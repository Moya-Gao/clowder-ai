...
| [Antigravity CLI](https://antigravity.google/cli) | Gemini / Google 账号侧选型 | plain text (`agy --print`) | CLI 管理 | Gemini 默认 |
| [Gemini CLI](https://github.com/google-gemini/gemini-cli) | Gemini | stream-json | 是 | 显式 fallback |
...
> Google consumer Gemini CLI / Gemini Code Assist individual 请求在 2026-06-18 停止服务，所以 Gemini 默认走 Antigravity CLI。只有 enterprise/API-key fallback 仍支持旧 CLI 时，才显式设置 `GEMINI_ADAPTER=gemini-cli`。
...
- **[第三方 AI Provider 配置指南](SETUP.opensource.zh-CN.md#模型接入ui)** — 配置 Kimi、GLM、MiniMax、Qwen、OpenRouter 等国产/第三方模型
