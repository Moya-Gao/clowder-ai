...
不教"什么是 i2v"。只沉淀：实测分工路由（中文乱码）、prompt 配方（四次翻车打磨）、工程纪律（防 $30-50 跑偏重演）、六个真金白银的坑。
...
1. **立项 charter**（CVO signoff 的 scope/路线/预算护栏/Non-goals）→ 范例 `docs/videos/cucu-pr-flow/episode-brief.md`；轻量立项规则见 shared-rules §21（LL-071：先对齐再批量产出）
...
| 先生产后对齐 | $30-50 跑偏（LL-071） | charter 先行；批量产出前一句话确认路线 |
