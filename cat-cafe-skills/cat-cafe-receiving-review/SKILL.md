...
2. CLASSIFY: 按 P1/P2/P3 分类问题
   🔴 区分两种反馈：
   - 代码级（bug/edge case/性能/命名）→ 正常修复流程
   - 愿景级（"这不是铲屎官要的"/"缺了多项目管理"/"UI 不可用"）→ 见下方愿景级反馈处理
...
如果 reviewer 指出"交付物不解决铲屎官的原始问题"，这不是代码 bug，是设计偏差：

```
WHEN 收到愿景级反馈:

1. STOP — 不要试图用代码 fix 修补设计问题
2. READ — 回读原始 Discussion/Interview 文档
3. VERIFY — reviewer 说得对吗？对照铲屎官原话判断
4. IF 确实偏离了:
   - 升级给铲屎官确认偏差范围
   - 重新设计受影响的部分（可能需要回到 brainstorming）
   - 不要在当前 review 循环里硬修——设计问题不是 patch 能解决的
5. IF reviewer 误判:
   - 用原始文档论证为什么当前实现符合愿景
   - 引用铲屎官原话
```

> **教训来源**：F041 的 10 轮云端 review 全在抓 edge case（代码级），没有一轮说"UI 不可用"或"多项目管理呢？"（愿景级）。因为 review 请求里没附原始需求文档，reviewer 没有判断愿景的上下文。
