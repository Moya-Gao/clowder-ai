...
4. **产出**：设计文档写到 `docs/plans/YYYY-MM-DD-{topic}-design.md`，commit 后问"要开始实现了吗？"
...
**CVO 介入**：价值 OQ 最后给 CVO，优先**提问**而非表态；表态标"价值偏好"不伪装事实约束。最终方案==CVO 初始未公开倾向 → 自检"论证结果还是锚定"（这是 convention 自检；高 stakes 要强制，再升硬层 sealed-commit，别误读成已有机械执法）。
...
**会议纪要模板**（存放：`docs/discussions/YYYY-MM-DD-{topic}-meeting-notes.md`）：
```markdown
# {主题} 讨论纪要
