...
**一个研究课题 = 一个子目录**。只要产出超过 1 个文件，就必须建子目录，禁止在 `docs/research/` 平铺散落。
...
- 如果只有 1 个文件（如单次 Mode B 咨询），可以不建子目录，直接放 `docs/research/`
...
**Step 1 — 写 Prompt 并落盘**
```
docs/research/YYYY-MM-DD-{topic}/prompt.md
...
```
结果存：`docs/research/YYYY-MM-DD-{topic}/{provider}-response.md`

**Step 3 — GPT-5.2 Pro 审阅**
输入三份报告 → 找逻辑漏洞、弱证据、三方分歧
存：`docs/research/YYYY-MM-DD-{topic}/gpt-pro-review.md`（注意：Pro 是审阅者，不是调研者，不要让他搜索）
...
实战范例：`docs/research/2026-04-16-f163-knowledge-lifecycle/research-brief.md`
...
docs/research/YYYY-MM-DD-{topic}/{model}-consult.md
```
例如：`docs/research/2026-05-18-finance-provider-stack/gpt-pro-consult.md`
