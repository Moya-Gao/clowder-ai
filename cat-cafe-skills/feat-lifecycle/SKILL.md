...
1. **扫描 BACKLOG + features/**：`grep -i "{关键词}" docs/BACKLOG.md docs/features/*.md`（或用 `search_evidence` 替代 grep）
2. **判定**：
...
**强制：Round 4 元审美审计（F163/Round 4 遗产）🔴**

**Design Gate 讨论收尾前，必须在当前 thread 完成以下三个灵魂拷问的自检并记录：**

1. **坐标变换 (Coordinate Transform) vs 多项式堆项 (Polynomial Fitting)**：
   - 这是一个让问题本身变简单的“坐标变换”吗？（例如：引入一个更准确的抽象，使得原本需要 10 个 if 的逻辑变成 1 行公式）
   - 还是仅仅在现有问题结构上“堆补丁、叠层数”？
   - **规则**：优先选择改变问题结构的方案。如果方案看起来很重，踹一脚坐标轴重想。

2. **认知脚手架 (Cognitive Scaffolding) 审计**：
   - 方案是否在增加不必要的 critic / refiner / planner 等“学术脚手架”？
   - 这些脚手架是为了掩盖对底层逻辑的不理解，还是真正的运行时必需（如权限隔离、中断点）？
   - **规则**：拒绝任何“为了让模型显得更聪明”而设计的复杂流程，只保留环境适配的必要支撑。

3. **负空间法则 (Negative Space Rule)**：
   - 如果我们**不**增加这个 Feature 或这段逻辑，系统会崩塌吗？
   - 是否存在一个更隐形的方案（例如：通过优化现有工具的 Prompt 或文档）就能达到目的？
   - **规则**：删掉它还能跑 = 多余。

**审计未通过 → 停下来，回到 Kickoff 或重新设计。**
