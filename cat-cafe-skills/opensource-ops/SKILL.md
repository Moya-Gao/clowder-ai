...
19. **Intake Guard = 硬约束 + 软约束**：硬约束负责机器阻塞：`Path Guard`（最终 merge diff 只能落在 Intent Issue 文件表 + exception list）、`Overlap Guard`（同文件社区改动 + 家里主线演化 = 禁止 safe-cherry-pick，强制 manual-port）、`High-risk File Guard`（入口接线、route 注册、DI、env、allowlist、auth/callback、sync 脚本等必须给 proof）。软约束负责猫的元认知：intake 不是覆盖文件，而是把 source intent replay 到当前 cat-cafe main，同时保住 home invariants。详见 [Inbound PR 文档](../refs/opensource-ops-inbound-pr.md) Step 1.1。
