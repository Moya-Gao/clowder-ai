...
> **R3.1 增量** (砚砚 OQ-5/6 final): `WaitSourceRef.slaUntilMs` REQUIRED + `anchorRef` for narrative kinds + `ownershipState` PR-O3 implement.
...
export type ClaimType =
  | 'owner'       // "这是 X 的活" / "这是我的活"
  | 'auth'        // "X 同意 / CVO signoff / 守护猫 APPROVE" (拆 AuthSubtype 区分子语义)
...
export type AuthSubtype =
  | 'cvo_signoff'        // claim "CVO 同意 / landy 签字"
...
### WaitSourceRef (R3.1 砚砚 OQ-5 final)
...
### OwnershipState (R3.1 砚砚 OQ-6, PR-O3 implement)
...
- **INV-O6**: `claimType='auth'` 时 `sourceRef.kind='messageId'` resolver 的 message author
  必须 = `'landy'`（catId 严格匹配；不接受 `'lysander'` / `'l.s.'` handle variant）才能 satisfy auth claim
