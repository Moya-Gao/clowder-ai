...
| 2 | **Authorization** | claim "X 同意 / CVO signoff / 守护猫 APPROVE" | `auth` | T0 (landy msg) / T1 (PR review) / T2 (转述) |
...
claim "CVO 同意" / "landy 签字" 后续行动。

| Resolver | sourceTier | 规则 |
|---------|-----------|------|
| `cat_cafe_get_message(messageId).author === 'landy'` | T0 | 严格 catId 匹配；`'lysander'` / `'l.s.'` handle variant 不算 |
| feature doc `## CVO Signoff` anchor 含 messageId reference | T2 | 必须能反推到原 landy messageId (T0) |

**verdict 规则**：T2-only → `insufficient`（不放行 merge / takeover / cvo_claim）。
转述（"X 说 CVO 同意"）= T2，不 satisfy。
...
| Family-level capability (布偶猫 / 缅因猫 / 暹罗猫 / 孟加拉猫) | T2 | family taxonomy |

**Limitation**：dossier 是猫可写 (T2)；硬限制（如 "暹罗猫禁写代码"）在 cat-config (T1) 才是 enforcement。
