...
date: 2026-06-21
authors: [opus-47 (宪宪)]
audience: cats (布偶猫 / 缅因猫 / 任何接手 cloud-cat onboarding 的猫)
purpose: |
  铲屎官说"宪宪/X 帮我接入 gpt-pro"或"在新机器上把云端 ChatGPT Pro 砚砚接进来"
  → 接球猫按本指南从头操作，全程不要让铲屎官点 CF dashboard（已经有过 4 次互相猜路径都猜错的教训）。
...
> dashboard 直觉**。
>
> 阅读对象 = 猫猫（不是铲屎官）。所以指令以 CLI + CF API 为主，dashboard 只在
> **铲屎官点 token scope** 那一步——这是 CF user-level token mint 唯一不能 API
> 做的事。其他全自动。
...
| 触发词 | 接球行动 |
|---|---|
| "宪宪帮我接入 gpt-pro" / "云端砚砚接入" | 加载本 SOP，从 §1 开始 |
| "新机器接砚砚 Pro" | 同上（多走 §A 新机器初始化）|
| "云端砚砚跑不通" | 跳 §7 debug clinic |
...
> 🔥 **关键教训（铲屎官 2026-06-21 04:56 UTC 原话）**：
...
6. 让铲屎官把 token 粘贴存到 `~/.cloudflared/cf-api-token`，`chmod 600`

> **B1b future 升级时再加** 3 个 scope：Access: Apps / Policies / Service Tokens（OAuth verified auth）。

如果铲屎官只给了部分 scope（之前其他猫接手时），猫必须**首检 B1a 2 个 scope 是否齐全**：
...
任何一行 `False` → 立刻让铲屎官回 dashboard 加 scope，不要凑合开干。

## 2. cat 注册（两层：mint allowlist + runtime catRegistry）

> ⚠️ **常见误判**（[LL-cat-cafe-api-has-hot-reload](../../docs/lessons-learned/LL-cat-cafe-api-has-hot-reload.md)）：之前以为 catId 加进 `cat-config.json` 重启 API 就能让 callback 路由认识它。**错**。
>
...
curl -X POST http://127.0.0.1:3002/api/cats \
  ...
  -d '{
    ...
    "mentionPatterns": ["@gpt-pro", "@gpt-pro-cloud", "@yanyan-pro", "@砚砚pro"],
    "accountRef": "codex",
    "roleDescription": "云端 ChatGPT Pro 砚砚 Pro，高阶判断 + 跨族 review + 设计方向收敛（F247 Phase B1a）",
...
# verify
curl -s http://127.0.0.1:3002/api/cats | python3 -c "
...
> "breeds[].variants[] Phase C 单独工程" 已被 B1a 实测 supersede（详见 [LL-cat-cafe-api-has-hot-reload](../../docs/lessons-learned/LL-cat-cafe-api-has-hot-reload.md)），
...
## 3. mint gpt-pro agent-key（不可逆，CVO 明确 OK 才执行）

```bash
cd /Users/lysander/projects/relay-station/cat-cafe  # 必须在 main 仓，不在 worktree
...
⚠️ 6399 圣域规则：mint 操作必须三重 flag 显式确认。猫不能自决跑 mint，必须铲屎官说 "OK mint gpt-pro" 才执行（F247 §10 KD-9）。

## 4. 公网 endpoint：cat-cafe tunnel（B1a Path A，无 CF Access）

> ⚠️ **绝大部分时间花在这里**。CF dashboard UI 重组频繁，**不要让铲屎官摸索 UI**，
> 全部走 CF API。
...
> 否则会触发"长得像 X 但行为像 Y"的诡异 bug — 详见 [LL-spike-server-env-contamination](../../docs/lessons-learned/LL-spike-server-env-contamination.md)。
...
| Env | 处理 | 原因 |
|---|---|---|
| `CAT_CAFE_INVOCATION_ID` | **unset** | 否则 MCP server `callback-tools.ts:662` gate 误判 spike 是 invocation-token caller，拒掉砚砚云端 post_message |
...
3 项全过 = ✅ onboarding 完成，可以传 ChatGPT 端配置给铲屎官。

## 7. 砚砚 ChatGPT 端配置（最后一步，铲屎官点一下）

铲屎官设置 ChatGPT custom MCP connector（路径：ChatGPT Settings → Connectors → 新建应用，需 Developer mode）。表单字段：

| 字段 | 填 |
|---|---|
| **名称** | `cat-cafe` |
| **描述** | `Cat Café MCP - 云端砚砚 Pro 工具集` |
...
**不要**截图发铲屎官（secret discipline）— 让他自己 `cat ~/.cat-cafe/spike-token` 看一眼粘贴。

然后灌 Custom Instructions（短 L0）：见 [`gpt-pro-custom-instructions.md`](./gpt-pro-custom-instructions.md)。

砚砚云端 ChatGPT 应该立刻能调 10 项工具。试 `cat_cafe_list_threads` 或 `cat_cafe_search_evidence` 看返回。
...
- 如连不上报 transport error，看 spike server log + ChatGPT 端报错截图发宪宪 debug
...
```bash
# 1. cloudflared login (会开浏览器，铲屎官 OAuth Google 一次)
...
| ChatGPT 端报 "工具调用被 OpenAI 的安全检查屏蔽" | MCP tool 缺 annotations (readOnlyHint/destructiveHint/openWorldHint) | 升级到含 `inferAnnotations()` 的 server-toolsets.ts (commit `994dfa665`+) + 砚砚 ChatGPT 端重开 connector force refresh schema |
...
| 砚砚云端 read 工具 OK，但 `cat_cafe_post_message` 返 `Unknown catId filter: gpt-pro` 或 502 | cat-cafe API catRegistry 没注册 gpt-pro | **不要重启 API**，走 `POST /api/cats` 热加载（§2.2）。详见 [LL-cat-cafe-api-has-hot-reload](../../docs/lessons-learned/LL-cat-cafe-api-has-hot-reload.md) |
| `post_message rejects threadId from invocation-token callers (F193 KD-1)` 错误（砚砚云端是 agent-key，本不该 fire）| spike server 继承父 invocation 的 `CAT_CAFE_INVOCATION_ID` + `CAT_CAFE_CALLBACK_TOKEN` env，MCP server gate 误判 | spike 启动加 `env -u CAT_CAFE_INVOCATION_ID -u CAT_CAFE_CALLBACK_TOKEN`（§5）。详见 [LL-spike-server-env-contamination](../../docs/lessons-learned/LL-spike-server-env-contamination.md) |
...
| `threadId required for agent-key auth` (cat-cafe API 错误) | agent-key caller 模式必传 `threadId`（vs invocation-token 必禁），砚砚 ChatGPT 端没传 | 告诉砚砚他是 agent-key 模式 `post_message` 必须显式带 `threadId`（短 L0 已说明）。详见 [LL-agent-key-vs-invocation-token-threadId](../../docs/lessons-learned/LL-agent-key-vs-invocation-token-threadId.md) |
| ChatGPT 端 write 工具偶尔被 OpenAI safety check 拦截，read 工具不拦 | ChatGPT 对 `readOnlyHint=false` 工具更严格 + first-time write 可能要 user 确认 | 让砚砚在 ChatGPT UI 点确认按钮 (如有)；或多试几次。属于 OpenAI 平台行为，**我们不可控** |

## §C 风险表 + Token Rotation SOP

> **B1a interim 是接受降级的设计**（F247 KD-7 + AC-B1-7）。这一节列**所有已知风险**
> 给猫和铲屎官知情，并提供 rotation / 应急 SOP。
...
| readonly + cat_id=gpt-pro + user_id=default-user 锁定 | 强 | 砚砚云端发的所有 message 标 `gpt-pro` 身份，audit log 完整 |

**实质暴露面**：拿到 token = 能伪装砚砚 Pro 身份发 message + 写 thread + 写 memory + 调 search/list 看 user-scope 记忆。
...
| Token 字符串 commit 到 git repo（包括 worktree / 历史 commit）| 🔴 P0 | rotate + `git filter-repo` 清史 + force push（铲屎官 + 跨族 review）|
| 铲屎官把 ChatGPT account 借给别人用 | 🔴 P0 | rotate + 看是否 setup 第二 connector |
| 砚砚 ChatGPT 端 connector list 截图被分享 | 🔴 P0 | rotate（截图含 URL = 含 token）|
...
# 3. 砚砚 ChatGPT 端更新 connector URL
...
- token 不出现在 URL，rotate 通过 OAuth provider 后端完成（不影响砚砚端）
- 风险等级降到生产可接受
...
| 信号 | 阈值 | 含义 |
|---|---|---|
| spike server `auth=present` 频率 | 正常 < N/分（按砚砚活跃度）| 异常高 = 可能被滥用 |
| `auth=absent` 频率 | 正常 < 1/秒（互联网扫描）| 异常高 = 公网扫描 / brute force 试探 |
| User-Agent 多样性 | 正常 1-2 个（砚砚 ChatGPT 一致）| 异常多 = 多客户端登录 |
| Egress IP 多样性 | OpenAI IP 段（Azure）| 突变 = 砚砚 ChatGPT 服务端 IP 变化或被劫持 |
...
2. **Token scope 残缺要回头补**（铲屎官原话见 §1.3）：
...
5. **mint 是不可逆**（F178 KD-9 + F247 §10 KD-9）：mint script 必须铲屎官签 "OK mint gpt-pro" 才跑。猫不自决。
