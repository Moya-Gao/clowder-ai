...
description: >
  ...
  GOTCHA: 必须有本机 `TTFUND_APIKEY`，且 key 只放环境变量或 gitignored `.env.local`，绝不写入 git 或回复全文。
triggers:
  - "天天基金 skills"
  - "ttfund"
  - "TTFUND_APIKEY"
...
`.env.local` 已被 `.gitignore` 忽略；不要把 key 写进任何 tracked 文件。
...
| 错误 | 后果 | 修复 |
|---|---|---|
| 把 API key 写进命令历史、文档或 git | 凭证泄露 | 只放环境变量或 `.env.local`，回复中只写 masked key |
