...
> 技术收敛纪要: [2026-04-05 三猫收敛](../../docs/discussions/2026-04-05-f138-video-pipeline-tech-convergence.md)

## 核心原则

**视频不是一只猫的活，是三猫流水线 + 铲屎官素材。**

- 宪宪：video-spec 编排 + Remotion 渲染 + 对齐集成
- 砚砚：音画同步 QA + 事实审查 + schema review
- 烁烁：节奏/调性审查 + 字幕/排版设计 + retiming 风格把关
...
| 配音方案 | 猫猫配音 / 纯字幕 / 原声 | 宪宪旁白 / 三猫声线 / 无配音 |
...
| 触发 | 场景 | 主导 | 说明 |
|------|------|------|------|
| 铲屎官说"做个视频" | **A: Brief + 素材盘点** | 宪宪 | 确认开局参数 + 分镜表 + 素材需求清单 |
| 铲屎官确认分镜 | **B: 素材入库** | 铲屎官录 + 宪宪压缩归档 | 素材放 `docs/videos/{project}/assets/`，粗标写 `asset-markers.md` |
| 素材到齐 | **C: video-spec 冻结** | 宪宪 | 写 video-spec JSON（4 层 segment contract），铲屎官确认 |
| spec 确认 | **D: 全局配音 + 对齐** | 宪宪 | CosyVoice 全局配音 → Qwen3-ForcedAligner → word_timestamps |
| 对齐完成 | **E: Remotion 渲染** | 宪宪 | schema → inputProps → preview render |
| 预览版出来 | **F: 审查 Gate** | 三猫 + 铲屎官 | 见下方审查标准 |
| 审查通过 | **G: Final Render + 交付** | 宪宪 | 高质量渲染 + 封面导出 + 发布 |
| 铲屎官不满意 | **R: Patch Loop** | 宪宪 + 烁烁 | retiming / 重录 / 重写段落 |
...
├── asset-markers.md       ← 素材标注表（铲屎官 + 宪宪共同编辑）
├── video-spec.json        ← segment contract（宪宪生成）
├── voice-script.md        ← 配音剧本（宪宪草稿 + 铲屎官确认）
...
路径 B 完整流程跑通后 → `quality-gate`（自检）→ `request-review`（砚砚审音画，烁烁审节奏）
