...
(async () => {
  ...
  if (page === undefined) { console.log("No viable target"); process.exit(1); }
  console.log("Connecting to:", page.title);

  const ws = new WebSocket(page.webSocketDebuggerUrl);
  await new Promise((resolve, reject) => { ws.onopen = resolve; setTimeout(() => reject(new Error("timeout")), 5000); });
  ...
  try {
    ...
    const tb = await cdp("Runtime.evaluate", {
      expression: '(() => { const tb = document.querySelector(\'[role="textbox"][contenteditable="true"]\'); return tb ? "FOUND" : "NOT_FOUND"; })()'
    });
    console.log("Textbox:", tb.result.value);

    if (tb.result.value === "FOUND") {
      const btn = await cdp("Runtime.evaluate", {
        expression: '(() => { for (const b of document.querySelectorAll("button")) { const l = (b.getAttribute("aria-label") || b.getAttribute("title") || "").toLowerCase(); if (l.includes("send") || l.includes("submit")) return "FOUND: " + l; } return "NOT_FOUND"; })()'
      });
