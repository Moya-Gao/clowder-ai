...
# subprocesses anchor cwd + module loading to /Volumes/Cat Cafe/...) and
...
# /Volumes/Cat Cafe/.DS_Store, which hdiutil bakes into the final image when
...
# with the same volume name (e.g. another "Cat Cafe" image left over from a
...
tell application "Finder"
  ...
  tell theMount
    ...
    set position of item "Cat Cafe.app" of container window to {140, 190}
...
for arch in "${ARCHS[@]}"; do
  ...
  hdiutil create -volname "Cat Cafe" -srcfolder "$dmg_staging" -ov \
