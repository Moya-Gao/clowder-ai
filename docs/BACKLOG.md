...
| F245 | Friction Signal Eval — 摩擦信号统一聚合（eval:friction）：猫爪感差/用户cancel/F222反馈/eval域摩擦 4 通道散落无统一视图 → 周期 rollup 聚合+分类(harness/工具/环境)+F128/code-as-harness 出口；频率可配（社区weekly/本家3天）；A聚合不搬迁 task-outcome；实证 signal 体量大（数百 invocation/天） | in-progress (Phase A/B/C merged, Phase D pending) | 缅因猫/砚砚 (gpt52)（2026-06-21 ownership reset） | internal (CVO 2026-06-18 "需要feat你可以创建") | [F245](features/F245-friction-signal-eval.md) |
| F248 | Eval Hub 人类可读性 — 让铲屎官看得懂 eval 在干嘛（F192 follow-up）：7 个 eval 域对人不可见/不可读、面板 bug（A2A 状态过时/归因包点不了）、verdict 结论不说人话、信息位置不对；前置 F236 AC-E3 + F245 friction 接入完成后启动 | spec (blocked: F236 AC-E3 + F245) | TBD | internal (CVO 2026-06-22 铲屎官痛点反馈) | [F248](features/F248-eval-hub-human-readability.md) |
