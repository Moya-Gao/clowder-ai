...
| 97 | **Connector Messages — 外部信息源抽象 + 自动唤起** | [ ] | [2026-02-25 Phase 3 设计](./plans/2026-02-25-connector-messages-phase3.md) | #81 的 Phase 3 升级为通用 Connector 抽象。3a: ConnectorSource 类型 + 前端 connector 气泡；3b: 自动唤起猫（route→invoke 闭环）；3c: Redis 持久化。GitHub Review 是第一个 connector，将来扩展 iMessage/Slack 等只需加 registry 条目。|
