...
| F8 | ~~Token 预算 + 深度可观测性~~ | **[x]** | [NDJSON 宝藏调研](./research/cli-ndjson-treasure-map.md) | 全部完成：char→token 迁移 (js-tiktoken, 16 files) + 三猫 CLI usage/cost/cache 捕获 + 前端 RightStatusPanel per-cat token 显示 + ParallelStatusBar 聚合 + inputTokens 归一化 (`da75aaf`) + review fix (`e8d1dbd`)。commits: `66a59e4`→`6f25a2b`→`e8d1dbd` |
...
| F31 | **PR 双层 Review 流程（本地猫 + 云端猫）** | **P1** | 2026-02-14 铲屎官提议 | 当前合入流程：本地缅因猫 review → 合 main。新增云端守护层：本地猫合入 main 后 → 开 PR → 云端 Codex（独立额度，目前 0 使用）自动 review → 通过后 merge。好处：**(1)** 本地猫互相熟悉容易互相放水，云端猫是"陌生审查员"；**(2)** Codex cloud 有独立额度不占本地配额；**(3)** F23 重构这种大改动多一层守护更安心。实现：`gh pr create` + Codex cloud `codex review --pr` 或 GitHub Actions 触发。**F26-30 做完后启用**，给重构（F23/F25）保驾护航。 |
| F30 | **消息代码块复制按钮 + 文件路径可跳转** | **P1** | 2026-02-14 铲屎官提议 | 两个 `MarkdownContent.tsx` 增强：**(1) 代码块复制按钮**：`<pre>` 右上角加 Copy 按钮 (`navigator.clipboard.writeText`)，点击后显示"已复制"反馈。**(2) 文件路径可跳转**：检测 `packages/api/src/foo.ts:42` 或绝对路径格式，渲染为 `vscode://file/...` 链接，点击在 VSCode 打开对应文件+行号（Codex app 已有此功能）。铲屎官说看不懂代码但能看文档，跳转帮助审阅猫猫引用的文件。改动量：~50 行前端。 |
| F29 | **删除右面板"任务统计"死区 + TaskExtractor 清理** | **P2** | 2026-02-14 铲屎官发现 | 右面板"任务统计"永远是 0——TaskExtractor 从对话文本提取 `- [ ]` / `TODO:` 标记，但猫猫实际用 CLI 工具 (TaskCreate/write_todos) 管理任务，两套系统不搭。删除：RightStatusPanel 的任务统计 section + taskSummary prop + ChatContainer taskSummary 计算。TaskExtractor 后端逻辑（TaskStore/fetchTasks）暂保留给 sidebar 毛线球用；前端右面板的任务展示由 **F26 的实时 task 进度**取代（放在每只猫的调用卡片里）。 |
| F28 | **授权请求跨渠道通知** | **P1** | 2026-02-14 铲屎官提议 | 猫猫请求权限时只通知网页 UI，铲屎官在 CLI/VSCode/其他 tab 里看不到，导致请求超时猫猫卡住。方案：浏览器桌面通知 (Notification API, `requireInteraction`) + Tab 标题闪烁 + Header pending badge + 可选 macOS `osascript` 系统通知。最小方案 ~65 行前端。计划: [`2026-02-14-f28-authorization-cross-channel-notification.md`](./plans/2026-02-14-f28-authorization-cross-channel-notification.md) |
| F27 | **A2A 路径统一 — 两条路合一 + 全链可取消 + 多 mention** | **P0** | 2026-02-14 铲屎官亲历 | **已复现 P0**: 2026-02-14 晚 thread `thread_mln54grb12u8v28h` 中两猫无限乒乓，铲屎官被迫强制重启。**三个缺陷叠加**：(A) 同一 @mention 被 Path A (worklist) + Path B (callback) 双重开火；(B) Path B 无深度限制导致无限递归；(C) callback child 不注册 tracker 不可取消。**修复方案**：callback 不再自己执行猫调用，改为追加到父 worklist（统一成一条路径 + 共享 AbortController）。`parseA2AMentions` 返回 `string[]` 支持多目标（上限 2）。**F24 合入后立刻开工**。Bug report: [`2026-02-14-a2a-feedback-loop`](./bug-report/2026-02-14-a2a-feedback-loop/bug-report.md)，计划: [`2026-02-14-a2a-path-unification.md`](./plans/2026-02-14-a2a-path-unification.md) |
| F26 | **UI Dashboard Upgrade — 右面板重构 + 实时计划进度** | **P1** | 2026-02-14 铲屎官提议 | 三个核心改进：**(1) 右面板信息丢失修复**：`targetCats` → `threadParticipants`，线程中出现过的猫始终显示（当前猫高亮、历史猫折叠）。**(2) 实时 Task 进度面板**：从三猫 NDJSON 流中识别 task 管理工具（Claude `TaskCreate/Update`, Gemini `write_todos`, Codex `reasoning` fallback），前端 checklist + 进度条。**(3) 信息层次重构**：平铺列表 → 分区折叠（当前调用/历史参与/统计/审计）。**前端交给暹罗猫用 Pencil Skill 设计 + 实现**。计划: [`2026-02-14-ui-dashboard-upgrade.md`](./plans/2026-02-14-ui-dashboard-upgrade.md) |
