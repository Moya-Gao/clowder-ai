...
| F226 | Presentation Surface / Demo Mode（右侧 surface 浮窗 PiP + AppShell 级 host + tear-off 快照，演示时 PPT 浮窗常驻、右侧切证据不打断）| spec | 宪宪 Opus-4.8 | internal | [F226](features/F226-presentation-surface-demo-mode.md) |
| F227 | Event Memory — 事件级记忆索引（拉闸记录 / 认知状态转折一等公民 + magic word 时间线 + teleport message 级跳转 + 飞轮可观测性层）| spec | 宪宪 Opus-4.8 | internal | [F227](features/F227-event-memory.md) |
| F192-sop-wiring | `eval:sop` Auto-Schedule Re-enable — 三件套 wiring：(1) SopTrace producer (session telemetry → SopTrace pipeline) + (2) file-writer 层 (~100-150 行，per F192 doc §323) 把 SopEvalResult[] 落 snapshot/attribution YAML → bundle → verdict.md + (3) `eval:sop` 加进 `PUBLISH_VERDICT_INSTRUCTIONS_BY_DOMAIN` 解 501。三件齐才删 `eval-sop.yaml` 的 `enabled: false` 恢复 weekly cron。优先级 P2（sunset 已止血 silent-fire，可逆改一行 yaml 即 re-enable，wiring 工作量独立、不紧急） | spec | 布偶猫 | internal | [F192 § 2026-06-06 sunset entry](features/F192-socio-technical-harness-eval.md) |
