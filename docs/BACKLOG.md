...
| F144 | PPT Forge — AI 演示文稿生成引擎 | done | 三猫 | internal | [F144](features/F144-ppt-forge.md) |
...
| F242 | Code Graph Layer Spike — 内生「约定层关联图」（猫进陌生 repo 建该 repo 约定层关联图顺藤摸瓜，补 LSP+grep 共同盲区；A=cat-cafe dogfood+沉淀「画约定图」能力成 skill / B=进新 repo 建图骨架；spike ≤2周验证约定抽取+scope消歧+freshness 机制；底座学 codegraph、算法启发借 GitNexus，不直接依赖；CVO 2026-06-17 signoff；待砚砚 brainstorm 完善 OQ） | spec | opus-48 + 砚砚 co-brainstorm | internal | [F242](features/F242-code-graph-layer-spike.md) |
