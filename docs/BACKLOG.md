...
> 维护者：布偶猫 | 最后更新：2026-02-07 (Phase 3.6 review 通过后整理)
>
...
| 5 | 目录浏览安全 (allowlist/blocklist) | [ ] | Phase 3.2 review | 当前仅限 `homedir()`，生产需更细粒度 |

## P2 — 建议做

| # | 项目 | 状态 | 来源 | 备注 |
|---|------|------|------|------|
| 6 | 上下文预算管理 (token 截断) | [ ] | 身份注入讨论 | systemPrompt + contextHistory + 串行回复叠加可能超 token 限制 |
| 7 | 前端图片压缩 | [ ] | Phase 3.2 review | 当前 10MB/张直传 |
| 8 | 对话级联删除 | [ ] | Phase 3.2 review | DELETE thread 不删消息，依赖 TTL |
| 9 | cancel_invocation 真正鉴权 | [ ] | Phase 3.3b review | 当前只有 `socket.rooms.has()`，无用户身份校验 |
...
| 13 | AgentRouter.ts 超 200 行 (379行) | [ ] | Phase 3.5 Step 3 | 加了 context history 后更长，提取 routeSerial/routeParallel 到 `route-strategies.ts` |
...
| 18 | start-dev.sh Redis 失败分支无自动化测试 | [ ] | Phase 3.6 缅因猫 review | `redis-server` 失败回退路径靠人工验证，可加 BATS/shell 级测试 |
