...
| F225 | Cat-Initiated Session Handoff — 猫主动封印会话接力（硬层 done + dogfood；软层 L0+skill 元思维注入 + context_management_hint + eval=capability-wakeup 进行中）| in-progress | 宪宪 Opus-4.8 | internal | [F225](features/F225-cat-initiated-session-handoff.md) |
...
| F231 | 启动胶囊 — per-user 画像注入与 L0 分层（猫醒来第一眼看到主人：profile capsule + relationship primer + breed/instance/user/relationship 四层，养成护城河机制本体）| spec | 宪宪 Fable-5 | internal (CVO 2026-06-11 "我同意立项的") | [F231](features/F231-user-profile-capsule.md) |
| L0-budget-defense | **P0** L0 预算结构性防御 4 件套 — **①+②+③ done (PR #2215 bf5a4f1e4) + ④ spec done (PR #2221 d7944b27)**：守护测试 3 件套 ✓ + ADR-038 L0 Staging Protocol 双向新陈代谢 spec ✓（三层 L0/Staging/Sunset + 双层守恒 + 三问 demote checklist + presence-normalized v1/v2 路径 + 雨刮器永居 staging 映射 + fallback 1 次 incident 作用域）。**剩余**：PR-B-impl (staging mechanism + 雨刮器减肥版 land 在 staging) + PR-C (first demote case 让 codex/gpt52 回 ≤6000)。**Promote 队列候选 #1**（fable-5 登记 2026-06-11）：shared-rules §21「Lifecycle 不分任务类型」投影行（LL-071，CVO 2026-06-10 拍板进 L0；投影行已在 worktree 验证 build ✓ + governance-l0 单测 8/8 ✓，token 实测把 gpt52 顶到 6142 撞 margin guard 故按协议排队不硬塞；文本与 anchor 见 9a30350d3 + 本条目 cross_post）。**Promote 队列候选 #2**（opus-47 登记 2026-06-11，Design Gate 决议）：F231 capsule（≤300 字 / ~285 tokens），注入锚 gated on PR-C 落地（demote codex/gpt52 回 ≤6000）；ADR-038 三问判定 capsule 属"全程身份/球权类"——必须留 L0（不进 Staging/不进 SystemPromptBuilder 运行时）；spec 字段已锚 commit `e5100a486`——见 [F231 KD-7](features/F231-user-profile-capsule.md) + [Design Gate 出口物](discussions/2026-06-11-f231-design-gate.md)。**Expiry**: 2026-06-13T00:00:00Z PR-A `L0_BUDGET_TODO_EXPIRY`。fable-5 trigger 链：staging 层雨刮器生效之时 (PR-B-impl) = "细则进 code-as-harness skill" trigger 锚 | in-progress (①+②+③+④spec done) | 宪宪/Opus-4.7 (ETA PR-B-impl + PR-C ≤ 2026-06-13) | internal (fable-5 + opus-47 + gpt52 build mode 2026-06-10，PR #2213 deadlock 倒逼) | [PR #2215](https://github.com/zts212653/cat-cafe/pull/2215) [PR #2221](https://github.com/zts212653/cat-cafe/pull/2221) [ADR-038](decisions/038-l0-staging-protocol.md) |
