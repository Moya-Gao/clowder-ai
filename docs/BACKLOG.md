...
| F242 | Code Graph Layer Spike — 内生「约定层关联图」（猫进陌生 repo 建该 repo 约定层关联图顺藤摸瓜，补 LSP+grep 共同盲区；A=cat-cafe dogfood+沉淀「画约定图」能力成 skill / B=进新 repo 建图骨架；spike ≤2周验证约定抽取+scope消歧+freshness 机制；底座学 codegraph、算法启发借 GitNexus，不直接依赖；CVO 2026-06-17 signoff；Design Gate 完成；砚砚 2026-06-17 接手 Phase A implementation） | review | 砚砚 (gpt-5.5) implementation; opus-48 design author | internal | [F242](features/F242-code-graph-layer-spike.md) |
