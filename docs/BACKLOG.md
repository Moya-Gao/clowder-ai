...
| 2 | ~~猫配置外置 `cat-config.json`~~ | [x] Phase 3.5 | Demo 发现 | `bab9fcf` Breed+Variant schema + 加载器 |
...
| 17 | ChatContainer.tsx 超 200 行 (297行) | [ ] | Phase 3.5 缅因猫 final review OQ#5 | 提取 fetchHistory/fetchTasks/handleSend 到自定义 hook |

## P3 — 可选优化

| # | 项目 | 状态 | 来源 | 备注 |
|---|------|------|------|------|
| 18 | blob URL 同 thread 连发大量图累积 | [ ] | Phase 3.3b review R1 | clearMessages 时已回收，但不切 thread 会累积 |
| 19 | 冷/热状态视觉反馈 (猫头像发光) | [ ] | 暹罗猫提议 | CSS class 切换，低成本 |
| 20 | Antigravity cancel 无效 (detached 进程) | [ ] | Phase 3.3b review | gemini-cli fallback 可选 |
