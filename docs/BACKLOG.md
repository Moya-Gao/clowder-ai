...
> 维护者：三猫 | 最后更新：2026-06-20（F168 ops-gap reopen truth sync）
>
...
| F237 | Prompt Injection Visibility — 社区 PR #859 intake（lifecycle viewer + 模板提取 + 3 段 overlay 编辑器；方向 2026-06-16 maintainer self-accepted；Round-3 audit 发现 P0 loopback-gate 安全洞 + P1 atomicity + P1 verify-coverage 8%，待作者 fix；F238 boundary guard 全 5 phase done 2026-06-16 — F237 仅剩等社区 @mindfn 修 Round-3 P0/P1 即可 intake dry-run） | awaiting-author-fix | community @mindfn + maintainers | community [#839](https://github.com/zts212653/clowder-ai/issues/839) + [#859](https://github.com/zts212653/clowder-ai/pull/859) | [F237](features/F237-prompt-injection-visibility-intake.md) |
...
| F244 | Capability Tips System — 等待态 Knowledge Feed 投影（家里能力 / magic words / 工作流 tips + feature tips 贡献机制；真实状态、tips、猫格表达三层分离） | spec | 缅因猫/砚砚 | internal (CVO 2026-06-18 "直接立项吧") | [F244](features/F244-capability-tips-system.md) |
| F245 | Friction Signal Eval — 摩擦信号统一聚合（eval:friction）：猫爪感差/用户cancel/F222反馈/eval域摩擦 4 通道散落无统一视图 → 周期 rollup 聚合+分类(harness/工具/环境)+F128/code-as-harness 出口；频率可配（社区weekly/本家3天）；A聚合不搬迁 task-outcome；实证 signal 体量大（数百 invocation/天） | spec | 布偶猫/宪宪 (opus-48) | internal (CVO 2026-06-18 "需要feat你可以创建") | [F245](features/F245-friction-signal-eval.md) |
| F246 | Approval Hub — 统一审批中心底座：CVO 审批散落各 thread（F128/F225/F193），铲屎官不在对应 thread 就看不到 → CQRS read-side index 聚合多 feature 审批项 + Hub "待审批" panel（跨 thread 可见 + 计数徽标 + 就地/跳转审批 + 过期提醒）；接入三条件 actor=CVO + binary outcome + 跨 thread；v1 接 F128/F225/F193 E3；F168 sibling 不迁；push channel 独立问题 | spec | 布偶猫/宪宪 (opus-46) | internal (CVO 2026-06-20 "好耶！我同意") | [F246](features/F246-approval-hub.md) |
