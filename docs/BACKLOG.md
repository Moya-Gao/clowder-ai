...
| 86 | **ImageExporter Puppeteer 进程泄漏** | [ ] | 2026-02-21 铲屎官排查耗电 | `ImageExporter.ts` 用 `puppeteer.launch()` 启动 Chrome 实例，复用 `this.browser`。但 API 服务退出/重启时无清理钩子调 `close()`，导致 Chrome 子进程变成孤儿僵尸。实测发现 5 组 Puppeteer（55 个进程）存活 9~10 天，吃内存 + CPU。修复方向：(1) 注册 `SIGTERM`/`SIGINT` handler 调 `imageExporter.close()`；(2) 考虑 Fastify `onClose` 钩子统一清理。关联 F17 导出长图。 |
