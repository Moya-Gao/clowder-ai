...
| 81 | **GitHub Review Email Watcher（自动唤醒猫猫处理 review）** | [x] | [2026-02-18 设计方案](./plans/2026-02-18-github-review-email-watcher.md) | Phase 1+2 已实现 `e50f99c`：IMAP 轮询 + 3 层路由 + PR tracking API + 67 tests（7 轮云端 Codex + 砚砚 R4）。Skill Step 2.5 注册已加 `454fe72`。Phase 3 → 见 #97。|
| 97 | **Connector Messages — 外部信息源抽象 + 自动唤起** | [~] | [2026-02-25 Phase 3 设计](./plans/2026-02-25-connector-messages-phase3.md) | Phase 3a 已实现 `91c66a1`：ConnectorSource 类型 + registry + StoredMessage.source + Redis 序列化 + 三路 type 判定 + ConnectorBubble 前端气泡 + ReviewRouter source + 15 tests。砚砚 R1 放行 + P3 URL 白名单 `aa0a01e`。**待做**: 3b 自动唤起；3c Redis 持久化。|
| 98 | **Session 查询工具升级 — 让猫更会查旧事** | [ ] | [08 课件 gap 分析](./plans/2026-02-25-session-query-tools-upgrade.md) | 对照 lesson 08 畅想的 3 个功能 gap：(1) read_session_events 加 view 模式 (chat/handoff/raw)；(2) 新增 read_invocation_detail 工具；(3) session_search 返回 invocationId 指针。三猫+铲屎官共识：先做功能 gap，性能债延后。|
