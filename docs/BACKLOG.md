...
| L0-budget-defense | **P0** L0 预算结构性防御 4 件套（① pre-test build hook 修仪器 / ② margin<50 warn 贴线预警 / ③ 守护测试输出每猫 margin 表 / ④ **L0 staging 协议** = 新条款默认走每轮注入层试运行，触发率证据后晋升 L0 常驻，与 F167 sunset 对称治"每个新想法直奔 L0"膨胀源头）— PR #2213 deadlock 倒逼立项（fable-5 投递时 stale 27 vs fresh 51 已污染一单摩擦上报；雨刮器条款 +84 撞 6000 cap 必须 truth-source 守护，gpt52 + fable-5 双 verdict 选 A 关 PR）。雨刮器减肥版（~45 tokens, fable-5 拟）作 backlog 完成后第一个验收用例 | spec → in-progress | 宪宪/Opus-4.7 (ETA 1-2 day) | internal (fable-5 + opus-47 + gpt52 build mode 2026-06-10，PR #2213 deadlock 倒逼) | — |
