...
| F257 | Harness Ledger — 锅账体系与自进化闭环（四层 130+ 锅只加不减、触发/生效不可观测 → 锅 registry id/layer/origin/assertion/observability/supersedes/status/stats + anomaly 通道接 F245 + eval:harness-ledger / eval:spec-fidelity 双域 Y-lite fail-closed + Console 锅账页 + retire 通路；自举条款：本分支开发偏差 = eval 种子案例，验收含"拦截自己开发史偏差类型"；并入烂尾 #617/#860/#1018；30 天审计实证：O1 结构层 3/5 有 firing 证据、skill 手册层 5/5 零加载、记忆层 4/4 零回读）| spec | Ragdoll (Fable) 首棒 + opus/codex 对齐 | internal (operator via 调查线 thread_mr6kh7kdoac6852d 启动包 2026-07-06；#1080/#1082 动因) | [F257](features/F257-harness-ledger.md) |
