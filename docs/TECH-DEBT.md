...
| TD116 | **scheduled web-digest 对接现有 browser-automation backend** | [ ] | F139 AC-H2b 转出 | `needs-browser` 路由、server-fetch 路径、SSRF 防护和失败语义都已落地；家里的 browser-automation / Playwright backend 已存在，剩余缺口是 F139 调度侧把 `needs-browser` URL 真正接到现有 backend，并定义好摘要/投递语义。此项不再阻塞 F139 close。触发条件：F139 scheduler execute 路径接入现有 browser-automation backend 时实现。 |
