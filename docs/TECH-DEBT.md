...
| TD093 | Gemini resume：按 thread 隔离目录 + `--resume latest` | [ ] | 铲屎官 2026-03-01 实测 | 现状：Gemini CLI 仅支持 index/latest resume（不支持 UUID），我们当前为避免跨 thread 串味，禁用 resume，改用 ContextAssembler prepend 历史。后续方案：为每个 thread 分配独立工作目录/会话目录，从而让 `latest` 语义稳定等价于“该 thread 的 latest”，减少限流与重复上下文传输。 |
