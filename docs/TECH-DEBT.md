...
| TD117 | **env 变量三源不同步 — registry / .env.example / code 缺口审计 + 防遗忘** | [x] | 2026-03-28 布偶猫审计 | PR #832 已修复：(A) 补齐 35+ 漏网变量到 `env-registry.ts`；(B) 清理 `.env.example`（删 8 孤儿，加 `exampleRecommended`）；(C) 新增 `check:env-registry` + `check:env-example` CI 门禁接入 `pnpm check`，防止再遗忘；(D) `gen:env-reference` 自动生成 `docs/env-reference.md`。教训沉淀 LL-042。2026-03-28 关闭。|
