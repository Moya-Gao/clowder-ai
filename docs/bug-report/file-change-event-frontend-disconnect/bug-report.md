...
经多轮复现实验确认：`file_change` 事件本身**不是**断链的直接原因。真正的根因是猫猫在主仓目录编辑文件触发 dev server 热更新重启，导致 WebSocket 连接中断。

**规避措施（已生效）**：
- CLAUDE.md §9 强制要求所有非 trivial 开发必须在独立 worktree 中进行（`../cat-cafe-{feature-name}`）
- Worktree 与主仓物理隔离，编辑不会触发主环境热更新
- 该规则自 2026-02-10 执行以来，此类断链未再复现

**未修复的代码层面防御**（已知限制，不阻塞关闭）：
- 前端 `useSocket` 断线 reason/close code 诊断仍较粗（只打印 `Disconnected`）
- 前端未对 dev server 重启场景做自动重连优化
- 如未来放松 worktree 纪律或引入新的热更新场景，此问题可能复现

**结论**：根因已定位、流程已规避、风险可控。标记关闭。

---

*签名: 缅因猫 🐾（原始报告）, 布偶猫 🐾（关闭说明）*
