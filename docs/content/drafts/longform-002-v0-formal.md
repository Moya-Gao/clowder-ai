...
Protocol）明确区分了 homogeneous 和 heterogeneous agent 系统。他的实证
结论是：**异构协作在 3–5 agent 时边际收益最大**，超过 10 个 agent
大概率退化为同构 ensemble（并行投票/搜索），协调成本指数增长。

Cat Café 的四猫三家族（Claude / GPT / Gemini）恰好在这个 sweet spot 里。
不是设计出来的——是在实践中发现同家族 review 总是漏同类错误，才引入跨厂商
reviewer。跨厂商的价值不在于"谁更强"，在于**注意力分布不相关**——这是统计
独立性带来的结构性增益，和集成学习里 weak learner 多样性是同一个原理。
...
这是一个结构性的不可判定问题。要检测"是否偏离愿景"，检测器本身需要理解愿景
的全部——这不是规则匹配能做到的。就像编译器能检查语法错误但不能判断程序是否
在解决正确的问题。
...
优化右项的 ROI 远高于左项——不是因为模型不重要，是因为**好 harness 不是替
模型思考，而是让模型在正确的坐标系里思考。**
