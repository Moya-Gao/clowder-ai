...
**第三代：管护工具链 + 多域联邦。** 解决"agent 怎么用"和"知识域怎么扩展"
的问题。把三个入口封装成 MCP 工具，agent 可以在任务中直接调用。同时建立
知识联邦——项目知识、全局方法论、外部领域知识库各自是独立的 collection，
有独立的治理规则和更新节奏，检索时通过 RRF 跨 collection 融合排序。
实践中已链接的域包括：项目本身的 spec/决策/教训、跨项目的技能和协作方法论、
一个与 AI 共创的虚拟世界设定集。架构上任何 git 仓库或结构化知识源都可以
作为新的 collection 接入——联邦是开放的。
