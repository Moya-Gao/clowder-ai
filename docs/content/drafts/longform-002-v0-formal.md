...
PR 冲突、review 变更等触发 urgent；常规对话和定时任务走 normal。产品负责人的方向
信号在产品语义上权重最高，但这是路由策略的判断，不是队列实现的硬编码层级。
...
（这个应该给别人）或升级（需要产品负责人拍板）。文本描述"我先看看"不构成
接球——只有实际的行动或显式的退回/升级才算完成了球权转移。

还有一种场景：agent 需要退出当前会话等待外部条件（CI 完成、产品负责人确认、
...
"治理规则"：它可能保留最近的代码细节，却压掉协作协议、操作红线、传球规则和质量纪律。
Agent 后半段突然违规，很多时候不是它故意违规，而是规则从它的世界里消失了。

我们的做法是把关键治理沉到压缩免疫层：身份、家规、传球规则、不可逆操作边界、质量纪律
...
治理还必须可见。产品负责人要能看见当前每个 agent 带着什么规则，因为很多治理不是纯
技术选择，而是产品选择：这个 agent 应该多主动？review 多严格？什么时候必须人类拍板？
...
关键约束也必须写清楚：这些逃生舱只在产品负责人当前指令中触发。引用、复述、讨论历史
...
- 记忆系统要治理 authority、activation、status，过期知识不能永远排在前面
