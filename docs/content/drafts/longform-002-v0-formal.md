...
*左上（高能力 × 低 harness）：**裸奔区**——大家最熟悉的场景：拿一个很强的模型，
直接对话或简单套壳。能出活，但每个 session 从零开始，没有积累。团队在这个区域
写的代码大多是脚手架（我们后面称为 "Build to Delete"）：详细的 prompt 模板、
多步推理引导、错误恢复样板——这些东西随模型升级而过时。*

*右下（低能力 × 高 harness）：**基建就绪区**——你说的就是这个："harness 做到了
目前的极致，但猫猫们还是跑不好。" 基础设施已经到位（git 接入、验证回路、记忆、
协议），就等更好的引擎来发挥。好消息是：这里的投入不会过时（"Built to Persist"），
模型一升级立刻享受现有基建的红利。*

*右上（高 × 高）：**乘数区**——强模型 + 成熟 harness，乘数效应活在这里。到这里
的路只有一条：两个轴都投。*

*行业的默认策略是沿着纵轴往上冲（换更强的模型），这意味着你一直待在左半边——
从死区冲到裸奔区，能力确实在涨，但永远享受不到乘数效应。只有同时向右移动
（投 harness），才能进乘数区。*
