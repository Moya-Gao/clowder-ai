...
行业大多数 multi-agent 讨论停在两个层面：prompt（怎么写提示词让 agent 更聪明）
和 orchestration（怎么编排 agent 之间的调用顺序）。我们在这两层也花了大量
时间——前五章都是这个范畴。
...
再跑测试、最后提交。文件写入成功了，但紧接着 stream 因为 provider 端的
grace window 过期而中断。用户看到一条错误信息。问题是：**文件已经写了，
...
我们遇到过一个真实的 split-brain：三个不同的 API 端点对同一只 agent 的
活跃状态给出了矛盾的回答——一个说"在线"，一个说"空闲"，一个说"未知"。
根因是三个数据源（进程内 tracker、持久化记录、草稿存储）各自维护了一份
liveness 状态，没有统一的真相源。进程重启后 tracker 被清空，但持久化记录
还卡在"运行中"——僵尸 invocation：没有对应进程的"活跃"记录。
...
四件套的 7 个实施阶段中 6 个已完成并合入：副作用分类器、副作用日志、恢复
决策引擎、cascade 健康评估、结构化恢复卡、持久化监管者 + 自动恢复分级全部
已实现。
