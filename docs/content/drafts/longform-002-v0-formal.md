...
我们的记忆系统解决的是一个不同的问题：多个 agent 在同一个项目里长期协作，
怎么让它们**不漂移、不重复犯错、不丢失已经学到的教训**？

这里的知识不是外部的——它是**团队自己产出的**。agent 写了一个 feature spec，
遇到了一个坑，提炼了一个教训，做了一个架构决策。这些知识散落在 spec 文档、
讨论记录、决策日志、教训总结里。记忆系统的职责是让这些知识**可导航、可搜索、
有治理**——不是把外部知识搬进来，是把团队自己的知识组织起来。

真相源仍然是仓库里的 markdown 文件——agent 能读、能改、能用 git 追溯。
数据库和向量索引只是编译层，坏了可以从 markdown 重建。这是一个有意的设计
决策：**真相源不能是只有系统能读的格式**。人类产品负责人要能直接打开文件
看到团队记忆里写了什么，而不是通过一个 dashboard 间接查看数据库里的 embedding。
...
恢复，可能是接手了一个不熟悉的任务。系统按时间倒序列出最近的文档、讨论、
记忆条目。这解决的是"发生了什么？我错过了什么？"
...
记忆系统不是一次设计出来的。它经历了四代演进，每一代解决上一代的核心缺陷：

**第一代：存储基座。** 解决"能存"的问题。建立两层 SQLite 架构——全局知识库
（只读，从技能/规则/共享记忆编译而来）和项目知识库（读写，项目内文档和讨论
的索引）。两层通过 RRF（Reciprocal Rank Fusion）融合排序。真相源是 markdown
文件，数据库是编译产物。

**第二代：治理层。** 解决"能找到对的"的问题。每条知识加上治理元数据：
**权威等级**（constitutional > validated > candidate > observed——架构决策
比观察笔记权威更高）、**激活模式**（always_on / scoped / query / backstop
——不同知识在不同场景下被检索）、**生命周期状态**（active / review /
invalidated / archived——知识会过期，过期的要标记）。

**第三代：管护工具链。** 解决"agent 怎么用"的问题。把三个入口封装成 MCP
工具，agent 可以在任务中直接调用。同时建立知识联邦——不同 collection
（项目/全局/外部）可以独立管理，检索时跨 collection 融合。

**第四代：反馈闭环。** 解决"怎么知道有没有用"的问题。这是最关键的一代
——不是因为它技术上最复杂，而是因为它闭合了一个之前敞开的循环。

### 用行为信号而非自评

第四代的核心创新：**用 agent 的真实行为判断知识的价值，不用 LLM 自评打分**。
...
**RAG**：数据源是外部文档，没有治理，没有反馈闭环。解决"上下文不够"——
把外部知识搬进来。知识的消费者和生产者分离。
...
没有治理，没有反馈闭环。解决"当前任务需要找资料"——实时获取外部信息。

**我们的记忆系统**：数据源是项目内文档 + 讨论 + trace（agent 自己产出的），
...
RAG、Wiki、Agentic Search 的知识都来自系统外部。我们的知识来自团队自身的
实践——这意味着知识的质量和团队的实践质量直接挂钩。
...
我们做的是**记忆运行时 + 项目现实导航**——不只编译知识，还追踪知识的使用
效果，并用使用效果反向优化知识的可发现性。

**〔 Figure 6 — 四种 Memory/Retrieval 范式对比 + 反馈飞轮 〕**

*低保真占位，待砚砚生成。四列对比（RAG / LLM Wiki / Agentic Search /
Cat Café 记忆）× 五维（数据源 / 治理 / 反馈闭环 / 知识归属 / 解决问题）。
突出 Cat Café 独有的"生产者=消费者"和"运行时结果反馈"两条。*
