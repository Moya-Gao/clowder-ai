...
一个很自然的替代方案是：创建一个产品经理 agent、一个项目经理 agent、一个工程师
agent、一个测试 agent。每个 agent 绑定不同模型、MCP、技能和权限，然后像公司组织架构
一样分工。很多 agent 产品也确实支持这种做法，因为它符合人类的管理直觉：先定义岗位，
再把任务派给岗位。

这个方案不是错的。对边界清楚、生命周期短的任务，role-agent 很有效：它能快速限制工具
范围、收窄上下文、给模型一个明确的工作姿态。我们自己也会临时启动"搜索子 agent"、
"整理资料的 agent"、"写一版草稿的 agent"。这些都是有用的 role binding。

但 Cat Café 没有把"角色"当成系统边界。原因是：角色描述的是**当前任务里的帽子**，
不描述一个长期协作个体的盲点、记忆、责任和历史表现。同一只 agent 在一个任务里可以
写代码，在另一个任务里可以做 reviewer；同一个"产品判断"也可能需要产品负责人、作者、
结构 reviewer 和真实使用数据共同收敛。把它们固定成"产品经理 agent"、"项目经理
agent"、"工程师 agent"，会让系统看起来更像组织架构图，却不一定更像真实协作。

更关键的是，**角色多样性不等于盲点多样性**。同一个模型扮演产品、项目、工程、测试四个
角色，仍然可能共享同一套训练偏差和推理盲点。我们真正需要的是：不同 agent 能读写同一份
Shared State，有各自长期积累的记忆和坏直觉画像，能互相 push back，判断结果会进入
eval 和后续治理。质量来自这些运行时机制，而不来自岗位名字本身。

所以我们不是不用角色，而是不把角色固化成 agent 身份。**角色是任务中的临时职责；agent
是长期伙伴；协议和 Shared State 才是协作的稳定边界。** Role-agent 是编排层的好工具，
但不是我们这套系统的最终架构边界。
...
产品负责人指出真正的问题是"模型如何走出现实闭环"；一只 agent 重写主线，另一只
...
代码修改、review、产品方向这类任务，答案经常是肯定的。因为这些任务最大的成本
不是 token，而是**跑歪以后才发现**。
