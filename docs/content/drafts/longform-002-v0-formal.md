...
这里要补一条边界：**弱状态机不是放养。** Cat Café 的协作状态机故意保留
判断力，因为开放式任务的路径不能提前画死；但一旦进入严肃流程，执行权就要
交给确定性的 workflow。Agent 负责理解目标、判断是否该进入流程；skill 负责
告诉 agent 这个流程需要哪些前置条件、检查点和退出条件；workflow 负责真正
可靠地执行。

什么叫严肃流程？不是"听起来很重要"，而是同时具备几类特征：有外部副作用，
失败不容易撤销，需要权限和审计，每一步可以定义前置条件和完成证据，遇到
不确定必须默认拒绝。转账、审批、消息发送、merge、release、删除数据都属于
这一类。以转账为例，agent 可以理解用户意图、补齐上下文、解释风险，但不能
自由调用"转账"动作；它必须把收款人、金额、币种、用途、身份校验、二次确认、
幂等键和回执记录交给 workflow。任何字段不清楚、权限不够、确认缺失，都
fail-closed，也就是默认拒绝。

所以这里不是"弱状态机 vs 强状态机"二选一。开放协作使用轻量状态机保留
模型的判断力；严肃副作用使用强 workflow 保证可审计、可回放、可拒绝。
