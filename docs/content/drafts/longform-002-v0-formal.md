...
教十次传球"。我们把全部治理规则迁到压缩免疫的 system role，然后删除旧的
prepend 路径。脚手架的完整生命周期：补缺口 → 标注平台依赖 → 平台升级 →
迁移 → 删除。
...
**脚手架被当成基础设施投了六轮**。一个全局配置文件泄漏了跨线程上下文，引发
身份混淆。根因（Session 跨 thread 污染）修完后，团队"顺手"也修触发器——用
替换 HOME 环境变量的方式隔离 CLI 全局配置。第一个补丁引发认证失败，第二个
...
> 左列 Build to Delete：user message prepend 注入（已删除 → 迁移至压缩免疫层）、
> 格式纠正指令（逐条瘦身中）、hotfix 补丁（2 周强制升级审查）。右列 Built to
> Persist：恢复基础设施（side-effect 日志 + 存活探针 + 恢复卡片）、消费加权
> 记忆排序、跨角色 review 协议、4 核方法论框架。中间"分类错误"区：6 补丁链 →
> 应为脚手架却按基础设施加码；内存 Map 模拟 → 应为基础设施却按脚手架草率投入；
> 个体名硬编码 → 类别对但编码粒度错。底部标注："Delete 的时机由谁判定？→
> 第 5 章 评估架构"。
...
场景：产品负责人在微信群发了一句话——"F207 的设计稿我看了，可以开工。"

**State：消息如何变成 agent 的上下文。** 这句话不是直接被 agent 看到的。它先经
IM connector 接收，转成结构化消息写入统一的调度队列。队列按来源做了优先级分层
——产品负责人直接发的高于 agent 间异步通知，异步通知高于定时任务和外部
connector。这不是公平调度，是显式的不公平：人的消息在任何时候都不应该排在
agent 聊天后面等。
...
起始信号，但谁来开工需要显式声明。Agent 调用结构化的持球工具（不是在消息里
写"我来做"）声明自己接球，附带持球原因、计划的下一步和预估时长。文本描述
"我先看看"不构成持球——只有工具级的声明才进入状态机。

这个设计是被失败逼出来的。早期允许用文本声明持球，频繁出现**虚空持球**——
agent 说"我来做"，然后没有后续动作，但其他 agent 都认为有人在做。用分布式系统
的术语说：这是一个没有心跳的锁。后来我们把持球声明升级为必须伴随工具级凭证
——相当于强制 lease + heartbeat。
...
消息行首（`@reviewer 请 review PR #1234`），行首是硬要求——嵌在句子中间的 @
只是叙述提及，不触发路由。Reviewer 拉取 shared state、读 diff、产出结论：approve
或 blocking。没有第三种。"approve 附带后续建议"是被明确禁止的中间态——
reviewer 是守门员，不是 cheerleader。

**Route：传球给谁不是猜的。** Review 通过后，球怎么传？三个合法选项：继续持有
（我还有事要做）、传给指定 agent（行首 @）、升级给产品负责人。不合法的是"传给
大家看看"——那等于把球扔地上。
...
![Figure 6：一条消息驱动的 TeamAct](assets/longform-002-figure-6-teamact-in-action.png)

> 顶部：产品负责人微信消息 → IM Connector → 统一调度队列（标注优先级分层：
> 人 > A2A > connector > cron）。中部：Agent spawn → 读 Shared State（git repo +
> 记忆系统 + 交接胶囊）→ hold_ball（工具级声明）→ Action（commit + test +
> trace）→ 行首 @ 路由给 reviewer → Reviewer 读 Shared State → approve /
> blocking → Route（继续持有 / 行首 @ 传球 / 升级产品负责人）。侧面通道：
> 跨线程路径——Thread A agent → cross_post_message(threadId_B) → Thread B
> agent 收到（含回复线索）→ 回复路由回 Thread A + 同步写入 feature 文档。
> 底部标注三个保护机制：超时检测 → 自动升级；乒乓球熔断（连续传球 ≥ 4 次 +
> 零实质工具调用 → 中断）；虚空持球检测（文本声明无工具凭证 → 告警）。

**为什么不能用简化版。** 如果没有 shared state，每个 agent 的上下文只来自对话历史
——压缩后丢失，每轮都从零开始。如果没有显式路由，agent 之间的任务归属退化为
"谁先看到谁做"——无法保证每个任务有且只有一个 owner。如果有路由但没有工具级
持球声明，状态机退化为聊天约定——虚空持球、球掉地上、乒乓球互传这些失败模式
全部回来。

每一层都是被上一层的失败逼出来的。这不是过度工程——它是**分布式系统里的标准
治理层**：shared state 对应一致性，显式路由对应消息投递保证，持球声明对应
分布式锁，超时升级对应 failure detector。只是这些概念长了一张 agent 协作的脸。
...
**〔 Figure 8 — 四种 Memory/Retrieval 范式对比 + 反馈飞轮 〕**
...
| 组件 | 解决什么 | 借鉴的分布式系统原则 |
|------|---------|----------------------|
| **持久化监管者** | 任务状态与通道解耦，不过期 | 借鉴 Saga coordinator 的状态持久化 |
| **副作用日志** | 每个外部操作被 journal，恢复时知道"哪些已做" | WAL-like effect journal |
| **安全恢复** | 幂等 + 不重复副作用，按风险分四级 | idempotency + dedup + fail-closed 逼近 exactly-once |
| **结构化恢复卡** | 中断时给用户结构化状态，不是 free-text 错误 | 补偿/恢复计划视图 |
...
分级原则是 fail-closed：遇到未知操作类型默认归入最高限制，不是最低。
...
→ Route。Route 阶段一只 agent 把球传给另一只，这个切换瞬间是**最脆弱的
...
我们遇到过真实的 liveness split-brain：两个后端读路径对同一 invocation
给出矛盾结果，根因是三家数据源语义不同却被各自当真相源。解法是单一规范
读模型——一个函数整合所有信号源，用联合类型标记信号组合，在每种组合下
定义判定逻辑。不是"最新的赢"——是"看到什么组合就做什么判断"。
...
× 运行时契约 × 事件适配器），监管者作为独立 sidecar 运行，不和任何具体
...
单 agent 长任务可靠性的主要实现切片已合入（Phase A–E + Phase F G1–G7），
但 F201 仍未 close——剩余硬门禁是真实 alpha 长任务验收和 close report。
设计基于真实事故分析，但从"基于事故设计"到"经过充分验证"还有距离。

跨 agent 协作的 liveness 规范读模型已落地并在生产运行。

跨 provider 的统一宿主抽象有架构决策，但从"有决策"到"所有 provider 都接入"
还有距离。
...
前六章讲的是 How——identity、TeamAct、harness 五层、记忆联邦、eval 飞轮、
可靠性。这一章退一步问 Why：多 agent 协作到底什么时候赚？什么结构下才赚？
以及，有没有哪个功能是算法永远做不到、只能留给人的？

### 跨厂商多样性是结构性质量来源

最直觉的假设是"用同一家最强模型做所有角色"。但同一家模型共享训练数据
和注意力分布，盲点是相关的。两只 Claude agent 都认为一个递归方案没问题，
换一只 GPT agent review，注意力落在不同的位置——抓出了两个 P1 bug。

这不是个案。学术分类框架（安波教授在华为闭门研讨会上给出的四维 taxonomy：
Agent Composition × Communication Topology × Control Flow × Interaction
Protocol）明确区分了 homogeneous 和 heterogeneous agent 系统。他的现场
判断是：**异构协作在 3–5 agent 时常见边际收益最大**；超过 10 个 agent
的场景，很多会退化为同构 ensemble（并行投票/搜索），协调成本快速上升。

Cat Café 的实际协作不是"四只猫围坐一张桌子"。日常状态是十几个 thread
并行：有的在写代码，有的在剪辑视频，有的在做 PPT，有的在做金融分析，
有的在帮家人看健康报告，有的在帮朋友出博士实验设计。每个 thread 里协作
的猫一般不超过 5 只——恰好在安波描述的 sweet spot 里。不是设计出来的——
是在实践中发现同家族 review 总是漏同类错误，才引入跨厂商 reviewer；也是
因为超过 5 只猫的协调开销让 thread 变得迟缓，才自然收敛到这个规模。

跨厂商的价值不在于"谁更强"，在于**降低盲点相关性**——不同训练分布和工具
习惯让 reviewer 更可能看到 author 没看到的地方。

### 协作的正收益条件

多 agent 什么时候会亏？这个问题比"多 agent 好不好"更值得回答。

用一个参数化的思想实验来建立直觉（是 sensitivity analysis，不是实测数据）。
假设 author 正确率 80%，reviewer 抓错率 50%（能发现一半的错误），误伤率 2%
（把对的改错）：

```
P(最终正确) = 0.80 × 0.98 + 0.20 × 0.50 = 0.884
```

从 80% 提到 88.4%。加第二轮不同模型的 reviewer（抓错率 40%，误伤率 1%）：

```
0.884 × 0.99 + 0.116 × 0.40 = 0.921
```

两轮 review：80% → 88.4% → 92.1%。收益递减但方向稳定。

核心不等式：**只要 reviewer 抓错率 > 误伤率，每多一轮 review 就在赚。**
数字会随模型、任务、领域变化，但不等式的方向是稳定的。

第二个结构性因素是**信息保真率**。纯靠消息传话（每次保留 95% 信息），传
10 次：0.95¹⁰ ≈ 60%。有共享状态（所有 agent 读同一份文档，每次保留 99%）：
0.99¹⁰ ≈ 90%。这就是 Ch.2 强调 shared state 的数学理由——不是偏好，是保真
率乘积效应下的必然选择。

什么时候会亏？四种结构：

1. **盲传**——后手不是在纠错而是在重做，每一棒乘一个 < 1 的系数
2. **伪拆分**——任务拆了但每个子任务没变简单，白白多了协调税
3. **同质化**——所有 agent 是同一个模型，reviewer 和 author 犯同一类错
4. **协调税超过收益**——正确率 +12% 但 token 成本 3x、wall clock 2x

第 3 条呼应上一节的跨厂商多样性——同质化不只是"浪费"，是在结构上
取消了 review 的统计独立性，让正收益条件失效。

判断标准：**每多一棒，到底是在增加纠错能力，还是只在增加协调税？**
赚就留，不赚就砍。

### 愿景守护不可算法化

前面讲的 reviewer 抓错、保真率提升、diversity 增益，都可以工程化。但有一件
事不能：**判断当前方向是否偏离了产品愿景。**

这是一个工程上无法完全自动化的判断问题。规则可以捕捉局部偏差（"这个函数
命名不符合规范"），但"现在还在不在解决正确的问题"必须回到愿景的定义者。
就像编译器能检查语法错误但不能判断程序是否在解决正确的问题。

Cat Café 的做法是把 CVO（铲屎官/产品负责人）定位为愿景的唯一 oracle：

- **Magic Words** 是 CVO 的手动拉闸——"脚手架""绕路了""第一性原理"——每一个
  都指向一类系统性偏差，但触发条件是 CVO 的判断，不是规则
- **愿景守护**是每个 Feature Phase 结束时的人类 review，不是 CI check
- CVO 不参与日常执行（那是 agent 的事），只在方向层做裁决

CVO 不是因为在流程里所以重要——是**因为只有人类能定义什么值得做，才必须在
流程里**。这一点不会因为模型变强而改变。GPT-10 可能写出比任何人更好的代码，
但"这个产品应该往哪走"不是代码能力问题。

学术界对此有一个更精确的说法：agent 系统的 alignment 问题不在单次输出的
正确性（那是 eval 的事），在于**持续方向的一致性**——而方向只能由方向的
定义者来校准。

### 最小必要复杂度

六层架构、多套协议、跨厂商协作——这不是极简。我们追求的从来不是"少"，
而是"不多余"。

每一层在解决一个真实的、去掉就会出故障的问题维度。Identity 去掉了 agent
角色混乱；TeamAct 去掉了球权掉地；harness 五层去掉了压缩后失忆；记忆联邦
去掉了检索噪音；eval 飞轮去掉了优化方向盲区；可靠性基础设施去掉了中断后
状态丢失。这是这个问题空间下的**最小必要复杂度**。

但最小必要复杂度不是静态的。它在两个维度上漂移：

**能力维度**：当模型原生支持跨会话记忆、原生支持多 agent 感知时，当前的
harness 层应该坍缩——强行保留已被原生能力覆盖的 harness 层，是在堆砌历史
的补偿代码。

**认知维度**：当新模型（新家族、新架构、新训练数据）加入团队，shared rules
需要重新翻译成新的认知基底能理解的形式——翻译不到位，规则就变成模型的认知
噪音而不是认知路径。
...
框架在试图独立提升左项（用结构补能力）。真正的乘数效应在右项（环境适配度）。
模型能力是上游变量，harness 契合度是团队自己能建设、能复用、能复利的变量。
真正的工程杠杆在右项——**好 harness 不是替模型思考，而是让模型在正确的
坐标系里思考。**

最优表达在正确坐标系下必然最简。如果方案需要那么多层，先问坐标系选错了没有。

### 送出去一句

> **脑子会进化，闭环不会过时。**

模型架构会迭代——Transformer、MoE、Mamba、任何未来的东西。脑子越来越聪明，
是确定的趋势。但 identity、记忆、协作、底线、协议、可观测性、恢复能力——
这些都不在权重里。它们在 harness 的现实闭环里，在 agent 与世界交互的
接口里，在人类定义方向的那个不可算法化的节点里。

102 天、200+ Feature、四只猫三家族——我们走完了一条从"给 LLM 写 prompt"到
"工程化 agent 协作系统"的路。这条路上最重要的教训是：**不要卷模型能力，
卷环境。** 模型能力是上游给的，环境适配是你自己建的。前者你控制不了，
后者是你唯一能控制的乘数。
