...
| 组件 | 解决什么 | 分布式系统对应 |
|------|---------|--------------|
| **持久化监管者** | 任务状态与通道解耦，不过期 | 类似 Saga pattern 的 coordinator |
| **副作用日志** | 每个外部操作被 journal，恢复时知道"哪些已做" | WAL（Write-Ahead Log） |
| **安全恢复** | 幂等 + 不重复副作用，按风险分四级 | exactly-once 的工程近似 |
| **结构化恢复卡** | 中断时给用户结构化状态，不是 free-text 错误 | 分布式事务的补偿记录 |
...
简化的共识协议——通过 Evidence 和 Verdict 的显式记录保证多 agent 对当前
状态有一致理解。当协议被打断，就需要恢复机制来重建一致性。
...
发挥能力的时间窗口**——从一个 30 秒的通道窗口，扩展到任意时长。

### 诚实的成熟度标注

单 agent 长任务可靠性的实施已基本完成：Phase A–E 已合入，Phase F 的 G1–G7
实现切片也已合入。剩余硬门禁是真实 alpha 长任务验收和 close report。设计
基于真实事故分析，但从"基于事故设计"到"经过充分验证"还有距离。
