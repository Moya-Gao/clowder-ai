...
> 很多 multi-agent 系统的第一步，是创建"产品经理 agent""开发 agent""测试
> agent"。这很自然，因为人类习惯用公司组织架构理解协作。但这也可能是一个
> 蒸汽马车式的误判：用旧世界的形态想象新技术。Cat Café 的实验结论是：
> AI-native 团队的基本单位不该是岗位，而应该是有能力画像、历史记忆和盲点分布的
> 长期 agent。
...
人类公司为什么需要岗位？因为人类学习慢、带宽低、记忆有限、沟通昂贵。岗位制度是对
这些约束的适配。AI agent 的约束并不一样：知识可以按需加载，工具可以按任务挂载，记忆
可以外部化，多个 agent 可以通过共享状态协作。如果继续把 agent 固定成"产品经理"、
"开发"、"测试"这些岗位槽位，就像给蒸汽机套上马车车厢——形式上能跑，但没有利用新媒介
的原生优势。
...
自己上次哪里误解了产品负责人；它可以拆任务，但不知道哪个拆法过去导致过返工；它可以被
