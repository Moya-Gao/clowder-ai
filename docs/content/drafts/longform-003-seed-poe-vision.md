...
**补充：FDE 前半段已拆为 longform-004（此处只留接口段）。**
上表讲的是“部署后”从真实轨迹里自动适配（Evolution FDE）。它的姊妹命题——“部署前 / 立项前”如何把一个行业用户的混乱现场语言蒸馏成第一条可验证产线（Discovery + Build FDE）——已独立成 [Longform-004：Workflow Distiller / 二阶 harness](longform-004-seed-workflow-distiller.md)。

> **接口段：003 讲「环境从轨迹里学习」；004 讲「先帮你制造第一条可验证轨迹」。** 二者同构（分层先验 + 验证器 + 选择压力），只是时间相位不同。
