...
### 分层可变性治理（A 粒度 × C 主体的投影，48 × 砚砚）

把 A 粒度（学到哪层）× C 主体（谁能动）交叉成一张权限矩阵——这是 AutoHarness 作为「会变的 harness」的专属技术挑战：会变，就必须回答**谁有权让它变、能变哪层、错了怎么发现 / 回滚、会不会串味**。

| 层级 | 谁能改 | 改什么 | 不能越过 |
|---|---|---|---|
| AutoHarness kernel | 我们 / harness 架构师（Landy） | 状态机、权限、验证器框架、回滚机制 | 客户不能直接改 |
| 行业包 | harness 专家 + 行业专家 | 行业 baseline、常见流程、通用 QA | 不能覆盖企业合规 |
| 公司层 | 企业落地工程师 / 管理员 | 审批、命名、交付格式、品牌、数据边界 | 不能破坏 kernel 安全 |
| 团队层 | 团队负责人 / domain champion | 分工、handoff、团队 SOP、模板 | 不能绕过公司规则 |
| 个人层 | 个人用户 | 工作偏好、质量阈值、输出风格、提醒 | 不能跨人泄露 / 覆盖合规 |
| 任务层 | Agent + 用户 checkpoint | 本次任务参数、约束、临时策略 | 高风险改动必须升级 |

**可变性梯度（第一性原理，48）**：越靠 kernel，越少人能改、blast radius 越大、验证要求越严；越靠任务层，越多人能改、影响越小、可逆性越高。**权限收紧程度 ∝ 影响半径。**

**为什么必须 code-as-harness（48）**：分层权限、版本化、回滚、blast radius 隔离，全依赖 harness 是**代码**——代码有模块边界、类型契约、测试、git 版本。prompt 大杂烩没有这些，做不出可治理的分层。这是 code-as-harness 不只「能执行」、而是「能被治理地演化」的根本原因。

**golden path（一体两面）**：分层权限定义「能改什么的边界」，golden path 定义「边界内怎么走」——用户不被丢进空白 workflow builder，系统先带行业地基进场，再用历史工件补 delta，只在关键 checkpoint 让人选。两者合起来 = 可治理的自定义。

这也精确回答了铲屎官那串「谁做什么」：Landy 改 kernel、落地工程师改公司层、行业专家改行业包、个人改个人层——每层 mutability surface 不同，这就是「学问」所在。
