...
**经济学指标时机**：砚砚裁"跑前预注册（secondary）"；46 初判"机制存在性阶段是噪声，deferred"。并集解：**预注册但定性为被动遥测**——token 成本/等效人工分钟数从 harness 日志免费记录（事后补采不可能，物理不可逆），**不进成功判据、不反推实验设计**。46 确认放行（2026-06-09）："三方实际上没有真分歧，只是措辞精度差；secondary 在实验设计语境下就是'记但不判'，§5 bis 当前措辞不用改。"砚砚的 secondary 定级同义。**决议：spike plan 维持 `d42a09efa` 措辞，零改动。**
