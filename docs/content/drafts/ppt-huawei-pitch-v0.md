...
**客户视角（分级，不是"全自动"）**：
1. **任务级摩擦 → AI 自己修**：限定在那个任务的工作流（如 commit 流程），可回滚、有跨 AI 审查、全程留 commit 记录
2. **系统级问题 → 用户一键提单，人来拍板修**：稳定性/安全/基础设施相关的问题，走提单流程（一键提交 + AI 接单调查 + 人审批），有工单、可追溯
3. 修复过程在后台进行，**当前工作不中断**
...
> **48 设计（v4 新增）**：这是 AutoHarness 区别于所有竞品的命门。
> 别人的 autoharness 靠刷 benchmark 验证进化效果（会被作弊/被刷分）。
> 我们靠三层真实信号，按可信度分级。**PPT 上去黑话，不写 F192/F222。**
...
| 层 | 怎么判 | 谁判 | 可靠度 |
|---|---|---|---|
| **客观事实** | 代码能不能跑、测试过没过、改动被采纳还是被推翻 | 机器自动判 | 最硬——有客观对错 |
| **专家校准** | 没有标准答案的事（设计好不好、方案合不合适），由负责人定期抽样评判 | 人在环 | 高——但频率有限 |
| **行为信号** | 从用户真实操作读线索——哪步喊停了、反复纠正什么、取消了什么 | 当线索用，不当判决 | 弱——但量大且免费 |

**对领导的杀手句**：
> "AI 不是自己说自己对——它用**客观事实兜底、人类专家校准、用户真实行为导航**，三层验证。改不对，会被这三层挡下来。"

**落地证据**（口头说，不放 PPT 文字里）：
> "这套信号采集系统已经在生产环境运行。4 个信号源在持续采集：用户取消行为、拉闸词触发、代码合入/回滚结果、重复纠偏模式。不是画饼，是上周刚合入的真实系统。"
