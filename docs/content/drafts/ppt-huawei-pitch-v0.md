...
status: v18-p2-discovery-agent-interview
...
> v17：铲屎官指出讲稿缺 P2“企业用户能感知到什么”。第二页从技术目标占位改成企业用户可感知变化 + FDE 三段压缩；“自己改自己凭什么可信”降为第二页后半治理边界，不再单独抢页。
> v18：修正 P2 Discovery 口径：不是把 FDE 访谈跳过，而是把真人 FDE 的访谈/建模能力下沉给 agent；agent 先读历史工件，再带着行业 baseline 追问 delta / 验证点，业务专家只在 checkpoint 校准。
...
> **对领导的杀手句**：我们不是卖更厉害的 FDE 人天，是卖一个**让你们自己的业务专家也能当 FDE**的环境。重复 FDE 人天，变成一次性 bootstrap + 业务专家 checkpoint + 算力。
>
...
> 变成环境先读工件、agent 追问校准、上线后从员工轨迹持续学习。
> **重复 FDE 人天，变成 bootstrap + checkpoint + 算力。**"
...
> **v17 重构**：第二页不再继续讲 L1-L5 或内部技术挑战。第一页已经回答“行业在哪 / 我们怎么做”，第二页必须回答企业听众最关心的问题：**这东西落到我公司，我和我的团队实际能感知到什么变化？**

### 【左半：用户可感知的五个变化】

| 企业用户今天的痛点 | AutoHarness 后的感知 | 背后机制 |
|---|---|---|
| 上来先教 AI 背景、术语、交付标准 | **不是空白 prompt**：进场自带行业通法，像懂行业基本功的新同事 | 行业默认 harness / golden path |
| 每家公司规矩不同，通用 AI 不懂现场 | **先读再追问**：agent 先读被毙方案、被改产出、被拒理由，再只追问 delta / 验证点 | Discovery：历史项目 + agent 访谈 → 公司规矩 / 团队分工 / 任务约束 |
| 同一任务在不同团队、不同负责人那里标准不同 | **越用越懂组织默契**：行业默认 + 公司规则 + 团队习惯 + 个人工作偏好 + 当前任务上下文组合执行 | 分层偏好 / 规则 / 上下文注入 |
| 新业务场景每次都等 FDE / 顾问排期 | **业务专家能自己长能力**：沿 golden path 把好方案、坏方案、修改理由沉淀成 SOP / 检查器 / skill | Build：业务专家 + 环境共建 harness |
| AI 越用越黑箱，没人知道为什么变了 | **变化可见、可回滚、可移交**：每次 harness patch 都有证据、版本、验证和回滚路径 | code-as-harness / review / audit / rollback |

> **口径**：企业用户不需要感知“六层架构”。他们感知到的是：它更懂行业、更懂公司、更懂团队、更懂个人，而且这种贴合不是黑箱，是可治理的。
...
这部分不要抢成独立页，只作为第二页过桥或 Q&A：用户感知是“持续贴合”，管理者要看到“持续贴合背后不失控”。

1. **真值 / 验证器问题**：Agent 自己改 harness，凭什么信它改对？真实轨迹先变成证据，用户自然决策、自动 eval、tracing 异常、代码合入、回滚、重复异常分层处理；行为频率只做线索，不直接判决。
2. **进化生命周期**：patch 要版本化、能继承、会退役；模型升级后，新模型自己能做对的旧补丁要主动 sunset。
3. **分层可变性治理**：行业包、公司规则、团队 SOP、个人偏好、任务策略分层授权；每层明确 blast radius、验证强度、回滚路径和 golden path。

**一句话收束**

> 企业用户感知到的是持续贴合；企业管理者要看到的是这套贴合背后有真值、有生命周期、有权限边界。
