...
不要一上来讲 L1-L5，也不要先解释 code-as-harness。那会像内部术语。先用一句听众能立刻理解的话开场：

> **今天我不想演示一个"会生成东西"的 AI。现在会生成的 AI 很多。我要演示的是另一件事：一个 AI 工作环境，在真实使用中发现摩擦、留下证据，然后把这些摩擦变成自己下一版的能力。**
...
>
> 我先说一个判断：过去一年，AI 产品最常见的形态是"你描述需求，AI 帮你生成一个东西"。比如生成一个页面、一个流程、一个低代码系统、一个 agent 工作流。这已经很有价值，但它有一个断点：**生成之后，如果真实使用中出了问题，通常还是人来维护、调参、改 prompt、改流程。**
>
> 我们想做的不是再做一个更会生成的 Agent 平台，而是往后走一步：**让这个工作环境在真实使用里自己发现哪里不好用，把用户的取消、拉闸、采纳、回滚、重复纠偏变成证据，再生成可验证、可回滚的 harness 改动。**
>
> 一句话记住我们和别人的区别：**别人的 AI 越用越旧——生成完就定型，之后靠人维护；我们的 AI 越用越好——它把每一次摩擦，改成自己下一版的能力。**
>
...
> 今天我想讲的不是"AI 能不能生成"，而是"AI 生成出来以后，能不能在真实工作里持续变好"。

### B. 更抓眼版

> 普通 agent 被用户纠正以后，最多下次回答得更礼貌一点。我们想做的是：它被纠正以后，能写代码修自己的工作环境。
...
诊断：钩子讲清了"问题"（生成 ≠ 持续变好），但缺一句 **"我们 vs 别人"的对比定锤**——竞争力是对比，没对比听众记不住"独一份"。这句放 60s 收尾、接 120 天硬证（已插入推荐开场 60 秒）：

**主推**（对仗、能复述、非技术秒懂）：
> **别人的 AI 越用越旧**——生成完就定型，靠人维护；**我们的 AI 越用越好**——它把每一次摩擦改成自己下一版的能力。

**变体**（按现场口语习惯选）：
- B：市面上的 AI 帮你生成一次就交给你；我们的 AI 工作环境，自己在真实里跑了 120 天，越跑越懂你、越跑越稳。
- C：别人做"会生成的 AI"，我们做"会自己变好的 AI 工作环境"——而且不是 demo，是真实 120 天的活系统。

**用法**：这句是**第一页右半（AutoHarness 方案）竞争力的开场预告**。开场砸下"越用越旧 vs 越用越好"，第一页右半再展开"为什么我们能做到（六层 + 自进化 + 验证器命门）"。听众带着这句进第一页，右半就有"啊这就是他们的杀手锏"的落点——解决你说的"说了但没重点说、没深刻感受"。
...
>
> 这里最重要的不是模型本身，而是真实轨迹。用户取消、拉闸、采纳、回滚、重复纠偏，这些平时看起来像摩擦的数据，在 AutoHarness 里会变成下一次系统进化的原材料。

### 左半不要讲的坑

- 不要说"我们已经 L4/L5"。
- 不要说"完全自动修自己"。
- 不要把竞品讲成弱智；只说它们停在不同责任层级。
...
> 这一页只讲一个核心命题：行业已经会生成，但还没有普遍解决"生成以后如何在真实使用中持续变好"。AutoHarness 的目标，就是把真实工作轨迹变成可验证、可回滚、可治理的 harness 进化。
