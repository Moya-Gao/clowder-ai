...
status: v2-auditable-evolution-opening
...
不要一上来讲 L1-L5，也不要先解释 code-as-harness。那会像内部术语。先承认行业语境：self-improving / AutoHarness 已经被 Hermes 等产品讲火了；我们的切口不是"我也会进化"，而是"进化能不能被审计、验证、回滚、治理"。

> **今天我不想演示一个"号称会自我进化"的 Agent。这个词已经很热了。我要演示的是更具体的一件事：一个 AI 工作环境，能不能在真实使用中采集信号、留下证据，并把改进变成可验证、可回滚、可治理的 harness 变更。**

这句话的作用：
- 避开"又一个 Agent 平台"的预期。
- 避免错误声称"行业没人做 self-improving"；承认概念已热，转向工程化证明。
- 把听众从"宣称会进化"拉到"进化能否被审计和治理"。
...
>
> 我先说一个判断：过去一年，"self-improving agent"已经不是新词了。Hermes 这样的产品把这个方向讲得很热，很多产品都会说自己会记忆、会学技能、会自我改进。问题是，到了企业场景里，不能只听"会进化"这个词，必须继续追问：**进化信号从哪里来？是人的纠偏，还是系统 eval、tracing 异常、代码合入/回滚这些世界结果？这些改进能不能被审计、验证和回滚？**
>
> 我们想做的不是再做一个更会营销"自进化"的 Agent，而是把自进化变成一条工程闭环：**从真实轨迹里采集信号，把信号归因成证据，再生成可验证、可回滚、可治理的 harness 改动。这里的信号不只来自人，也来自自动 eval、tracing、代码合入、回滚和重复异常。**
>
> 一句话记住我们和别人的区别：**不是宣称会进化，而是让每次进化都有账可查、有尺可验、有闸可停。**
>
...
> 今天我想讲的不是"AI 会不会说自己在进化"，而是"这个进化能不能被审计、验证和回滚"。

### B. 更抓眼版

> 很多产品会说自己 self-improving。我们关心的是：它每一次 self-improve，有没有信号、有无证据、能不能上线、能不能回滚。
...
诊断：原来的"别人越用越旧、我们越用越好"太大白话，而且会误伤 Hermes 这类已经在讲 self-improving 的产品。新的定锤句要承认行业已经在讲"会进化"，但把我们的竞争力压到**可审计工程闭环**上。

**主推**（稳、硬、可复述）：
> **不是宣称会进化，而是让每次进化都有账可查、有尺可验、有闸可停。**

**变体**（按现场口语习惯选）：
- B：从 self-improving 的口号，到 auditable evolution 的工程闭环。
- C：别人讲的是 AI 会不会进化，我们讲的是进化能不能被治理。
- D：一次改进必须走完整链路：signal → evidence → patch → review → rollout → rollback/sunset。

**用法**：这句是**第一页右半（AutoHarness 方案）竞争力的开场预告**。开场砸下"进化可审计"，第一页右半再展开"为什么我们能做到（六层 + 适配入口 + 执行公式 + 验证器命门）"。听众带着这句进第一页，右半就有"啊这就是他们的杀手锏"的落点。
...
>
> 这里最重要的不是模型本身，而是真实轨迹。真实轨迹不只包括人的取消、拉闸、采纳、回滚、重复纠偏，也包括系统自动 eval、tracing 异常、代码合入/回滚这些世界结果。它们在 AutoHarness 里会变成下一次系统进化的原材料。

### 左半不要讲的坑

- 不要说"我们已经 L4/L5"。
- 不要说"完全自动修自己"。
- 不要说行业没人做 self-improving；Hermes 这类产品已经把概念讲火了。我们讲的是"可审计、可验证、可回滚"。
...
> 这一页只讲一个核心命题：行业已经会讲 self-improving，但企业真正需要的是可审计、可验证、可回滚的自进化。AutoHarness 的目标，就是把真实工作轨迹变成可治理的 harness 进化。
