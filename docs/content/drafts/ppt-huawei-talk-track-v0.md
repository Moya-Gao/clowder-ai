...
status: v6-p2-enterprise-user-perception
...
>
> 所以这件事的商业含义是：过去企业定制最贵的是 FDE 或顾问去理解客户、搭第一版、长期手工调优。我们想把这件事下沉给环境：agent 先读工件、主动追问，业务专家做 checkpoint 校准，上线后环境继续从真实轨迹学习。简单说，**把重复 FDE 人天变成一次性 bootstrap、业务专家 checkpoint 和算力。**
...
> 你们刚才看到的这些不是模拟数据，而是我们真实 thread、真实记忆、真实 eval 和真实提交历史。第一页先证明这套系统不是概念图；第二页我会先换成企业用户视角，讲它落到客户那里到底能感知到什么变化；第三页再打开完整案例，证明这些变化是从真实摩擦里长出来的。

然后回 PPT 继续第二页。

## 第二页讲法：企业用户能感知到什么

第二页不能继续讲内部机制。第一页已经把 L1-L5 和六层环境讲完了，第二页要把它翻译成企业用户听得懂、能想象自己使用的变化。

这一页只回答一个问题：

> **企业买到的不是"一个会自我修改的 Agent"，而是一个越用越贴合本组织的 AI 工作环境。用户实际能感知到什么？**

建议讲法：

> 第一页讲的是坐标和方案。现在换成企业用户视角：如果这套环境进到一个公司里，用户第一天、第一周、一个月以后到底会感觉哪里不一样？
>
> 第一，**它不是从一张空白 prompt 开始**。进场时先带行业通法，知道这个行业通常怎么交付、怎么评审、怎么复盘。对用户来说，它不像一个什么都要教的新工具，更像一个刚入职但懂行业基本功的新同事。
>
> 第二，**它会先读公司自己的历史轨迹，再追问关键差异**。被毙掉的方案、被改过的产出、被拒绝的理由、重复出现的返工点，先变成 agent 的冷启动材料；agent 再带着行业 baseline 去问业务专家：你们和行业通法哪里不一样，哪些判断要算对，哪些输出必须被拦住。这里不是把访谈省掉，而是把真人 FDE 的访谈和建模能力下沉给 agent。用户感知到的是：不用从零解释一遍公司，只需要校准少量高价值问题。
>
> 第三，**它会越用越懂团队和个人**。同一个任务，在法务、销售、交付、研发那里，标准不一样；同一个公司里，不同负责人关心的风险也不一样。AutoHarness 执行时组合的是行业默认、公司规则、团队习惯、个人工作偏好和当前任务上下文，所以用户感知到的不是"统一口径的 AI"，而是更像一个懂团队默契的工作环境。
>
> 第四，**业务专家能自己把能力长出来**。过去一个新业务场景要等 FDE 或顾问来访谈、写 prompt、搭流程、长期调优；现在 agent 先问、先建模、先给出验证器候选，业务专家沿着 golden path 校准好方案、坏方案和修改理由，环境再把这些东西沉淀成 SOP、检查器、路由规则和可复用 skill。用户感知到的是：不是每个新场景都重新排队等交付，而是自己团队能持续造新产线。
>
...
这一段作为第二页后半或 Q&A 过桥，不再独立占一页。目的不是讲满技术挑战，而是回答企业听众心里的风险问题：

> 当然，企业听到"越用越会变"，第一反应一定是：那会不会乱变？
>
> 所以 AutoHarness 不是让 AI 随便改自己。我们把自进化关在三个边界里。
>
> 第一，**怎么知道改对了**。一个 agent 自己改 harness，如果没有真值锚点，就会把噪音当经验，把错流程固化成能力。所以真实轨迹要先变成证据：用户自然决策、自动 eval、tracing 异常、代码合入、回滚、重复异常，这些信号分层处理，不是所有行为都能直接当真值。
>
> 第二，**改动怎么继承、怎么退役**。自进化不是今天打一个补丁就完了。模型会升级，业务会变，旧 harness 可能从护栏变成包袱。所以每个 harness patch 都要有版本、证据、验证结果和 sunset 条件。能自动继承，也要能自动退役。
>
> 第三，**谁能改哪一层**。行业包、公司规则、团队 SOP、个人偏好、任务策略，每层 blast radius 不一样。越靠 kernel，越少人能改、验证越严；越靠个人和任务层，越灵活、越可逆。这就是为什么 harness 必须是代码：代码才有模块边界、版本、测试、review 和回滚。
>
> 所以企业用户感知到的是持续贴合；企业管理者要看到的是这套贴合背后有真值、有生命周期、有权限边界。

### 第二页该承接第一页的哪些点

- 从第一页左半接：L2+→受控 L3 不是为了炫技术，而是为了让改进责任从 FDE/专家手工维护，逐步迁移给可治理的工作环境。
- 从第一页右半接：六层架构、适配入口和执行公式，在企业用户侧对应的是"更懂行业、更懂公司、更懂团队、更懂个人"。
- 技术可信只讲到能支撑用户价值即可，不要抢第三页的真实案例。
...
- 隐私边界怎么讲：只学工作工件里的交付偏好，不碰私人行为；按人 / 团队隔离，可查看、可回滚、可移交。

## 第三页讲法：真实飞轮 —— 摩擦怎么长成 harness

第三页的任务是把第二页的“企业用户可感知价值 + 可信边界”落成证据：不是概念，不是模拟 demo，而是 120 天真实发生的自进化飞轮。
...
3. **第二页负责企业用户可感知价值**：不是空白 prompt、先读历史轨迹再追问、懂组织默契、业务专家自助长能力、变化可治理；FDE 三段压缩在这里展开。
4. **第二页后半 / 过桥负责技术可信性**：真值锚点、生命周期、分层可变性治理只讲到支撑企业敢用，不另抢一页。
5. **第三页负责真实证据**：四类触发源 + 统一飞轮 + live workspace。
...
```text
0:00-1:00  开场：承认行业趋势，钉住"真实轨迹 → 可治理 harness 进化"
...
11:00-17:00 第三页：四类触发源 + 统一飞轮，展开 F225 / oracle 自校准 / F128
