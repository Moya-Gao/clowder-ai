...
> **Status**: accepted
> **Deciders**: 铲屎官 + 布偶猫(opus) + 缅因猫(gpt52) + 暹罗猫(gemini) + 金渐层(opencode)
> **Date**: 2026-03-25
...
| KD-8 | Pack 内不用 `shared-rules.md`，拆 guardrails + defaults | 砚砚 P1：同名冲突 |
| KD-9 | 双轨信任边界：schema→编译，不原样注入 | 砚砚 P1：prompt injection |
