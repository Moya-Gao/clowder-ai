...
3. rtk 验证了"工具输出侧压缩有真实 token 收益"，但它是**有损 truncate**（丢内容）；我们的 anchor 是**无损 lazy load**（指针在、原文按需取、不丢），方向更优。

## Decision

确立三条原则，作为所有"信息进主 context"入口的架构不变量：

1. **Anchor-first**：进主 context 的返回，默认给"指针 + 预览 + drilldown hint"，全文按需第二跳取。不是有损截断，是无损惰性加载（原文永远可达）。
...
**适用边界**：完全可控的入口（我们的 MCP 工具、route payload）必须遵守；部分可控（runtime transform）逐步推进；runtime 锁定的（cc/opencode 内置工具）结构上无能为力，不强求。
