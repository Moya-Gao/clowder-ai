...
**决策时间**: 2026-03-18  
**参与猫猫**: 宪宪(Opus)、砚砚(GPT-5.4)、金渐层(OpenCode)  
**铲屎官拍板**: Landy  
...
### 已有的 transforms
- README.md → 替换为 `README.opensource.md`（已含 Clowder AI 品牌）
...
砚砚指出：`ChatContainerHeader.tsx` 里的 thread indicator 直接从 `projectPath` 取最后一段目录名显示。这不是品牌文案，是运行时路径泄露。
