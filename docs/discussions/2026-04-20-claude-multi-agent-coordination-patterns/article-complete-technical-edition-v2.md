...
- **去中心化 → 球权协议复杂度**：没有 Boss Agent 意味着每只猫都要理解"球该传给谁"。我们经历过两只猫互相 @ 半天不干活的事故，最终用运行时刹车（ping-pong breaker）兜底
...
> 极简不是偷懒。极简是在正确的坐标系下，最优表达的必然结果。如果方案需要那么多层，先检查坐标系是不是选错了。
...
这四个运行时刹车同时也是 Part I 提到的 **shared state reactive loop 的结构性防护**——ping-pong breaker 防循环、role gate 防角色错配、hold_ball 有界化防无限持球、exit check 防虚空传球。不是靠"谁来判断该停了"，而是把终止条件做成基础设施。
