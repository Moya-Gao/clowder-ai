...
| 问题 | 立场 | 待定 |
|------|------|------|
| 外部 connector event 改 thread 级判忙 | 4/4 一致 | ✅ 收敛 |
| A2A agent callback 是否也改 thread 级 | 46/47 认为保持 slot 级 + fairness gate；54/55 未明确表态 | ❓ ADR-034 OQ-1 |
| tryAutoExecute 是否加 thread-level 闸门 | 47 建议分档；46 建议 P1 修复；54/55 未涉及 | ❓ ADR-034 OQ-2 |
| messages.ts @/whisper 路径的真实语义 | 54/55 指出 spec/test/code 不一致：前半 cat 级 → 后半 tryStartThreadAll thread 级 | ❓ ADR-034 OQ-3 |
