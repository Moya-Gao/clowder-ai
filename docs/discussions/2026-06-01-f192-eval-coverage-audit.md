...
上表是**默认落点，不是刚性绑定**。传感器类型回答"信号怎么来的"；A1 / A2 / Proxy 回答"这个信号经过内容和上下文解释后有多可信"。例如 cancel reason 里说"编译不过"仍先是 reason 事件，但要通过 build/test 验证后才绑定到 A1；cancel burst 密度很高会提高排查优先级和置信度，但在绑定 episode / reason 前仍不能单独做 verdict；merge 只证明"被合入"，不自动证明"质量好"。
