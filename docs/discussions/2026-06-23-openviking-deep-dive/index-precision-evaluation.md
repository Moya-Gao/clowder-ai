...
**Strong-model paper pass with a human-query caveat.** After adding the missing human-query gate, the OpenViking-style L0/L1 output is index-ready for **9/10** samples by manual review, including **5/6** hard cases.

Compared with F243 prompt v3's old aggregate (**4/10 production-ready; hard cases 1/6, 83% fail**), this run strongly improves the index surface, but it is not a clean sweep:
...
Scoring follows `prompt-quality-blind-test.md`: each dimension is 0/1/2; `index-ready = all dimensions >=1 and total >=7`.

| ID | Type | Core coverage | Distinction | Query hit | Human query | Total | Verdict | F243 old aggregate |
|----|------|---------------|-------------|-----------|-------------|-------|---------|--------------------|
| F008 | hard | 2 | 2 | 2 | 2 | 8 | index-ready | needs-fix |
| F038 | hard | 2 | 2 | 2 | 2 | 8 | index-ready | needs-fix |
| F155 | hard | 2 | 2 | 1 | 1 | 6 | needs-fix | needs-fix |
| F161 | hard | 2 | 2 | 2 | 2 | 8 | index-ready | needs-fix |
| F170 | hard | 2 | 2 | 2 | 2 | 8 | index-ready | production-ready |
| F189 | hard | 2 | 2 | 2 | 2 | 8 | index-ready | needs-fix |
| F009 | easy | 2 | 1 | 2 | 2 | 7 | index-ready | production-ready |
| F012 | easy | 2 | 1 | 2 | 2 | 7 | index-ready | production-ready |
| F013 | easy | 2 | 1 | 2 | 2 | 7 | index-ready | production-ready |
| F119 | easy | 2 | 2 | 2 | 2 | 8 | index-ready | needs-fix |
...
Note on `Human query=1`: F155 is only recoverable if retrieval indexes the L1. The L0 alone is not human-readable enough to match likely human phrasing.

Aggregate:

| Metric | F243 prompt v3 old result | OV prompt + strong model result |
|--------|---------------------------|---------------------------------|
| Total ready | 4/10 | 9/10 |
| Hard ready | 1/6 | 5/6 |
| Easy ready | 3/4 | 4/4 |
| Hard fail rate | 83% | 17% |
...
For human-facing discovery, OV's resource-style L0 is too document-shaped. Cat Cafe needs a separate `product_summary` or `human_description` field that says what the capability does in plain language before mentioning architecture.
