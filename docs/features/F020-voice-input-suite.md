...
- **F20f**: ASR streaming 质量退化修复。**根因**：`useVoiceInput.ts` 的 streaming 逻辑每 3 秒把全部累积 chunks 拼成 blob 重新发给 ASR（Qwen3-ASR），音频越长质量越差（幻觉、乱码、奇怪符号）。Qwen3-ASR 还多一步 ffmpeg 转 WAV，长音频开销更大。**修复方向**：前端改增量发送（只发最近 chunk，不重发历史音频）；加 backpressure（上一次转写没完成不发下一次）。Qwen3-ASR 可调参数有限（仅 `context`），优化主要在前端。2026-03-13 铲屎官发现录音越长识别越差。
