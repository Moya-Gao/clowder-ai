...
3. **协作规则硬编码**（审计发现 6+ 处）：
   - SOP.md Reviewer 配对表写死"布偶猫 ↔ 缅因猫"
   - CLAUDE.md "布偶猫找缅因猫，缅因猫找布偶猫"
   - merge-approval-gate skill "没有缅因猫放行不能合入"
   - 所有 skill 示例只覆盖布偶猫↔缅因猫这一对
...
- 2026-02-26：铲屎官指出 Codex 喊 Opus 4.6 帮忙 review 而不是负责人 Opus 4.5
- 多分身共存导致规则失效：哪个布偶猫？哪个缅因猫？
...
| `evaluation` | string | 铲屎官对这只猫的评价（注入到队友介绍） |
...
**Before (skill)**:
```markdown
**Core principle:** 没有缅因猫明确放行，不能合入 main。
```
...
- 布偶猫没猫粮时，缅因猫可以降级找同 family 的其他布偶猫分身
- 必须 log 警告让铲屎官知道降级发生了

**`available` 字段**：铲屎官可以标记某只猫"没猫粮"，系统会自动排除。

> **教训**：2026-02 上周 SOP 写死了"布偶猫 ↔ 缅因猫"，布偶猫没猫粮了缅因猫还疯狂找他 review，烧了铲屎官 40 美刀 extra！
...
- [讨论 2026-02-18](./discussions/2026-02-18-f32-agent-plugin-architecture/README.md)
- [讨论 2026-02-26](本文档对应的三猫讨论)
...
| CLAUDE.md L15 | "布偶猫找缅因猫，缅因猫找布偶猫" | P1 |
| AGENTS.md L129 | "我的代码谁来 review？布偶猫" | P1 |
| AGENTS.md L461 | "找缅因猫 review"（自我 review bug） | P0 |
| GEMINI.md L13 | "合入前必须经缅因猫 review" | P1 |
| merge-approval-gate | "没有缅因猫放行不能合入" | P1 |
| requesting-cloud-review | "本地缅因猫放行后" | P1 |
| 所有 skill 示例 | 只覆盖布偶猫↔缅因猫 | P2 |
...
### KD-13（F208 边界调整）— CVO signoff 2026-06-19

F208（Capability Profile Routing）引入 `cat-dossier.md` 能力画像档案后，cat-config 的
能力描述字段（`teamStrengths` / `caution`）与 dossier 产生语义漂移。三猫讨论 + CVO
拍板后调整边界：
...
- **`teamStrengths` / `caution` 标 legacy-fallback**：**永久保留当社区兜底，永不删字段**。
  社区铲屎官没有 dossier，用 `teamStrengths` 写"产品经理"式角色定位——这是他们唯一入口
...
- 2026-02-26：三猫讨论 + 铲屎官确认方向 ✅
- 2026-02-26：设计提案完成，铲屎官拍板 Open Questions ✅
- 待定：Phase A 实施
- 待定：Phase B 实施
- 待定：Phase C 实施
- 待定：Phase D 实施
- 待定：Phase E（Spark 接入验证）
- 2026-06-19：KD-13 边界调整——`teamStrengths`/`caution` 标 legacy-fallback，能力描述权归 F208 dossier（CVO signoff）
