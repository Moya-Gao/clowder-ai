...
| 暹罗猫 | **烁烁** | "灵感的闪烁"，暹罗猫在 Google AI Studio 里自己取的名字 (2026-02-27) |

## 运行时观察（2026-02-27 发现）

以下问题在 runtime 运行中观察到，需要持续跟踪：

### 观察 1：Context Compacted 后身份丢失

**现象**：缅因猫在 Context compacted 后自称"宪宪"，去 @ 砚砚（把自己当成布偶猫了）

**根因分析**：
- 压缩后 MCP 注入只包含队友列表："你的队友：布偶猫/宪宪..."
- **缺失"你是谁"信息**：没有注入"你是缅因猫/砚砚"
- 猫猫看到队友列表后，误认为自己是列表里的某只猫

**触发条件**：长对话 → Context compact → 再注入时身份段被压缩掉

**建议修复**：Context compact 后的 MCP 注入应**始终包含身份声明**，不能只有队友列表

**优先级**：观察中（之前没出现过，需要多观察）

### 观察 2：A2A @ 能力退化

**现象**：猫猫们经常忘记使用 @ 队友的能力，不主动协作

**根因分析**：
- 当前 A2A 协议（@ 规则、队友名册）**只在 new session 和压缩后注入**
- 不是每个回合都注入
- 随着对话推进，猫猫逐渐"忘记"自己有 @ 队友的能力

**建议修复**：
1. 考虑在每个回合注入精简版 A2A 提示（如 "记得你可以 @ 队友协作"）
2. 或在 system prompt 中更强调 @ 能力的存在
...
4. **持续观察**: 跟踪上述运行时观察，收集更多案例后决定修复优先级
