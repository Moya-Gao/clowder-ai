...
- **Phase A+ 实时广播也走 `connector_message` + `source.meta.presentation='system_notice'`**（2026-04-16 #505 收敛）：避免“持久化回放是 notice，实时 socket 却是 error 气泡”的分叉。restart notice 在 transport 上复用 connector 协议，在呈现上归属 Tier 3 system notice
