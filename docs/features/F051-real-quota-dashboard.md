...
- [ ] AC-1: Codex 卡片显示的用量% 与 `chatgpt.com/codex/settings/usage` 页面一致
- [ ] AC-2: Claude 卡片显示 session/weekly 用量% 与 Claude 官方 usage 一致
- [ ] AC-3: Codex 和 GPT-5.2 只显示一张共享卡片，不分开
- [ ] AC-4: 抓取失败时显示"抓取失败"而非推导值
- [ ] AC-5: Antigravity 显示"待接入"占位（不是推导值）
- [ ] AC-6: 支持手动刷新
...
|----|---------------------------|---------|----------|------|
| R1 | "三只猫的额度现在到底有多少" | AC-1,2 | screenshot: 对照官方页面 | [ ] |
| R2 | "codex 和 gpt 52 他们是一个额度" | AC-3 | screenshot: 只有一张卡 | [ ] |
| R3 | "用chrome去各个公司额度页面查看都比你算半天靠谱" | AC-1,2 | 实现使用浏览器抓取 | [ ] |
| R4 | "官方有什么我们看什么" | AC-4 | test: 抓取失败不显示推导值 | [ ] |
| R5 | "antigravity 下次一定" | AC-5 | screenshot: 占位文案 | [ ] |
...
1. Claude 官方 usage 页面的 URL 是什么？（需要确认）
2. 抓取频率：每次打开看板实时抓？还是后台定时抓 + 缓存？
3. 已合入的 PR #161 UI 骨架（`HubQuotaBoardTab`）是保留改造还是重写？
