...
### 铲屎官原话（2026-03-02 Thread `thread_mm8nkwlcwmwhmfgz`）
...
// 跨线程消息（新增来源标注）
[msgId] [14:30 缅因猫 ← from thread:mm806zbc] 内容...
```
...
| # | 问题 | 状态 |
|---|------|------|
| OQ-1 | 跨线程 A2A 是否需要单独的 depth 限制（比主线程更严）？ | 待讨论 |
| OQ-2 | `sourceInvocationId` 是否必要？还是只要 `sourceThreadId` 就够？ | 倾向可选 |
| OQ-3 | Phase B 前端标签的具体设计（颜色/位置/文案）？ | 待暹罗猫出设计 |

## Review Gate

- Phase A: 跨家族 review（缅因猫 codex 或 gpt52）
- Phase B: 前端部分额外需要暹罗猫视觉 review
