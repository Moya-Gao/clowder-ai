...
> **Updated**: 2026-03-03（立项 + Phase A 落地）
...
1. **系统文档同步**：清理仓库内“Gemini 不支持 UUID resume”的陈旧描述。
2. **观测与告警**：补充 Gemini resume 失败分类（missing session / cli exit / auth）统计。
3. **策略对齐**：将 Gemini 纳入和 Claude/Codex一致的 session strategy 讨论（seal/rotate/retry 语义）。
...
- [ ] AC-4: 全仓文档清除“Gemini 不支持 UUID resume”旧说法并统一口径。
- [ ] AC-5: 补充 resume 失败观测项并输出到可追踪日志。
...
| R2 | “先拨乱反正，别再按错误前提实现” | AC-2, AC-4 | 单测 + 文档检查 | [ ] |
