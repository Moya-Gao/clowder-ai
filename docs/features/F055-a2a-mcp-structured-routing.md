...
猫猫们在 CLI 文本里写 `@队友` 的格式经常出错（行中 @、忘记换行、冒号后紧跟 @），导致 A2A 路由失败，铲屎官变成人工转发站。
...
- **Evolved from**: [F050](F050-a2a-external-agent-onboarding.md)（A2A 接入契约，定义了 callback 协议）
- **Related**: [F046](F046-anti-drift-protocol.md)（anti-drift，路由策略）
- **Related**: [F042](F042-prompt-engineering-audit.md)（提示词优化）
- **Trigger**: PR #206 — 移除关键词门控后，铲屎官观察到猫猫仍然不好好 @ 的问题
...
| # | 决策 | 理由 |
|---|------|------|
| D1 | targetCats 是路由信号，不是可见性控制 | 铲屎官确认 debug/play 可见性逻辑不变 |
| D2 | 双通道取并集（Phase 1），逐步移除文本解析 | 平滑过渡，不破坏现有流程 |
| D3 | 不做 7B 意图识别 | CJK 准确率不够 + 结构化字段已解决根因 |
| D4 | targetCats 放在 callback response，不是独立 MCP 调用 | 铲屎官确认"一个动作"体验更好 |
...
1. Phase 2→3 的移除时机：观察多久后移除文本解析？（铲屎官说"先试试现在改之后 a2a cli 解析"）
2. 非 Cat Café 原生猫（F050 外部 agent）的 targetCats 适配
3. 前端是否需要展示 targetCats 信息（如"此消息路由到 codex"的标签）
...
| 日期 | 事件 |
|------|------|
| 2026-03-04 | Kickoff + spec（来自 PR #206 后铲屎官反馈） |
