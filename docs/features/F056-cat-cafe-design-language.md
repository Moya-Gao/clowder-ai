...
> **Status**: doing（Phase E 11/12 done + Phase E Sweep 2026-05-25~28 完成：bubble routing 统一 / variant slug 补齐 / 350-line split / clowder-ai#784 review-response — AC-E12 Playwright baseline deferred 到集成验证） | **Owner**: 砚砚/GPT-5.2 + 布偶猫 主导设计执行 + 布偶猫工程架构 + 烁烁概念方向
> **Priority**: P1
...
> 详见 `docs/research/2026-03-24-frontend-design-system-refactor-gpt-pro-consult.md` Part 3
...
#### A1: 设计语言收敛（三猫打样竞赛 → 铲屎官定调）

**设计语言公式**（砚砚 GPT-5.2 提出，铲屎官拍板选中）：
...
|---|------|-----------|------|----------|
| 宪宪 (Opus) | v1 Apple-inspired | `VJghG` | 日式瑞士，珊瑚渐变线 + 极细边框 | "干净但缺灵魂" |
| 烁烁 (Gemini) | Cat-ified | `pq1cf` | 猫爪印 + 奶油白 | "看不出区别，头像碎了" |
| **砚砚 (GPT-5.2)** | **Postmark** | **`Nfif0`** | **奶油纸底 + 蜡封角标 + 邮戳 pill** | **"做的最好！理解、语言、完成度、认真"** |
| 宪宪 (Opus) | v2 Apple-refined | `lydod` | 暖象牙 + 珊瑚点缀 + 精排 | "精致但偏冷" |

**结论**：砚砚版 Postmark 风格胜出，作为 F056 的参考标准。
...
- **Token 三层架构**（砚砚 GPT-5.2 提案，布偶猫拍板）：
...
| KD-4 | 砚砚(GPT-5.2) 主导设计执行，烁烁出概念方向 | 三猫打样竞赛铲屎官选中砚砚版（Postmark） | 2026-03-04 |
| KD-5 | Token 三层架构：base(猫名) → semantic(工程语义) → persona(身份色) | 砚砚(GPT-5.2)提案，避免改一次色板全站手抖 + 代码可读性 | 2026-03-04 |
...
| KD-10 | 五层夹心架构：Layer 0 治理 → Layer 1 tokens → Layer 2 primitives → Layer 3 patterns → Layer 4 enterprise | 三方共识（宪宪+砚砚+GPT Pro），详见 GPT Pro 咨询报告 | 2026-03-27 |
| KD-11 | 在 TW3 上做，不叠加 TW4 升级风险 | 当前 Tailwind 3.4.0，TW3→TW4 迁移是正交风险源 | 2026-03-27 |
| KD-12 | Radix headless 做 a11y 密集型控件（Dialog/Select/Menu），shadcn 当参考不当宪法 | GPT Pro + 砚砚共识，a11y/focus 管理自建风险高 | 2026-03-27 |
| KD-13 | 术语覆写只做 UI glossary，route/API/schema rename 不在 F056 范围 | 砚砚 review：thread 已进路由/API/Redis key，scope 边界必须写死 | 2026-03-27 |
...
- **Research**: `docs/research/2026-03-24-frontend-design-system-refactor-gpt-pro-consult.md`（GPT Pro 咨询 + 三方综合）
...
| # | 问题 | 状态 |
|---|------|------|
| OQ-1 | 谁主导 Phase A 视觉设计？ | 砚砚(GPT-5.2) 主导执行，烁烁出概念（铲屎官定调） |
| OQ-2 | 是否需要 dark mode 的猫猫化版本？ | Phase A 先留 token，Phase D 做全量切换（KD-6/KD-10） |
...
- Phase D: 铲屎官拍板企业定制边界 + 跨家族 review
...
| 2026-03-04 | F056 立项，烁烁 (Gemini) 确立 4 大设计原则并完成”猫猫化”打样 |
| 2026-03-04 | 三猫讨论：砚砚(GPT-5.2) 提出 Token 三层架构 + 动效上限 + 对比度检查 |
| 2026-03-04 | 布偶猫拍板：KD-5~8（token分层/dark mode/动效限制/禁硬编码hex） |
...
| 2026-03-27 | 铲屎官提出前端组件化+企业 fork 定制需求（R8） |
