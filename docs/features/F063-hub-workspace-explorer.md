...
# F063: Hub Workspace Explorer — 铲屎官不用打开 IDE
...
| R8 | "这个是我们非常重要的一环体验？如何 ux 如何布局？" | AC-6 | visual: 布局方案烁烁 review | [ ] |
...
| 布局方案 | 侧边栏 / Tab / Modal / 可拖拽 | **待烁烁参与 UX 讨论** | 待定 |
...
1. 布局方案需要烁烁出 UX 设计稿 — 侧边栏？可拖拽？怎么和现有聊天 UI 共存？
2. 前端预览的技术选型：iframe sandbox vs Sandpack vs 其他？
3. 文件搜索用什么后端？直接 `grep -r` 还是建索引？
4. 需不需要支持文件编辑（铲屎官在 Hub 里改文件）？还是只读就够？
5. 猫猫消息中的文件路径自动识别：regex 够用还是需要更智能的方案？
