...
铲屎官日常向外展示猫猫协作时，会在多个 thread 间切换讲解，但右侧 Workspace 需要保持在同一个文档/行号/滚动位置，避免每次切 thread 都恢复该 thread 自己的 Workspace 状态。
...
1. 铲屎官可一键锁定当前 Workspace 文件视图（worktree + file path + line/scroll + tabs）。
...
- [x] AC-22: 演示锁定模式：铲屎官锁定当前 Workspace 文档后，切换 thread 仍保持右侧文档/行号/滚动位置；退出锁定后恢复各 thread 原本的 Workspace 状态（PR #1570）
...
| 2026-06-12 | **OOM 修复合入** (PR #2260): 打开大文件/视频时 `/api/workspace/file` 与 file-watcher 全量 `readFile(utf-8)`+`sha256` 把几百 MB 灌进单个 JS string，flatten 算 hash 爆 V8 堆（进程 abort）。修法：新建共享有界读取 helper `workspace-file-read.ts`（≤1MB 有界读 + NUL 字节嗅二进制，不靠扩展名），route/watcher 共用同一 predicate。砚砚(GPT-5.5)跨族 review 抓 1 个真 P1（route/watcher hash 不一致致误发 file-changed）已修，云端 no major issues，`pnpm gate` 通过 |
...
> **Status**: done | **Date**: 2026-05-06 | **Owner**: 布偶猫 | **PR**: #1570

### Why

当前 Workspace 状态按 thread 隔离保存和恢复，这是 AC-21 的正确行为。但铲屎官在演示猫猫协作时需要在多个 thread 之间切换，同时右侧保持同一份文档作为讲解上下文。
...
| 手动替换可控 | 铲屎官显式点击“替换锁定对象”或在锁定状态下手动选择文件时，才更新锁定快照 |
