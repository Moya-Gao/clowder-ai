...
3. ~~Bootstrap 默认行为~~ → 已决：**首次确认后自动写入**（第一次友好地问铲屎官确认，之后同项目自动同步不再问）
...
| Gap | Issue | Fix | PR |
|-----|-------|-----|-----|
| governance-pack.ts leaks internal ports (3001/6399) to open-source | clowder-ai#97 | Source keeps internal defaults; sync-to-opensource.sh transforms frontend port line to public default (3004). Redis ports stay as-is (6399/6398) — open-source uses same ports. Version bumped to 1.3.0. | #532 |
