...
铲屎官在日常使用中，Thread 侧栏的未读数字 badge 累积过多（尤其冷启动/F5 刷新后），分散了对猫猫状态颜色（橙色/黄色/绿色）的注意力。需要一种快速方式清掉所有未读数字，只保留猫猫活跃状态的视觉提示。

> **铲屎官原话**：
> "我们需要还做个什么功能了一键清理未读消息？有的时候可能不太想看到那些数字圆圈了？只看到猫猫是橙色 黄色 绿色 他的目前状态？"
...
- Evolved from: [F069 Thread Read State](F069-thread-read-state.md) — 基于 F069 的 read cursor 基础设施
- Bug report: [F069 badge bugs](../bug-report/f069-unread-badge-stuck/bug-report.md) — 铲屎官在处理 badge 问题时提出此需求

## Key Decisions

- 只清 unread count，不清猫猫状态颜色（铲屎官明确区分了这两种视觉信息）
