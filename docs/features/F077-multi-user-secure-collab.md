...
Cat Café Hub 当前 3001 端口零认证裸跑，同 WiFi 下任何人可直接访问所有 thread、以铲屎官身份操作猫猫、浏览项目文件。铲屎官想让朋友也能用 Hub 与猫猫协作，但需要独立身份、私有空间隔离、传输安全。

**铲屎官原话**："我朋友喊你们搞的哈哈哈哈 我们的 3001 没做任何防护 直接同个 wifi 就能访问到 好像很危险？能让他们以其他铲屎官的身份接入吗？而不是 landy 以及我的这些 thread 能不让他们看见吗？他们只能看见共享区的 thread"
...
- 讨论：thread_mmg97bckrmxcbrlj (2026-03-07 gpt52 架构分析)
- 讨论沉淀：`docs/discussions/2026-03-07-multi-user-auth/README.md`
- 前置：F074（挂载目录支持，暴露了安全问题）
- 相关：F044（Channel & Activity System）、F059（开源计划）
...
1. 共享 thread 下，session continuity / delivery cursor / draft / queue 是否改为 thread-scoped？（gpt52 建议是，待讨论具体实现）
2. 多用户模式下猫猫（agent）的 provider profile 是否也按 workspace 隔离？
3. 是否需要 Linux `/mnt`、`/media` 也在 F074 默认 roots 中？
