...
- ~~历史是全局的还是按 thread？~~ → **全局**（Key Decision #2）
- ~~是否需要"清除历史"功能？~~ → **不做**（MVP 够用，localStorage 本身可清）
- ~~敏感内容是否需要排除？~~ → **接受风险**（localStorage 纯本地，不跨设备同步，与浏览器密码管理器同等安全级别）
