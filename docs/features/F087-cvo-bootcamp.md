...
- [ ] AC-1: 前端有"新手引导"入口按钮，点击触发 Bootcamp 模式
- [ ] AC-2: 猫猫天团轮流登场自我介绍（宪宪→砚砚→烁烁，各自风格）
- [ ] AC-3: 自动检测用户环境（MCP、依赖、配置），主动帮用户解决问题
- [ ] AC-4: 提供任务候选菜单（≥3 个不同难度），用户自选感兴趣的任务
- [ ] AC-5: 用户从头到尾走完 feat lifecycle（立项→设计→开发→review→完成）
- [ ] AC-6: 过程中用户做了 ≥3 次 CVO 决策（方案选择、纠偏、冲突裁判）
- [ ] AC-7: 完成后用户能看到自己参与的成果（功能上线 / commit 记录）
- [ ] AC-8: 成就/进度接入 F075 猫猫排行榜系统（终态基座，不搞临时版）
- [ ] AC-9: 训练营可作为 clowder-ai 的 Quick Start 引导
