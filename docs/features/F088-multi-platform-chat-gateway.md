...
详细设计见 [架构归一设计](assets/F088/architecture-unification.md) · [设计讨论纪要](../discussions/2026-03-10-f088-connector-thread-unification-meeting-notes.md)
...
- **ISSUE-16**: 外部 IM 创建线程后 spawn 的猫 cwd 错误 — 从微信/飞书/Telegram 进来的消息创建线程后，spawn Claude Code 时 `workingDirectory` 未正确设置（fallback 到 API server 的 `process.cwd()` = `packages/api`），导致：(1) Claude Code session 文件存到 `~/.claude/projects/...-packages-api/` 而非 `...-cat-cafe-runtime/`，`claude --resume` 需要在 `packages/api` 目录下才能找到；(2) 猫的工作目录在子包下而非 monorepo root。根因在 `invoke-single-cat.ts` 的 `workingDirectory` 解析链路：thread 无 `projectPath` → undefined → ClaudeAgentService spawn 时不设 cwd → 继承父进程 cwd。相关已有修复：`c46682711 fix(F136): use monorepo root for accountStartupHook projectRoot`（同类问题不同链路）。
...
2. ~~群聊排期？~~ → Phase 7（F077 前置）
3. ~~多猫外显策略？~~ → 方案 A（消息前缀 `[布偶猫🐱]`），单 bot 最简

## Timeline

| Date | Event |
|------|-------|
| 2026-03-09 | Kickoff: F088 立项，Phase 1 + 2 完成 (PR #328 + #336) |
...
| [设计讨论纪要](../discussions/2026-03-10-f088-connector-thread-unification-meeting-notes.md) | 布偶猫+缅因猫 thread 统一讨论 |
