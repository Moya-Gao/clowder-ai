...
> **Status**: done | **Owner**: 布偶猫 | **Priority**: P1 | **Completed**: 2026-04-04 (Phase A~J) | **Reopened**: 2026-04-13 (Phase K) | **Re-closed**: 2026-04-14 (Phase K done; AC-K3 later completed by F209 Phase A; AC-K4 remains future work)
...
**影响**：搜 "why are cats named 宪宪 砚砚 烁烁" 时，BM25 召回不到猫名故事，
...
7. 知识对话 — "我们为什么放弃 Hindsight？" → 综合叙事回答（布偶猫，IReflectionService 终态）
...
- [x] AC-B2: 索引覆盖 `docs/features/`, `docs/decisions/`, `docs/plans/`, `docs/lessons/`
...
- [x] AC-F1-1: `project-init` CLI 命令存在（`pnpm project:init <dir>`），在目标目录创建 13 个标准 KIND_DIRS 子目录 + 基础骨架文件（BACKLOG.md / VISION.md）
...
| KD-19 | **Embedding 模型选 Qwen3-Embedding-0.6B**（主方案）+ multilingual-e5-small（兜底），MRL 维度可调 | 铲屎官指示统一 Qwen 技术栈；C-MTEB 66.33 远超 MiniLM；中英混排核心场景 | 2026-03-12 |
...
| KD-32 | **Thread 索引不导出 markdown**——直接从 messageStore 读消息内容编译索引，不转中间层 md 文件 | 铲屎官明确否决 + 砚砚方案共识：真相源在 Redis（TTL=0 永久），索引是编译产物，导出 md = 重复真相源 | 2026-03-18 |
| KD-33 | **Thread 索引不靠 threadMemory.summary**——340 thread 中 326 个 summary 为空，必须从消息内容本身提取可搜文本 | 铲屎官指出"threadMemory.summary 不靠谱"，回溯 QMD proposal 确认：正确做法是 turn-by-turn 消息拼接 | 2026-03-18 |
...
| 2026-03-12 | Phase B 实现完成：IndexBuilder + frontmatter 解析 + eval corpus + kind-priority + 67 tests |
| 2026-03-12 | codex review 放行（3 轮：DI wiring + anchor collision + incrementalUpdate priority） |
...
| 2026-03-18 | PR #534: 索引覆盖 127→413 docs + 4 skills recall guidance |
| 2026-03-18 | PR #541: SessionBootstrap auto-recall（所有猫 session 启动自动注入项目知识） |
| 2026-03-18 | PR #543: thread 索引根因修复（从 messageStore 读消息内容，不靠 threadMemory.summary） |
| 2026-03-18 | PR #544: messageStore append listener（36 个调用点全覆盖，30s debounce flush） |
| 2026-03-18 | LL-033 教训：云端 review inline code comments 必须检查，不能只看 review body |
