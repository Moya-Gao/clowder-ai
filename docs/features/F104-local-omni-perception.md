...
当前猫猫的感知管道由三个独立小模型串行组成（ASR 1.7B + LLM后修 4B + TTS 1.7B），总延迟高、能力有限、且猫猫完全没有视觉理解能力。铲屎官的 128GB M4 Max 有充足算力跑 MoE 30B 级模型（3B 激活），升级后猫猫从"只会读写的文字 AI"变成"能听能看能说的全感知伙伴"。

铲屎官原话："我们能做什么！？他们能帮你们干嘛？"
...
| # | 问题 | 状态 |
|---|------|------|
| OQ-1 | Qwen3-Omni 是否有 MLX 量化版？mlx-community 现状？ | ⬜ 待调研 |
| OQ-2 | Omni 的 voice clone 能力如何？能否保持三猫声线？ | ⬜ 待调研 |
| OQ-3 | 两个 MoE 模型并行推理的实际显存和延迟？ | ⬜ 待实测 |
| OQ-4 | Omni 端到端延迟 vs 现有管道延迟的实际对比？ | ⬜ 待实测 |

## Key Decisions

| # | 决策 | 理由 | 日期 |
|---|------|------|------|
| KD-1 | 优先 Phase A（Omni），再 Phase B（VL） | Omni 替换三模型管道收益最大，且铲屎官日常语音交互最频繁 | 2026-03-11 |
...
| 日期 | 事件 |
|------|------|
| 2026-03-11 | 立项，三猫讨论 /loop + 感知层升级 |

## Review Gate

- Phase A: 跨家族 review（优先 @codex/@gpt52）
- Phase B: 跨家族 review + 暹罗猫视觉验证

## Links

| 类型 | 路径 | 说明 |
|------|------|------|
| **Feature** | `docs/features/F066-voice-pipeline-upgrade.md` | 前代语音管道（Evolved from） |
| **Feature** | `docs/features/F103-per-cat-voice-identity.md` | 声线身份（Related） |
| **Feature** | `docs/features/F092-voice-companion-experience.md` | 语音陪伴体验（Related） |
