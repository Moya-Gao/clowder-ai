...
铲屎官定性：**金渐层**（Golden Chinchilla / British Shorthair）——毛色渐变如同 opencode 的"开放渐进"理念，圆润沉稳的英短体型体现稳定可靠。
...
- **Cat Cafe CatOrchestration**：管理跨猫调度（金渐层 ↔ 布偶猫/缅因猫/暹罗猫）
...
- **API Proxy**: `https://chat.nuoda.vip/claudecode`（Anthropic 格式，铲屎官配置）
