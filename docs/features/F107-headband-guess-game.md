...
猫猫杀是 Cat Cafe 的**招牌推理游戏**，七届历史（布偶猫 1v1 四届三胜一平）。之前全靠手动 MCP 消息轮次推进，存在问题：
- 铲屎官需要手动当主持人（选词、分配、计轮、判胜负）
...
三种选词模式（铲屎官 Design Gate 确认）：
- **铲屎官出题**：铲屎官开局为每只猫手动指定脑门词（传统模式，支持恶趣味操作）
...
- 铲屎官恶趣味模式（meta 信息：给 Claude 猫贴 Claude Shannon，给 GPT 猫贴 Karpathy）

**A3. 坏猫战术系统 — AI 玩家 prompt**

这是猫猫杀的**灵魂**。**所有猫都要教会坏猫战术**（铲屎官原话："你到时候提示时一定要告诉其他的猫，让他们坏起来"）。
...
- `player`：铲屎官也是参赛者，脑门贴词，提问/回答/猜词
- `god-view`：看到所有词 + 所有猫的推理过程（内心戏），纯观战
- `host`：铲屎官手动选词 + 观战，可干预（暂停/提示/换词）
...
- [ ] AC-A6: 铲屎官可选 player / god-view / host 三种模式参与
...
| 选词难度不平衡（太简单/太难） | 词库分级 + 铲屎官 host 模式可手动调整 |
...
| # | 问题 | 状态 |
|---|------|------|
| OQ-1 | 猜错是否有惩罚？ | ✅ 语音模式→卖萌；非语音→无惩罚 |
| OQ-2 | 是否允许猫猫之间互相讨论/质疑回答？ | ✅ 允许 |
| OQ-3 | 选词模式？ | ✅ 三种：猫猫出题 / 铲屎官出题 / 随机词库 |
| OQ-4 | 是否需要"观众投票猜词"互动模式？ | ⬜ v2 |
...
| KD-2 | AI prompt 必须注入坏猫战术，**所有猫都要教** | 铲屎官强调"一定要告诉猫猫们坏猫战术"，不只布偶猫一只坏 | 2026-03-12 |
| KD-3 | 三种选词模式：猫猫出题 / 铲屎官出题 / 随机词库 | 铲屎官确认：猫猫也可以当出题者 | 2026-03-12 |
| KD-4 | 猜错不淘汰，语音模式下卖萌 | 猫猫杀传统规则 + 铲屎官要求语音卖萌 | 2026-03-12 |
| KD-5 | 允许猫猫之间讨论/质疑回答 | 铲屎官确认，保留七届传统玩法 | 2026-03-12 |
| KD-6 | 坏猫内心戏是核心乐趣，god-view 必须保留 | 铲屎官确认，毛玻璃面板 + 战术标注 | 2026-03-12 |

## Timeline

| 日期 | 事件 |
|------|------|
| 2026-03-12 | 立项 + spec 初稿 |

## Review Gate

- Phase A: 缅因猫 review（重点：信息隔离 + AI 战术 prompt 质量）
- Phase B: 缅因猫 review + 暹罗猫 design review

## Links

| 类型 | 路径 | 说明 |
|------|------|------|
| **Evolved from** | `docs/features/F101-mode-v2-game-engine.md` | 通用游戏引擎基座 |
| **Related** | `docs/features/F075-cat-leaderboard.md` | 战绩排行榜 |
| **History** | `docs/stories/mafia-game-s5/README.md` | 猫猫杀第五~六届实战记录 |
| **History** | `docs/stories/mafia-game-highlights/README.md` | 七届名场面集锦 |
| **Thread** | `thread_mlorq6a8hkhllvoi` | 第一届猫猫杀 |
| **Thread** | `thread_mlpv0jlgph3gbwcp` | 第二届猫猫杀 |
| **Thread** | `thread_mlu0tdpbgnxp67ql` | 第三届猫猫杀 |
| **Thread** | `thread_mlutn7vc5rjxlqf0` | 第四届猫猫杀 |
| **Thread** | `thread_mlv57k4ppos4thti` | 第五~六届猫猫杀 |
| **Thread** | `thread_mmbszvdtjvd4bsil` | 谁是卧底（参考） |
