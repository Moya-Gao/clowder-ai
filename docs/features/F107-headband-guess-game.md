...
开局配置界面（参考狼人杀 UI，铲屎官截图确认）：
1. **选择参赛猫猫**：点击头像添加，铲屎官决定几只猫参赛、分别是哪些猫
2. **选择铲屎官角色**：
...
**自动流程**：
- **开局**：铲屎官说"开一局脑门贴词" → GameMaster 自动发词、分配身份、注入坏猫战术 prompt
...
- [ ] AC-A10: GameMaster 全自动驱动：开局→教学→回合推进→裁判→结算，铲屎官零操作
- [ ] AC-A11: GameMaster 自动管理可见性切换（公开发言 vs 内心戏 scope）
- [ ] AC-A12: 开局配置界面：铲屎官可选参赛猫（哪些猫 + 几只）、选角色（玩家/上帝/host）、选词模式
...
| R9 | 铲屎官选参赛猫（几只 + 哪些猫），参考狼人杀开局 UI | AC-A12,A13,B1 | screenshot | [ ] |
...
| KD-8 | GameMaster 系统全自动驱动，走现有消息管线 | 铲屎官当上帝不需要手动组织流程，复用 post_message/multi_mention/富文本 | 2026-03-14 |
| KD-9 | GameMaster 是独立角色，有专属头像 | 像飞书 bot / GitHub bot，在消息流中有清晰视觉区分 | 2026-03-14 |
| KD-10 | 开局由铲屎官选参赛猫 + 数量 | 参考狼人杀开局 UI：选板子→选猫猫→选视角→开始游戏 | 2026-03-24 |
