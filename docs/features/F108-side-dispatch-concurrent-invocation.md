...
| 两只猫并发改同一文件 | Phase A 硬约束：不承诺双写，只允许一写一读 | 铲屎官用例（修 bug + 反思）天然一写一读 |
...
> 缅因猫(GPT-5.4) 安全评审 2026-03-12 提出：必须在 Design Gate 前把这些写死。
...
| DG-10 | 前端消息渲染——交错还是分栏 | ⬜ 待定 | Phase B UX，铲屎官确认 |
...
| KD-2 | WorklistRegistry 从 `threadId` 改为 `parentInvocationId` 绑定 | 防止 A2A callback 在并发 invocation 间串台（缅因猫安全评审） | 2026-03-12 |
...
| KD-6 | Phase B 双模发送 UX：锁头悄悄话 + 广播默认 | 铲屎官 2026-03-12 亲自定义的交互流 | 2026-03-12 |
...
| 2026-03-12 | 缅因猫(GPT-5.4) 完成安全评审，定位 5 处单活跃假设，确认方向可行 |
| 2026-03-12 | 布偶猫吸收评审结论更新 spec：引入 ExecutionSlot 概念，Open Questions → Design Gate Checklist |
| 2026-03-12 | 铲屎官定义 Phase B 双模发送 UX（锁头悄悄话 + 广播模式） |
| 2026-03-12 | 布偶猫完成 UX 线框图 `designs/f108-side-dispatch-ux.pen` |
...
| **Design** | `designs/f108-side-dispatch-ux.pen` | Phase B UX 线框图 |
...
| R2 | "给缅因猫一直发悄悄话避免影响你的修复" — 旁路消息不中断主执行 | AC-A2, AC-B1 | test: 主执行猫不被 abort | [ ] |
...
```
场景：布偶猫修 bug 中，铲屎官同时想让缅因猫反思架构

当前（F108 之前）：
  铲屎官 → @opus 修这个 bug
  铲屎官 → @gpt52 你反思一下为什么过了
  ❌ @gpt52 的消息 abort 了 @opus 正在进行的修复
  ❌ 或者铲屎官被迫开新 thread，结果分散在两个地方

期望（F108 之后）：
  铲屎官 → @opus 修这个 bug       → 布偶猫在主执行流修 bug
  铲屎官 → 🔒 悄悄话 @gpt52       → 缅因猫在旁路执行流反思（不打断布偶猫）
