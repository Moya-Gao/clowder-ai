...
1. **软删除前端不生效**
   - **根因**（砚砚分析，高置信）：`removeMessage` 只改 flat `messages` 数组，不改 `threadStates[threadId]`。socket 回调 `onMessageDeleted` 丢了 `threadId`。切线程后气泡保留在 background thread state 里。
...
> **前置依赖**：message `kind` 显式化必须先做，否则 revision note 会被错误塞进 `user`/`connector` 语义（砚砚@gpt52 提醒）
...
- [x] AC-A6: 铲屎官可以在任何 thread 中 Branch From 任意消息（含 system thread）
...
| # | 问题 | 状态 |
|---|------|------|
| OQ-1 | ~~就地编辑是否保留原消息？~~ | ✅ KD-3 三档分层解决 |
| OQ-2 | 下游回复是否标记"基于旧版本"？ | ⬜ Phase B2 讨论 |
| OQ-3 | ~~铲屎官能否编辑猫的消息？~~ | ✅ v1 不支持（KD-3） |
| OQ-4 | 软删除是否需要占位符（类似 Slack）？ | ⬜ 待铲屎官确认 |
...
| KD-2 | `participants` 不做 ACL，branch 用 `createdBy === userId \|\| createdBy === 'system'` | participants 是 CatId[] 路由元数据，不是权限模型（砚砚@gpt52） | 2026-03-12 |
| KD-3 | 编辑三档分层：tail-edit / branch-rewrite / cat-no-edit | 避免破坏上下文链和审计（砚砚@gpt52） | 2026-03-12 |
| KD-4 | Revision 独立 store，主消息只放元数据 | 不污染 message hash（砚砚@gpt52） | 2026-03-12 |
| KD-5 | WebSocket 用 `message_edited` 专用事件 | 删除/恢复/编辑是不同前端语义（砚砚@gpt52） | 2026-03-12 |
| KD-6 | Revision note 不走 unread 计数，引入显式 message `kind` | 防止铲屎官编辑触发 UI 未读 badge（砚砚@gpt52） | 2026-03-12 |
| KD-7 | Phase A 同时覆盖 soft/hard delete + restore | hard/soft 走同一回调，restore 前端是 no-op（砚砚@gpt52 R2 补充） | 2026-03-12 |
| KD-8 | B2 前置：message `kind` 必须先显式化 | 历史接口靠推断，不先解约束 revision note 会串语义（砚砚@gpt52 R2 补充） | 2026-03-12 |

## Timeline

| 日期 | 事件 |
|------|------|
| 2026-03-12 | 立项，铲屎官报告软删除前端 bug |
| 2026-03-12 | 砚砚（@gpt52）技术讨论，收敛 KD-2 ~ KD-6 |
| 2026-03-12 | 砚砚 R2 补充：hard delete/restore 一并纳入 Phase A + B2 前置依赖明确（KD-7, KD-8） |
| 2026-03-28 | Phase A merged (PR #827) — socket wiring + toast + branch permission + restore |

## Links

| 类型 | 路径 | 说明 |
|------|------|------|
| **Discussion** | `docs/discussions/2026-03-12-f109-message-actions/README.md` | 技术方案讨论记录 |
| **Code** | `packages/api/src/routes/message-actions.ts` | 删除/恢复 API |
| **Code** | `packages/api/src/routes/thread-branch.ts:131` | Branch 权限判断 |
| **Code** | `packages/web/src/components/MessageActions.tsx` | 前端操作 UI |
| **Code** | `packages/web/src/hooks/useChatSocketCallbacks.ts:85` | WebSocket 事件处理 |
| **Code** | `packages/web/src/stores/chatStore.ts:592` | removeMessage（flat only） |
| **Code** | `packages/web/src/stores/chatStore.ts:802` | background thread 写路径 |
| **Code** | `packages/api/src/domains/cats/services/stores/redis/RedisMessageStore.ts` | Redis 存储层 |
| **Test** | `packages/api/test/soft-delete.test.js` | 现有删除测试 |
| **Test** | `packages/web/src/components/__tests__/message-actions-identity.test.ts` | Branch identity 回归测试 |
