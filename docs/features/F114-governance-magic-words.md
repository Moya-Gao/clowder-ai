...
> "我感觉做愿景守护的喵，要么他没有认真的看我的愿景，要么你们两只猫的人类意图理解能力有问题。但是这个问题在于，按照你跟砚砚的智力水平，这句话不应该理解错。" — 铲屎官 2026-03-13

> "我们的家规或者喵约里面很多很重要的该怎么办？" — 铲屎官 2026-03-13

> "我觉得这种脚本需要加一个 log 写进度条，不然都不知道他是不是死了" — 铲屎官 2026-03-13（评价平行 session 跑 sync 脚本）

> "如果一天就能写完，你还要做那些先搞一个垃圾版本……可能又搞了一周都没干完" — 铲屎官 2026-03-13（评价 Claude Code 系统提示词的"avoid over-engineering"倾向）

### 问题根因

1. **重要规则写了但不遵守**：喵约已在系统提示词级别注入（`SystemPromptBuilder.ts` L220-L371），与 Anthropic 原生指令同一强度。但猫仍然违反"P1 终态基座不是脚手架"等关键原则。证明"注入了 ≠ 会遵守"。

2. **愿景守护流于形式**：F101（mode v2 / 狼人杀）案例——铲屎官愿景是"删旧 mode，把狼人杀加入 mode"。第一次：狼人杀做了但没加入 mode。第二次：旧 mode 删了但狼人杀还是没入口。两次愿景守护都没拦住。规则 `shared-rules.md §9` 写着"AC 全打勾 ≠ 完成"，但守护猫只审计了 checkbox，没有从用户视角验证体验。
...
4. **声明式规则的固有弱点**：写"问自己铲屎官体验如何"就像在公路上立"注意安全"牌子——没有任何机制能验证猫是否真的执行了这个思考过程。

### 解决什么

让关键治理规则从"写在那里靠自觉"变成"必须产出证物才能通过的硬门禁"，同时给铲屎官一组 magic words 作为紧急注意力锚点。
...
1. **`request-review` Gate**：发 review 请求时，缺少铲屎官原话摘录或证据 → BLOCKED（复用现有 request-review skill 的 BLOCKED 机制）
...
- 铲屎官原话摘录（≥1 条）
- 证据（测试输出 / 截图 / 命令输出）
- 对照结论（一句话："铲屎官说 X，现在的状态是 Y，匹配/不匹配"）
...
- [ ] AC-A2: 铲屎官发送「脚手架」时，猫输出"当前产物 vs 终态"对比分析
- [ ] AC-A3: 铲屎官发送「星星罐子」时，猫停止新增副作用并进入等待指示状态
- [ ] AC-A4: 喵约层精简到 ≤5 条宪法级原则
- [ ] AC-A5: `pnpm --filter @cat-cafe/api test` 通过（SystemPromptBuilder 测试）

### Phase B（Evidence Gate）
- [ ] AC-B1: 愿景守护输出包含"铲屎官原话 vs 实际状态"对照表
- [ ] AC-B2: 对照表中有未匹配项时，守护猫明确拒绝放行
- [ ] AC-B3: 完成声明必须附铲屎官原话摘录 + 证据
- [ ] AC-B4: `request-review` skill 缺少铲屎官原话摘录/证据时输出 BLOCKED
...
| # | 问题 | 状态 |
|---|------|------|
| OQ-1 | Magic words 是否需要多语言支持（中/英）？ | ⬜ 未定 |
| OQ-2 | Evidence gate 的证物格式是否需要 parser 自动校验？ | ⬜ 未定 |
| OQ-3 | 升级管道的"2 次复发"如何计数？人工记录 or 自动化？ | ⬜ 未定 |

## Key Decisions

| # | 决策 | 理由 | 日期 |
|---|------|------|------|
| KD-1 | 四层架构（喵约/Magic Word/Skill/Gate） | 宪宪 + 砚砚独立思考后合并的共识方案 | 2026-03-13 |
...
| 日期 | 事件 |
|------|------|
| 2026-03-13 07:00 | 铲屎官 + 宪宪闲聊发现问题（sync 脚本 + 图灵测试 + 愿景守护失败） |
| 2026-03-13 14:30 | 宪宪 + 砚砚独立分析 → 合并四层架构方案 → 立项 |
| 2026-03-13 15:01 | 砚砚 review 初版 spec → 4 条 P1 修改（gate 落点/星星罐子语义/5 条锁定/定位调整） |

## Review Gate

- Phase A: 跨家族 review（缅因猫 review SystemPromptBuilder 改动）
- Phase B: 铲屎官亲自验收（用真实 magic word 测试猫的反应）

## Links

| 类型 | 路径 | 说明 |
|------|------|------|
| **Discussion** | `docs/discussions/2026-03-13-governance-magic-words/` | 铲屎官 + 宪宪 + 砚砚讨论原始记录 |
| **Feature** | `docs/features/F086-meta-thinking-reflection.md` | 前置：元思考触发器 |
| **Feature** | `docs/features/F073-bulletin-board.md` | 前置：公告板（当前是告示牌不是门禁） |
| **Feature** | `docs/features/F041-*` | 教训源：AC 全 ✅ 但 UI 不可用 |
| **Plan** | `docs/plans/2026-03-13-f059-sync-modular-proposal.md` | 触发案例：sync 脚本质量问题分析 |

## 需求点 Checklist

> 来源：铲屎官 2026-03-13 对话

- [ ] RC-1: 铲屎官说「脚手架」→ 猫停下来检查产物是否终态
- [ ] RC-2: 铲屎官说「星星罐子」→ 猫立刻停手
- [ ] RC-3: 愿景守护必须对照铲屎官原话，不能只看 AC checkbox
