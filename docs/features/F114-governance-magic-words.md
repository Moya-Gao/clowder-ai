...
1. **愿景守护流于形式**：F101（mode v2 / 狼人杀）案例——铲屎官愿景是"删旧 mode，把狼人杀加入 mode"。两次愿景守护都没拦住：守护猫只审计了 checkbox，没有从用户视角验证体验。

2. **声明式规则的固有弱点**：shared-rules §9 写着"AC 全打勾 ≠ 完成"，但没有任何机制能验证猫是否真的执行了这个检查。

3. **铲屎官缺少紧急拉闸手段**：猫走偏时，铲屎官只能用自然语言描述问题，没有能快速触发特定行为模式的触发词。

### 解决什么

两件事，不多不少：
1. 给铲屎官 4 个 **magic words** 作为手动拉闸手段
2. 把愿景守护从"审计 checkbox" 升级为 **必须交证物才放行的 Gate**

### 不做什么（v1 讨论后明确排除）

- ~~喵约瘦身到 5 条~~ — 铲屎官指出：从系统提示词移走 = 注意率从 70% 降到 1%，是开倒车
...
| **「星星罐子」** | P0 不可逆风险 | 停止新增副作用（不发新命令、不写新文件、不 push），等铲屎官指示 |

> **定位**：Magic Words 是注意力锚点（铲屎官手动拉闸），不是最终安全机制。最终约束以 Gate 为准。
...
| KD-6 | 现有 shared-rules 17 条结构不动 | 铲屎官 + 宪宪 + 砚砚三方共识：当前结构是最优折中 | 2026-03-13 |

## Timeline

| 日期 | 事件 |
|------|------|
| 2026-03-13 07:00 | 铲屎官 + 宪宪闲聊发现问题 |
| 2026-03-13 14:30 | 宪宪 + 砚砚独立分析 → 四层架构方案（v1） |
...
- 跨家族 review（缅因猫 review SystemPromptBuilder + feat-lifecycle 改动）
- 铲屎官亲自验收（用真实 magic word 测试猫的反应）
...
**蚊帐机制**：magic word 表只放「下次一定」一个（铲屎官好喊），但 F177 Phase A 的 quality-gate follow-up 字样扫描（`check-followup-tails.mjs`）同时覆盖语义同族关键词列表。
