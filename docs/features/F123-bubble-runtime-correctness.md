...
铲屎官关于“气泡为什么又双影 / 又丢 / 又要 F5 才正常”的痛点，从第一天修到现在还在复发。`F081` 已经完成了第一阶段的连续性修复和写路径审计，但现状仍然是：
...
| # | 问题 | 状态 |
|---|------|------|
| OQ-1 | callback text 与 stream text 在哪些场景应该明确共存，哪些场景必须视为同一 bubble 的不同阶段？ | 🟡 默认规则已收敛：同 invocation 且语义重叠时 callback 替换 stream；边界留给 Phase A fixture 验证 |
| OQ-2 | `MessageWriter`/统一 reducer 是否在 F123 本轮就落地，还是先做 shared helper + invariant 过渡？ | 🟡 先 shared helper + invariant，MessageWriter 暂不作为本轮前置 |
| OQ-3 | provenance / timeline dump 只做 debug dump，还是同时在 UI 暴露“查看来路”？ | 🟡 先做 debug dump，无 UI 入口 |
