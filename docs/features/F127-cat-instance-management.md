...
> **Status**: done | **Completed**: 2026-04-29 | **Owner**: 金渐层 + 砚砚 | **Priority**: P1
...
- [x] AC-A3: 现有 F062 布偶猫 profile 无缝迁移到新账户系统
...
| OQ-2 | 运行时新增猫是否需要重启 CLI 进程 | ✅ 不需要重启。新会话通过 registry-first roster 获取动态猫；resume 会话在 CatRegistry revision 变化后会重新注入 system prompt，刷新 roster，同时保留普通 resume 的 F-BLOAT 跳过行为 |
...
| KD-4 | `owner` → `coCreator` 术语统一 | F127 intake 引入了 `owner` 概念（指铲屎官），但 `owner` 在 CS 领域是过载术语（repo owner, worklist owner），改为 `coCreator` 对齐愿景（共创者） | 2026-03-21 |
...
| 2026-03-21 | Intake 后修复 — PR #631 修复 UI 退化（head avatar/color 丢失）+ owner→coCreator 重命名（114 files），5 轮 review（codex R1-R3 + gpt52 R4-R5） |
