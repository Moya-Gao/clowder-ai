...
猫目前无法帮助铲屎官准备新 thread。当话题需要独立上下文时（如新 issue 调查、子任务分配），猫只能口头请求铲屎官去前端手动创建，打断了自主工作流。

> 发现场景（issue #82）：铲屎官要求"新开一个 thread"，但猫没有 API 可调，只能等铲屎官手动操作。

但直接给猫暴露 `cat_cafe_create_thread` 也不对。Thread 是用户可见、持久化、会改变工作空间结构的对象；猫可以起草创建信息，但不应悄悄创建。F128 的产品目标不是"猫绕过铲屎官创建 thread"，而是：

> 猫猫把新 thread 的信息填好，以卡片形式展示；铲屎官确认或编辑后，系统再创建。
...
| 谁确认 | 铲屎官或具备 thread create 权限的用户 |
| 谁执行创建 | 后端使用用户确认上下文执行 |
...
> 铲屎官反馈：`final-only` 模式下猫仍不断 ACK 主 thread、浪费 token。
...
上述 review 聚焦在 PR #85 的技术拆分与 P2 缺陷；铲屎官在 2026-05-22 补充了更上层的产品判断：
