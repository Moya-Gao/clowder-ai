...
铲屎官 2026-03-21 进一步明确：

> 铲屎官不会给精确路径。"帮我打开日志""看看 F131 的设计图""打开那个 discussion"——猫猫自己能 glob/grep 到路径，自己去传精确路径到 API。对人类来说说全路径太不友好了。

核心痛点：铲屎官让猫猫一起看某个文件/目录时，只能靠自己在 Workspace Explorer 里手动点击层层目录。猫猫有 `setWorkspaceOpenFile` / `revealInTree` 等前端能力，但没有对外暴露的 HTTP 端点供猫猫调用。browser-preview 的 `auto-open` 已证明这种模式可行且体验好。
...
| 猫猫意图匹配不准（找错文件） | Skill 文档提供明确的搜索策略 + 多结果时让铲屎官确认 |

## Open Questions

| # | 问题 | 状态 |
|---|------|------|
| ~~OQ-1~~ | ~~Phase B 的模糊匹配是否需要调用 LLM 做意图解析？~~ | ✅ 已定：不需要后端做，猫猫自己用 glob/grep 解析 |
...
| KD-3 | 不分 Phase A/B，单 Phase 三层：基础设施层 + 硬实力层 + 软实力层(Skill) | 铲屎官拍板——模糊路径解析是猫猫的 Agent 能力（硬实力），不需要后端做；Skill 教猫猫怎么做（软实力）；API/Socket 是管道（基础设施） | 2026-03-21 |
| KD-4 | 猫猫传给 API 的路径必须是精确路径，模糊意图解析在 Agent 侧完成 | 铲屎官："我不会告诉你全路径，你自己能 glob 到的" — Agent 本身就是路径解析器，无需后端 LLM | 2026-03-21 |
