...
**Design Gate 决策（砚砚 2026-03-23）**：不用 `check-suites` 或 `check-runs` 做主轮询入口。GitHub 的 CI 有两套体系（Checks API + commit statuses），raw Checks API 只覆盖前者，会漏掉仍走旧式 `commit status` 的 CI 提供方。
...
| OQ-2 | 前端 ConnectorBubble 是否需要 `github-ci` 专属 icon？还是复用 GitHub icon + 不同颜色？ | ⬜ 未定（Phase A 不阻塞） |
| ~~OQ-3~~ | ~~CI 成功是否默认静默？~~ | ✅ 已定：CI 成功发消息但不 trigger，不静默 |
| ~~OQ-4~~ | ~~PrTracking 注册时是否需要显式指定 `ciTracking: true`？~~ | ✅ 已定：v1 默认 true，后续需要 review-only 场景再加开关 |
| OQ-5 | Raw Checks API 对 fork push 有已知 caveat，PR 语义和 SHA 语义并不总是同一个视角。用 PR 级 rollup 是否足够覆盖 fork 场景？ | ⬜ 待实测验证 |
...
| KD-2 | 数据源用 PR 级 rollup（`gh pr view --json statusCheckRollup`），不用 raw Checks API | 一次请求拿 headSha + lifecycle + 聚合状态；覆盖 Checks + commit statuses 两套体系；砚砚 Design Gate 提出 | 2026-03-23 |
...
5. Design Gate 建议 CI failure trigger 用 `normal`；后续经铲屎官确认，升级为 `urgent` 以与 review 优先级行为归一（F175 修正：urgent 语义为队列内优先出队，不再走抢占旁路）
