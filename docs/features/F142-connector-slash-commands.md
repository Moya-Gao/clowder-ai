...
**Scope 边界：只改 connector 侧（`ConnectorCommandLayer` + 后端 API），不碰 Web 端类型定义和 `useChatCommands`。**
...
- `/cats` — 查看当前 thread 的猫猫（已加入 + 可调度但未加入 + 不可用）
- `/status` — thread 概览（标题、创建时间、参与猫数、最近活跃）

**A2 — 聚合 API**：
- `GET /api/threads/:id/cats` — 返回结构化猫猫数据（含 owner/participant 权限校验）
...
```typescript
{
  ...
  unavailable: CatSummary[];         // 已注册但当前不可用（离线/忙）
...
2. **Skill 命令声明**（走现有 manifest/capabilities 链路，不直接扫 SKILL.md）：
   - `manifest.yaml` 新增 `slashCommands` 字段（zod schema 校验：命令名白名单正则 `/^\/[a-z][a-z0-9-]{1,30}$/`、描述长度上限 200 字符、纯文本）
...
- [ ] AC-A2: `/cats` 在 connector 中返回：participants（已加入）+ routableNow + routableNotJoined + unavailable，口径 = `registeredService ∩ isCatAvailable`
- [ ] AC-A3: `/status` 在 connector 中返回 thread 标题、创建时间、参与猫数、最近活跃时间
- [ ] AC-A4: 清理幽灵命令（`/game status`、`/game end` 等），注册表-执行器一致性测试通过
- [ ] AC-A5: `GET /api/threads/:id/cats` 聚合 API 可用，返回上述结构
- [ ] AC-A6: `/api/threads/:id/cats` 有 owner/participant 权限校验，非授权请求返回 403
...
- [ ] AC-B3: `slashCommands` 字段 zod 校验：命令名白名单正则、描述长度上限、纯文本（禁止 HTML/脚本注入）
...
| 新 API 数据泄露（thread 猫列表） | `/api/threads/:id/cats` 强制 owner/participant 权限校验 |
...
| OQ-2 | `/cats` 返回的 unavailable 猫是否需要区分"离线"vs"忙"？ | ⬜ 未定 |
...
| KD-3 | 命令注册增加 surface 维度（Phase B） | 避免"注册了但当前入口不能用"的用户困惑（砚砚建议） | 2026-03-27 |
| KD-4 | Phase A 不碰 Web 端类型定义 | 砚砚 P1：`surface` 字段改类型定义会波及 Web 端，超出 scope（推 Phase B） | 2026-03-27 |
| KD-5 | `/cats` 口径锚定 AgentRouter 现有逻辑 | 砚砚 P1：不另造计算口径，绑定 `registeredService ∩ isCatAvailable` | 2026-03-27 |
| KD-6 | Skill 命令走 manifest/capabilities 链路 | 砚砚 P2：已有元数据扫描机制，不直接解析 SKILL.md 避免双份解析器 | 2026-03-27 |
...
| 2026-03-27 | 砚砚 review spec v1，提出 3P1+2P2，宪宪修订 spec v2 |

## Review Gate

- Phase A: 砚砚 review（已深度分析现状 + connector 双轨架构）
- Phase B: 砚砚 review + 铲屎官确认扩展机制
...
| **Code** | `packages/api/src/infrastructure/connectors/ConnectorCommandLayer.ts` | connector 命令层（Phase A 主改动点） |
| **Code** | `packages/api/src/domains/cats/services/agents/routing/AgentRouter.ts` | `/cats` 口径来源 |
| **Code** | `packages/web/src/config/command-registry.ts` | 命令注册表（Phase A0 清理幽灵命令） |
