...
| # | 问题 | 状态 |
|---|------|------|
| OQ-1 | 行业是否会出现统一的 Agent Hosting Protocol 标准？ | ⬜ 持续观察 |
| OQ-2 | WebSocket agent 的真实需求场景有哪些？ | ⬜ 待验证 |
| OQ-3 | 现有 provider 迁入顺序如何排优先级？ | ⬜ Phase D 前决定 |
...
| 日期 | 事件 |
|------|------|
| 2026-03-27 | 立项。起源：playground ACP 代码分析 → 两猫讨论 → GPT Pro 咨询 → Part 3 综合 |

## Review Gate

- Phase A: 架构级——猫猫讨论 → 铲屎官拍板（ADR-023 审批）
...
| 类型 | 路径 | 说明 |
|------|------|------|
| **Research** | `docs/research/2026-03-27-hostable-agent-runtime-abstraction-gpt-pro-consult.md` | GPT Pro 咨询（Part 1 提示词 + Part 2 回答 + Part 3 两猫综合） |
...
| **Decision** | `docs/decisions/023-hostable-agent-runtime.md` | 统一宿主抽象架构决策（ADR-023） |
| **Decision** | `docs/decisions/001-agent-invocation-approach.md` | Agent 调用方式选择（历史决策） |
| **External** | `clowder-labs/clowder-ai playground` 分支 | ACP 参考实现 |
