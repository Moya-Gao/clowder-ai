...
**根因**：F041 的 capability orchestrator 只做到"统一真相源 + 自动生成三份 CLI 配置"，但没有区分"期望态"和"本机解析态"。`capabilities.json` 混进了机器特定的绝对路径（如 pencil 的 `/Users/lysander/.antigravity/extensions/highagency.pencildev-0.6.32-universal/out/mcp-server-darwin-arm64`），导致：
...
**愿景**：铲屎官在新电脑上 `git clone` + 一条命令，所有 MCP 自动解析、配置生成、就绪报告。不需要手动挂任何 MCP。
...
|---|---|---|
| 布偶猫（Claude Code） | ✅ 原生支持 | Claude Code 运行在项目目录，自动发现 `.mcp.json` |
| 砚砚（Codex） | ✅ 原生支持 | 同上 |
| 烁烁（Gemini ACP） | ❌ 不支持 | `acp-mcp-resolver.ts` 的 `projectRoot` 硬编码为 `findMonorepoRoot()`，只读 Cat Café monorepo 的 `.mcp.json` |
...
- Phase A: 缅因猫 review（砚砚参与了架构讨论，由他验收实现）
- Phase B: 跨家族 review
...
| **Workaround** | `mcp-config-adapters.ts:37-40,188` | Gemini skip pencil |

## 需求点 Checklist

| # | 需求点 | AC | Phase | 来源 |
|---|--------|-----|-------|------|
| R1 | capabilities.json 不存机器特定路径 | AC-A1 | A | 铲屎官原话 |
| R2 | Pencil 支持 Antigravity + VS Code 双宿主 | AC-A2 | A | 铲屎官原话 |
| R3 | 新机器 clone 后 MCP 自动解析 | AC-A3,A4,B5 | A+B | 铲屎官愿景 |
| R4 | Skill 能声明 MCP 依赖 | AC-B1,B2,B3 | B | 砚砚提议 |
| R5 | 一条命令看全局 MCP 就绪状态 | AC-B4,B5 | B | 铲屎官愿景 |
