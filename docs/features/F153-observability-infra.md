...
Phase E 只回答“发生了什么”（traces、metrics、健康状态），不做质量判断或打分。记忆命中率、A2A 接力成功率等 eval 色彩的指标留给未来 phase，首版不实现。UI 命名为「观测台 / Observability」，不叫「Eval Dashboard」。

**L1: 数据层（API 侧）**
1. `LocalTraceExporter` — 自定义 `SpanExporter`，在 `RedactingSpanProcessor` 之后消费 redacted spans，投影为 DTO 后写入内存 ring buffer（双阈值：`maxSpans` + `maxAgeMs`）
...
5. **时序快照 ring buffer**（`MetricsSnapshotStore`）
   - `setInterval`（默认 30s）调用 `PrometheusExporter.collect()`，序列化为快照 DTO，再写入内存 ring buffer
   - 快照 DTO：`{ timestamp, metrics: Record<string, number> }`，只保留 gauge/counter 当前值（非全量 Prometheus 文本）
...
10. **burn-rate 阈值检查**
    ...
    - 超标时通过 SSE/WebSocket 推送 `system_notice` 事件，前端 `SystemNoticeBar` 弹出 notice（复用 F508）
...
| KD-16 | F153 = descriptive observability，不做 normative eval | Phase E 只展示“发生了什么”，eval 信号留给未来 phase（讨论见 `2026-04-19-online-evaluation-trends-and-cat-cafe.md`） | 2026-04-21 |
...
| **Issue** | `zts212653/clowder-ai#544` | Phase E 对应 issue（Hub 内嵌观测台） |
| **PR** | `zts212653/clowder-ai#545` | Phase E spec PR（Hub 内嵌观测台） |
| **Issue** | `zts212653/cat-cafe#1341` | Intake Intent Issue（Phase E spec manual-port） |
