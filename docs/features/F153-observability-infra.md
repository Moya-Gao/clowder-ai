...
> **Discussion**: 2026-04-22，三猫讨论（布偶猫 + Sonnet + GPT-5.4）
>
...
> **Discussion**: 2026-05-22，Design Gate（Ragdoll + 砚砚/codex GPT-5.5 + gpt52/GPT-5.4 + 布偶猫/Sonnet 4.6）— Sonnet 提了 "Hybrid A+C" 替代方案（transformer 内 UUID 状态机 + 栈 fallback），与 codex/gpt52 的"明确降级"立场冲突，最终采纳 codex/gpt52 的明确降级路线（KD-41），Sonnet 提案 rejected
> **Implementation PRs**:
...
- [x] AC-G3: `capturePromptIfEnabled` 在 `invoke-single-cat` fire-and-forget 调用，不阻塞 invocation hot path. Wired at [invoke-single-cat.ts:1548](packages/api/src/domains/cats/services/agents/invocation/invoke-single-cat.ts); AC-G10 follow-up keeps the fire-and-forget invariant via async `runCapture()` wrapper inside the bridge.
...
| # | 决策 | 理由 | 日期 |
|---|------|------|------|
| KD-1 | 社区 PR 先不放行，P1 修完再 intake | 缅因猫 review 发现 counter 泄漏 + 端口硬编码 | 2026-04-09 |
...
| KD-45 | Phase K 归入 F153（不开新 F 号）：复用记忆系统 `IndexStatus.tsx` 的功能开关面板机制（`filterEvidenceVars` → `PATCH /api/config/env`），在 `HubObservabilityTab` 中按 `category === 'telemetry'` 过滤渲染 toggle。启动时变量（`OTEL_SDK_DISABLED` / `TELEMETRY_HMAC_SALT`）标"需重启"放配置参考区，热更新变量（`PROMPT_CAPTURE`）做 toggle | 铲屎官确认（2026-06-26）：改的就是 F153 Phase E 产物，env 全在 telemetry 分类，scope 小不值得新 F 号 | 2026-06-26 |
...
| 2026-04-13 | Phase A merge + intake 完成（OTel SDK + Metrics + Health Check） |
| 2026-04-15 | Phase C merge + intake 完成（inline @mention observability） |
| 2026-04-18 | Phase D merge + intake 完成（`TELEMETRY_DEBUG` + startup semantics） |
| 2026-04-22 | Phase E spec: `clowder-ai#545` merge；`cat-cafe#1341` 建立 Intake Intent Issue，按 `manual-port` 吸收 Phase E 真相源 |
| 2026-04-24 | Phase E implementation: `clowder-ai#546` merge；`cat-cafe#1371` 建立 Intake Intent Issue，按 `safe-cherry-pick + manual-port` 回家吸收 |
...
| 类型 | 路径 | 说明 |
|------|------|------|
| **PR** | `zts212653/clowder-ai#393` | Phase A 社区 PR（OTel SDK + Metrics + Health Check） |
| **Issue** | `zts212653/clowder-ai#388` | F153 对应社区 issue |
...
| **Issue** | `zts212653/clowder-ai#544` | Phase E 对应 issue（Hub 嵌入式可观测） |
| **PR** | `zts212653/clowder-ai#545` | Phase E spec PR |
| **PR** | `zts212653/clowder-ai#546` | Phase E implementation PR |
| **Issue** | `zts212653/clowder-ai#592` | Phase F 对应 issue（trace persistence via pointer association） |
| **PR** | `zts212653/clowder-ai#579` | Phase F implementation PR |
| **Issue** | `zts212653/cat-cafe#1341` | Phase E spec 的 Intake Intent Issue |
| **Issue** | `zts212653/cat-cafe#1371` | Phase E implementation 的 Intake Intent Issue |
| **Issue** | `zts212653/cat-cafe#1448` | Phase F implementation 的 Intake Intent Issue |
| **Issue** | `zts212653/clowder-ai#583` | Phase G 对应 issue（Prompt X-Ray + A2A trace propagation） |
| **PR** | `zts212653/clowder-ai#619` | Phase G implementation PR |
| **Issue** | `zts212653/clowder-ai#721` | Phase I 对应 issue（Step Summary / Agent Loop count） |
| **PR** | `zts212653/clowder-ai#732` | Phase I implementation PR |
| **Issue** | `zts212653/cat-cafe#1836` | Phase I implementation 的 Intake Intent Issue |
| **Feature** | `docs/features/F130-api-log-governance.md` | 日志治理（logging 层） |
| **Research** | `docs/research/2026-04-19-online-evaluation-trends-and-cat-cafe.md` | Online Eval 行业趋势与猫咖对照 |
