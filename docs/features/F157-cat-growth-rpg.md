...
| 维度 | 数据源 | 说明 |
|------|--------|------|
| 架构力 | ADR/spec 撰写、方案决策参与 | 设计与系统思考 |
| 审查力 | Review 拦截率、bug 发现数 | 质量守护 |
| 审美力 | 设计反馈被采纳次数、UI 贡献 | 视觉与体验 |
| 执行力 | Task 完成数、代码贡献、测试覆盖 | 落地交付 |
| 协作力 | 跨猫协作频率、handoff 成功率 | 团队配合 |
| 洞察力 | 主动发现问题次数、self-evolution 事件 | 自驱与发现 |

**经验值结算引擎**：
- 每个 task / review / 讨论闭环后自动结算
- 数据源：session events、task 状态变更、PR tracking 记录
- 经验值公式透明可审计
...
- [ ] AC-A1: 六维属性从 session events / task / PR 数据自动结算，无需人工打标
