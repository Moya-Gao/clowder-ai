...
# F158: ACP Carrier Generalization — 多载体复用同一 Runtime Policy

> **Status**: spec | **Owner**: TBD | **Priority**: P2

## Why

F149 交付了完整的 ACP runtime operations（进程池 / session lease / lifecycle / watchdog），但第一载体只有 Gemini。铲屎官原话（2026-03-31）：

> "我们要支持acp这个协议 支持烁烁acp接入 其实 codex 和claude code也支持这个协议。"

当前 `AcpProcessPool` 和 `AcpClient` 的接口没有 Gemini-specific 的硬依赖，但"没有 hard dependency ≠ 已验证可泛化"。F158 的目标是让第二个 ACP carrier 走通同一套池化/lease 模型，验证泛化性。

**Scope 来源**：从 F149 Phase D 拆出（2026-04-13 铲屎官拍板）。Gemini 作为第一个已有实现，有需求时再继续。
...
| 日期 | 事件 |
|------|------|
| 2026-04-13 | 从 F149 Phase D 拆出立项（铲屎官拍板 scope 收窄） |
