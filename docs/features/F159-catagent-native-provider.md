...
> **Status**: in-progress | **Owner**: 社区 (bouillipx) + 布偶猫 + 缅因猫 | **Priority**: P1
...
| # | 问题 | 状态 |
|---|------|------|
| OQ-1 | CatAgent v1 是否只允许 first-party 使用，不对第三方 provider 开放同一路径？ | ⬜ 未定 |
| OQ-2 | provider 内部的 compaction 应该完全 provider-owned，还是需要宿主层提供约束槽位？ | ⬜ 未定 |
| OQ-3 | read-only tools 在 v1 是否必要，还是先用 text-only provider 验证 northbound contract？ | ⬜ 未定 |
...
| 日期 | 事件 |
|------|------|
| 2026-04-10 | 社区 PR `clowder-ai#397` 被 maintainer review 退回，定性为 architecture-blocked spike |
| 2026-04-10 | 社区 RFC `clowder-ai#434` 创建，开始按 “native provider” 方向重述 |
| 2026-04-11 | 旧 issue `#396` 关闭，保留 `#434` 作为唯一讨论入口 |
| 2026-04-11 | 正式 kickoff F159，进入真相源 |
| 2026-04-25 | `clowder-ai#549` 合入：Phase E SSE streaming + strict fail-closed turn handling |

## Review Gate

- Phase A: 布偶猫 + 缅因猫架构 review → 铲屎官拍板
...
| 类型 | 路径 | 说明 |
|------|------|------|
| **Community** | `zts212653/clowder-ai#434` | RFC 讨论入口 |
| **PR** | `zts212653/clowder-ai#397` | 已关闭的 spike PR |
| **PR** | `zts212653/clowder-ai#549` | 已合入的 Phase E — SSE streaming |
| **Feature** | `docs/features/F143-hostable-agent-runtime.md` | 宿主抽象母 feature |
| **Feature** | `docs/features/F149-acp-runtime-operations.md` | runtime ops 经验输入 |
| **Feature** | `docs/features/F153-observability-infra.md` | 审计与 usage 可观测能力 |
| **Decision** | `docs/decisions/001-agent-invocation-approach.md` | CLI vs API path 边界 |
