...
毛线球（TaskPanel）自上线以来 **从未被任何猫主动使用过**。三猫头脑风暴（宪宪+砚砚+烁烁，2026-04-11）诊断出四个根因：
...
| 层级 | 组件 | 范围 | 数据源 | 生命周期 |
|------|------|------|--------|---------|
| **Mission Hub** (F049) | BacklogCenter | 项目/Feature 级 | BACKLOG.md + feature specs | 跨 thread |
...
适用场景：铲屎官提了需要跟踪的事项、多猫协作分工、长期追踪项。
...
- Phase B: **铲屎官 review**（前端 UI/UX，需 Design Gate 确认 wireframe）+ 跨家族 code review
- Phase C: 跨家族 review
...
| 类型 | 路径 | 说明 |
|------|------|------|
| **Feature** | `docs/features/F055-plan-board.md` | 猫猫祟祟 — 调用级任务面板 |
| **Feature** | `docs/features/F049-mission-hub.md` | Mission Hub — 项目级任务中心 |
