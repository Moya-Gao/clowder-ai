...
| 2026-06-27 | Phase P 已知问题 **软层修复** merged (PR #2617, squash `17b0b7494`) — hold_ball 概念边界：`callback-tools.ts` description Not-for 增"等铲屎官/猫回复→@landy/@cat（消息触发你，timer=冗余第二触发）" + Phase M GOTCHA 明确"人/猫消息 IS a callback" + wakeWhen 收窄"仅本地命令非万能 smart wait" + `routing-guard-remedial.ts` REMEDIAL_PROMPT 等人→@landy/@句柄。2 regression 锁（21 mcp-server + 13 api guard 全绿）。gpt52 跨族 APPROVE (No findings)；cloud 豁免 (text-only scope, opus-47/4.6 directive)。硬层（waitSourceRef enforcement）留 PR-O3。调查+实施 thread `thread_mqwe66e0xpxwhi9o`。 |
...
2. ✅ **软层（概念边界，改 3 surface）— 已实施 PR #2617 (`17b0b7494`，gpt52 APPROVE + cloud 豁免)**：① `callback-tools.ts` hold_ball Not-for 补"等铲屎官/另一只猫 → @landy/@cat，他们的消息会触发你，再挂 timer = 冗余第二触发源"；② `routing-guard-remedial.ts:76` "等外部条件"收紧为"等**无回调**的外部条件"；③ Phase P wakeWhen 描述回收 Not-for 边界
