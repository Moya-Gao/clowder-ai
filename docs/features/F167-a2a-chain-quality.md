...
| 2026-06-20 | Phase O **PR-O2b-fix** — hotfix for vision guardian block: `redactGroundingSample()` applies hmacId() to invocationId/threadId/sourceThreadId, removes claimSummary (free-text outside spec whitelist). Aligns grounding-samples endpoint with traces endpoint redaction pattern. LL: cloud review trigger #N must wait for result before merge; author pushback is not terminal verdict. |
