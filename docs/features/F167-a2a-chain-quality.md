...
**F167 Close 门槛更新**：Phase O 收敛（**PR-O1 ✅ → O2a ✅ → O2b ✅ → O2b-fix ✅ → O3 ✅** → O4）+ PR-O2b shadow 1 周 telemetry 观察 + F192 weekly verdict 通过后才可 close。Phase N 与 Phase O 关系：keep PR-tracking hard-block + patch issue_tracking/hold_ball 已确认；Phase N 不再是 F167 close 的最后一步。
