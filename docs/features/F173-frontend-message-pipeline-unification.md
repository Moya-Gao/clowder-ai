...
| 2026-04-24 18:47 | **Phase A hotfix3 诊断（Orphan Draft Bubble）** — Phase B-1 merge 后实机验收发现新 dup-bubble 变种：`draft-{inv}` 持久化占位 + `msg-{inv}-{cat}` live stream 并存。根因 = `messages.ts:1265-1312` draft merge 缺 invocation-alive 验证，invocation 在产生 formal message 前异常终止留下孤儿 draft。详见 [bug-report](../bug-report/2026-04-24-f173-orphan-draft-bubble/bug-report.md)。Hotfix PR 待启动。 |
