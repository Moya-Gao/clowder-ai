...
| 2026-04-26 23:14 | **Phase E Task 3-5 merged (PR #1426, squash `862d978b4`)** — KD-1 handler unification 真闭环。砚砚 GPT-5.5 接手实施: handleBackgroundAgentMessage + helpers (recoverStreamingMessage / shouldSuppressLateBackgroundStreamChunk / findBackgroundCallbackReplacementTarget / ensureBackgroundAssistantMessage / markThreadInvocationActive/Complete + text/error/done 三段) inline 到 useAgentMessages.ts (+1038 行 → 3043 行)。**3 个 bg 源文件真删除** (useSocket-background.ts 634 行 + .types.ts 111 行 + useSocket-background-system-info.ts 341 行)。drift risk 结构性消除：active+bg 走同一份业务逻辑实现，未来 race fix 自动覆盖两条路径。第一轮砚砚提交 rename-only PR (+32/-34 file rename) 被宪宪 P1 push back ("rename 不是迁移，stub anchor 变种")，重做后 +1124/-1159 真 inline。云端 Codex review no major issues ("Delightful")。527/527 hooks + tsc + biome 全绿。**铲屎官特批 same as PR #1421/#1423** api gate `mcp-config-adapters.test.js` + `capability-orchestrator.test.js` pre-existing env leak 跳过。**未来 TD**: useAgentMessages.ts 现在 3043 行远超 350 行硬上限，建议 follow-up 拆 cohesive sub-modules (msg-text-handler / msg-error-handler / msg-done-handler)，挂 TD list 不挂 stub feature spec。 |
...
| 2026-04-27 02:45 | **Closed-state hotfix merged (PR #1429, squash `9e0301cde`)** — `thread_mogj6kvwp3l80x56` dup bubble follow-up（铲屎官 04-27 02:08 实测质询：F173 close 后又见"我的消息 / 布偶猫 / 布偶猫重复气泡 / 我的消息 / 布偶猫气泡"）。砚砚 GPT-5.5 诊断：outer wrapper `msg.invocationId`（user-turn parent）vs inner `parsed.invocationId`（auth child）在前端 identity 里混用——active path 绑 outer id，bg path 绑 inner id → 同一逻辑响应两 bubble。Fix：`useAgentMessages.ts` 8 处 invocationId derivation 全部 canonicalize 成 outer-priority (`msg.invocationId ?? parsed.invocationId`)，覆盖 `consumeBackgroundSystemInfo` (invocation_created / session_started / task_progress) + 主 callback active branch (同 3 类 + web_search / thinking / rich_block)。其中 web_search/thinking/rich_block 三处之前写反了 (`parsedInv ?? msg.invocationId`)，是历史遗留 inversion。新 fixture `useAgentMessages-invocation-id-canonicalization.test.ts` 9 测试 (active+bg × invocation_created/session_started/task_progress + active × web_search/thinking/rich_block)，断言 `deriveBubbleId` 返回 `msg-{OUTER}-{cat}` 不是 `msg-{INNER}-{cat}`。砚砚 GPT-5.4 review 2 round（第 1 轮串了 PR #1364 / `ff1487364` SHA → 47 push back 给 GitHub 真相源 + git log 实证 → 砚砚撤回错误 blocker → 第 2 轮基于 PR #1429 真实 2 文件 diff 正式放行 `b9126c19a`；47 补 3 个 fixture 覆盖砚砚指出的 web_search/thinking/rich_block 缺口 → 砚砚续签 `9da9a1588`）。云端 Codex no major issues ("Hooray!")。2562/2562 web 全套 + tsc + biome 全绿。**铲屎官 implicit 特批 same as Phase E**：api gate `capabilities-route.test.js > realigns stale managed cat-cafe MCP paths` pre-existing `CAT_CAFE_RUNTIME_ROOT` 环境泄漏跳过——前端 only PR 不动 packages/api。**教训沉淀**：reviewer SHA 串掉是真实风险，merge-gate Review Continuity Guard 不是仪式，HEAD 对不上必须 push back。 |
