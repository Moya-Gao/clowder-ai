...
> **Status**: in-progress (reopened — close attempt #2 失效，alpha 验收 #3 否决) | **Owner**: 布偶猫（Opus-47）+ 缅因猫（GPT-5.4，跨家族独立 review） | **Priority**: P1 | **Reopened**: 2026-04-26 (D2b-2 interaction semantics 三次否决 — badge 点不掉 + 强制跳 deep-link + 视觉撑爆)
>
...
> **Phase D2b-2 (system-level health indicator, rev2)**: ✅ merged 2026-04-26 via PR #1419 (squash `0a202dad1`) — HubButton badge merge after rev0 (per-cat dot, PR #1403/#1410 否决) + rev1 (standalone plug indicator, PR #1419 否决) two alpha rejections
...
- **D2b frontend** 📐 design稿 ready (`designs/F174-callback-auth-health-card.pen`)
  ...
  - **D2b-2 system-level health indicator**（P0 · 实体层 — twice revised after alpha 反馈）— PR #1403 (rev0 per-cat dot) → PR #1410 (rev1 standalone plug indicator) → PR #1419 (rev2 HubButton badge merge ✅ merged)
    ...
    - [ ] AC-D6 (rev3): 系统级 affordance merge 进 HubButton (top-bar 现有 entity)，复用 GitHub bell icon / Slack unread / iOS app badge **未读 → 看过 → 消失** 通知 mental model；非 owner 不渲染；不在 top-bar 增加新图标
    - [ ] AC-D7 (rev3): HubButton badge 显示 `unviewedFailures24h`（不是 totalFailures24h）— 进入 observability/callback-auth subtab 自动 mark-viewed → badge 消失 / 仅显示 viewed 之后的新失败；click HubButton **始终走 default openHub()**（撤回 deep-link，尊重用户原意图）；badge size 严守 max-width 22px（即使 99+ 也不撑爆 hub icon）
