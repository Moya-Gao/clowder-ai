...
- **D2b frontend** 📐 design稿 ready (`designs/F174-callback-auth-health-card.pen`)
  ...
  - **D2b-2 system-level health indicator**（P0 · 实体层 — twice revised after alpha 反馈，**badge 已于 PR #2606 移除**）— PR #1403 (rev0 per-cat dot) → PR #1410 (rev1 standalone plug indicator) → PR #1419 (rev2 HubButton badge merge) → PR #2606 (badge 从主界面移除)
    - **当前形态（PR #2606 后）**：主界面 settings gear 不再显示 callback-auth badge。内部诊断指标不应在主 UI 通知用户（铲屎官裁定）。mark-viewed 基础设施（`lastViewedAt` / `unviewedFailures24h` / `markViewed()`）保留，仅服务 observability panel 的 unviewed watermark 和诊断状态。`CallbackAuthSnapshotMount` 从 chat layout 提升到 AppShell，确保所有路由（含 settings）的面板数据可用
    - ~~rev2/rev3 形态（已被 PR #2606 移除）~~：HubButton badge merge — `ActivityBar.tsx` SettingsButton 内部加 `useCallbackAuthAggregate` + `useCallbackAuthAvailable` hooks；24h unviewed failures > 0 时右上角渲染 amber/red 数字 badge；进入 observability/callback-auth subtab 自动 mark-viewed → badge 消失。经 4 次 alpha 否决 + 8 轮 cloud Codex review 后收敛，但铲屎官最终裁定：callback auth 作为内部诊断指标不值得占用主界面通知位（"为什么这个 callback auth 失败这么特殊需要在我的主界面知道"）
...
| 2026-04-28 00:13 | **D2b-1 alpha #6 fix merged via PR #1444** (squash `0cd6c20e`)。铲屎官 alpha #6: 「小红点点进去看板没消除」+「某只猫挂过 MCP 现在没在跑还显示 60 个红点」。根因 = Cloud Codex P2 round 1 prescription `viewedUpTo = snapshot's "as of"` 是 over-engineered 对真实用户 mental model — 用户进 panel 那一刻 = "ack everything to now", 不是 ack to snapshot time。Issue 2 是 Issue 1 症状 (badge 不清 → 历史 60 失败一直显示)。修法：`callbackAuthStore.ts` markViewed POST 不传 viewedUpTo body, server 默认用 `Date.now()`。Trade-off: snapshot-to-now 间隙失败被自动 ack (active failure 场景 badge 自然 re-populate; idle-cat 场景 badge 干净清除并保持)。Round 4/5/6 race protections (pendingMarkViewedCutoff / effectiveCutoff / stale-poll guard) 保留。Tests +1 (60-failure offline cat → markViewed → badge 0)，web 全 358/358/2571/2571 + biome clean。@gpt52 cross-family PASS + Cloud Codex round 1 = "Delightful" (0 P1/P2)。**第 5 维度教训新增**: panel-open = ack-now 是用户 mental model, 不是 snapshot-consistency theory; theoretical race < real UX |
