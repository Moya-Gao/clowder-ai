...
- [~] AC-Z1: 铲屎官 2026-04-30 报告的 5 类症状（裂 / 不见 / F5 才正常 / F5 才出来 / 发完才出来）在 alpha 通道实测全部消失 — **R1/R3 强通过**（identity contract + reducer single-writer + IDB merge filter 验收通过）；**R2/R4/R5 重新打开** — 2026-05-02 铲屎官 thread `thread_mop35hgupruuwsuz` 实测复现 "F5/切 thread 才出来" 症状。根因：Phase C 的 gap detection 仅在新 `agent_message` 到达时触发；如果 WebSocket 断开窗口期 server broadcast 了消息、重连后又没有后续 live event，则 `processThreadSeq` 永远不触发 catchup，必须 F5/switch 才出。F183 follow-up PR 修复方向：在 `useSocket.ts` reconnect handler 里调用 `useChatStore.getState().requestStreamCatchUp(threadId)`（复用 Phase C catchup version + debounce + retry + ack + Phase D merge filter）。砚砚 R1 design 拍板。修完后 @gemini 重跑 R2/R4/R5 验收。
...
| R2 | "气泡不见了" | AC-C1, AC-C2, AC-Z1 | replay test + alpha | [~] reconnect catchup gap (F183 follow-up) |
| R3 | "F5 之后气泡不裂了" | AC-D1, AC-D2, AC-Z1 | manual + alpha | [x] |
| R4 | "F5 之后气泡出来了" | AC-C1, AC-Z1 | replay + alpha | [~] reconnect catchup gap (F183 follow-up) |
| R5 | "猫猫发完消息气泡才出来" | AC-C1, AC-C2, AC-Z1 | replay + alpha | [~] reconnect catchup gap (F183 follow-up) |
