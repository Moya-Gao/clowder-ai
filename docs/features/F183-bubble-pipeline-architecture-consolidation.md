...
- [~] AC-B1: `MessageWriter` / reconcile reducer 落地，所有写入口收敛（B1.1 reducer core；B1.2.1-1.2.5 active text + callback explicit + hydration；B1.3-1.6 active done/error/tool；B1.7 background tool/error + `replaceThreadMessages` API — **active path 100% + bg tool/error 收口**；剩 bg text branch（callback merge + replacementTarget + batchStreamChunkUpdate hot path + cross-thread handoff）独立设计议题留 B1.8 收尾）
