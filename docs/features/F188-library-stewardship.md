...
砚砚 Design Gate 一审 P1×3 退回原 G.1/G.2/G.3 三件套：G.3 schema 改造与 G.1/G.2 hotfix 风险等级 + 验证半径完全不同，**强行同 PR 把 P0 runtime fix 拖进大改**。收窄方案：
...
- 砚砚 P2: 不 oversell——`SCOPE_KIND_MAP` 是 `evidence_docs.kind` 过滤，不是真的跨 surface raw thread/message/memory 全量扫描
...
- [x] AC-I6: archive/unbind 不等于 hard delete——compiled index 归档到 `~/.cat-cafe/library/archives/{collection-id}/`，manifest 标记 `archived`，search/graph 不再返回该 Collection 结果；**可恢复**（unarchive → rebuild）
...
- [x] AC-I9: 端到端验收场景：铲屎官通过 MCP/UI 创建 `domain:finance`，bind `docs/library/finance/` 下 5+ 篇理财知识 .md 文件，触发 rebuild，search/graph/catalog 全部能查到 finance 内容，默认 `private` sensitivity
...
| 2026-06-09 | **Phase K implementation complete (PR open pending)** — 5 detectors + evaluator + functionalStatus 同步评估在 `/api/evidence/status` (KD-16)，`healthy` 语义保留 (KD-14)，UI degraded banner (Tailwind amber)，33 tests pass (18 pure + 4 route + 4 backward-compat snapshot + 7 frontend)，dogfood harness 3 scenarios（healthy-baseline / reporter-880 / docs-root-broken）live JSON 验证。Commit 链：`a0b0a6865` RED → `7f070522e` GREEN → `4f6368b14` route → `4a8f3fc9b` frontend → 本 commit closeout。Dogfood report: `docs/harness-feedback/2026-06-09-f188-phase-k-dogfood-report.md` |
