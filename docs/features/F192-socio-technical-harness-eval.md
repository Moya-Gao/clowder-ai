...
| 2026-06-15 | **Direction B fix: eval-domain-daily publish-prereq gate (clowder-ai#923)** — Inbox bug accepted as F192/harness-eval territory: `eval:a2a` daily cron lands on **whichever active runtime** in dual-instance setup (Hub 3001/3002 + dogfood worktree 3011/3012, shared Redis, both schedulers active). Dogfood worktree (`feat/f719-multi-project-skills`, HEAD `5b588cb1b`) lacks publish-verdict prerequisites (no `isA2aSourceRefs` validator) → eval cat receives full publish+cross-post instructions → hits infra blocker at publish → per its prompt cross-posts the blocker text into the dogfood runtime's owning **feature** thread `thread_mpdic4arzqaku586` (not eval thread). Not a delivery-layer bug; **eval-cat behavior + dual-runtime binding bug**. Owner split with 砚砚 (cross-thread route-back from inbox): **Direction B (mine, narrow eng)** — add `publishPrereqProbe?: (domainId) => boolean \| Promise<boolean>` to `EvalDomainScheduleOpts`, run it at `execute()` BEFORE legacy-gate / override / `buildEvalCatInvocation`; on `false` (or throw → fail-closed) post a stable-header `SKIPPED (publish prereq missing)` status message to the domain's OWN `systemThreadId` and **return early — eval cat is never invoked**. No LLM = no LLM-discretion path to `cross_post_message` = leak class eliminated at source. **Direction A (runtime pin) + C (dual-scheduler convergence)** out of scope — those need CVO Decision Packet for runtime-cron-ownership + scheduler-convergence policy, separate from this code change. Bootstrap (`index.ts`) wires the probe via cached dynamic-import introspection: probes `typeof (publish-verdict/validation).isA2aSourceRefs === 'function'`, present-only-after-fix on current main; first call resolves + caches per domain. Tests: 6 new prereq-gate cases — backward-compat (probe omitted), probe-false (skip + blocked message + no trigger), probe-true (proceed), probe-throws (fail-closed), probe-Promise (async API), all-domains-skip threadId-never-leaks-to-feature-thread invariant. 756/757 harness-eval (1 unrelated skip), biome 0 errors on changed files. **R1 fixes (砚砚 review on PR #2297)**: P1#1 — probe target corrected from `a2a-generator-adapter.js` to `publish-verdict/validation.js` (adapter only imports/calls `isA2aSourceRefs`; the export lives in `validation.ts`, so the original probe would always mark even a healthy runtime as "missing prereq" → eval:a2a permanently skipped → silent functional regression worse than the bug being fixed); user-facing SKIPPED message text also updated to point operators at the correct file. P1#2 — new prereq-gate tests split into dedicated `eval-domain-daily-prereq-gate.test.js` (211 lines) to keep both test files under the 350-line hard cap (original file restored to 337 lines). All 21/21 tests (15 existing + 6 new) green post-fix. Will cross-post PR URL back to clowder-ai#923 trail when merged. |
| 2026-06-17 | **eval:a2a C1 sample-window build fix merged (PR #2337, `7b071f31`, verdict `2026-06-17-eval-a2a-c1-sample-window-build`)** — 砚砚 cross-thread verdict from `thread_eval_a2a` 03:10 UTC: 06-17 eval shows C2 below threshold post #2314 (3/274 = 1.1%), C2 void-hold also below floor (13/281 = 4.6%); new top finding C1 `c1.zombie_hold_count=4` with sampleCoverage 2/4 incomplete despite PR #2250 per-fire event instrumentation. Root cause: `run-f167-eval.mjs` calls `fetchTraces(..., {limit:500})` while trace store has 1826 spans for the window → metrics count full window but sample extraction sees only the latest 500-span slice. Direction: **build** (not C1 guard tuning yet). Smallest mechanical fix — add `?expandLimit=true` opt-in to `/api/telemetry/traces`: default 500 cap unchanged for UI/dashboard traffic; when literal "true" the cap rises to `traceStore.stats().maxSpans` (still bounded, still session-gated, same redacted DTO shape, strict string check — no truthy coercion). Adapter `fetchTraces({...expandLimit:true})` forwards the param; `run-f167-eval` fetches `traceStats` first then sizes the trace fetch to `traceStats.maxSpans` — no second magic number. Not cursor pagination and explicitly not a general bulk-export API (PR body / route doc both call it out). 砚砚 R1 caught 2 P1 (rebase + count drift on PR #2331/#2336 + test-file size split) → R2 LGTM on `d0137f55`; cloud Codex R1+R2 clean. Bundled 3 main-blocking drift fixes (run-checks.test.mjs 23→25 / incident-containment ratifies #2331 / sync-manifest reverse-sanitizer entries) to unblock every concurrent猫's pnpm gate. Acceptance: next C1 zombie finding has `sampleCoverage.complete=true` (`sampleCount == metricCount`) once events are beyond the first 500 but inside maxSpans, or C1 stays under threshold for 2 consecutive evals. |
| 2026-06-18 | **eval:a2a C1 zombie-hold semantics fix merged (PR #2368, squash `2d0e2f88e`, verdict `2026-06-18-eval-a2a-c1-zombie-hold-semantics-fix`)** — 砚砚 cross-thread verdict from `thread_eval_a2a` 03:10 UTC: PR #2337 worked — 06-18 eval fetched 1212 spans, C1 sampleCoverage 4/4 complete. With complete samples, C1 is `c1.zombie_hold_count=4/6=66.7%` BUT samples show `prior_long=3/4 + prior_short=1/4` — the metric was mostly measuring benign single-slot replacement churn (per F167 Phase G KD-23), not actionable zombie suppression. Direction: **fix**. Mechanical bucket-based split per `bucketWakeDelay()`: (1) `prior_overdue`/`prior_imminent` → new `c1.hold_zombie_count` + `c1.hold_zombie_fired` span event (actionable: scheduler stuck or wake interrupted <60s); (2) `prior_short`/`prior_long` → new `c1.hold_replacement_count` + `c1.hold_replacement_fired` span event (benign churn). Legacy `c1.zombie_hold_count` removed (no alias — eval is the only consumer, historical bundles in `docs/harness-feedback/bundles/` document the prior name in-place). Producer (`callback-hold-ball-routes.ts`) routes both counter + span event by `isTrueZombie = bucket==='prior_overdue'||bucket==='prior_imminent'`. Sample-evidence extractor renamed `c1-zombie-hold-sample-evidence.ts` → `c1-hold-sample-evidence.ts` with paired `extractC1HoldZombieSamples` + `extractC1HoldReplacementSamples` (shared discipline + `C1_HOLD_EXTRA_ATTR_KEYS` for `priorTaskIdHash`/`newTaskIdHash`). `f167-eval.ts buildC1` consumes both metrics + surfaces both sample sets under split friction keys. `attribution.ts SAMPLED_METRICS` updated. F167 component registry yaml updated. Tests: 41 cases on the new c1-hold-sample-evidence (parameterized over both extractors + cross-bucket bleed guard) + callback route emits routed by bucket + f167-eval friction split + per-fire-sample-lookup C1 variant. Acceptance: post-runtime-reload eval reports separate replacement and true-zombie metrics; true-zombie below count<3 or ratio≤5%, or any remaining finding has complete samples without benign `prior_long` majority. |
