...
> **Status**: in-progress (Phase F re-eval closure + Phase G `eval:task-outcome` closure + `eval:sop` re-enable) | **Owner**: 布偶猫 | **Truth sync**: 2026-06-09
...
5. ✅ Unbiased sampling v1：去掉 `sessionIds` 硬必填，支持 runtime-session window scan + per-family snapshot split。Durable trial store / stable trial IDs 仍是未来增强，不作为 Phase F closure 前置。
...
| 2026-06-09 | **Phase F coverage expansion merged (PR #2180, squash `4a06390af`)** — `eval:capability-wakeup` 覆盖从 3 条扩到 L0 §8 v1 全 13 条 Tier 1 predicate，补齐 Tier 1 MCP tool-use mapping；`sessionIds` 从硬必填收窄为 optional，省略时走 owner-scoped runtime-session window scan + pagination + spanning-session inclusion；live verdict snapshot 新增 `components[].byFamily` family split。merge-gate 在最新 main 上全绿，云端 review 多轮 P1/P2 已闭，最终 no major issues。AC-F7 / AC-F8 关闭；AC-F9 仍等连续 re-eval 数据做 promote / demote / drop closure。 |
| 2026-06-09 | **Phase G PR-D episode verdict writeback implemented** — 补上 PR #2162 明确 out-of-scope 的 7-class per-episode writeback gap：`task-outcome-snapshot` sourceRefs 增加可选 `episodeVerdicts[]`（`episodeId` + `success/corrected_success/needs_investigation/harness_fix_needed/routing_failure/taste_mismatch/abandoned`），MCP schema + API validation 同步；adapter 只允许写 selected replay window 内且 terminal 的 episode，调用既有 `TaskOutcomeEpisodeStore.updateVerdict()`，避免从 4-class packet verdict 自动伪造 per-episode judgment。目标测试锁住 happy path（写回后不再出现在 `listNeedingVerdict()`）+ non-terminal 400 + MCP passthrough/schema。剩余 Phase G closure：manual runtime Eval Hub acceptance；剩余 F192 backlog：rollup/batch aggregation。 |
