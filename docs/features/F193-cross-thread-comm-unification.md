...
- [ ] AC-A3: API route 层 — `CallbackPrincipal × threadId × targetCats` 组合校验，违反 KD-1 时 400 + alternatives
...
- [ ] AC-B2: 收到 `extra.crossPost.sourceThreadId` 触发的 invocation，SystemPromptBuilder 自动注入 reply hint（含 sourceThreadId / senderCatId / 回复方式）
- [ ] AC-B3: `post_message` 描述更新为 principal-conditioned 语义说明
- [ ] AC-B4: 测试 — 构造 cross-post 触发的 invocation，断言 system prompt 含 reply hint 字段
...
### Phase D（废弃工具清理）
- [ ] AC-D1: 移除 `reflect` 工具注册（含 server-toolsets / tools/index）
- [ ] AC-D2: 移除 `guide_resolve` legacy alias
...
| 接收侧 reply hint 注入位置选错（system prompt vs invocation context vs user turn 头部）影响覆盖率 | OQ-1 在 Phase B 起步前定，参考 SystemPromptBuilder 现有 invocation context 注入路径（如 parallel mode 禁路由提示） |

## Open Questions

| # | 问题 | 状态 |
|---|------|------|
| OQ-1 | 接收侧 reply hint 注入 surface — system prompt 静态段 / invocation context 动态段 / user message turn 头部？倾向 invocation context 动态段（每次新跨线程触发都注入，避免 system prompt 缓存污染） | ⬜ Phase B 起步前定 |
| OQ-2 | KD-2 选项确认：A 新增 `cat-cafe-limb` server / B 暂不进默认 harness（保留 all-in-one 作为可选） / C 合并到 collab。倾向 A | ⬜ Design Gate 拍板 |

## Key Decisions

| # | 决策 | 理由 | 日期 |
|---|------|------|------|
| KD-1 | **Principal-conditioned threadId semantics**：invocation-token principal 调 `post_message` 必须省略 `threadId`（F043 #316 防误投），传了 → reject；agent-key principal 调 `post_message` 必须显式 `threadId`（F178 无默认 thread）；任何 principal 跨 thread 写入只能用 `cross_post_message`；`cross_post_message` 必须有 `targetCats` 或 content 行首 @，否则 reject。两层 enforcement（MCP handler + API route）。 | F043/F178 两个已拍板契约 reconcile，不是开放问题——证据齐全。reject 优于 silent strip：silent strip 会丢 F052 sourceThreadId 溯源链路 + 同名猫跨线程豁免（route-helpers.ts:615）。 | 2026-05-07 |
| KD-2 | **Limb tool harness placement** — 倾向选项 A：新增 `cat-cafe-limb` server entry point（按需加载）。备选：B 暂不进默认 harness / C 合并到 collab。Design Gate 拍板。 | A 最小破坏现有 split-only 设计意图（F043 立项原意），且 limb 是布偶猫专属能力，独立 server 更符合 F041 配置编排"按猫加载"模型。 | ⬜ Design Gate |
