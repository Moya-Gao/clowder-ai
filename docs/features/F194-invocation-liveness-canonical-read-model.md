...
- [x] AC-Z30: CLI Output 块行为与是否有 post_msg 无关——纯 stream / stream+post_message 都显示猫的 CLI 工作日志（tools + stream stdout）；post_msg speech 自己单独成泡，不吞掉或改写 CLI 工作日志。
  - 诊断（2026-05-16 19:xx 47，raw record 实证）：thread `thread_mov3a7qva8mtsbs1` 同 turn `964b43c5` 有 stream record（origin=stream, content="Confirmed — branch...", 4 tools）+ callback record（origin=callback, content="@codex Review continuity..."）。Z8 KD-27 按 `(catId, turnInvocationId)` group → 合并 1 bubble，origin=callback。`ChatMessage.tsx:122` `toCliEvents(toolEvents, isStreamOrigin ? content : undefined)` —— 合并后 origin=callback → isStreamOrigin=false → CLI Output 拿 undefined → stdout 丢失，只剩 tools。纯 stream turn 则 origin=stream → CLI Output 有 stdout。**行为不一致是痛点**（铲屎官 R17：CLI Output "无论有没有 post msg 行为要保持一致"）。
  - R17 follow-up（2026-05-17 02:34 砚砚接手）：铲屎官进一步澄清 "post msg 自己单独气泡；其他消息就是正常的 Thinking + CLI Output + cli 原本消息"。第一版 Z11 保留 Z8 合并导致 `post_msg` speech 跑进 CLI Output 上方/同泡，坐标仍错。修正 contract：stream/work-log records 按 `(catId, turnInvocationId)` 合并；callback-origin records with own message id 按 callback id 单独成泡；只有 exact-key `callback_final`（id 等于已有 stream record id）作为 terminal update 留在 stream 组，避免 duplicate id
...
- [x] AC-Z32: `invocation_created` 已建立当前 cat turn 后，后续仍只带 parent invocation 的 live `text/tool_use/tool_result` 事件必须绑定到当前 turn bubble；不得按 parent stable key 回收旧 turn 的 finalized bubble。
  - 诊断（2026-05-19 19:13 铲屎官）：F207 thread `thread_moa8ndjmv4x8j93u` 同 parent sequential chain 中，Opus turn1 已完成后，Codex handoff 回 Opus turn3；live UI 显示两个 Opus bubble，其中第二个 bubble 复制了 turn1 开头内容。后端 `/api/messages` raw records 正确：Opus turn1 `turnInvocationId=3889fd9f...`，Opus turn3 `turnInvocationId=068257df...`，且 turn3 raw content 不含 turn1 文本；`projectCanonicalBubbles` hydrate 也正确分泡。因此根因是 live recovery 在 parent-only chunk 上仍用 parent key 命中旧 parent-only finalized bubble。
...
| R19 | "又出现重复气泡了！ 砚砚猫！" (2026-05-19 19:13) | AC-Z32 | live parent-only stream chunk 在当前 turn 已知时按 turn 绑定，不再回收旧 same-parent finalized bubble | [x] (Phase Z13 hotfix) |
...
| 2026-05-16 19:30 | **Post-close CLI Output stdout consistency hotfix (Phase Z11, R17)** — 铲屎官 alpha catch：猫 post_msg 后，同 turn 的 CLI Output 块"只剩下 tools"，stream stdout 工作日志消失；纯 stream turn 则正常。raw record 实证（thread `thread_mov3a7qva8mtsbs1` turn `964b43c5`）：Z8 把 stream + callback 合并成 origin=callback bubble，`ChatMessage.tsx:122` 只在 origin=stream 时把 content 喂 toCliEvents → 合并后 CLI Output 丢 stdout。第一版 Z11 保留合并并用 `cliStdout/speechContent` 分桶渲染，但铲屎官 2026-05-17 02:34 澄清正确模型是 **post_msg 自己单独气泡，CLI 工作日志保持原 stream bubble**。Follow-up 修正：projection group key 增 `originBucket`，callback-origin own-message-id 记录按 callback id 单独成泡；exact-key callback_final 才归 stream bucket。Full web `src/stores src/hooks src/components`: 371 files / 2713 tests GREEN。 |
| 2026-05-17 05:08 | **Z11 live active recovery follow-up** — 铲屎官给出 thread `thread_motr8u5i1dtjigtd` 最新 invocation：Opus post_msg callback bubble 正确独立，但 live tool_use 被 active recovery 追加进 same-turn callback bubble，视觉上变成 callback speech 在 CLI Output 上方。根因：`getOrRecoverActiveAssistantMessageId(... ensureStreaming=true)` 复用 same stable key 的 callback bubble。修复：ensureStreaming 恢复路径只接受 stream-origin bubble；callback active ref 视为 stale，tool/text stream 会重建独立 stream bubble。Focused + broad web regression 通过。 |
| 2026-05-19 08:40 | **Z12 ideate targetCats hydration hotfix** — 铲屎官 alpha catch：三猫独立观点采样中，从 thread 切出再切回后采样条从 3 只缩成 2 只。根因：`useChatHistory.fetchQueue` 把 `/queue.activeInvocations`（server active slot 子集）当作本轮 full `targetCats`，直接 `replaceThreadTargetCats`，破坏 Z5 AC-Z15 依赖的 targetCats 全量不变量。修复：ideate 模式且本地有 previousTargetCats 时，queue hydrate 使用 `previousTargetCats ∪ activeCatIds`；execute/普通场景继续收敛到 server active subset；非 active 猫保留已有状态（done 等）。Focused regression 54/54 + `pnpm check` GREEN。 |
| 2026-05-19 19:13 | **Z13 live parent-only chunk recovery hotfix** — 铲屎官 alpha catch：F207 sequential handoff 后 Opus turn3 live bubble 复制了 Opus turn1 开头内容。raw `/api/messages` 与 hydrate projection 均正确，问题限定 live recovery：`invocation_created` 已知道当前 turn，但后续 parent-only `text/tool_use/tool_result` 仍用 parent stable key 回收旧 parent-only finalized bubble。修复：active stream lookup / bubble id seed / reducer event 在 `catInvocations[cat].invocationId === parent` 时统一使用当前 `turnInvocationId` 作为 effective turn id。Focused + broad web hooks/stores regression 1135/1135 + `tsc --noEmit` GREEN。 |
| 2026-05-20 01:04 | **Z13 follow-up: active parent-only stream upgrade** — 铲屎官 alpha catch：Opus post_msg 后 live UI 仍出现重复 CLI Output，F5 后 hydrate 正常。raw `/api/messages` 与 `projectCanonicalBubbles` 对最新 Opus records 投影为正确的 stream + callback 两泡，说明不是后端双写；live root cause 是首个 stream bubble 先 parent-only 创建，server stream re-emission 带 turn id 返回时被 active stale check 误判，旧泡未升级、新泡另建。修复：active ref 命中 parent-only stream 且当前 turn 已知时原地补写 `turnInvocationId`，再让 reducer 复用同泡。Focused regression `useAgentMessages-bubble-merge` 45/45 + Z8/Z11 related 18/18 GREEN。 |
| 2026-05-16 | **Post-close parallel slot cleanup hotfix merged (PR #1716, squash `e97f9491`)** — 铲屎官 runtime 发现 `@opus @opus47 @codex` 三猫场景里两只布偶猫已完成、缅因猫尚未出现，但底部 active bar 仍显示已完成的 Opus 4.6 在跑。诊断：后端 `/queue` 已空，server truth 正确；前端 `activeInvocations` 仍按 parent liveness id 建 slot，而 terminal `done` event 携带 per-cat turn id，旧 cleanup 只删 turn-key slot，删不到 parent-key slot。修法：`findTerminalActiveInvocationSlot` 用 `catInvocations[cat].turnInvocationId → invocationId(parent)` 桥接 Z9 dual identity，只清完成猫 parent-key slot、保留其他 cats；`isStaleTerminalEvent` 同步允许 turn-key terminal 对 parent-key slot 判 fresh。回归测试：三猫 slot + `done(isFinal=false)` turn id → 只移除完成猫 parent slot。Opus-46 review APPROVE，`pnpm gate` 通过。 |
| 2026-06-15 | **Post-close Codex live tool/text convergence fix merged (PR #2304, squash `ec5accdc6`)** — 铲屎官 runtime catch：Codex live turn 偶发裂成 tool-call-only 工作日志泡 + text 泡，F5 后 hydrate 合并。根因是 parent-only fresh seed 只证明“曾经新鲜”，不能证明属于当前 `invocation_created` boundary；同 parent 后续 turn 可把旧 active/background residue 升级进新 turn。修复：active/background `fresh-parent-seed` 增 runtime-only timestamp/seq current-boundary proof，缺 proof 或 stale proof 时 finalize 为 residue，不升级。新增 active/background stale fresh-seed RED regressions。Cloud Codex final verdict clean on `a8c2d55cc6`；focused/convergence/ledger/bubble-merge/full web tests + `tsc --noEmit` + `pnpm check` + architecture ownership + `git diff --check` GREEN。 |
| 2026-06-16 | **Post-close stream-residue reconcile merged (PR #2319, squash `38b824ac4`)** — #2304 saga 接力。铲屎官 runtime catch：codex 多工具 turn 在 final empty CLI/tool stream 泡请求 catch-up 后，server history 把权威 persisted message 以另一 turn key 返回，而错 key 的本地 `msg-*` 泡从未持久化 → 仅 F5 后裂（history 泡 + 幽灵 tool-only 泡）。根因诊断（双猫思辨）：projection 早在 Z8 已统一，真因在 live identity/key 时序的 wrong-key local-only 残留。修复（砚砚）：`mergeReplaceHydrationMessages` 出口新增 `isUnclaimedTerminalToolOnlyStreamResidue` 守卫，仅在满足全部条件时 drop——`origin==='stream'` + settled `msg-*` + 纯 tool-only payload + **存在全量匹配的 persisted sibling evidence**（`hasFullToolResidueEvidence` 多重集计数，防 persistence-lag 误删尚未落库的终态 tool 输出）+ 未被 live invocation claim + 不跨 turn 边界（parent invocation / A2A / user boundary 三重 scoping）。`origin==='stream'` 早返回守 #814/F194 explicit-post no-swallow 红线（callback origin 不被吞）。砚砚 author + 宪宪/opus-48 review APPROVE (`bb5f4e659`)；cloud Codex 11 轮 evidence-guard 演进（R4-R11）收敛 clean on `fd536a09c`（"Didn't find any major issues. Breezy!"）；fresh-session merge gate 复验（SHA 三方对齐 / inline P1/P2 全 traced 已解决 / shared drift additive 非 breaking / 非 hotfix / 红线不变量守住）通过。alpha 验收 @sonnet 待执行。Status 不变（done）。 |
| 2026-06-18 | **Post-close R18 background invocation-created ledger bridge merged (PR #2363, squash `a8198a22`)** — #2304/#2319/#2349 saga 第 18 轮。saga 17 修了 dual-path turn 解析同源（background fallback 查 ledger 代替 catInvocations），但遗留三类 collision edge case：(1) `invocation_created` 自己也走 background path（operator 切 thread 后到达），此时 `bgStreamRefs` 为空但 active ledger 有 fresh-parent-seed → 新增 ledger bridge 让 background `finalizeStaleBackgroundInvocationStreams` 也能升级 parent-only seed；(2) turn-stamped background text/tool chunk 抢先创建 `msg-{turn}-codex` 泡，延迟的 `invocation_created` 做 `replaceThreadMessageId` 直接覆盖 → 改为 `buildTurnBubbleCollisionPatch` merge（seed content/toolEvents/richBlocks/metadata 全保留，`isStreaming` 尊重 finalized target，`textMode=replace` 不重复 prepend stale text）；(3) 所有 6 个 finalization 路径（`finalizeStaleBackgroundInvocationStreams` / `stopTrackedStream` / `drainPendingBackgroundCallback` / `handleBackgroundAgentMessage` ×3）增加 `clearActiveBubbleLedgerForFinalizedMessage`，确保 active ledger 只指向仍在 streaming 的泡（防止后续 turn no-turn tool events 被 stale ledger 吸入旧泡）。`bubble-reducer.ts` 新增 `textMode` 从 event payload 传播到 `extra.stream.textMode`，collision merge 可据此跳过 seed prepend。砚砚(codex) author + 云端 codex 6 轮 review（R1-R6 循环到封板 LL-072）+ 宪宪/opus-46 final continuity review APPROVE on `99972ca1`。10/10 dual-path + 317/317 useAgentMessages tests GREEN + TSC clean + fallback net -1。Status 不变（done）。 |
| 2026-06-19 | **Post-close R19 contentful stream-residue reconcile merged (PR #2418, squash `c950b35db`)** — 铲屎官 runtime catch：PR #2363 alpha/unit 全绿后，真实 Codex 长工具流仍在 live UI 裂成两个 assistant stream 泡；但刷新后 `/api/messages` 对 thread `thread_mqkivws1k5e07s6k` 只返回一条权威 persisted stream（parent `b3f2a25e...` / turn `7386eb91...`，content + 14 tool events），runtime 已含 #2349/#2363，排除旧代码未同步。根因边界从 R18 ledger/turn stamp 转到 live catch-up 合并：#2319 只 drop tool-only wrong-key `msg-*` residue，明确保留 contentful wrong-key residue 以防误删 partial text；现场证明当同 parent persisted sibling 已覆盖文本和工具证据时，这条保守规则会保留 ghost work-log 泡。修复（砚砚）：`mergeReplaceHydrationMessages` 将 residue guard 扩展为 persisted-evidence covered terminal stream residue——terminal `origin='stream'` + `msg-*` + 未被 live invocation claim + 不跨 user/A2A/other-cat boundary；有 text 时要求同 parent persisted text 归一化后覆盖 residue text，有 toolEvents 时要求 persisted tool evidence 多重集覆盖，text+tool 双存在则双证据都必须满足；callback/explicit-post/contentBlocks/rich blocks 仍不吞。新增 RED→GREEN `drops contentful terminal residue when persisted same-parent sibling already covers text and tools`、`preserves contentful residue when text coverage is only manufactured across persisted siblings`，保留 partial-text preserve 守门。cloud clean on `3205546bd`；Sonnet scoped continuity APPROVE on bridge `851666b5`；full `pnpm gate` GREEN on gate SHA `9ea60013`，bridge tree-identical。Status 不变（done）。 |
| 2026-06-19 | **Post-close R20 terminal live CLI ghost fix merged (PR #2429, squash `26d6d5101`)** — R19 合入并重启 runtime 后，铲屎官在 `thread_mqcb399ktegukxdy` 仍看到 Codex 正文泡旁边残留独立 CLI Output 泡；bounded devtools dump 显示后端 persisted truth 只有一条 Codex stream message（`0001781875470094-000205-a8335457`，parent `34a012f5...` / turn `4666bc59...`，content + tools），UI 多出的泡来自 completed task-progress snapshot 仍被 `isClaimedByLiveInvocation` 当作 active claim，挡住 R19 persisted-evidence residue drop。修复（砚砚）：`mergeReplaceHydrationMessages` claim gate 新增 `isActiveInvocationClaim`，仅 `snapshotStatus` 非 `completed/interrupted` 时保护 live residue；running snapshot 仍 preserve，terminal snapshot 不再阻止同 invocation persisted sibling 覆盖证据 drop。新增 RED→GREEN drop/preserve 双测（completed snapshot drops covered residue；running snapshot preserves empty `msg-*` residue）。Opus-47 APPROVE-equivalent on `f93144f92` + scoped continuity to `ee6210b1`；cloud R1 P1 docs frontmatter fixed in `1b2bff1e`；cloud R2 clean on `1b2bff1e10`；full `pnpm gate` GREEN on `1b2bff1e`（4354/4354 web tests + lint/check）。Status 不变（done）。 |
| 2026-06-23 | **Post-close R21 live carrier cleanup + terminal stream speech projection rolled back (PR #2514, squash `40ea51c5`)** — R21 曾把问题拆成 A path live parent-only carrier cleanup 与 B path terminal stream speech projection，并通过 cloud R6 clean + unit gate；runtime dogfood 随后证明这是失败方案：live 首屏仍会裂成两个 bubble，F5/hydration 后才收敛，同时 B path 把产品 invariant 从 **Thinking -> tool call -> 消息** 改成 **消息 -> Thinking -> tool call**，破坏消息折叠语义。R21 因此只保留为反例证据，不再作为可复用修复基础；行为改动已回滚，失败样本截图保留在 `/assets/screenshots/f194-alpha-spark-v2-after-f5.png`。后续 saga 必须从 R20 baseline 重做，并先由铲屎官签字钉死产品语义 invariant。Status 不变（done；R21 rolled back）。 |
| 2026-06-17 | **Post-close codex live dual-path bubble split merged (PR #2349, squash `96e39f867`)** — #2304/#2319 saga 第 17 轮。铲屎官 devtools 抓真实裂泡样本（thread `thread_mqevrpipz1prrvh3` parent `ff1d8e85`：active 泡 turn `1ba442cb`（持久化匹配） + background 泡 turn `ac378b26`（live-only shadow），4 工具 ts/command 逐字相同 = 同一回复被双重处理）坐实 dual-path 根因：同一 codex 回复因 operator 中途切 `currentThreadId`（Y→X）跨越 active + background 两路径，codex tool/work-log stream 事件无 `msg.turnInvocationId`，两路径 fallback 不同源（active 用 ledger-bound-turn / background 只查 `catInvocations`，被新 invocation context 留的 shadow turn 污染）→ reducer `getStableInvocationKey` 视作两个 turn 劈开。修复（48 author）：`thread-runtime-ledger.ts` 新 `getActiveBoundTurnInvocationId` helper（`seedSource==='bound'` Z3-safe gate，从 `messageId='msg-{turn}-{catId}'` 反解 turn）+ `useAgentMessages.ts` `ensureBackgroundAssistantMessage` turn 解析在 `catInvocations` fallback 之前插入按 `msg.threadId` 查 ledger（不是 `threadIdRef.current`，避免查错 thread）→ 同一回复无论事件走哪条路径都解析出 active 已 bound 的同一 turn → 单泡。Z3 redline 守住：fix 只在 `msg.turnInvocationId` absent 时同源 fallback，真不同 `invocation_created` 的 turn 仍分开。红测 `useAgentMessages-codex-dual-path-thread-switch.test.ts` 覆盖 saga 16 轮未覆盖的"双路径中途切 thread"维度（用真实裂泡 fixture 复现 + Z3 redline 反测 2/2 GREEN）。47 cross-individual review COMMENT on `63cb818a6` (APPROVE-equivalent — cat-cafe family 共享 GitHub identity `zts212653`，known pattern：COMMENT state 落 `reviews[]` 当 standing review，`gh api .../reviews --jq length=1` 反幻觉锚定)；subagent fresh-context 复跑全量 `pnpm gate` EXIT=0 / tsc clean / check 27/27 / lint baseline only。**Long-session confabulation 警示**（saga 17 轮独有教训）：48 本 session 自报 4 次假 git PUSHED + 虚构整段 "47 review 交互"（thread participants 实测无 opus-47），CVO 重定向真 47 fresh-context review 兜底；47 review 全程命令锚定 GitHub 真相（`reviews=1` `commit_id=63cb818a6` `mergedAt=2026-06-17T13:19:07Z`），不信任何转述。诊断真相源：`docs/bug-report/2026-06-16-codex-live-bubble-split-race/README.md`（main `44972c1aa`）。alpha 单元层 PASS by sonnet（34/34 含 2 × #2349 专属 dual-path 红测；alpha env synced to `5b1ae3148`）；端到端真人 codex 触发卡云端 budget，CVO sign-off A 方案接受：单元层 + 真实样本 fixture 红测 + Z3 redline 反测 + 全量 gate 已构成足够 close 信号，本 PR close；future 撞到 = 开 saga 18 新账本（race condition 信任时间检验，不挂本 PR）。Status 不变（done）。 |
| 2026-06-26 | **Post-close Codex live bubble split ledger bridge fix merged (PR #2589, squash `ae67556da`)** — saga 17+ 缺口。铲屎官 runtime catch：Codex（GPT-5.5）气泡偶发裂成两个（text + 空 tool-only），F5 刷新后收敛（持久化层正确）。根因：`shouldSkipBackgroundParentOnlyResidueRecovery` 只信 `bgStreamRefs`，active→background transition（operator 切 thread）清了 bgStreamRef（line 4100）但 runtime ledger 仍持有当前 `fresh-parent-seed`，导致后续 background tool events 被错误跳过、新建独立气泡。修复（宪宪/opus-46 author）：在 bgStreamRef check 之后增加 ledger bridge——same `messageId` + `seedSource='fresh-parent-seed'` + `isCurrentFreshParentSeed` timestamp/seq guard，与 `invocation_created` handler line 655-668 已有 bridge 同模式。Z3 redline 守住（genuinely different turns 仍分开）。新增红测 `[pre-invocation_created tools]` 覆盖 saga17 后遗缺口 + Z3 反测不动。砚砚/GPT-5.4 独立 review APPROVE + Codex/GPT-5.5 独立 review APPROVE + `pnpm check` 双方跑绿；云端 Codex LGTM "Chef's kiss" on `1b8a5cf76a`。Status 不变（done）。 |
