...
| 2026-06-23 | **Post-close R21 live carrier cleanup + terminal stream speech projection merged (PR #2514, squash `40ea51c5`)** — R20 后 alpha/spark dogfood 把问题拆成两类：样本 A 是 live 增量路径残留 parent-only local CLI carrier，F5 后 hydrate 能合并；样本 B 是 F5 后仍只有 CLI Output、正文被吞进 stdout，根因是 hydrate projection 只在 stream+callback merge case 才拆 `cliStdout/speechContent`，单条 terminal stream-origin content+tool record 会被 `ChatMessage` 当作 CLI stdout 并跳过正文渲染。修复（砚砚）：A path 在 `useAgentMessages` 增加 evidence-covered parent-only live carrier cleanup（terminal `msg-/draft-*`、parent-only、无 sidecar content、同 turn sibling 覆盖 text + tool multiset，且尊重 user/A2A/other-cat turn boundary）；B path 在 `bubble-projection` 增加 terminal stream speech sentinel，把单条 turn-bound terminal stream content 投影为 body + 独立 CLI block，并在 callback replay/drain 时 sentinel-aware preserve/strip；`ChatMessage` 改为 `hasCliBlock && stream-origin && !bodyText` 才跳过正文。R5 cloud feedback 修正 coalesced live chunks 误判与 callback body changed sentinel 丢失：final-stream sentinel 只在 raw final chunk 等于整泡 content 时设置，callback drain 保留空 `cliStdout` sentinel。新增 live/projection/render 三层 RED→GREEN tests；根目录 `f194-alpha-spark-v2-after-f5.png` 归档到 bug-report evidence。Opus-47 APPROVE-equivalent on `2f7d80918e`；cloud R1-R5 findings 全处理，final cloud clean on `23f1a3d778`；full `pnpm gate` GREEN on `23f1a3d77`。Status 不变（done）。 |
