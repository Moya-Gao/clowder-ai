...
| 2026-06-18 | **Post-close R19 contentful stream-residue reconcile prepared** — 铲屎官 runtime catch：PR #2363 alpha/unit 全绿后，真实 Codex 长工具流仍在 live UI 裂成两个 assistant stream 泡；但刷新后 `/api/messages` 对 thread `thread_mqkivws1k5e07s6k` 只返回一条权威 persisted stream（parent `b3f2a25e...` / turn `7386eb91...`，content + 14 tool events），runtime 已含 #2349/#2363，排除旧代码未同步。根因边界从 R18 ledger/turn stamp 转到 live catch-up 合并：#2319 只 drop tool-only wrong-key `msg-*` residue，明确保留 contentful wrong-key residue 以防误删 partial text；现场证明当同 parent persisted sibling 已覆盖文本和工具证据时，这条保守规则会保留 ghost work-log 泡。修复（砚砚）：`mergeReplaceHydrationMessages` 将 residue guard 扩展为 persisted-evidence covered terminal stream residue——terminal `origin='stream'` + `msg-*` + 未被 live invocation claim + 不跨 user/A2A/other-cat boundary；有 text 时要求同 parent persisted text 归一化后覆盖 residue text，有 toolEvents 时要求 persisted tool evidence 多重集覆盖，text+tool 双存在则双证据都必须满足；callback/explicit-post/contentBlocks/rich blocks 仍不吞。新增 RED→GREEN `drops contentful terminal residue when persisted same-parent sibling already covers text and tools`，保留 partial-text preserve 守门。F194 9-file focused group 94/94 GREEN。Status 不变（done）；review pending。 |
