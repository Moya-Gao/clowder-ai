...
- [x] AC-Z30: CLI Output 块行为与是否有 post_msg 无关——无论纯 stream 还是 stream+post_message 合并，CLI Output 都显示猫的 CLI 工作日志（tools + stream stdout）；post_msg speech 渲染为气泡主体。
  ...
  - 决策：铲屎官选 (A) 保留 Z8 合并 + 恢复一致性（不推翻 KD-27，不动 bubble 数量；R12/R13 不回归）
  - Fix：投影 `projectGroup` 按 origin 分桶 content；merge case（同时有 stream + callback content）时在 `extra.stream` 暴露 `cliStdout`（stream 段）+ `speechContent`（callback 段）。`content` concat 保持不变（R12/R13/search/hydrate 契约不动）。`ChatMessage` CLI Output 用 `cliStdout ?? (isStreamOrigin ? content : undefined)`；主体渲染 merge case 用 `speechContent` 避免重复显示 stream 段
  - RED tests：`bubble-projection-z11-cli-stdout.test.ts` 4/4（merge 暴露 cliStdout/speechContent / 纯 stream 不设 / 纯 callback 不设 / 多 stream record concat）+ `ChatMessage-z11-cli-stdout-consistency.test.ts` 3/3（merge CLI Output 显示 stream 日志 / 主体不重复 / 纯 stream 不变）。Full web vitest 3085/3085 GREEN
...
| R17 | "你的cli output 无论有没有这个post msg行为要保持一致啊！！...异常的他是 CLI Output (tools) 只剩下tools了" (2026-05-16 19:14) | AC-Z30 | CLI Output 块行为与 post_msg 无关：merge 后仍显示 stream 工作日志（tools + stdout），post_msg speech 作主体 | [x] (Phase Z11) |
...
| 2026-05-16 19:30 | **Post-close CLI Output stdout consistency hotfix (Phase Z11, R17)** — 铲屎官 alpha catch：猫 post_msg 后，同 turn 的 CLI Output 块"只剩下 tools"，stream stdout 工作日志消失；纯 stream turn 则正常。raw record 实证（thread `thread_mov3a7qva8mtsbs1` turn `964b43c5`）：Z8 把 stream + callback 合并成 origin=callback bubble，`ChatMessage.tsx:122` 只在 origin=stream 时把 content 喂 toCliEvents → 合并后 CLI Output 丢 stdout。铲屎官拍板 (A)：保留 Z8 合并、恢复一致性（不推翻 KD-27）。修法：投影按 origin 分桶 content，merge case 在 `extra.stream` 暴露 `cliStdout`（stream 段）+ `speechContent`（callback 段），`content` concat 不变（R12/R13/search/hydrate 不动）；ChatMessage CLI Output 用 cliStdout、主体用 speechContent 避免重复。AC-Z30。RED→GREEN：projection 4/4 + ChatMessage 3/3 + 全量 web 3085/3085。砚砚 review pending。 |
