...
> **Status**: in-progress (Phase A + B + Z merged; **close gate REVERTED 2026-05-09 17:09** — 砚砚 catch parallel route gap：`route-parallel.ts:399` 调 `invokeSingleCat` 漏传 `parentInvocationId`，多猫 ideate/parallel 场景 child invocation 命名空间断链，helper 又把 parent record + child draft 当 split → Phase Z2 修补 + 重做 KD-23 四件套) | **Owner**: 布偶猫(Opus 4.7) | **Priority**: P1
>
...
- [ ] AC-Z6 (Z2 extension): `route-parallel.ts` 调 `invokeSingleCat` 必须传 `options.parentInvocationId`，与 `route-serial.ts:725` 对齐。RED test 覆盖 ideate 多猫场景：parallel chain 的 child registry record 必须有 `parentInvocationId === 当前 parent record.id`；helper 不能再把 parallel parent + child 误判为 `tracker+draft_missing_record`
- [ ] AC-Z7 (Z2 extension): KD-23 四件套重做（含 ideate 场景）— 单测 + 集成测 + alpha runtime + 守护猫对照表新增"ideate 双猫并行 parent/child namespace bridge"行
