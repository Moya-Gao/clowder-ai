...
5. **Design Gate PASS**：HW-1 spec research done（`6cb3d7d22`）→ Design Gate 两猫收敛（宪宪+砚砚）→ writing-plans → worktree → TDD
...
- **Related**: F242（Code Graph Layer Spike — convention graph 提供 HW-1 coverage search expansion 数据源方向，2026-06-19 铲屎官提议交叉）
...
| 2026-06-19 | **HW-1 + F242 交叉设计灵感落盘**（铲屎官提议 convention graph 作为 coverage search expansion 数据源 + 宪宪 brainstorm 4 交叉点：expansion source / scope 消歧 / edge provenance / freshness 共享）。HW-1 前置条件已满足（SW-1 运行 1 个月+），进入 spec 调研 |
| 2026-06-19 | **HW-7 Telemetry 三態校准 + Eval Correctness merged**（PR #2427）— 三件修复：(1) `computeSearchQuality` 三態校准（`resultCount=NULL` 不算 zero-hit）+ `observedSearches` 精确分母；(2) `applyConsumptionRerank` shadow baseline 用 partition 前原始 BM25 顺序（Cloud-P2 修复 lexical protection 偏差）+ shadow mode 存 would-be reranked order（Cloud-P1-3 修复 shadow≡live 零信号）；(3) `eval-memory-adapter` 加 `search_zero_hit_rate` 为 recall 首要信号。gpt52 local review APPROVE + 云端 codex 3 轮（R1 1P1+1P2 修复 + R2 1P1 shadow-mode 修复 + R3 0 P1/P2）。17 tests（4 HW-7 telemetry + 13 consumption-rerank）全绿 |
| 2026-06-19 | **HW-1 spec research 完成**（`6cb3d7d22`）— 全面调研文档 `docs/discussions/2026-06-19-f200-hw1-coverage-search-research/README.md`：数据信号（7d abandon 65% + reformulation 30% + graphTraversal 0%）、F242 API surface 评估（types/queries/engine）、当前搜索管道架构 review、5 步 pipeline 提案（新建 CoverageSearchService + intent=coverage 参数扩展 + F242 soft dep）、6 个 OQ + 风险矩阵 + 3 Phase sequencing 建议 |
| 2026-06-19 | **HW-1 Design Gate PASS**（宪宪+砚砚两猫收敛）— 6 个 OQ 全部 resolved：intent=coverage 参数扩展 / 第一版只 nudge 不自动切 / F242 soft dep / max 50 + per-source quota / 新 CoverageSearchEvent / 不需要 shadow 但需要 telemetry。Architecture: memory primary + code-intelligence soft dep, map delta none。砚砚补充 5 个 writing-plan 约束（不改核心 search 语义 / provenance 必须 / stale guard / nudge-only / 6 类测试覆盖）|
| 2026-06-20 | **HW-1 coverage search merged**（PR #2437，squash `045439952`）— CoverageSearchService 5-step pipeline + intent=coverage MCP/API integration + MCP coverage response formatter + coverage-intent nudge。8 files (4 new + 4 modified), 23 tests。gpt52 local review 2 轮（2 P1 修复：scope leak rebase + MCP response shape crash）+ 云端 codex 1 轮（3 P2 pushback：scoped options by design / per-source caps informational / searchWithMeta type contract）。三类 expansion 数据源：frontmatter aliases + source-thread links + F242 convention graph (soft dep, graceful degradation) |
