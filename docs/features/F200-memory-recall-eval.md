...
| HW-4 | **P1: Consumption attribution fix（消费归因可信度修复）** | 铲屎官 challenge 成立且必须修：大规模并发搜索下，当前 consumed 是同 invocation 后续 tool event 的反推 proxy，不是真实用户点击/确认。初筛已发现 `candidates_json=[]` 高达 59.2%，且 Codex 大量 `sed/rg/nl` shell 读文件不被算作消费。需要先抽样 20-50 条 recall_events，对照原始 session events / 猫最终回复 / Read/Bash/graph 链路，统计 false negative（真实用了但没记）和 false positive / ambiguous attribution（归错 search 或重复归因），再修 candidate extraction / Bash file-read parsing / sourcePath 归因 / `resultSetId` 等。**在修复前，F200 consumption-based eval 不可信，只能看粗趋势，不能作为排序策略裁判。** | **HW-3 前置**：没有 attribution audit + 归因修复，不用 consumption 数据 close OQ-6/OQ-7 |
