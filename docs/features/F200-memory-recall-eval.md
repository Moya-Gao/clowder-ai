...
| HW-4 | **Consumption attribution audit（消费归因准确率审计）** | 铲屎官挑战成立：大规模并发搜索下，当前 consumed 是同 invocation 后续 tool event 的反推 proxy，不是真实用户点击/确认。需要抽样 20-50 条 recall_events，对照原始 session events / 猫最终回复 / Read/Bash/graph 链路，统计 false negative（真实用了但没记）和 false positive / ambiguous attribution（归错 search 或重复归因）。审计后再决定是否加 `resultSetId` / explicit consume marker / Bash file-read 解析 / source-thread drilldown 归因 | **HW-3 前置**：没有 attribution audit，不用 consumption 数据 close OQ-6/OQ-7 |
