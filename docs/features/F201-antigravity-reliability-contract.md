...
| 2026-06-01 | **Post-close retry-fragment hardening implemented**：F201/F211 live batch exposed zero-message automatic retry attempts being created as full visible sessions. Decision: keep every Antigravity cascade as an auditable `SessionRecord`, mark failed 0-message automatic retry attempts in `RuntimeSessionMetadata.lifecycle.retryFragment`, and fold explicit retry fragments in Session Chain while preserving legacy `tool_conflict` fallback and drilldown. |
