...
close_gate_report:
  ...
  reflection_capsule:
    status: written
    path: docs/reflections/2026-05-19-f201-antigravity-reliability-capsule.md
...
| 2026-05-17 | **Phase F Task 1 merged (PR #1739, squash `52debba44`)**：durable supervisor store foundation。Adds provider-internal `AntigravitySupervisorStore` with in-memory + Redis implementations, `antigravity:supervisor:v1:` no-expire persistence, copied Phase B journal summary snapshots（not a second side-effect truth source）, JSONL audit redaction, malformed Redis payload fail-closed validation, and narrow F194 liveness projection. 46/47 local review + cloud Codex LGTM；`pnpm gate` passed on `badb9b78`. |
| 2026-05-17 | **Phase F Task 2b merged (PR #1740, squash `51859d2e`)**：durable supervisor service wiring。Antigravity runtime now injects Redis-backed supervisor persistence, service writes started/batch/liveness/probe/resumable/done audit records, liveness evidence persists without consuming stall probe budget, post-side-effect interruptions persist `status=resumable`, and the P1 empty-response overwrite regression is guarded by a red→green test. 46/47 continuity + cloud Codex LGTM；`pnpm gate` passed on `35e52d26`. |
| 2026-05-17 | **Phase F Task 3 merged (PR #1741, squash `f7c82cfe`)**：receipt conflict split。Native executor success followed by trajectory/upstream error now records `receipt_conflict`, retries fresh cascade only when no side effect was observed, and fail-closes side-effect risk into resumable/manual-card supervisor recovery with `receiptState=native_success_trajectory_error`. 46/47 continuity + cloud Codex LGTM；`pnpm gate` passed on `192e5a07`. |
| 2026-05-17 | **Phase F Task 4 merged (PR #1743, squash `472329280`)**：resume tier classifier foundation。Adds `classifyAntigravityResumeTier()` as a provider-internal, fail-closed classifier over Phase B journal summaries + deterministic probes: Tier 1 read/build/test/lint, Tier 2 owned-probe auto candidate, Tier 3 shared/external manual, Tier 4 irreversible/manual. Fourteen rounds of cloud hardening covered manual-before-auto order, shell-chain guard, split `rm` flags, git global options, redacted targets, absolute/embedded shared paths, plus refspecs, mutating git subcommands, shell-wrapped git/rm payloads, and broad GitHub CLI writes. 46/47 continuity + cloud Codex LGTM；`pnpm gate` passed on `b0dcef20`. |
| 2026-05-17 | **Phase F Task 5 merged (PR #1744, squash `061b27a6`)**：AC-G6 auto-resume execution wiring。Safe Tier 1/2 decisions now auto-resume through a capped fresh-cascade retry path with Phase C `resumeContext` prompt injection, durable `auto_resuming` supervisor state, `ANTIGRAVITY_AUTO_RESUME=false` rollback, post-side-effect `empty_response` coverage, and safe-prompt preservation across later retries. 46/47 continuity + cloud Codex LGTM；`pnpm gate` passed on `26e7b5ef`. |
| 2026-05-17 | **Remaining Phase F compression decision**：CVO pushed back on “one AC per PR” pacing. 46 proposed and 55 accepted a compressed finish: PR1 bundles AC-G2/G3/G4 as one liveness/evidence/keepalive closeout, PR2 isolates AC-G7 controlled YOLO for safety review, and AC-G8/AC-F close as validation/docs only. Heartbeat upstream compatibility is verified inside PR1 Red→Green, not as a standalone spike PR. |
| 2026-05-17 | **Phase F PR1 merged (PR #1749, squash `5b7569c97`)**：AC-G2/G3/G4 liveness bundle closeout。Bridge/service liveness now distinguishes bounded trajectory heartbeat, pending approval/tool, RPC reconnect, native executor activity, and delivered progress; timestamp and numTotalSteps churn cannot bypass stall bounds; terminal/idle guards beat heartbeat; supervisor persists durable evidence summaries without becoming a second side-effect journal. 47 strict review + cloud Codex LGTM；`pnpm gate` passed on `b242d0b7`. |
| 2026-05-18 | **Phase F PR2 merged (PR #1751, squash `f1e373d49`)**：AC-G7 controlled YOLO closeout。Antigravity `run_command` now has bounded executor timeout, Bridge socket buffer, fail-closed env bounds, abort/error listener ordering, and hard-refusal-before-dispatch invariants for root delete / Redis 6399 / fork bomb. 47 strict safety review + cloud Codex LGTM；`pnpm gate` passed on `ae80e8c2`. |
| 2026-05-18 | **AC-G8 alpha unblock hotfix merged (PR #1756, squash `3339b86fe`)**：connector token CSS moved to a static vendor stylesheet and `start-dev.sh` syncs vendor assets before bypassing package lifecycle hooks, unblocking the first alpha web 500 class; Windows portable and Electron lifecycle bypasses remain deferred follow-up risk outside the alpha path. 47 cross-cat review + cloud Codex LGTM. |
...
| 2026-06-01 | **Post-close retry-fragment hardening merged (PR #2021, squash `fc34cf6f1`)**：F201/F211 live batch exposed zero-message automatic retry attempts being created as full visible sessions. Decision: keep every Antigravity cascade as an auditable `SessionRecord`, mark failed 0-message automatic retry attempts in `RuntimeSessionMetadata.lifecycle.retryFragment`, and fold explicit retry fragments in Session Chain while preserving legacy `tool_conflict` fallback, partial-output attempts, and drilldown. |
