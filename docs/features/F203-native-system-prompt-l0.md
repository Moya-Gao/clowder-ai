...
- [ ] AC-I1: S8 spike 完成——OpenCode `instructions` 压缩免疫性验证 ✅（源码验证，非运行时实验）
- [ ] AC-I2: `opencode-config-template.ts` `generateOpenCodeRuntimeConfig()` 支持 `instructions` 字段
- [ ] AC-I3: `OpenCodeAgentService` 加 `compileL0ViaSubprocess` 调用 + `injectsL0Natively() → true`
- [ ] AC-I4: `invoke-single-cat.ts` OpenCode 分支传 compiled L0 路径到 runtime config `instructions`
- [ ] AC-I5: `compile-system-prompt-l0.mjs` 加 `golden-chinchilla` workflow triggers（评估复用 ragdoll 或独立定义）
- [ ] AC-I6: `OPENCODE.md` 保留 OpenCode 专属内容（question deny、交互通道规则、commit 签名）作为第二个 instructions 文件
- [ ] AC-I7: 守护测试——`compile-system-prompt-l0.test.mjs` 加 `opencode` catId 编译 + token 预算 ≤ AC-B3 上限
...
| S8 | OpenCode `instructions` 压缩免疫性源码验证 | 46 (宪宪) | ✅ 2026-06-03 reachable + immune | 浅克隆 `sst/opencode` dev 分支（TypeScript 单仓，v1.15.13），trace 三关键文件：① `packages/opencode/src/session/instruction.ts`:153-167 — `config.instructions` 每轮 fresh 读取文件内容 ② `packages/opencode/src/session/llm/request.ts`:94-105 — 非 OpenAI-OAuth provider（含 Anthropic）注入为 `role: "system"` messages ③ `packages/opencode/src/session/compaction.ts` — compaction 只压缩 user/assistant 轮次，system prompt 每轮从文件重建不受影响 | **OpenCode `instructions` = compression-immune by design**：文件内容每轮 fresh 读 → `role: "system"` 注入 → 不进对话历史 → 不被 compaction 吃。功能等价于 Claude `--system-prompt-file` / Codex `-c developer_instructions`。实现路径：`opencode-config-template.ts` 加 `instructions` 字段 + `OpenCodeAgentService` 加 `compileL0ViaSubprocess` + `injectsL0Natively() → true` + 加 golden-chinchilla workflow triggers |
