...
- [ ] AC-I2: `opencode-config-template.ts` `generateOpenCodeRuntimeConfig()` 支持 `instructions` 字段（接受 compiled L0 path + OPENCODE.md path 数组）
- [ ] AC-I3: `OpenCodeAgentService` 加 `compileL0ViaSubprocess` 调用 + `injectsL0Natively() → true`。**前置守卫**（砚砚 P1）：`injectsL0Natively()` 返回 true 的前提是所有 OpenCode invocation 路径都有 native L0 覆盖——当前 `invoke-single-cat.ts:1326` 的 `OPENCODE_CONFIG` 只在 API-key + resolvedAccount + effectiveModel/provider 条件分支生成，"known legacy model 无 runtime config"路径会断层。解决方案二选一：① 扩展 invoke-single-cat 让所有 OpenCode 路径都生成含 instructions 的 runtime config；② service 层在 `buildArgs` 或 `invoke` 自行编译 L0 并通过 `--instructions` 或 env 兜底。Phase I 实现时选定方案并加测试钉住。
- [ ] AC-I4: `invoke-single-cat.ts` OpenCode 分支传 compiled L0 路径到 runtime config `instructions`。**三路径守护**（砚砚 P1）：API-key/custom、subscription/unresolved、known-model/no-mcp 三类 OpenCode invocation 路径均需测试证明 native L0 到达或 fail-closed，不允许 pack-only + 无 L0 的断层态
- [ ] AC-I5: `compile-system-prompt-l0.mjs` 加 `golden-chinchilla` workflow triggers（评估复用 ragdoll 或独立定义）
- [ ] AC-I6: `OPENCODE.md` 保留 OpenCode 专属内容（交互通道规则、commit 签名）作为第二个 instructions 文件。**注意**（砚砚 P2）：`question deny` 的真正 enforcement 在 `opencode.json` 的 `permission.question = "deny"` 而非 OPENCODE.md 文案。Phase I 不得破坏 `permission.question=deny` / OMOC plugin (`oh-my-opencode`) / `compaction` 设置——加测试钉住 runtime config merge 后这三项保持不变
- [ ] AC-I7: 守护测试覆盖全链（砚砚 P1 扩展）：① `compile-system-prompt-l0.test.mjs` 加 `opencode` catId 编译 + token 预算 ≤ AC-B3 上限；② `opencode-config-template` 输出含 `instructions: [compiledL0Path, "OPENCODE.md"]`；③ `invoke-single-cat` 把含 instructions 的 config 传到 `callbackEnv.OPENCODE_CONFIG`；④ route 层在 `injectsL0Natively()=true` 下对 OpenCode 走 `buildStaticIdentityPackOnly`（不 prepend full static identity）；⑤ compile 失败 → fail-closed（不进入无身份 invocation）
...
| S8 | OpenCode `instructions` 压缩免疫性源码验证 | 46 (宪宪) | ✅ 2026-06-03 reachable + immune | 浅克隆 `sst/opencode`，证据 pin 到 **`v1.15.13` tag**（commit `385cb694419f98103af0e8fc6187ddcbcbb6eecb`），trace 三关键文件：① [`instruction.ts`](https://github.com/sst/opencode/blob/v1.15.13/packages/opencode/src/session/instruction.ts) `system()` — `config.instructions` 每轮 fresh 读取文件内容 ② [`request.ts`](https://github.com/sst/opencode/blob/v1.15.13/packages/opencode/src/session/llm/request.ts) `prepare()` — 非 OpenAI-OAuth provider（含 Anthropic）注入为 `role: "system"` messages ③ [`compaction.ts`](https://github.com/sst/opencode/blob/v1.15.13/packages/opencode/src/session/compaction.ts) — compaction 只压缩 user/assistant 轮次，system prompt 每轮从文件重建不受影响 | **OpenCode `instructions` = compression-immune by design**：文件内容每轮 fresh 读 → `role: "system"` 注入 → 不进对话历史 → 不被 compaction 吃。功能等价于 Claude `--system-prompt-file` / Codex `-c developer_instructions`。实现路径：`opencode-config-template.ts` 加 `instructions` 字段 + `OpenCodeAgentService` 加 `compileL0ViaSubprocess` + `injectsL0Natively() → true` + 加 golden-chinchilla workflow triggers。**实现风险**（砚砚 P1）：`injectsL0Natively()=true` 后 route 层走 pack-only，但 `invoke-single-cat.ts:1326` 的 `OPENCODE_CONFIG` 有条件守卫——需确保所有 OpenCode invocation 路径都有 native L0 覆盖 |
...
| 2026-06-01 | **F203 Phase H 污染收口 + S6/S7 spike merged（PR #2036, squash `b64a23f28`）**：砚砚跨族 final APPROVE（本地）+ 云端 review "no major issues" clean；merge 前 rebase 到最新 main（18 commits）+ biome format 非行为 delta，`pnpm gate` 全绿（`913643edb`→squash `b64a23f28`）。dir-size 全队 blocker 由 F23 thread 延期解决（`11b839282`，未来架构调整时拆子目录），非本 PR scope。本 PR 闭合 **AC-H0**（GEMINI.md home+repo-root / AGENTS.md AGY-safe 污染收口）+ **AC-H1/H2**（S6/S7 spike = Google/Antigravity 全线 prompt-level 天花板）+ KD-22。剩余 Phase H：AC-H3 Gemini CLI fallback policy（KD-20 已定，待落档）+ AGY/Antigravity prompt-level 做扎实实现。|

| 2026-06-03 | **S8 OpenCode `instructions` 压缩免疫性源码验证 = reachable + immune**（宪宪 46 独立 spike）。铲屎官发现"给烁烁 宪宪 砚砚都做了 F203 但忘记 opencode 金渐层"。浅克隆 `sst/opencode` dev（TypeScript 单仓 v1.15.13），trace 三关键源文件确认：`instructions` 数组文件内容每轮 fresh 读 → `role: "system"` 注入（Anthropic provider 走 system messages，非 OpenAI OAuth `options.instructions`）→ compaction 只压缩对话历史 → **instructions = compression-immune by design**。功能等价 Claude `--system-prompt-file`。Cat Café 当前 gap：`OpenCodeAgentService` 不调 `compileL0` / 不声明 `injectsL0Natively()`、route 层走 full buildStaticIdentity prepend（被 compaction 吃）、runtime config 不注入 instructions、golden-chinchilla 无 workflow triggers、OPENCODE.md 手写静态身份无 roster/governance。立 Phase I 补齐，铲屎官 directive "给砚砚审核 ok 就开 wktree"。|
