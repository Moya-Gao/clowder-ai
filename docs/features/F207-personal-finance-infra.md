...
在做任何分析之前，猫猫必须先了解铲屎官是**什么类型的投资者**。不同风格/风险偏好/资产倾向决定了后续所有层的行为。
...
**数据敏感性**：投资者画像包含个人财务数据（净资产、收入、资产配置），属于**敏感 PII**：
- 仅本地存储，**禁止同步到任何云端/远程服务**
- 存储文件使用 GPG 或系统级加密（macOS FileVault / 加密卷）
- 猫猫查询时解密读取，不在日志/消息/rich block 中暴露具体金额
- 分析报告中使用百分比和相对值，不暴露绝对数字（除非铲屎官显式要求）

交付物：本地加密的 `investor-profile.json`（或等效结构化文件），猫猫分析时作为上下文注入。
...
- **频率监测**：检测铲屎官 7 天内查询同一标的 >3 次时，主动提醒"你已经关注 X 多次了，要不要先做点别的？"
...
- [ ] AC-03: 分析报告中默认使用百分比/相对值，不暴露绝对金额
- [ ] AC-04: 猫猫能基于画像回答"我适合买 X 吗？"（匹配风格+风险偏好）
...
| 猫猫分析质量不可靠（训练数据 ≠ 专家判断） | 所有分析标注置信度 + 强制包含反方观点章节，低置信度建议铲屎官找专业人士 |
| **CVO 角色错位**（铲屎官信任猫过头，review 流于形式） | 每季度强制 1 次"盲测"——铲屎官先独立判断，再看猫猫报告，对比偏差 |
...
| # | 决策 | 理由 | 日期 |
|---|------|------|------|
| KD-1 | 架构分五层（画像/知识/数据/分析/决策） | 铲屎官 + 三猫讨论收敛 + CVO 补充画像层 | 2026-05-18 |
...
| KD-5 | 投资者画像数据本地加密，禁止云端同步 | 财务 PII 敏感性 — CVO 指令 | 2026-05-18 |
