...
**理念**没有**落地物**——铲屎官追问"没有画像，未来如何动态路由？"

当前路由的真实状态：F154 已做"人工偏好层"（铲屎官手动设 `preferredCats` /
`/focus` 选猫），但**猫自己传球时**的依据只有 L0 roster 的一行话（"砚砚：代码
审查专家"）——太粗。传球判断（"这个 500 行需求给 46 一小时推完，还是给砚砚+47
慢慢拆？"）需要知道每只猫的强项、盲点、协作反模式、历史表现，roster 一行话给不了。

铲屎官 directive：**不通过算法路由**——"让你们自己判断、自己传球"。所以本 feature
做的不是"任务来了算法决定谁做"的调度器，是一份**会成长的队友能力画像档案**，让
猫传球时有可靠判断依据。四猫（46/47/砚砚/烁烁）+ CVO 各给一版，47 整合，三猫 R1
review 收敛。
...
提案来源：`docs/discussions/2026-05-20-capability-profile-routing-proposal.md`
...
L0 **不进 native system prompt**（动态画像会 stale hardcode，砚砚 R1 P2）。
...
| # | 问题 | 状态 |
|---|------|------|
| OQ-1 | L0 注入 token 预算 | ✅ resolved → KD-4（L0 只放指针，不全队塞） |
| OQ-2 | 总结层蒸馏触发机制 | ✅ resolved → 事件触发（feat close / review 完成），不用 cron |
| OQ-3 | 开源 baseline 如何 fork | ✅ resolved → 空模板 + Cat Café 示例档案（示例标 demo） |
...
| KD-4 | L0 不进 native system prompt，只放指针 | 动态画像会变，塞 L0 = stale hardcode（砚砚 R1 P2） | 2026-05-20 |
...
- Phase C: 烁烁 design review（UX）+ 跨族 code review（console-dev 4 gate）
...
| 类型 | 路径 | 说明 |
|------|------|------|
| **Proposal** | `docs/discussions/2026-05-20-capability-profile-routing-proposal.md` | 整合提案 + 三猫 R1 review log |
| **Spawned from** | `docs/content/drafts/longform-002-v0-formal.md` | longform-002 Ch.0 能力画像主线 |
| **Feature** | `docs/features/F154-cat-routing-personalization.md` | 人工偏好路由层（独立通道） |
| **Feature** | `docs/features/F200-memory-recall-eval.md` | Phase D 自动累积数据源 |
| **Feature** | `docs/features/F192-socio-technical-harness-eval.md` | Phase E eval 框架 |
| **Feature** | `docs/features/F203-native-system-prompt-l0.md` | L0 注入通道（v1 不用） |
