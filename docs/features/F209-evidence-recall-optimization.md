...
> **Status**: done | **Completed**: 2026-05-26 | **Owner**: 缅因猫/砚砚 | **Priority**: P1 | **Close evidence**: Phase A-D merged and dogfooded; PR #1910 fixed final thread-scope degraded blocker; independent vision guardian Opus 4.6 approved close. Phase E eval ownership transferred to F200 per KD-6 / HW-5.
...
> **来源**：2026-05-23 F209 post-Phase-C dogfood 反思（47 / 砚砚 alignment + 铲屎官指示）。Phase B alias registry 在生产里**机制 ship 但字典为空**、Phase C drillDown **post-merge 才被 author 真用一次抓到 file-slice bug** —— 都是"AC pass ≠ 用户感受到"的同型走偏。该定义钉死后，未来 Phase close 必须证据双足。F209 最终 close 由 Opus 4.6 作为非作者非 reviewer 愿景守护猫吃猫粮复验并 APPROVE。
...
- [x] AC-B6: **transferred to F208 AC-A5** (2026-05-23 post-Phase-C reflection). F209 不再阻塞此 AC；F208 spec 持有对偶 AC `cat-dossier consumes F209 entity_id; no parallel namespace`。这是 ownership cleanup，不是新决策——47 / 砚砚 owner 对齐即可，不需 ping landy。
...
- 加 3 条真实 dogfood query（铲屎官指定 / 历史真实场景），跑 `search_evidence → drillDown → reader` 端到端。
- 同时跑 B.1 seed 后的"CVO 找到只提铲屎官的消息"作为 Phase B 真实可感知验证。
...
| OQ-11 | 是否需要 **invocation-level passage** 作为第二条召回轴（补充 message-level，不替代）？message 是 chat UX 原子，invocation 是执行原子；按 invocation 切对"那次召唤干了啥"更连贯 | ⬜ 留观察——**必须等 D.0 eval 证明 message-level 太碎才做**；可能与现有 session-chain readers（`read_session_digest/events/invocation_detail`）职责重叠，需在 OQ 关闭前论证不 redundant |
| OQ-12 | Phase A `passages[].content` 是否应 **truncate to first-200-chars + drillDown on demand**？现在全文 inline 是"看着够用"的经验判断，未验证 | ⬜ **跟 D.0 eval 一起测**——看猫到底读不读 inline，如果大量靠 drillDown 才展开则改为 200-char + reader on demand |
...
| R7 | “Perspective 不是给铲屎官搜，但能给铲屎官看；和 search_evidence 明厨亮灶联动” | AC-D6, AC-D7, KD-10 | API/MCP run trace exposes plan id, steps, hit counts, typed reader route hints, degraded/effectiveMode | [x] |
...
| 2026-05-26 | Phase D runtime merged (PR #1887) and F209 perspective dogfood passed after multi-round cloud/local review hardening |
| 2026-05-26 | Final close blocker fixed (PR #1910) — thread/session scoped recall now stays project-local and no longer inherits degraded metadata from global store |
| 2026-05-26 | Independent vision guardian Opus 4.6 ran F209 dogfood and APPROVE close; live `cat-cafe-runtime` sync remains a deployment timing note, not a F209 code blocker |
...
| Decision | `docs/decisions/2026-05-23-f209-d0-readiness.md` | D.0 readiness BLOCK → UNBLOCK history and dogfood metrics |
| Close Gate | `docs/decisions/2026-05-26-f209-close-gate.md` | Final CloseGateReport: Phase A-D evidence, Phase E ownership transfer, guardian verdict |
| Reflection | `docs/reflections/2026-05-26-f209-evidence-recall-close-capsule.md` | F209 close reflection capsule |
| ADR | `docs/decisions/036-f209-retrieval-surface-multi-layer.md` | F209/F193 multi-layer retrieval surface and topology lessons |
