...
| 2026-05-31 | Phase G model-selector recon：official docs and local AGY `1.0.3` language-server probes confirm selector labels and `settings.json` key `model` as the exact profile storage surface, closing AC-G1 while leaving AC-G2 open for live per-profile E2E onboarding/user-facing exposure |
