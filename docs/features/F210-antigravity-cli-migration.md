...
- **H2**（砚砚拍 H2a/H2b 拆分，2026-06-02）：trajectory 内容提取，替换 resumed stdout final text；红测复现 `[1]→[1,2]→[1,2,3]` 只输出本轮（根治重放）。proto 解码首选逆向 schema，备选趁临时 LS 活着调 `ConvertTrajectoryToMarkdown`（fallback，生命周期不可靠）。
  ...
  - **H2b/extractor**（下一 PR）：proto text extractor 解 step_payload final assistant text，替换 resumed stdout（根治重放，`[1]→[1,2]→[1,2,3]` 只输出本轮）。B spike §7.4 已证 assistant text 明文存 step_payload（路径 A 可行）。
