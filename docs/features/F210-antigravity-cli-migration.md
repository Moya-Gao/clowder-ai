...
| R8 | “像 F198 拯救布偶猫那样接入 AGY 互动式 CLI？” | AC-G6 | local language-server API probe + PTY fallback smoke + carrier decision packet | [x] |
...
| OQ-3 | Does `agy` support model override, and how does that map to Cat Cafe cat identity? | Partial — no top-level `--model`; deterministic profile routing uses isolated `settings.json` key `model` plus post-run log-label verification, not a per-call flag |
...
| OQ-6 | What exact AGY selector ids map to Opus, Gemini 3.1 Pro, and Gemini 3.5 Flash? | Answered for profile settings — use selector labels `Claude Opus 4.6 (Thinking)`, `Gemini 3.1 Pro (High)` / `Gemini 3.1 Pro (Low)`, and `Gemini 3.5 Flash (High)` in `settings.json`; placeholder ids remain recon-only |
...
| OQ-8 | Can Cat Cafe isolate AGY profiles by HOME / `~/.gemini/antigravity-cli` config while preserving auth, model selection, trusted worktree, and MCP access? | Partial — PR #2004 implements HOME/settings/trustedWorkspaces isolation and fail-closed selected-model verification; the F210 profile E2E smoke runner now makes the live no-bleed proof repeatable, but real auth material remains per-profile onboarding and user-facing Opus/Gemini cats still wait for a passing live report |
| OQ-9 | What is the minimum safe AGY permission posture for unattended runtime calls? | Answered for runtime — use `--dangerously-skip-permissions` only when Cat Cafe has created and preflighted an isolated AGY profile sandbox; strip user-supplied yolo flags elsewhere |
| OQ-10 | Can AGY's local language-server API send user messages, stream updates, cancel turns, and set/verify selected models? | Answered no for production in AGY 1.0.3 — read endpoints and `StreamAgentStateUpdates` work with Connect framing, but API-created send/model/cancel lifecycle is not proven |
| OQ-11 | If the structured local API is insufficient, what is the smallest acceptable PTY/tmux bridge? | Answered — PTY/tmux is manual takeover/observation only; no ANSI-to-AgentMessage parser, no `--continue` routing, no real-HOME yolo, and no unattended local-API writes |
...
- **H3** ✅ **merged（PR #2063, squash `aa941f6cd4`, 2026-06-02）**：step_type 粗标签（15=assistant activity / 14·98=lifecycle / 23=metadata / 9=operation activity 不硬标 tool call / unknown=neutral）+ 折叠单行 agy 进度 UI（`formatAgyProgressDetail` "AGY working · N steps · latest" → thread 级 catStatusDetails → ThinkingIndicator chat 区单行 + ThreadCatStatus tooltip，复用现有不加新组件，不刷 bubble，done/cancel workingCats 过滤自动折叠）。砚砚 APPROVE + 云端 codex 0 major。后端 22 + 前端 20 测 + gate 绿。**遗留**：live AGY browser dogfood 待 alpha 验真实 UI；cache leak（gemini/gemini25 未配 agyProfile → repo root 泄漏 cache/projects.json）= F210 P2 follow-up（bug report 已 commit）。
...
| 2026-06-02 | Phase H1-hotfix merged via PR #2047 (squash `b30da6f9e2`)：前端两 render path 未识别 `agy_trajectory_progress` → 渲染原始 JSON system bubble 刷屏；修为两 path `consumed=true` 静默消费（仿 timeout_diagnostics）。gpt52 跨族 review 0 P1 + 云端 codex 0 major。教训：alpha 验收 gap（前端集成未 review/未 runtime 验收）。遗留 non-blocking：background path 专门红测留 H2 补 |
| 2026-06-02 | Phase H2a-locator merged via PR #2048 (squash `7154e0c538`)：B spike 确认 H1 resume progress P2 gap（agy resume 不写 --log-file + 另起新 cascade db），抽 `AgyTrajectoryLocator` + `listAgyConversationDbs` + `resolveAgyAppDataDir`（effective child HOME）修复。砚砚 APPROVE + Re-confirm；云端 codex 抓真 P2（appDataDir 漏 accountEnv.HOME）已修。全 78/78 + gate 绿 + 真数据 dogfood。carryover→H2b：invocationStartMs guard / trajectory_meta 消歧 / H1 background 红测。附带修 PR #2049 带来的 SessionChainPanel biome format 债 |
| 2026-06-02 | Phase H2b-extractor merged via PR #2056 (squash `ad49f9c87c`)：**根治 resume 重放**。手写 minimal proto wire parser（不引 protobufjs）解 step_payload field 20→1 final / 8 fallback / 排除 3 thinking；`readAgyTrajectorySteps` + `extractAgyFinalTextFromSteps` + `classifyAntigravityCliPlainText` resumed 替换 + `GeminiAgentService` 接入（locator→db read→extractor→替换 stdout）。全 fail-open。砚砚 APPROVE + Re-confirm（真实 AGY DB dogfood 216-1957 chars final 正确抽）+ 云端 codex 抓 2 真 P2 全修（accountEnv.HOME / empty stdout 丢 final）。95/95 + gate 绿。遗留：无 live resume golden fixture（AGY resume §8.5 上游串台 bug），真实 resume 替换待 alpha |
| 2026-06-02 | Phase H3 merged via PR #2063 (squash `aa941f6cd4`)：step_type 粗标签（15=assistant/14·98=lifecycle/23=metadata/9=operation activity 不硬标 tool call/unknown=neutral）+ 折叠单行 agy 进度 UI（catStatusDetails → ThinkingIndicator chat 区单行 + ThreadCatStatus tooltip，复用现有不刷 bubble）。砚砚 APPROVE + 云端 codex 0 major。后端 22 + 前端 20 测 + gate 绿。**H Phase（H1/H1-hotfix/H2a/H2b/H3）全部完成**。遗留：live AGY browser dogfood 待 alpha；cache leak = F210 P2 follow-up（agyProfile rollout） |
| 2026-06-03 | cache-leak fix merged via PR #2068 (squash `1abd4fe34`)：**AGY cwd-relative cache (`cache/projects.json`) 不再泄漏到 repo root**。spawn cwd 与 workingDirectory 解耦 → per-worktree hash sandbox（`<base>/<sha256(resolve(wd))[:16]>`；base = profile `cwdPath` / no-profile `~/.cat-cafe/agy-cwd/<catId>`，`CAT_CAFE_AGY_CWD_ROOT` override）；symlink fail-closed（base+sandbox）；workingDirectory normalize 成绝对路径；workspace 仍由 `--add-dir` 授权；`diagnostics.antigravityCli.spawnCwd` 暴露隔离 cwd。砚砚（缅因猫）跨族 review 全程 APPROVE。**云端 codex 5 轮复审**：relative-path P2 / symlink P2×2 / per-worktree-isolation P1 全修，第 5 轮 "cwd 破坏 AGY 工作区" P1 经砚砚 probe 实测 **refute**（AGY active workspace 绑 `--add-dir` 非 process cwd：`cd sandbox && agy --add-dir work` → tool pwd=work、读 work 文件、GEMINI.md 加载、仅 cache 落 sandbox）。**教训**：cache-leak 修复不止"挪 cache 出 repo"——AGY 把 conversation/cache 都 scope 到 cwd，sandbox 必须 per-worktree（非 per-cat）；4 轮补丁后第 5 轮触发"补锅匠/绕路"自检 → 停手质疑根基 → 实测 refute 而非盲 pivot。**作废上方 H3 旧判断**（"cache leak = agyProfile rollout"：agyProfile 只钉 HOME 不改 cwd，治不了 cwd-relative leak）。遗留：runtime 重启验 `spawnCwd` 落 sandbox + repo root 无 cache（alpha 验收）。bug-report: `docs/bug-report/f210-agy-project-registry-cache-leak/` |
| 2026-06-04 | roster/alias PR #2102 merged (squash `cda2cca93`)：gemini25（暹罗猫 3.5）画像/别名更新（@gemini35 alias）+ L0 roster 放开为「可开发：明确边界 + 强 review gate，不独立 owner/merge」。**伴随 dev-discipline 事故**：gemini25（烁烁）在**主仓**直接 checkout `fix/gemini35-roster-alias` + rebase/amend（reflog 签名实证）而非开 worktree → 主仓 off-main ~7h，触发多 thread 开工自检告警。责任链：gemini25（reflog 签名）。根因：第一次独立写代码/配置的新猫缺整条 dev workflow 肌肉记忆（同晚第 2 起：#2097 假绿 self-merge broke main + #2102 主仓 checkout）。**教训/防复发**：(1) worktree 纪律不止 `packages/`——**roster/config/governance 改动同样开 worktree**，主仓恒停 main；(2)「approve ≠ 放手」扩到 whole workflow：谁 enable 新猫写代码，谁 shepherd worktree setup→实现→merge→收尾验主仓干净，不只 review diff（ties `feedback_approve_then_enforce_merge`）；(3) 触主仓的 session 结束前必 `git switch main` + `git status --short --branch` 自证；(4) 砚砚边界：当前是新猫训练缺口非系统性复发，**短期不加硬 guard**（先靠 pre-end 自证），复发再升级 session-start/pre-end warning；勿 scope-creep 进 `fix/runtime-sanctuary-guard-cwd`。现场已恢复：主仓 `## main...origin/main` 干净（be6600c66）+ 遗留本地分支 `fix/gemini35-roster-alias` 已清。愿景守护 PASS（平行 opus-4.6）。 |
| 2026-06-08 | Phase H5 resume progress delta fix merged via PR #2141 (squash `3dc59a0e`)：AGY resume 轮次的 step/tool-call UI 不再显示历史累计 1..N，而是以 pre-spawn DB baseline cursor + bounded prefix continuity proof 只展示本次 invocation 新增 trajectory rows。云端 codex 多轮 P2（same-path DB recreate/truncate、inode/birthtime 不可靠、baseline retry、全历史 hash 热路径）全修；observer/Gemini service targeted tests + full gate green。 |
...
| Official | https://antigravity.google/docs/cli-settings | AGY CLI persistent settings surface; refreshed 2026-05-31 |
...
| Recon | `docs/features/assets/F210/phase-g-interactive-carrier-decision-2026-06-01.md` | Phase G AGY 1.0.3 interactive carrier decision, local API rejection, and PTY/tmux fallback boundary |
| Recon | `docs/features/assets/F210/phase-g-agy-1.0.3-capability-refresh-2026-05-31.md` / `agy-1.0.3-help.txt` | Phase G AGY 1.0.3 help/docs refresh, profile-auth smoke, and auth-required guardrail |
| Recon | `docs/features/assets/F210/phase-g-agy-model-selector-recon-2026-05-31.md` | Phase G official/current model availability, selector labels, and profile settings storage surface |
