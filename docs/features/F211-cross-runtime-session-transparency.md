...
| 2026-05-29 | **REG4 真因纠正 + REG5/6/7 立项（opus-4.8 日志取证）**：REG4 真因不是 catch{} 抛异常，而是 runtime catalog 的 stale `sessionChain:false` overlay 把创建块整个 gate 掉了（template 自 F061 #1299 已 true，overlay 盖 base）；CVO 在 hub 手动 toggle 已修复 live，auto-heal migration 暂停。继续挖 CVO 反复报的"静默错误→换session→失忆"：drill `thread_mpqtwr9ltb3nf7gb` + `api.2026-05-29.1.log` 锁定真因 = **`getOrCreateSession:967` 只 resume IDLE cascade；孟加拉还在干活（680 步研究任务，cascade `cbaa41a9`）时 CVO 11:23:28 发 follow-up → busy cascade 无法 resume → 起全新空白 cascade `3d91d911` → 失忆**（REG5；session-chain toggle 不修这条，故"开了还会出现"）。旧 invocation 被当 `runtime_error_reset` 崩溃 seal（REG6）；WS 2 分钟抖 6 次 + done-looking-but-still-running streaming 误导 CVO 以为已完成（REG7）。CVO 强调他**没手动 cancel**——`SocketManager:211` reason 写死 `user_cancel` 是误导。下一步：摸 Antigravity cascade API 选 queue / inject / interrupt-resume 修法（保记忆优先）。 |
| 2026-05-30 | **REG5 substantial fix MERGED via PR #1959**（squash `bca4c6b82`）：核心失忆（busy cascade 收到 follow-up → 复用而非起新空白 cascade，保全 native 记忆）+ 同类边界/abort/terminal 边缘全套修复（跨 `getOrCreateSession` reuse 与 preflight health-rotation 两个子系统）。经云端 Codex ~18 轮 + 一次对全 diff 的 adversarial 同类审视（CVO directive "不只修他找到的那个，审视同类边界问题"）收敛：reuse 谓词 `continuable = IDLE \| RUNNING`、对 IDLE/RUNNING/awaiting in-flight 的 count-gated `settleRunningCascadeForReuse`、model-only RUNNING 不 drain（不引入多分钟延迟）、in-flight settle 以工具最大超时 + 60s 兜底、abort 端到端贯通（settle/drain/trajectory-read/pre-send）、terminal/unknown → replace、preflight 不退役仍 RUNNING 的 oversized cascade（cascade-health `cascadeStatus` gate）。根因洞察：`getInFlightCount` 是进程内活动量计，不是 owed-result 神谕。antigravity 套件全绿。剩终极 HARD-CORE busy-reuse 边界残差（**保记忆**；stale-output + count==0-WAITING）CVO 批准 defer → **REG8**（先合记忆修复止血孟加拉的 live 失忆，边界残差待部署后在 live runtime 上验证 Antigravity 排队语义再修）。`pnpm gate` 唯一红 = main 上既有、与 F211 无关的 F203 system-prompt 体积预算超标（正交；本 diff 仅 antigravity，绿）。**孟加拉此刻仍跑旧代码 → runtime sync（部署）是 CVO 的活。** |
| 2026-05-30 | **REG5 LIVE e2e VERIFIED post-deploy（决定性日志铁证）**：CVO 同步+重启 runtime（runtime HEAD `1fd5d369e` 含 `bca4c6b82`）→ 修复上线。opus-4.8 在 `thread_mp5lezi1hp0cft3w` 做受控 mid-work 测试：给孟加拉（antig-opus，cascade `633808bb`）派一个长多步任务，趁他忙时插一句去 confound 的探针。runtime log `api.2026-05-30.1.log` 决定性证明：task（invoke 10:17:36）与 follow-up（invoke 10:22:30）**复用同一个 cascade `633808bb`**，且该 cascade 在 10:22:30 复用那一刻**持续 RUNNING**（`RUN_COMMAND`/`view_file`/`grep` tool 流不间断，10:22:17 `RUN_COMMAND WAITING`、10:22:29.939 `batch processed` 一路到复用之后）→ 是真·busy 路径（preflight 日志 "skipped … a non-IDLE busy cascade we must not rotate mid-turn"），不是一直好使的 IDLE 路径。孟加拉完整记得任务/文件/进度/自己发的 messageId；旧 bug 会起空白 cascade → 冷启动失忆。事前 flag 的 confound（孟加拉主观说"你来晚了我干完了"）被日志推翻——复用时他还在干活。REG5 真·闭环（合入 + live 证明）。NOTE：follow-up 10:17:58 发、10:22:30 才处理（~4.5min，排在长任务后），记忆保住了（REG5 ✅）；这个排队延迟本身属 REG8 边界范畴，非 REG5 失败。 |
| 2026-05-30 | **REG8 (busy-reuse boundary, facet a) MERGED via PR #1976**（squash `b65e94fd`）：follow-up 到达 model-only RUNNING turn 中途时 `getOrCreateSession` 立即复用（REG5 no-drain）、`sendMessage` 中途取 `stepsBefore` → `pollForSteps` 原本流式旧 turn 尾巴并在旧 turn IDLE 提前终止 → 队列消息回复被静默丢弃。修法 #3（poll-aware，非 #2 drain-before-reuse——#2 会回退 REG5 no-drain 决策）：`sendMessage` 返回 `{stepsBefore, wasBusy}`；`pollForSteps` busy-reuse 模式在队列消息自身 `USER_INPUT` step 出现前不认终止性 IDLE；`invoke` 只对首个 poll 启用。先验证（孟加拉 antig-opus 确认 Antigravity 原生 QUEUE，严格在当前 turn 终止 IDLE 后拾起）再实现。TDD red→green；REG5 的 5 个 no-drain 测试保持绿；61/61 + 480 antigravity 套件 + `pnpm gate`(19 checks) 全绿。opus-46 跨个体 APPROVE，作者按 review-continuity 自合（APPROVE 后 delta 均非行为性：2 处 gate 抓到的 test-mock 形状 + biome format + commit-msg reword）。**孟加拉 antig-opus 的 invocation 在实现 GREEN 时静默挂死（REG9 现场）→ opus-4.8 CLI 接手完成。** facet (b) owed-WAITING-step 属 abort/seal 路径 → REG6/REG9 跟踪。 |
| 2026-05-31 | **REG9 MERGED via PR #1984**（squash `7abb22e68`）：根因 = (i) O(全历史) 轮询（`pollForSteps` 每 2s 重拉整段 ~4MB trajectory 只为判断有无新 step；长命 busy cascade 无限长大 → 越来越脆 + stall 时白拉）+ (ii) `invoke` 无 top-level finally → 被弃 generator 静默蒸发。live-probe 先杀死 delta-via-field（`startStepIndex`+16 字段名+cursor-token 全部静默忽略；真·O(delta) 只能 push）。修法 (b)：`getCascadeStatus` 廉价摘要（~57KB）做变更信号 + status-gate（仅变更才拉满量；RUNNING throttle 兜 mutation/awaiting；no-change 保留 idle-timeout）+ `invoke` try/finally side-effect seal（runtime_disconnected）。砚砚 review 抓 1 真 P1（optimistic status-key commit → transient fetch fail 吞变化 → false stall），`pendingStatusKey` deferred-commit 修复（红→绿）。**#4 runtime 证据**（只读真 cascade 633808bb/357步）：unchanged poll 57KB vs 满量 3.99MB = **70× 降**，key 稳定 → gate 跳过满量。`pnpm gate` 绿（antigravity 488/488）。云端 Q4 降级砚砚 review。**Deferred → REG10**：push/streaming transport（asymptotic O(delta)）+ getTrajectorySteps-2× on-change。**Post-deploy alpha**：finalizer live-proof（真被弃 invocation seal）需 CVO 部署后验。 |
| 2026-05-31 | **REG6 part 1 MERGED via PR #1986**（squash `eab674998`）：top-level catch 之前对所有 caught error 硬编码 `sealReason: 'runtime_error_reset'`（crash）→ 一个 interruption-abort（signal abort：新消息抢 slot / WS reconcile，抛 'Aborted'）也被当 crash 封 → 触发 cascade-replacement，丢掉下条消息该复用的 cascade（REG5 连续性丢）。修法：catch 加 `isInterruptionAbort = options.signal.aborted \|\| /^Aborted/.test(msg)`；是 abort 就省掉 seal（preserve cascade，等同正常 turn-end），真错误仍 crash-seal。TDD red→green；gpt52 跨族 review APPROVE + 采纳其 test-tightening（abort 断言 `sessionLifecycle === undefined`）；antigravity 490/490；云端 Q4 降级 gpt52。**Part 2（`SocketManager:211` `'user_cancel'` 归因）仍 open**：需客户端传真实 trigger（前端+协议）+ 牵 auto-resume 副作用，单独搞。NOTE：本轮两次 index-sync stale（我的 REG9 doc-sync + 别人的 F216 PR#1987）暴露"doc-sync 漏 regen feature index"系统缺口 → 待加固 merge-gate Step 7.5 / pre-commit hook。 |
| 2026-05-31 | **Batch-live validation post-restart（CVO 重启 runtime；sonnet 驱动 2 轮 + 孟加拉并行，opus-48 owner 接 subtle 判定）**：REG5 **LIVE-reconfirmed**——Round 2 直接 log 铁证：6 步任务 cascade `d492e8e1` + 中途 follow-up（step 43 到达、cascade RUNNING）复用**同一个** cascadeId，记忆完整。REG9 无 4MB（status-gate 1–37KB、满量 204–735KB）；REG7 全程无 WS flap（50+ steps，与 REG9 缓解假设一致）；REG6 受控 seal（非 `runtime_error_reset`）。**REG8 本轮未被直接压到**（Round 1 走 F201 retry、Round 2 走 F216-c3 supersede——都复用 cascade、都保记忆，但 REG8 的 queue-边界轮询具体路径未直接 live-stress；单测+review+目标达成旁证 → 低风险 live-residual）。**观察**：Round 2 follow-up 走 F216-c3 supersede（中断+同 cascade 重启）而非 REG5 原生 queue——两条都保记忆，canonical 行为是否被 F216 改了待厘清。**F201 corpse-churn 根因（CVO 在 UI 见 4 具 0-msg 尸体追问"是不是 F216"）**：孟加拉模型反复 `view_file` 猜错 F211 文件名（`F211-session-lifecycle-transparency.md` ← 真名 `cross-runtime-session-transparency.md`）→ invalid tool call → F201「Antigravity retry」seal 旧 cascade `tool_conflict` + 开新的 → 同错名重复 → 连环 4 次 → 第 5 次跑对。**实锤排除 F216**（`[F216-c3] supersede` 整 log 仅 fire 1 次、Round 2、不同时间；sonnet Round 2 纯 search_evidence 零尸体）。能 recover、非回归、非 REG5/8/9/6 → 毛线球 `0001780239122393`（候选硬化：F201 复用/清理 cascade 而非每 retry seal+换新；勿盲目 cap 重试）。**另**：上行 REG6 NOTE 的 index-sync stale 系统缺口已由 **decommit `docs/features/index.json`**（PR #1993 `ac18cfb3d`，砚砚 APPROVE）根治——派生产物零活跃消费方、不再 commit、check-feature-truth 删自比对。 |
| 2026-05-31 | **F201 retry-corpse UI declutter MERGED via PR #2006**（squash `355062d0c`）：SessionChainPanel 把 ≥2 具 0-msg `tool_conflict`-sealed 的 F201 重试残骸折叠成单行 summary（`N 次 tool_conflict 重试残骸`），不再每具 retry 糊一张 corpse 卡——消掉 CVO 在 UI 看到的"4 具空尸体"噪音。纯 UI surface（recovery/seal 逻辑没碰）。砚砚（codex/GPT-5.5，CVO 点名）review 抓 1 真 P2（in-flight `sealing` 的 0-msg `tool_conflict` 不该被折叠，会藏掉 live 状态 + 查看/解封）→ predicate 收窄到 `status==='sealed'` + 加锁测；rebase 到最新 main 后砚砚 continuity-APPROVE `4a39ad685`。`pnpm gate` 全绿（439 files / 3626 tests）。**仅 UI 症状缓解**——F201 根因（每 retry seal 旧 cascade + 起新 → 留尸体）的 cascade-reuse/cleanup 硬化仍是毛线球 `0001780239122393`。 |
