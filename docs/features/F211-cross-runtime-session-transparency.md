...
> **Status**: done ⚠️ **post-close regression open (2026-05-28) — see F211-REG1/REG2/REG3; reopen decision pending CVO** | **Owner**: 缅因猫（砚砚） | **Priority**: P1 | **Completed**: 2026-05-26
...
| F211-REG1-2026-05-28-dispatch-visibility-filter | F211 Phase E / external-runtime-sessions route | **P1 (post-close regression)**: Cat-Cafe-dispatched Antigravity sessions are invisible to the user. List/detail/deep-dive runtime surfaces filter `surface==='ide-direct'` only (`external-runtime-sessions.ts:82`/`:122`), but dispatched sessions are written `surface:'cat-cafe-dispatch'` (`invoke-single-cat.ts:378`). User cannot find the session or its cascadeId. Directly causes 铲屎官 2026-05-28 "看不到他的 session / 不知道他的 id". | ⬜ Open. Fix direction: list/detail/deep-dive runtime surfaces must include `cat-cafe-dispatch` (or expose a combined runtime view); red test asserts a dispatched cascade appears in `/api/external-runtime-sessions` + MCP list + deep-dive header. |
...
|----|---------------------------|---------|----------|------|
| R1 | “Antigravity 的 session 得是透明的” | AC-A1~A9, AC-E1~E4 | SessionChainStore tests + session reader proof + Hub/session-chain display | ⚠️ regressed 2026-05-28 — dispatched sessions structurally invisible (F211-REG1) |
...
| R5 | IDE 直开和孟加拉猫聊天也要能找回 | AC-B1~B6 | IDE-direct registration fixture / list/read discoverability validation | ⚠️ partial 2026-05-28 — registration endpoint works but IDE-direct never auto-calls it; dispatched path invisible (F211-REG1) |
