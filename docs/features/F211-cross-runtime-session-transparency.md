...
> **Status**: done ✅ REG1 + REG2 + REG3 (Layer B urls + Layer C image media) FIXED + merged + e2e-verified (PR #1940 `a0f7cf208`, PR #1944 `fb37ecc7e`, opus-4.8) ⚠️ **REG4 RESOLVED 2026-05-29 (stale `sessionChain:false` catalog overlay gated the block off — NOT a thrown exception; CVO toggled session-chain ON; durable migration paused). NEW open: F211-REG5 (P1) — follow-up-while-busy spins a fresh BLANK cascade (`getOrCreateSession:967` only resumes IDLE) → Bengal amnesia whenever messaged mid-work; the session-chain toggle does NOT fix it (this is the recurring "换session→失忆"). + REG6 (interruption misclassified as `runtime_error_reset` crash) + REG7 (WS flap / premature done affordance). opus-4.8 owning the fix; probing Antigravity cascade API next.** | **Owner**: 缅因猫（砚砚）; regression + REG4/5/6/7 fixes 布偶猫 Opus 4.8 | **Priority**: P1 | **Completed**: 2026-05-26
