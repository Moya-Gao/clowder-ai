...
`dump.rdb` / `dump.rdb.backup-*`（Redis 持久化，**6399 圣域**）、`evidence.sqlite*`（Hindsight 记忆证据库）、`world.sqlite*`（World Engine 世界状态）。
...
| 类型 | 路径 | 说明 |
|------|------|------|
| **Decision** | `docs/decisions/010-directory-hygiene-anti-rot.md` | 上游防腐化决策 |
| **Feature** | `docs/features/F023-directory-corrosion-defense.md` | 子目录代码防腐化（互补） |
| **Thread** | `thread_mpq6yy8q88py274n` | 初版提案讨论（EP-002，未落地成文件） |

## 需求点 Checklist

| ID | 需求点（铲屎官原话/转述） | AC 编号 | 验证方式 | 状态 |
|----|---------------------------|---------|----------|------|
| R1 | "从根源解决猫们乱扔垃圾问题"（提案总结） | AC-A1/A2, AC-C1 | test + manual | [ ] |
| R2 | "不准动 redis相关的那些！！"（铲屎官 2026-05-28） | AC-A2/A3, Fixture 4 | test（硬拒绝 .rdb/.sqlite） | [ ] |
| R3 | 有状态存储迁移"甚至不应该列"为后续 Phase（铲屎官 2026-05-28） | Scope 边界节 | manual（spec 审查无尾巴） | [ ] |
| R4 | §N 根目录卫生公约（提案三件事之一） | AC-B1/B2 | manual | [ ] |
| R5 | pre-commit hook 白名单（提案三件事之一） | AC-C1/C2 | test | [ ] |

### 覆盖检查
- [ ] 每个需求点都能映射到至少一个 AC
- [ ] 每个 AC 都有验证方式
- [ ] 前端需求已准备需求→证据映射表（不适用，无前端）
