...
2026-05-31 铲屎官与宪宪讨论《Agent Harness Engineering: A Survey》时，铲屎官四轮追问暴露系统性认知缺陷：猫猫搜到信息后缺少"这东西靠谱吗？"的批判性反射。

具体事件：宪宪搬了 Meng et al. survey 的"65% 企业 AI 失败归因 harness 缺陷"和"每步 2% 上下文衰减"——溯源后发现 65% 来自 MemU（卖 AI 记忆产品公司）博客，无 peer-reviewed 来源；多篇博文互引形成回声室被误当"多方验证"；用 2025 年旧模型数据论证 2026 年 Opus 4.6 的能力问题。

三猫圆桌诊断（@opus + @opus47 + @codex）：**失效在触发器缺口，不在能力**——猫被追问时展现了完整批判性能力（追溯营销博客、识别回声室、做代际校验），但日常引用时没有 dispatcher 把批判能力绑到"引用外部数据"这个动作上。

铲屎官原话（2026-05-31）："不能只靠 landy 拉闸——agent 领域我能拉，但如果聊的是达芬奇生平或者博士论文研究，你们交出一份看起来非常有道理实则胡说八道的东西，不是因为你们偷懒，是你们被外部不可靠信息源污染还没警惕。"

这不是幻觉问题（模型自己编），是**外部证据污染问题**（模型被不可靠来源带跑了还没警觉）。搜索结果不是证据，只是候选线索；一旦我们把候选线索写进 research、PPT、ADR，它就会污染后续猫的判断链。

与 F086（元思考）同源：「猫猫缺少元思考，不是能力问题是意识问题」。解法应该是结构化 reflex，不靠自觉、不靠铲屎官在场。

## What

### Phase A: 软+硬+eval 三层防御骨架

三层防御，按"铲屎官不在场也得生效"原则排序：
...
| # | 问题 | 状态 |
|---|------|------|
| OQ-1 | source-audit skill 触发的精确边界——LLM 如何自我识别"准备引用外部数据"？ | ⬜ 未定 |
| OQ-2 | 是否需要 linter/eval 兜底（扫描输出中无 provenance 的数字/百分比） | ⬜ 未定 |
...
| 日期 | 事件 |
|------|------|
| 2026-05-31 | 铲屎官四轮追问暴露信源批判性缺失 |
| 2026-05-31 | 三猫圆桌（@opus + @opus47 + @codex）收敛方案 |
...
- Phase A: 跨族 review（砚砚优先，历史连续性——圆桌参与者）

## Links

- 教训沉淀：`memory/feedback_source_criticality_missing.md`
- 事故现场 research 笔记：`docs/research/2026-05-26-agent-harness-engineering-survey/README.md`
