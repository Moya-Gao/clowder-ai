...
> **Status**: in-progress（解耦的"立项愿景硬度自检 SOP gate"已 PR #2034 merged；核心引擎债主体待启动）| **Owner**: 布偶猫（宪宪/Opus-4.8）| **Priority**: P1
>
> **CVO signoff**：铲屎官 2026-06-02 04:45 UTC 明确立项 "f219 这个立项哦"，scope = 聚焦核心引擎债（不贪全量），routeSerial 走快线打头。立项前讨论见 Links 讨论草案。
...
F216 是从 F215 thread handoff 立项的。铲屎官诊断根因：
...
对登记册排优先级，每个高优项给"演进方案 + tradeoff + 分期"。多猫圆桌评估（布偶猫主导设计 + 缅因猫 review + 多视角）。
...
### Phase A（盘点）
- [ ] AC-A1: 产出技术债登记册（`docs/discussions/` 或独立文档），覆盖 IN scope 全部核心引擎模块，**每项含 4 要素**（脆弱度实测证据 / 历史 bug 频次 / 演进选项 / 预估投入）
...
| # | 问题 | 状态 |
|---|------|------|
| OQ-1 | "核心引擎"边界精确定义——哪些 ownership cell 纳入 IN scope（routing/dispatch/identity-session 确定，bubble-pipeline 是否纳入？） | ⬜ Phase A 开盘前定 |
| OQ-2 | routeSerial 快线 complexity 目标值（255 → ?） | ⬜ Phase B 定 |
| OQ-3 | routeSerial ownership cell 归属——新建 routing cell 还是并入 dispatch？ | ⬜ Phase A 盘点产出建议，Design Gate 拍 |
...
| 日期 | 事件 |
|------|------|
| 2026-06-02 | 立项（CVO 用 F 号语言 signoff "f219 这个立项"；scope/快线/解耦三项确认） |
| 2026-06-02 | 解耦的「事情 B」（立项愿景硬度自检 SOP gate）落地 — PR #2034 merged（`feat-lifecycle/SKILL.md` + `feature-doc-template.md`），砚砚跨族 review R1 P1/R2 P2/R3 放行 |

## Review Gate

- Phase A: 盘点登记册经 ≥1 只非 owner 猫 review（AC-A3）
- routeSerial 快线: 跨族 review（缅因猫优先）+ 云端 review + 真实 runtime 愿景守护（LL-064）

## Links

| 类型 | 路径 | 说明 |
|------|------|------|
| **讨论草案** | `docs/discussions/2026-06-02-tech-debt-architecture-evolution/README.md` | 立项前提案（commit `a2b2a3f51`），含元教训分析 |
| **Feature** | `docs/features/F216-route-serial-refactor.md` | 直接前身；complexity 255 转技术债的出处；close 口径与 AC-B2 |
| **Feature** | `docs/features/F215-malformed-toolcall-recovery.md` | routeSerial 7 轮 review 引爆点 |
| **Feature** | `docs/features/F185-dispatch-busy-gate-unification.md` | dispatch cell（invocation 债候选所在） |
| **ADR** | `docs/decisions/034-dispatch-busy-gate-unification.md` | dispatch busy-gate 分层决策 |
| **Lesson** | `docs/lessons-learned.md` → LL-064 | 核心路径 feat 必须真实 runtime 验证（关联 F215 PR #1953/#1960/#1966） |
| **Ownership** | `docs/architecture/ownership/cells/dispatch.md` | InvocationQueue/QueueProcessor/InvocationTracker 归属 |
| **代码（债务本体）** | `packages/api/src/domains/cats/services/agents/routing/route-serial.ts` | routeSerial 2376 行 / complexity 255 |
| **代码（routing 模块）** | `packages/api/src/domains/cats/services/agents/routing/` | AgentRouter / route-helpers / routing-decision / WorklistRegistry / route-parallel |
| **代码（invocation 模块）** | `packages/api/src/domains/cats/services/agents/invocation/` | InvocationQueue / QueueProcessor / InvocationTracker（dispatch cell 债候选） |
| **Bug 史** | `docs/bug-report/a2a-same-turn-handoff-coalesce/` | F216 Phase D coalesce/supersede 上下文 |
| **F216 相关 PR** | #1987 / #1991 / #1997 / #2002 / #2003 / #2008 / #2010 | routeSerial 决策/执行分离的全部落地 PR |
