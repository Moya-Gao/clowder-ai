...
现有记忆三层各有主体：Session/Invocation 的主体是**工具调用**，Thread Digest 的主体是**话题**，Raw Message 的主体是**消息**。但"48 在哪 aha 了 / 铲屎官在哪拉了闸 / 坐标系在哪被纠正"——这些 harness 运行轨迹中**信息密度最高的认知转折点**（cognitive-state-transition），在系统里根本不存在。普通消息流信息密度低，"48 发现坐标系错了"才是黄金信号，而它没有一等公民的位置。后果：**连当事猫自己都回溯不了自己的认知轨迹**——F225 起源时，是铲屎官人肉记得"那只猫是 48"，不是系统记得。
...
铲屎官原话场景（2026-06-06，PPT 演示编排讨论）：想在台上说"我说过脚手架"然后**瞬间跳转到那个 thread 那条 message**——正是这个动作暴露了记忆系统的真空层。
...
| 趋势被误读为"自进化有效"（砚砚 push back）| 强制配 resolution 链，趋势不单独展示为结论（AC-C2） |
...
| # | 决策 | 理由 | 日期 |
|---|------|------|------|
| KD-1 | 内核是 Event Memory（事件级索引），不是 Magic Word 面板 | Magic Word 只是第一条 lane（砚砚收敛）| 2026-06-06 |
| KD-2 | 核心 schema 字段是 cognitive-state-transition，不是 magic word | 真空层是"认知状态不是一等公民"（48 reframe）| 2026-06-06 |
| KD-3 | 两轨采集，猫自拉闸必须主动声明 | no-classifier 红线：系统不判断哪条是 aha（48 批判 + 铲屎官裁定）| 2026-06-06 |
| KD-4 | 系统是小本本记录员，不 push 猫，猫主动翻阅 | 铲屎官裁定 | 2026-06-06 |
| KD-5 | v1 schema 面向 v5 终态，走正确路叠不脚手架叠 | 铲屎官裁定 | 2026-06-06 |
...
- Phase C: 重点守"趋势不单独当证据"（AC-C2，砚砚 push back 落地）
