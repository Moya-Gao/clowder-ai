...
- **素材池已开仓**：`assets/F229/desktop-pet-sprite/`（README 含 production pipeline 五步 + 砚砚验证的云端生图 prompt 模板）——缅因猫 raw sheet ×2 已入库（fbb0e8add）；v1 默认布偶猫 + 孟加拉/暹罗 sheet 待生成。后续四猫视觉刷新以醋醋喵漫画母图和 `docs/videos/cucu-pr-flow/character-bible-v0.1.md` 为上游 canon：砚砚猫猫球的母图就是醋醋喵漫画里的砚砚，不从通用缅因猫 prompt 重新采样。
- 主动冒泡（新版本发布等白名单事件，安静优先）
- OpenCLI 式页面操作演示（#841 终态收编：猫操作页面给用户看，操作前用户确认）

#### Phase E3: Resizable Ball（猫猫球可拖拽缩放）

> CVO 2026-06-26："我能够拖动猫猫球的大小让这只猫变大点吗？现在有的时候感觉这只大猫猫太小了"
...
> CVO 2026-06-26："猫应该能自己跑起来！甚至和我们的界面交互！比如拖到界面边边可以猫猫躲起来！加点让他真的在 Hub 内生活的感觉"
...
**互动玩法扩展**（CVO 2026-06-26："投喂小鱼干？猫粮？"）：
...
| 2026-06-18 | **Phase E1 atlas animation merged** (PR #2373, squash merge)：yanyan-codex 9-state animated spritesheet atlas integration。`CodexPetState` 4→9 states + V1 projection + CSS sprite animation engine (`useSpriteAnimation` setTimeout chain + per-frame timing + reduced-motion) + `AtlasSprite` renderer (aspect-ratio-aware 192×208→59×64 height-fit) + `computeScaledBackgroundPosition` integer-math position helper (P2 fix: float rounding drift on non-square cells)。gpt52 local review 1 P2 found→fixed→confirmed + cloud review 1 P2 pushed back (intentional locked skin picker, KD-14)。4282 web + 20 shared tests green。AC-E1-1~E1-4 ✅ |
...
| 2026-06-18 | **UX polish merged** (PR #2398)：CVO 反馈一把修——emoji→SVG icons（bell/bell-off/cross/cat 四个 CafeIcon）+ "前台猫"→"猫猫球" 全量 rename（ConciergePanel/RailToggle/settings-nav/ConciergeSettingsContent 6 处 + test 2 处）。gpt52 local review（1 P2 漏改 settings 页已修）+ cloud R1 0 P1/P2 + pnpm gate 全绿 |
| 2026-06-18 | **Skin transparency + idle animation fix merged** (PR #2385)：CVO 报告猫猫球黑底 + 动画太快。根因 Tailwind preflight `img { max-width: 100% }` 约束 atlas img → 加 `maxWidth: 'none'`；idle frameDurations 1.1s→3.8s。gpt52 + cloud R2 全绿 |
...
| 2026-06-22 | **BUG-UX-7/8 fix merged** (PR #2488, squash `13828ec9c`)：ConciergeMessageContent 统一使用 MarkdownContent 渲染（`buildMdComponents(tp?)` 工厂 + `processMarkers` textProcessor，覆盖所有文本容器，code/pre 排除）+ peek toggle/dismiss（BUG-UX-8）；gpt52 local review 2 轮 + cloud 0 P1/P2；16 tests |
| 2026-06-18 | **CVO UX feedback fix merged** (PR #2401, squash `ab8f70b1e`)：cancel/stop button for in-progress invocations（scoped per-cat cancel via `/cancel/:catId`，非 force-reset 核弹——gpt52 P1 修正）+ text overflow fix（`overflow-hidden` + `[overflow-wrap:anywhere]` 治 CJK+EN 混排溢出气泡边界）；5 新测试（overflow + cancel 4 tests），4289 全绿；cloud P2 "queued 窗口 disabled" downgraded P3（sub-second gap，pre-PR 无 cancel 能力，非回归）|
