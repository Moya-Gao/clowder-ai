...
- [ ] AC-A3: 记忆导航——3 个真实历史讨论 query 给出正确 thread/message 链接，且**两种动作都可用**：跳过去（teleport）+ 原地看（卡内 inline 展开）→ R3/Why-3——**基础设施 ✅（CardBlock handler + peek API 200 + KD-17 HandleMap merged PR #2266）；待 alpha 验收补验**（证据 `ac-a3-*.png`）
