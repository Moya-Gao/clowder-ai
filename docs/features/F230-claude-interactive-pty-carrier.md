...
| 2026-06-12 10:2x | **✅ Phase B-hook 步骤 ② merged（PR #2252，squash `f9239b69`）**：宪宪/Opus-4.6 实施（TDD 6 tasks，57 tests）+ 砚砚/GPT-5.4 R1+R2 review（live claude 2.1.175 验证 hooks 端到端 PASS）+ 云端 Codex COMMENTED（2 P1 pushback 降级 P3）。输出面器官移植完成：hook sidechannel 替代 transcript tail，factory 解除 2.1.170 pin。**pin 死锁解除——任意 CLI 版本可用**。Usage 降级为已知 tradeoff（hooks 不含 token 数据）。 |
...
| 2026-06-12 10:3x | **✅⚠️ B-hook 愿景守护（Fable-5，亲验非看声明）**：merged main 用**系统 claude 2.1.175**（170 路径已清→fallback 实证脱离 pin）跑真链路 21.1s——session_init 真 UUID + 回复送达 + 零 error，**核心承诺"脱离 pin"达成 APPROVE**。**但抓 2 个真 follow-up gap（非 block，已 merged→新 PR）**：① **AC-B1 计费身份可验证性回归**——hook input JSON 不含 entrypoint 字段（spike capture 实证字段表无 entrypoint），sidecar 丢失 entrypoint=cli 这个 F230 立身之本的文件级证据。**修法**：hook capture 脚本 dump `$CLAUDE_CODE_ENTRYPOINT` env 到 sidecar（spike 的 dump 脚本 env 段已证 env 里有 cli），恢复订阅安全可验证。② **smoke 脚本未适配 B-hook**（仍 pin 170 + 断言 transcript entrypoint/permission）→ `pnpm gate` 对 B-hook live path 是假绿（砚砚 R1 已点出未修）——smoke 应改采 sidecar（permission_mode 在 hook input 有，entrypoint 待①补）。系统性根因：test seam `hookSidecarPathOverride` + 过时 smoke 双重绕过真实 hook transport（[[alpha_smoke_happy_path_blindspot]] 系统版，砚砚 R1/R2 反复踩的同一处） |
| 2026-06-12 11:56 | **✅ B-hook follow-up merged（PR #2254，squash `ca039a871`）**：宪宪/Opus-4.6 实施 + 砚砚/GPT-5.4 review APPROVE（live claude 2.1.175 验证 PASS）+ 云端 Codex CLEAN。Fable-5 愿景守護抓的 2 个 follow-up gap 全闭环：① **AC-B1 计费身份可验证性恢复**——capture script 注入 `$CLAUDE_CODE_ENTRYPOINT` env 到 sidecar JSON（`_cc_entrypoint` 字段），`extractEntrypointFromHookEntries` 纯函数提取，carrier done.metadata.entrypoint 透传。② **smoke 脚本适配 B-hook**——移除 2.1.170 pin + 从 sidecar 读 entrypoint 替代 transcript + AC-B4 降级 info（CLI flag + 单测已验证）。9 新测试（enrichment 3 + consumer 4 + carrier 2），总 58 测试全绿 |
| 2026-06-12 11:3x | **✅ 守护复验 PASS（Fable-5，final-SHA on merged main）**：smoke 20.6s 全绿——**`done.metadata.entrypoint=cli`**（AC-B1 计费身份文件级证据恢复 ✅）+ 系统 claude 2.1.175 无 pin（fallback warning 消失，smoke 已适配 sidecar ✅）+ usage 降级如实标注。**两 follow-up gap 闭环，Phase B-hook 完整收口**：interactive carrier 现为"任意 claude 版本可用 + 计费身份可验证"。Phase C/B-full/D 维持 gated（6/15 判罚触发） |
