...
| OQ-3 | 安乐死通道是否提前到 Phase A 轻量版（一个显式标记 + why） | 🟡 推荐方案（2026-06-18 opus-47 整理待 CVO 拍板）：**不提前到 Phase A**，走 Phase C 完整版进 ball-custody event stream（与 KD-2 单账本一致 / 与 KD-1 "球权由言语行为构成" 哲学一致——CVO 显式说"这球死了" = 真正的安乐死事件，比 "mark flag" 更符合愿景；Phase A 提前轻量版会引入双写绕过事件流，违反 P4 单一真相源）。**短期缓解**（Phase A 已生效，不需新代码）：简报"异常优先+晾龄折叠"已经把 ≥60 天僵尸球折叠不抢视线，CVO 实际看简报时不会被淹没。**Phase C 落地形态**：球可被 owner 或 CVO 显式发"球.冷冻 / 球.降级 / 球.放弃"事件 + 留 why，进事件流后简报僵尸球区自然消项 |
...
| OQ-7 | 全景网络渲染模式（all-lines 系统图 + 动效 + 暗色霓虹）——CVO 明确定位为**传播资产**非运维仪表（铲屎官 2026-06-12："有噱头 可以宣传"）。差异化 vs graph-RAG 3D 球：每条线/每个站可点开验证真实协作史。同一事件流数据零额外埋点。**Phase C stretch 或 close 后衍生，不阻塞 A/B/C 主线**；视觉可交烁烁/antig-opus | 🟡 推荐方案（2026-06-18 opus-47 整理待 CVO 拍板）：**单独立项不挂 Phase C 主线**——Phase C 主线 (AC-C1/C2/C3) 是 CVO 自用的"安乐死 + 轨迹 + 单账本验证"，业务工具属性；全景渲染是对外传播资产，scope 不同。**数据复用**：直接读 Phase B `BallCustodyEventLog` 投影，零额外埋点。**视觉分工**：烁烁（暹罗猫，审美/前端风格判断，禁码）出地铁图风格稿 + antig-opus 出 3D demo / 烁烁 review → 出实现 spec 再交开发猫。**形态边界**：暗色霓虹 + edge=真实球权传递 + node=thread/cat，点开 node = thread 历史 + 球权时间线。**触发时机**：Phase C close 后或并行立单独 feat（如 F-xxx 立项），不阻塞 Phase C |
