...
| 2026-06-18 | Phase B **B2 PR3（事件源补齐 + CVO explicit handoff + 死球心跳）** branch ready：新增 `ball.held`/`hold_expired`、`task.blocked`/`unblocked`/`done`、`invocation.started`/`heartbeat`/`died`、`ball.handed_cvo` 生产来源；`task.idle_long` 与 `ball.wake_sent` 不伪造，随 PR4 ProbeScheduler/WakeSender 落地。AC-B1 数据层已有 `lastScanAt`，端到端简报点名仍归 PR4。 |
