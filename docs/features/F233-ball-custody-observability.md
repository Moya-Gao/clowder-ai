...
| OQ-8 | **「提包球」新形态评估**（2026-06-19 F188 Phase K 10 天 stale case discovery）：猫正常完成 turn + push commits 到 `fix/feat/*` branch，但**没接入任何 structured ball 通道**（无 PR / 无 task / 无 hold_ball / 无 follow-up @）——invocation 自然结束、CLI 正常退出、git 痕迹完整、所有 thread/message 痕迹**戛然而止**。F233 现役 5 形态（搁置 / 死球 / 睡美人 / 虚空 / 僵尸）共同前提是**结构化球权痕迹存在**——「提包球」**完全没有这些痕迹**，只有 git push event。F188 Phase K 实证 10 天零 detector 触发，纯靠 CVO 心血来潮 backlog 大扫除发现。完整 case study 见 [docs/discussions/2026-06-19-f188-stale-branch-case-study/case-study.md](../discussions/2026-06-19-f188-stale-branch-case-study/case-study.md)。需 Phase C planning 时评估：① 这种形态是否值得提前 prioritize（在 Phase C 主线之外）② 信号源接入方案（cron 扫 git ref / git hook / Phase C 轨迹视图反向覆盖）③ Phase C 轨迹视图数据源是否包含 git ref state ④ 历史 forensic 调查是否有同形态 case 趋势 | 🟡 待 Phase C plan 评估（2026-06-19 F188 case 实证记录） |
...
| 2026-06-19 | **「提包球」新形态 case discovery**（OQ-8 实证记录）：F188 Phase K（PR [#2414](https://github.com/zts212653/cat-cafe/pull/2414) merge commit `1ec99732`）暴露的 stale branch 形态——平行 opus-47 2026-06-09 08:23 UTC 完成 5 commits + push 到 `fix/f188-phase-k-config-health-surface`，10 天零 detector 触发（thread `thread_mov0in6qfn2j2nvg` 最后活动早于最后 commit 49 分钟），2026-06-19 02:27 UTC CVO 心血来潮 backlog 大扫除发现。完整 case study + thread 锚点 + F233 现役 5 detector 漏报矩阵 + 三方案建议（轨迹反向 fixture / 球权事件流加 git source / git hook）见 [docs/discussions/2026-06-19-f188-stale-branch-case-study/case-study.md](../discussions/2026-06-19-f188-stale-branch-case-study/case-study.md)。本 case 进 OQ-8 待 Phase C plan 评估；author + F233 owner（opus-47 / 宪宪）倾向「方案 A 等 Phase C 轨迹自然覆盖 → 数据观察后再升级」，不擅自动 detector。 |
