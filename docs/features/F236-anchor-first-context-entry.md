...
6. **🔗 CROSS-FEATURE 协调 → F245（`eval:friction`）= 近同构姊妹 feature，平行 opus-48 owner（2026-06-18 recall 发现 + 双向协调完成）**：F245 也在 F192 下新增 phenomenon eval domain + rollup aggregator + bump 同一对枚举（OQ-4「2 enum 扩展」= domainId + sourceAdapter）。cross-post F245 thread `thread_mqjnm2ymjj17f1md` 协调，**平行我 ack 如下**：
   ...
   - **✅ Track 拆分 + 排序**：**Track-1（AC-E1 anchor telemetry emit as OTel metrics，不碰 contract）= 实现完成 → PR #2411（in review @gpt52，分支 `feat/f236-track1-anchor-otel-telemetry`）**。新增 `routes/anchor-telemetry.ts` recorder（镜像 callback-auth-telemetry）+ 4 instruments `cat_cafe.anchor.{returned,full_drill}.{count,chars}` + `anchor.tool` allowlist；4 emit 点 additive；raw counts only（ratio 留 rollup）。⚠️ limitation：OTel emit 测法是 in-memory mirror（仓库无 exporter-readout harness），end-to-end OTel 验证留 Track-2 eval-adapter 集成。**Track-2（AC-E4 domain 注册）**等 **shared Y-lite infra PR** 先 land（砚砚建议 F245 Phase C 先 land——更宽 consumer——F236 rebase 继承；F236 先到也可，但须 scope 到 F192 contract、不夹带 anchor-specific）。**两 feature 都禁再提"硬 enum +1/+2"的 feature-local PR**。
   - **🛑 开放设计岔路（2026-06-19 cloud review round-2 暴露 → 升 eval owner 砚砚）**：Track-1 的"独立 per-tool 聚合计数器"**结构上算不出 per-tool open-rate**。云端 2 个 P2 指向同一根：F236 的 open-rate 本质是**跨 endpoint / 跨请求 / per-item 的 drill↔preview 相关性**（① get-message drill 未归因回源 preview tool thread-context/pending-mentions → 那些 tool 永远 0 drill；② returned 按 payload 计、drill 按 item 计，单位错配可 >100%）。**根因 = eval 信号模型未设计就埋点**（退一步换层，不补第 N 个锅）。待砚砚定信号模型：(i) emit per-event 可关联 telemetry（带 correlation key，Track-2 join） / (ii) 穿 source-tool + item-count（碰 drillDown pointer contract，跨切面） / (iii) Track-1 scope 到省(chars)信号、open-rate 整块设计进 Track-2。**PR #2411 暂停 merge，等信号模型裁定后重塑 Track-1 emission**。返回值省(chars)部分本身 OK，但 open-rate 那块不能就这么 ship。
...
## Links
- 收敛纪要: `docs/discussions/2026-06-15-anchor-first-context-entry-meeting-notes.md`
- research: `docs/research/2026-06-15-context-entry-anchor-audit/`
- rtk 对照: `docs/discussions/2026-06-15-rtk-deep-dive/`

## Key Decisions
- KD-1: 新 F 号不 reopen F148（边界不同：消息侧 vs 返回侧）— 宪宪×砚砚共识
- KD-2: 第一刀落 callback route projection helper（最内层封顶）— 砚砚 sharpen
