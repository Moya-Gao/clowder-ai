...
- KD-7: **Bash/WebFetch 不做 anchor（per-tool 策略，2026-06-26 宪宪自决）**——Bash 输出是自由格式文本（无结构化 metadata 可提取 locator），WebFetch 返回整页 HTML/text（无有意义的 "drill" 概念）。anchoring 这些需要 content-aware truncation 违反 ADR-031 locator-not-synopsis 不变量。F236 anchor scope = 有结构化输出 shape 的工具（Read/Grep/Glob/MCP 协作工具），freeform 输出不 anchor
