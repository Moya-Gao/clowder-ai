...
> **Status**: in-progress (Phase A+B done · **Phase A/B-Eval DONE** [Track-1 chars/volume + Track-2 open-rate + AC-E3 sunset trigger] · Phase C = cat-controlled mode（2026-06-24 pivot）：**MCP-tools cat-mode V1 MERGED（PR #2546）** + cc-native Read/Grep/Glob MERGED（#2552，opus-47 VG）+ Phase E MERGED（#2559）+ cleanup batch MERGED（#2565：AC-C4 drill-side ✅ 收口、opus-48 VG）；剩 modeSource `legacy_equivalent` emit-wiring + 小尾巴 backlog（stale Read / per-tool 策略）) | **Owner**: 布偶猫 (宪宪 opus-48 愿景守护；Track-2/AC-E3 实现 opus-4.6) | **Priority**: P1 | **Created**: 2026-06-15 | **Companion ADR**: ADR-203
>
