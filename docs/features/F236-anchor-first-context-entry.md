...
- [~] AC-C4: 双边 eval 对 cc 工具同样适用（Read drill 净收益 = 省 − drill 成本）——**preview side done（PR #2559）, drill side remaining**：`AnchorEvalBridgeConsumer` transforms eval jsonl → `recordAnchorPreviewEvent()`；carrier wires second `TranscriptTailer` for eval jsonl polling + final drain + cleanup；`setupHookInfrastructure()` registers F236 anchor hook with matcher `Read|Grep|Glob`。**Remaining for full AC-C4 closure**: cc drill telemetry（hook emits drill events for bounded Read pass-through + `AnchorDrillTool` type extension for cc tools）
