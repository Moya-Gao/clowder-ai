...
> **Status**: spec | **Owner**: Community (彭潇/bouillipx) + 布偶猫家族 maintainer | **Priority**: P1
...
> 来源：开源社区 clowder-ai issue #941（提案人彭潇/bouillipx）+ 2026-06-16 clowder-code 接入猫咖讨论。CVO 2026-06-17 signoff 立项。
...
> Phase 拆分来自 issue #941 的 consolidated decision packet（社区彭潇 + 我们家砚砚收敛）。
...
- [ ] AC-A10:【治理】core 侧安全实现（transport registry / token / MCP 注入 / sandbox）+ 本 feat 所有 PR 的 merge-gate 由**布偶猫家族 maintainer 守门**，不委托社区/plugin 代码（F129 继承；分工见 KD-3）
...
| # | 问题 | 状态 |
|---|------|------|
| OQ-1 | 最终 feature 号/名/owner | ✅ 本次立项：F241 / Agent Provider Plugin / Community(彭潇)+maintainer，CVO 2026-06-17 signoff |
| OQ-2 | Phase A 是否要求 ACP 立即可用，还是允许 A2A 作 smoke path（等 clowder-code 暴露 ACP server） | ⬜ 待 Design Gate（取决于 clowder-code ACP 能力） |
| OQ-3 | registry 契约定后，`agentProvider` 精确 manifest shape | ⬜ 待 Phase B |
| OQ-4 | 多少 Hub UI 属于 Phase B vs reference runtime phase | ⬜ 待 Design Gate |
| OQ-5 | 与 F143/F161/ADR-023 精确划界——Phase A transport registry 与 F143 hostable runtime 抽象谁先落地/谁拥有 | ⬜ 待 Design Gate + F143 owner（opus-4.6）对齐 |

## Key Decisions

| # | 决策 | 理由 | 日期 |
|---|------|------|------|
| KD-1 | 定位为独立 provider-extension feature，**不当 F202 Phase 3** | F202 是 manifest/config/UI 支撑设施，本 feat 的 ownership 边界是 agent-provider 扩展面；社区彭潇 + 我们家砚砚在 #941 收敛 | 2026-06-17 |
...
| KD-3 | owner = Community(彭潇/bouillipx) + 布偶猫家族 maintainer；彭潇 own 提案+reference，**core 安全 + merge-gate maintainer 守** | 沿用 F150/F202/F205 社区核心主导 + maintainer 把关模式；社区账号够不到私有仓 core | CVO 2026-06-17 signoff |
...
| 日期 | 事件 |
|------|------|
| 2026-06-17 | 立项（源自社区 issue clowder-ai#941，提案人彭潇；CVO signoff；宪宪/Opus-4.8 建档） |

## Review Gate

- Phase A: 架构级 + 最高危安全边界 → 跨族 review（缅因猫家族）+ 愿景守护；core 安全实现禁 self-merge（AC-A10）
...
| 类型 | 路径 | 说明 |
|------|------|------|
| **Community Issue** | [clowder-ai#941](https://github.com/zts212653/clowder-ai/issues/941) | 源提案 + consolidated decision packet（彭潇/砚砚收敛） |
| **Feature** | `docs/features/F032-agent-plugin-architecture.md` | Evolved from——CatId 身份松绑（done） |
| **Feature** | `docs/features/F143-hostable-agent-runtime.md` | Blocked by——hostable runtime 契约 + transport registry lineage |
| **Feature** | `docs/features/F161-acp-carrier-generalization.md` | Related——ACP 载体泛化（ACP transport 依赖） |
| **Feature** | `docs/features/F202-plugin-framework.md` | Related——`agentProvider` 资源挂其上 |
| **Feature** | `docs/features/F050-a2a-external-agent-onboarding.md` | Related——a2a 外部 agent 契约（done） |
| **Feature** | `docs/features/F211-cross-runtime-session-transparency.md` | Related——session 可见性（done，runtime enum 待扩） |
| **Feature** | `docs/features/F159-catagent-native-provider.md` | Related——provider intake 教训（须避坑） |
| **Feature** | `docs/features/F129-pack-system-multi-agent-mod.md` | Inherits constraint——no same-power plugin |
