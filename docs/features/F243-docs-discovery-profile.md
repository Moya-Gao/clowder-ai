...
**砚砚钉死的一句话（vision 锚）**：不是为了符合 OKF，也不是为了自动生成 description，**而是让 `docs/features` 从平铺文件堆变成可渐进探索的知识入口**。

铲屎官 2026-06-15 启动："来吧你来综合一下三只喵喵的想法的"（OKF 学习路径）→ "这个 description 要如何保证不漂移？" → "小心 这可能会变成我们提防的 小猫代替大猫做决策" → 2026-06-17 CVO signoff："a 吧先 feat 立项 然后！ 然后砚砚喵回来了！你可以喊他讨论了"。

**真实痛点（实证 ≠ 感觉乱）**：
- `docs/features/` 有 200+ 个 F 号文档，**无入口索引**。任何猫初次进入要么 `ls` 200 行眼花、要么 `grep -i` 多轮碰运气、要么 `search_evidence` 但 snippet 不一定够判断
- **凭记忆引路必错**：本 brainstorm thread 内孟加拉猫 Opus 引用 F186 时把文件名记成 `F186-library-stewardship.md`，实际是 `F186-library-memory-architecture.md`（`F188` 才是 `library-stewardship`）。**两只猫凭记忆引路都会错认文档名**
- **F242 立项时（5 小时前）我自己也要靠 `grep -E "^\| F[0-9]+" docs/BACKLOG.md | tail`** 才知道最大 F 号——`BACKLOG.md` 是任务跟踪不是知识地图
...
四阶段实施（砚砚 co-design 4-Phase 框架）。**description generation 形态在 Phase A 判定后才进入 B/C 固化**——不预设小模型/大猫/模板。
...
- 三猫盲评（@codex / @antig-opus / 宪宪），盲评协议在独立 spike thread 执行避免互看
...
- **Non-goal**：`> Summary:` blockquote 正文镜像 v1 不进（砚砚 guardrail，多一个漂移面）

**子能力 3 — Eval rubric**：
- Baseline 定义：`BACKLOG.md + rg/search_evidence + ls docs/features/`
- Friction metric（多元，砚砚 KD）：找正确 feature 的 tool calls / 时间 / 误点率 / 漏判率 / description-in-index 让读者愿意点开转化率
...
- 数据驱动 Go/No-Go：是否扩到 `docs/decisions/` 和 `docs/research/`
...
- [ ] AC-D3: 扩展 Go/No-Go decision packet：是否扩到 `docs/decisions/` 和 `docs/research/`，有数据支撑（含子能力的 generalize 测试）

## 需求点 Checklist

| ID | 需求点（铲屎官原话/转述）| AC 编号 | 验证方式 | 状态 |
|----|---|---|---|---|
| R1 | "docs/features 从平铺文件堆变成可渐进探索的知识入口"（砚砚钉句）| AC-D1 | test: 冷启动 tool calls/时间 vs baseline ≥30% 下降 | [ ] |
| R2 | description 防漂移："这个 description 要如何保证不漂移？"（铲屎官 06-15）| AC-B1, AC-B3, AC-C3 | test: profile lint + sync gate + 触发节流 | [ ] |
| R3 | 防"小猫代偿决策"："小心 这可能会变成我们提防的 小猫代替大猫做决策"（铲屎官 06-15）| AC-A4, KD-4 | test: PR-time 大猫 confirm gate；抽查不可代替 gate | [ ] |
...
- Activation = 进入 docs/features/ 的第一个动作（`cat docs/features/index.md` / `ls` / `search_evidence` / 查 BACKLOG.md）
- Secondary（不在 primary scope）：`search_evidence` 命中后看 snippet 渲染（那是 F236 surface）

**Friction Metric（多元，砚砚 KD）**：
...
| # | 问题 | 状态 |
|---|------|------|
| OQ-1 | Phase A 若选"小模型生产"路径，烁烁 quota 长期可持续吗？需要 cost telemetry | ⬜ 未定（Phase A 输出）|
| OQ-2 | index.md 分组维度：按 status（spec/in-progress/done/parked）还是按 topic？或两个 view？| ⬜ 未定（Phase A 框架时定）|
| OQ-3 | description i18n 支持？目前文档中文为主+英文术语混合，OKF SPEC 用英文，互操作时需考虑 | ⬜ 未定（Phase D 扩展时定）|
| OQ-4 | Phase C 回填 200+ 文档 description 优先顺序？已假定 spec/in-progress 优先 → done 批量，是否需 CVO 调优 | ⬜ 未定（Phase B/C 过渡时定）|
| OQ-5 | F236 数据契约：F243 generated index 是否需要也走 F236 anchor-first preview（index 文件可能也大）？| ⬜ 未定（Phase B 时与 F236 sync 决定）|

## Key Decisions

| # | 决策 | 理由 | 日期 |
|---|------|------|------|
| KD-1 | F 号主名 `Docs Discovery Profile`，OKF 仅在 H1 副标（不入 BACKLOG 主名）| OKF 是 lineage 不是依赖/目标，避免外部规格绑架 feature 中心（砚砚 sharpen）| 2026-06-17 |
| KD-2 | 内核保留：`search_evidence` / `graph_resolve` / `list_recent` / 消费加权 ranking (F200) 不动；只对外补 OKF-compliant export profile | OKF 是 lingua franca 不是 OS（宪宪/砚砚/孟加拉猫 opus 三猫共识 2026-06-15）| 2026-06-17 |
| KD-3 | description 走"小模型生产 + 大猫 confirm + 强 prompt 规则"模式，**但 Phase A 验证后才固化** | mini-spike R1/R2/R3 三轮证明 prompt v3 9 条能矫正烁烁默认偏差（formal pass），但 1 sample (F186 easy mode) 不能 generalize → stratified sample 验证是 Phase A 前置 | 2026-06-17 |
| KD-4 | "抽查"不能代 gate，**PR-time 强制 confirm 才是 gate**；抽查只在 eval 层做 | feedback_intake_visual_parity_required / feedback_reviewer_no_middle_state：质量门禁不能"部分放过"（铲屎官 P0）| 2026-06-17 |
| KD-5 | `index.md` 是 **checked-in generated artifact**，永不手写 + CI sync gate 守门 | 手写 index = 第二个 BACKLOG.md 漂移源（砚砚 sharpen）| 2026-06-17 |
| KD-6 | 评估单元是 **description-in-context**，不是 description 单看 | "好"description 涉及与 H1/status/相邻文档对比的 contextual readability（砚砚 sharpen）| 2026-06-17 |
| KD-7 | Primary user = **冷启动探索**（不知道 F 号），不是 search_evidence post-snippet | 后者是 F236 surface；冷启动是真正的 discovery 痛点（砚砚 sharpen）| 2026-06-17 |
| KD-8 | `> Summary:` blockquote 正文镜像 v1 **不进** | 多一个漂移面；除非 Phase A 证明"frontmatter 藏起来导致作者不维护"才作为 Phase B 备选（砚砚 guardrail）| 2026-06-17 |
| KD-9 | Sunset signal **必含"变瞎子"**（误点率/漏判率），不只测 token | 变瞎子比 anchor tax 更隐蔽，token 账看不到；学 F236 双边 sunset 设计 | 2026-06-17 |
| KD-10 | Phase A stratified sample = **6 硬骨头 + 4 easy mode** | F186-类 easy mode 不能 generalize；6:4 比例（砚砚 sharpen）| 2026-06-17 |

## Timeline

| 日期 | 事件 |
|------|------|
| 2026-06-15 | OKF 一手核实 + 三猫综合（宪宪 / 砚砚 / 孟加拉猫 opus 4 轮 brainstorm）|
| 2026-06-15 | description 漂移讨论 + "小猫代偿决策"反模式识别（铲屎官原话）|
...
| **External** | https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f | Karpathy LLM-wiki gist（OKF 灵感源）|
