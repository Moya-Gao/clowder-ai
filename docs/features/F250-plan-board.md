...
铲屎官发现猫猫祟祟面板在多猫场景下存在对接缺口：
1. 金渐层（opencode）完全没有对接猫猫祟祟
2. 缅因猫（Codex）做完任务后不打勾、进行中没有动画效果
3. 暹罗猫（Gemini）不确定是否对接
...
```
CLI 工具调用 → provider event-transform → invoke-single-cat.ts → 前端 useAgentMessages → PlanBoardPanel
```

task_progress 有**两条入口**：
1. **通用路径**（invoke-single-cat.ts L1138-1148）：拦截 `tool_use` 事件，若 `toolName ∈ TASK_TOOL_NAMES`（= `TodoWrite` | `write_todos`）则 `extractTaskProgress()` 从 `toolInput.todos` 提取
2. **Codex 专属路径**（codex-event-transform.ts L45-79）：直接识别 `todo_list` 类型的 item 事件，转为 `system_info(task_progress)`

#### 各猫对接状态

| 猫猫 | Provider | task_progress 对接 | 入口路径 | 问题分析 |
|------|----------|-------------------|---------|---------|
| 布偶猫 (Claude) | anthropic | ✅ 完整工作 | 通用路径：TodoWrite tool_use → extractTaskProgress | 正常 |
| 缅因猫 (Codex) | openai | ⚠️ 有双路径但可能状态映射问题 | ① codex-event-transform 原生 todo_list ② 通用路径 extractTaskProgress | Codex todo_list 事件的 item status 值可能不是标准三值（`pending`/`in_progress`/`completed`），导致前端 TaskStatusIcon fallback 到灰色方块 |
| 金渐层 (opencode) | opencode | ❌ 未对接 | opencode-event-transform.ts 无 todo 处理；通用路径依赖 toolName 匹配 | opencode CLI 的 tool_use 事件中 `part.tool` 值需确认是否包含 `TodoWrite` |
| 暹罗猫 (Gemini) | google | ⚠️ 仅通用路径 | gemini-event-parser.ts 无原生处理 | Gemini CLI `tool_use` 中 `tool_name` → 通用 extractTaskProgress 可匹配，取决于工具名 |
| 狸花猫 (Dare) | dare | ❌ 未验证 | 待确认 | — |

#### 前端渲染逻辑（确认无问题）

- `PlanBoardPanel.tsx` 的 `TaskStatusIcon` 正确实现了三种状态：
  - `completed` → 绿色勾 ✓
  - `in_progress` → 蓝色旋转 `animate-spin`
  - `pending`（default）→ 灰色方块
- `PlanCard` 的 dot 在 `status === 'running'` 时有 `animate-pulse` 脉冲动画
- `snapshotStatus` 由 `finalizeTaskProgress()`（invoke-single-cat.ts L382-418）在 invocation 结束时设为 `completed`/`interrupted`

#### 根因

1. **金渐层缺失**：`opencode-event-transform.ts` 只处理 `step_start`/`text`/`tool_use`/`error`/`step_finish` 五种事件，**没有 todo/task 相关逻辑**。通用路径的 `TASK_TOOL_NAMES = Set(['TodoWrite', 'write_todos'])` 需要 opencode CLI 发出完全匹配的工具名才能触发。
2. **缅因猫状态映射**：`codex-event-transform.ts` L57-66 的 status 映射逻辑 (`t.status ?? t.completed ? 'completed' : 'pending'`) 可能与前端期望的标准三值不匹配。特别是 `in_progress` 状态可能在 Codex 原生事件中不存在。
3. **暹罗猫不确定**：Gemini CLI 是否使用 `TodoWrite` 工具名未确认。

### 修复计划

1. 确认各 CLI 实际发出的 `tool_use` 事件中的工具名
2. 必要时扩展 `TASK_TOOL_NAMES` 或在各 provider 的 event-transform 中添加原生 todo 事件识别
3. 统一 Codex todo_list 的 status 值映射到标准三值（`pending`/`in_progress`/`completed`）
4. 为 opencode event-transform 添加 todo 事件支持（如果 opencode CLI 有自己的 todo 事件格式）
