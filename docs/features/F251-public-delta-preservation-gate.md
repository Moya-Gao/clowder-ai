...
| 2026-06-27 | **Task 4c override-with-reason merged via PR #2607 (squash `f5ad3280e`); AC-A4 NOW LIVE.** Per-path `--override <path>:<reason>` (CLI + bash, both `=` and split forms) flips classifier BLOCK → `override-pass` with reason recorded in `report.items[].overrideReason`. > 3 overrides per sync triggers `cvoApprovalRequired=true` exit 1 unless `--cvo-approved-public-delta-overwrite` is set (report still records the alarm for audit). Review chain: 砚砚 R0/R3/R4 cross-review + opus48 R0/R3 ship-ready audit (caught §16e signal: same-class option-looking-value exploit via two forms → extracted `reject_option_looking_override_value()` helper as single source of truth) + cloud R0–R3 (R0 P1 split-form parsing / R1 P1 split-form option-looking exploit / R2 P2 equals-form sibling exploit / R3 clean). Tests 63 → 70. **Phase A AC-A1/A2/A3/A4/A5 LIVE; only AC-A6 (30-day retroactive eval, gated by calendar) still DEFERRED.** |
