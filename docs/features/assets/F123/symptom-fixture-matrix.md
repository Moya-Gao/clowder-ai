...
| rich block 落错 bubble / CLI Output duplicate bubble | R1, R5 | `packages/web/src/hooks/__tests__/useAgentMessages-richblock-correlation.test.ts` | covered | rich_block 无 `messageId` 时必须附着到 formal callback；即使先创建了无 `invocationId` 的 streaming placeholder，callback text 到达时也必须就地回收，而不是裂成第二个 assistant bubble |
