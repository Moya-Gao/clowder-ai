...
| 13 | `cat_cafe_update_workflow` | L0 §8, workflow/SOP docs | Existing MCP `cat_cafe_update_workflow` | Mission Hub workflow board readback | Workflow/eval handoff signals | workflow/SOP + `harness-eval` (renders via `hub-action-surface`) | Keep; improve trigger and resumeCapsule examples. |
