...
- waving R1-alpha: pending peer QA. Generated after reloading the canonical base and `waving` layout guide immediately before the roll; chroma-key removed with `remove_chroma_key.py --soft-matte --despill --edge-contract 1`; `extract_strip_frames.py --method auto` extracted `waving` via `components`; row has 4 frames, `edge_pixels=0`, and `chroma_adjacent_pixels=0`.
