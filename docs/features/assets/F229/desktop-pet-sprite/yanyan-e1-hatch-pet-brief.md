...
```text
看附件的猫猫参考图。按这只猫的样子，画一条 idle 动画行条。

角色固定：银灰缅因猫、额头呆毛、大尖耳（深边粉内）、白胸白嘴、灰虎斑花纹、蓬松条纹尾巴、认真严肃表情。6 帧必须是同一只猫，不许变脸、变大小、变花纹。

画布：一张宽横图，约 3000×540。分成 6 个隐形格子（每格约 500×540），每格中间放一只完整猫（约 360×450），格子之间留足空白，猫和格子边缘至少 60px 距离。不要挤在一起。

背景：真透明 PNG（有 alpha 通道）。如果做不到，用单一纯色背景。绝对不要画棋盘格。

这是安静的 idle 循环，猫坐着微微动。不要挥手、走路、跑、跳、工作、大动作。

6 帧分镜（每帧必须不一样，禁止复制粘贴）：
第1帧：睁眼，安静坐姿，尾巴自然卷在身边。这是标准姿势。
第2帧：睁眼，身体整体微微上移1-2像素（吸气），尾巴尖轻轻抬起。不要挺胸。
第3帧：半眨眼，身体在最高点，尾巴尖微微内卷。
第4帧：闭眼眨眼，头微微低一点，尾巴尖保持内卷。眨眼顶峰。
第5帧：睁眼，身体回落（呼气），尾巴尖放松下垂。
第6帧：睁眼，接近第1帧但不完全相同，头微微不同，尾巴回到基线附近。能自然接回第1帧循环。

硬性要求：
- 6 帧猫的大小必须一致，脚底基线必须一致
- 只有整体微移、眼皮、尾巴尖在变，其他部分不动
- 尾巴根部和整体形状不变，只有尖端在动
- 不许出现 4 帧以上看起来一样的情况
- 不许变表情（除了眨眼）、不许挺胸、不许换姿势

禁止：文字、UI、场景、阴影、发光、星星、水滴、速度线、灰尘、动效线、漂浮符号、guide 标记。
