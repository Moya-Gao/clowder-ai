...
- R5-alpha scale superseded on 2026-06-17 after full-state preview: identity was acceptable in isolation, but the row was visually too large against the later 8 rows. Metrics from `/tmp/cat-cafe-evidence/f229-yanyan-pet-run/preview-check` showed idle averaging about `35.9k` nontransparent pixels per frame while the later non-jumping rows were mostly `18k-22k`, causing a visible size pop from idle into other states.
- R6-scale: peer identity, motion, and scale QA passed by Sonnet on 2026-06-17. Regenerated only `idle` after the scale issue, anchored to the later accepted rows rather than old idle. Source was cleaned with `remove_chroma_key.py --auto-key border --soft-matte --despill --edge-contract 1`; `extract_strip_frames.py --method auto` extracted idle via `components`; full 9-row preview with R6 returned `inspect_frames.py ok=true` and `validate_atlas.py ok=true`, atlas size `1536 x 1872`, transparent RGB residue `0`. R6 idle averages about `18.0k` nontransparent pixels per frame with bbox width `132..150`, matching the later rows' footprint much better. Sonnet noted two integration/watch items: R6 has less white chest fur than R5 as a style-unification side effect, and F4-F5 have a slightly larger head-angle change than a pure breathing idle; both are acceptable for QA but should remain visible for CVO taste review.
- running-right R1-alpha: peer identity and motion QA passed by Sonnet on 2026-06-17. Generated after reloading the canonical base and `running-right` layout guide immediately before the roll; chroma-key removed with `remove_chroma_key.py --soft-matte --despill --edge-contract 1`; `extract_strip_frames.py --method auto` extracted `running-right` via `components`; row has 8 frames, `edge_pixels=0`, and `chroma_adjacent_pixels=0`.
- running-left R1-mirror: peer identity and motion QA passed by Sonnet on 2026-06-17. Derived from the approved `running-right` R1-alpha frames with `derive_running_left_from_running_right.py` after normalizing the rightward components into `8 x 192` frame slots; `extract_strip_frames.py --method auto` extracted `running-left` via `components`; row has 8 frames, `edge_pixels=0`, and `chroma_adjacent_pixels=0`.
- waving R1-alpha: peer identity and gesture QA passed by Sonnet on 2026-06-17. Generated after reloading the canonical base and `waving` layout guide immediately before the roll; chroma-key removed with `remove_chroma_key.py --soft-matte --despill --edge-contract 1`; `extract_strip_frames.py --method auto` extracted `waving` via `components`; row has 4 frames, `edge_pixels=0`, and `chroma_adjacent_pixels=0`.
- jumping R1-alpha: rejected by Sonnet on 2026-06-17. Identity passed, but the motion looked like seated forepaw lift / backward lean rather than a real jump; it had no clear all-limbs-off-ground airborne frame.
- jumping R2-alpha: peer identity and motion QA passed by Sonnet on 2026-06-17. Generated after reloading the canonical base and `jumping` layout guide immediately before the roll; source was cleaned with `remove_chroma_key.py --soft-matte --despill --edge-contract 1`; `extract_strip_frames.py --method stable-slots` preserved the vertical jump arc and `inspect_frames.py --allow-stable-slots` accepted it; row has 5 frames, `edge_pixels=0`, and `chroma_adjacent_pixels=0`.
- failed R1-alpha: rejected before peer QA on 2026-06-17. The frame count and failure pose direction were promising, but frame 8 touched the outer canvas edge and was cropped; the chroma background also had a stronger gradient than acceptable for production.
- failed R2-alpha: peer identity and motion QA passed by Sonnet on 2026-06-17. Generated after reloading the canonical base and `failed` layout guide immediately before the roll; source was cleaned with `remove_chroma_key.py --auto-key border --soft-matte --despill --edge-contract 1`; `extract_strip_frames.py --method auto` extracted `failed` via `components`; row has 8 frames, `edge_pixels=0`, and `chroma_adjacent_pixels=0`.
- waiting R1-alpha: peer identity and motion QA passed by Sonnet on 2026-06-17. Generated after reloading the canonical base and `waiting` layout guide immediately before the roll; source was cleaned with `remove_chroma_key.py --auto-key border --soft-matte --despill --edge-contract 1`; `extract_strip_frames.py --method auto` extracted `waiting` via `components`; row has 6 frames, `edge_pixels=0`, and `chroma_adjacent_pixels=0`. Sonnet noted F4 is slightly close to `failed`, but the loop still reads as expectant waiting because F2-F3 clearly lift the front paw into an asking/input pose and no question marks, bubbles, UI, props, or floating symbols appear.
- running R1-alpha: peer identity and motion QA passed by Sonnet on 2026-06-17. Generated after reloading the canonical base and `running` layout guide immediately before the roll; source was cleaned with `remove_chroma_key.py --auto-key border --soft-matte --despill --edge-contract 1`; `extract_strip_frames.py --method auto` extracted non-directional `running` via `components`; row has 6 frames, `edge_pixels=0`, and `chroma_adjacent_pixels=0`. This candidate intentionally interprets `running` as active task processing rather than locomotion: F2-F4 show front-paw work and head lean with no directional travel, speed lines, dust, or running gait.
- review R1-alpha: peer identity and motion QA passed by Sonnet on 2026-06-17. Generated after reloading the canonical base and `review` layout guide immediately before the roll; source was cleaned with `remove_chroma_key.py --auto-key border --soft-matte --despill --edge-contract 1`; `extract_strip_frames.py --method auto` extracted `review` via `components`; row has 6 frames, `edge_pixels=0`, and `chroma_adjacent_pixels=0`. Sonnet noted F3 has slight paw-position overlap with `waiting`, but the loop reads as focused review because the posture is more inward and evaluative rather than an outward asking pose.
- Full-state preview checkpoint on 2026-06-17: before producing or integrating more rows, composed the 9 approved rows into a temporary preview atlas at `/tmp/cat-cafe-evidence/f229-yanyan-pet-run/preview-check/final/spritesheet.webp`; `inspect_frames.py` and `validate_atlas.py` both returned `ok=true`, atlas size is `1536 x 1872`, and transparent RGB residue is `0`. Visual spot check: identity is consistent; `jumping` preserves a visible airborne arc; `waiting` and `review` remain the closest semantic pair and should be differentiated by state context.
