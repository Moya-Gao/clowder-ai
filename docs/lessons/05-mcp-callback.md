...
**2026-02-06**，铲屎官第一次在前端同时召唤三只猫。结果暴露了一堆问题，其中一个是 P1 级别的 `[事实: docs/discussions/2026-02-06-first-demo-findings.md]`：

> **P1 — MCP 挂载不完整**
>
> 现象：只有 Claude CLI 通过 `--mcp-config` 动态注入了 MCP，Codex 和 Gemini 没有。

这里需要解释一下背景。三只猫的 CLI 工具都支持 MCP——这不是问题。问题是 **MCP 配置有三种挂法**，而且只有一种适合我们 `[事实: docs/discussions/2026-02-06-identity-injection-open-invite.md]`：
