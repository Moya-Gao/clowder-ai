...
**前端展示**（跨版本布偶猫圆桌更新）：
- Phase 3.5: 时间线交织 + ideate 模式时显示 "独立观点采样中 · 三只猫各自思考" UX header
- Phase 4: 暹罗猫探索翻牌式揭示（先缓冲完整回复，同时揭示）
...
### 4-E：Deliberate 两轮制（来自跨版本布偶猫圆桌）

- `#deliberate` 模式：Round 1 并行独立 → 同时揭示 → Round 2 看了别人观点后各自修正
- 解决"独立性 vs 增量性"的张力：Round 1 保独立，Round 2 补增量
- 代价：2x token 消耗和延迟，需要缓冲完整回复
- 前端配合翻牌式 UX（3.5-D 的进阶版）

### 4-F：多版本猫召唤（来自跨版本布偶猫圆桌）

- CatBreed + CatVariant 两层结构：品种(布偶猫) → 个体(团子4.5/年糕4.6/棉花糖Sonnet)
- `@布偶猫` 召唤品种默认个体，`@团子` 召唤指定个体
- 召唤 UX：角色+展开子菜单（方案 C）+ 场景预设（方案 D）
- 影响面：召唤解析、AgentRouter、前端头像、消息铭牌全需改造
- **Phase 3.5 准备**：CatConfig 外置时就用 Breed/Variant schema，但只实现单默认个体

### 4-G：协作元反思（来自跨版本布偶猫圆桌）

- 系统能反思"协作过程本身"能改进什么，而不只是"讨论内容的总结"
- 区别：纪要 = "我们得出了什么结论"，reflect = "我们的协作模式有什么问题"
- 实现方向：定期自动生成协作质量报告（谁的声音被淹没？讨论效率？）
- 这是猫咖真正进化的来源
...
|---|---|---|
| 布偶猫 | Task 后端 + Intent 路由 + 并行改造 | Bridge 后端 + 依赖编排 + deliberate 两轮制 |
| 缅因猫 | Code review + 安全审计 + 测试 | 成本治理 + 审计日志 + 协作元反思数据 |
| 暹罗猫 | Task UI 视觉 + 纪要卡片 + 并行展示 | 氛围系统 + 翻牌式 UX + 多版本猫头像 |
| 铲屎官 | 产品决策 + 验收 + 踹猫出思维定势 | 同左 + 多版本猫召唤 UX 决策 |
