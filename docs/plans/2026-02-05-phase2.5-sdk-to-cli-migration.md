...
> 状态说明：Antigravity 路径已定；Claude/Codex 已完成参数存在性核对，仍需实机 smoke。

**Claude CLI**:
- [x] 验证 `--output-format stream-json` 选项存在
- [x] 验证 `--permission-mode` 选项存在
- [x] 验证 `--resume` 选项存在
- [ ] 实机验证 `--output-format stream-json` 的事件结构
- [ ] 实机验证 `--resume` 恢复行为
...
- [ ] 实机验证 `codex exec --json` 事件结构
- [ ] 实机验证 `codex exec resume [SESSION_ID] [PROMPT]` 恢复行为
- [ ] 验证 `--full-auto` 是否满足服务端自动化要求
...
| 🔄 进行中 | Task 1: CLI 研究（Antigravity 已定，Claude/Codex 待实机 smoke） | 进行中 |
