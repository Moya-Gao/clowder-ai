...
4. exposes the run process to CVO with plan id, steps, hit counts, opened anchors, and degraded / effectiveMode.
...
The Perspective returns a compact route trace: which searches ran, which anchors were opened, which docs or messages looked relevant, and which surfaces degraded. The new cat can then inspect the same anchors directly and form its own judgment.
...
CVO sees the run process in the same spirit as `search_evidence` visibility: plan id, step labels, queries, hit counts, opened anchors, and degraded / effectiveMode. CVO is not operating a search UI; CVO is watching the cat's evidence trail.
...
- opened anchors per reader step
...
| AC-D6 | Visibility contract requires plan id, step, hit count, opened anchors, degraded / effectiveMode. |
