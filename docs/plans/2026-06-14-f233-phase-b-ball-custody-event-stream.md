...
- **PR3（done #2364）— 事件源补齐 + CVO explicit handoff + 死球心跳**：
  ...
  - **callback-routing state contract（done #2378）**：PR3/#2374 后续发现 callback guard / source worklist / `mentionsUser` / local CVO / target CVO 五个消费者曾共享隐式状态，已以 `classifyCallbackContentRoutingState` + 8-cell consumer matrix 封板（merge commit `81a7fa6b5`），不再按边角 patch 继续 re-review。
