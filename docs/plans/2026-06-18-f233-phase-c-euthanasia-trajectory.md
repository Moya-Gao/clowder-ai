...
- **git-ref-snapshot source contract**（OQ-8 锁定 + 砚砚 P2-1 修正补 feat/thread join 字段 + 砚砚 step 3.6/4 护栏补真实 PR timestamps + entry.at per-kind contract）：
  ...
  - **join 字段 provenance**：`{ confidence: 'high'|'medium'|'low', joinedVia: ('feat_index'|'thread_keyword'|'commit_message_F#'|'branch_name_F#')[] }`——`featureCandidates` 因 branch 命名 `fix/f188-*` / commit message `F188:` / feat_index 关联等启发式可能多候选，confidence 反映 join 强度。**Cloud round 2 P2 fix (PR #2439)**：当前 collector 只实现了 `branch_name_F#` + `commit_message_F#` + `feat_index` 三种 discovery；`thread_keyword` 作为 discovery 需要新的 `ThreadSearch.findByBranchKeyword(branchName)` IO，超出 C2a scope（step 5+ 或 follow-up F#）。当前 thread search 用于 post-discovery 关联（拿 last activity 时间戳给 F188 invariant 用），不计入 joinedVia 的 discovery 来源
