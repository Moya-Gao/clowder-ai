...
Topic 1 (25-30 min) — 元认知鸿沟：工具调用 → 好坏直觉 → Build to Delete
  ↓
Topic 2 (20-25 min) — 协同与制衡：Agent×Agent → Human×Agent (HITL vs HOTL)
  ↓
☕ 茶歇 (10-15 min)
  ↓
...
- **会被吞掉的**：工具调用的 prompt 路标（模型原生支持后不再需要）、格式转换层（模型理解更好后自动处理）
- **不会被吞掉的**：可观测基础设施、治理规则、知识生命周期管理、跨厂商协调层
- **判断标准**：补偿模型认知缺陷 → 技术债，终将被吞；解决模型无法解决的系统问题（多 agent 协调、knowledge governance、审计追踪）→ 持久价值
- **何时剥离**：模型升级后，harness 某层 bypass 率 >80% → 说明模型已经吞掉了这层 → 安排剥离

**🐱 Topic 1 总结（猫猫录音）**

> **Harness 工作本来就是 Build to Delete。**
>
> 好的 harness 工程师应该希望自己写的东西有一天被模型吞掉——那意味着模型真的变强了。但"认知路径塑造"会被吞掉，"系统级治理"不会——后者不是在补偿模型弱点，是在解决模型根本不该解决的问题。

---

## Topic 2：协同与制衡：Multi-Agent 架构下的协同模式与人机指挥边界（HITL vs HOTL）

> Multi-Agent 的核心问题不是"用哪种模式"，而是"Agent 之间怎么协作"和"人站在哪里"。
...
### 2.2 人与 Agent 关系：Human-in-the-Loop (HITL) vs Human-over-the-Loop (HOTL)

**🎙️ Host Hook（郭良）**

> 两个概念：
> - **HITL**：人在流程**里**，每一步都参与
> - **HOTL**：人在流程**上方**，设方向、监督，不阻断每一步
>
> 在你们的实践里，人到底站在哪个位置？
...
- 不审批 ≠ 不知道——**可观测性让 HOTL 成为可能**

**吴浪**

社区贡献者视角：
- 作为 fork 仓 CVO，自己也在实践 HITL → HOTL 的转变
- 刚开始每步都看、每个 PR 都自己审 → 效率瓶颈
- 后来只关注方向和愿景，细节让猫自治 → 产出质量反而上去
- 关键感悟：**信任是渐进建立的**——从 HITL 到 HOTL 不是一步切换，是随着对猫的了解逐渐放手
...
```
愿景 / 战略方向  ← 铲屎官深度参与（HITL）
  ↓
架构 / 技术选型  ← 铲屎官把关 + 猫提案（过渡区）
  ↓
实现 / 代码细节  ← 猫自治 + 跨猫 Review（HOTL）
  ↓
格式 / 命名 / 琐碎  ← 完全自治
```

根本原因：Agent 没有真实后果。Reward signal 必须最终锚在承担后果的人身上。
