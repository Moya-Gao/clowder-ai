...
- **四件套还不够 — interaction semantics 是第五维度**（D2b-2 rev2 → rev3 三次否决教训）：rev2 解决了 placement，但 badge 当 "alert/CTA" 设计 — 数据驱动不能消除、click 强制 deep-link 抢用户原意图、数字撑爆 hub icon。被铲屎官 alpha 验收 #3 否决"红点点开也点不掉！很难受！点开直接进入了可观测性！万一我想看的是原本的成员呢？" + 截图证据。**信号设计的第 5 维度 = interaction semantics**：把 badge 当 "alert/CTA"（强制跳转、不可消除）还是 "unread notification"（用户决定何时看、看了就清）决定所有交互预期。GitHub bell / Slack unread / iOS app badge 是行业标准 unread notification 范式 — 偏离需明确论证。最终 rev3：lastViewedAt 后端追踪 + viewedUpTo body 防 30s poll 间隙 loss + monotonic cutoff + safe-side >= same-ms + effectiveCutoff for optimistic + reconcile-from-snapshot + watermark stale-poll guard，6 P2 + 1 P1 共 8 轮 cloud Codex review 全修，把 unread notification 范式工程上做到位。
...
- `cat-cafe-skills/refs/in-context-observability-checklist.md` — 加新必检项：「信号设计 = affordance × placement × legend × **scarcity-of-realestate** × **interaction-semantics**，五件套缺一不可。affordance 自检：去掉所有外部解释（spec/legend/tooltip），用户能从信号本身 parse 出含义吗？scarcity 自检：top-bar 是稀缺位，新增 icon 前先问'能不能 merge 进现有 entity 的 badge slot'（GitHub/iOS 通知范式）。interaction-semantics 自检：badge 是 'alert/CTA'（强制跳转、不可消除）还是 'unread notification'（用户决定何时看、看了就清）？默认走 unread notification 范式（GitHub bell / Slack / iOS app badge 行业标准），偏离需明确论证。」
