...
| **优化器 (optimizer)** | "拿到梯度后，具体怎么调权重"的算法。梯度只说方向，优化器决定步子怎么迈 |
| **Adam / AdamW** | 最常用的优化器（**不是参数，是调参数的策略**）。聪明在给每个参数记两本账：① **动量**——最近一直往哪走，保持点惯性别乱晃；② **波动大小**——梯度抖得狠就走小步、稳就走大步（自动调步长）。这两本账是训练一路攒的"肌肉记忆"，所以断点续训得存它，丢了要重新热身。AdamW = Adam + 防过拟合小改进 |
