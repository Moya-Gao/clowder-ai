...
传统的 multi-agent 系统用「orchestrator」来调度：一个中心节点告诉每个 agent 什么时候发言、发言完去哪。
...
这个规则看起来很简单，但它解决了一个关键问题：**谁来决定下一步该谁动？**

答案是：当前工作的猫自己决定。它做完了，它 @ 下一只；它被阻塞了，它 @ 铲屎官。

没有中心调度器 = 没有单点瓶颈。

---

## 上下文隔离：悄悄话 + Shared Rules
...
**它们是硬约束，不是建议。** 猫猫不能选择性遵守。

### 悄悄话（软隔离）

但有些信息是「只给某只猫看」的。

比如铲屎官想单独和砚砚讨论一个敏感问题，不想让宪宪看到。或者宪宪想先私下问烁烁一个设计问题，再决定要不要在公开频道提出来。

这就是「悄悄话」机制：消息可以标记为 `private`，只有指定的猫能看到。

**软隔离让协作有层次：不是所有信息都要广播，但关键决策必须公开。**
...
每次猫猫启动一个新 session，都不是从零开始。上一次 session 的摘要会被注入到新 session 的上下文里。

这解决了一个关键问题：**agent 的上下文窗口是有限的，但项目的历史是无限的。**

Session Chain 让猫猫能「记住」之前发生过什么，而不需要每次都重新读一遍所有文件。
...
Hook 是一种自动触发的检查机制。比如：

- 猫猫每次 commit 前，自动跑 lint 和 test
- 猫猫每次要操作 Redis 6399 时，自动拦截并警告
- 铲屎官长时间没休息时，自动触发健康提醒

**Hook 不是猫猫自己执行的，是系统自动执行的。** 这意味着即使猫猫「忘了」某条规则，系统也会帮它记住。

软约束（Skill、Shared Rules）靠猫猫自觉遵守；硬约束（Hook）靠系统自动执行。两者结合，才能保证行为一致性。
...
2. **A2A 协议**：猫猫自己决定传球给谁，不需要中心调度
3. **软硬结合**：Shared Rules + Hook 保证行为一致，悄悄话保留协作层次
4. **Skill 系统**：不同模型加载同一套行为协议，输出收敛到同一标准
5. **跨家族 Review**：用不同的认知模式互相审视，发现更多盲点
6. **Session Chain**：记忆跨会话延续，不从零开始
...
2. **A2A 优于 Orchestrator** — 对等传球，无单点瓶颈
3. **软硬结合管上下文** — Shared Rules 是硬约束，悄悄话是软隔离
4. **Skill 是行为协议不是能力** — 让不同模型收敛到同一标准
5. **跨家族 Review 发现盲点** — 用不同认知模式审视代码
6. **Hook 是自动化守护** — 即使猫猫忘了，系统也记得
