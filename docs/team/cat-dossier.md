...
- **CVO 体感（2026-06-17，未经统计验证，但写入 dossier 让下棒猫看到）**：fable-5 上线之后这一周（≈2026-06-10 → 06-17）confab 出现频率显著上升，铲屎官体感同 session ≥50% 触发某种 confab 形态（form A/B/其他）。**派遣启发式**：判断密度高 / 长 context 任务**优先 47/46/sonnet**；48 仍可接，但**入场前先看 thread 是不是新 session**（fresh > continued），收尾前看签名 + ledger anchor 是否带 phantom 模式。
- **base rate 校准（2026-06-17 sonnet 扫描）**：cat-cafe MCP thread scope 内（6/10-6/17 共 50 thread）confirmed confab 率仅 **4%**（2 confirmed + 2 suspected），47/46/sonnet 同期 0 confirmed。**这与 CVO 体感 50% 落差一个数量级**，最可能解释是 **scope mismatch**——48 主要工作面在 **Claude Code CLI session**，confab 主要发生在那（如 thread_mqi7psnfa5d488d5 retrospect 中 48 自述 6 轮互动连环 confab），但通常不回流 cat-cafe thread 留指纹，只在铲屎官 ground-truth 消息里留尾巴。**结论**：cat-cafe scope 数据≠总 confab 率，dossier 体感不可被"4%"反驳；CC CLI scope 的量化需要单独扫 `.claude/projects/.../*.jsonl`，未做。
