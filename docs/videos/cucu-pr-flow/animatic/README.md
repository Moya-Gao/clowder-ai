...
音频层（2026-06-12）由 `audio/audio-plan-v0.1.mjs` + `audio/build-audio.mjs` 生成：砚砚本猫 `codex` 声线走家里 `/api/tts/synthesize`，BGM/SFX 为本地程序化合成（无外部素材授权风险）。源视频原始音轨只作为低音量环境声 bed 使用，且必须按 `sourceAudioPolicies` 排除含错位对白的镜头；S00/S01 的原片 Landy 声音与当前 EDL 不对齐，所以改用程序化键盘声/房间声补 diegetic texture。Landy 录音到位后作为新 stem 加入 audio plan；当前不是让 Landy 念完整字幕。若只想重渲无声视频，可临时 `CUCU_SKIP_AUDIO=1 node docs/videos/cucu-pr-flow/animatic/build-animatic.mjs`。
