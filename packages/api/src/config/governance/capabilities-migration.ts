...
import { DEFAULT_MOUNT_RULES, STANDARD_PROVIDER_IDS } from '@cat-cafe/shared';
...
function getDefaultMountRuleEntries(): MountRuleEntry[] {
  return STANDARD_PROVIDER_IDS.map((id) => ({
    name: id,
    path: DEFAULT_MOUNT_RULES.providers[id].path,
    enabled: DEFAULT_MOUNT_RULES.providers[id].enabled,
  }));
}
