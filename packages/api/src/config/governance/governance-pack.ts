...
function getRuntimePorts(): RuntimePorts {
  // Home defaults: frontend 3001 / API 3002 (sanitizer transforms `?? '3001'` → `?? '3003'`,
  // `?? '3002'` → `?? '3004'` on outbound sync). Redis ports stay as-is across both repos.
  return {
    frontendPort: process.env.FRONTEND_PORT ?? '3001',
    apiPort: process.env.API_SERVER_PORT ?? '3002',
