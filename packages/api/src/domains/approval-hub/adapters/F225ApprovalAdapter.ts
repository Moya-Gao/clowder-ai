 ...
 * decidedAt = updatedAt (set at approve/reject time); decidedBy = userId (always CVO).
 */
...
function toSettledItem(p: SessionHandoffProposal): SettledApprovalItem {
  // F225 has no dedicated decidedBy field — the decision-maker is always the CVO (userId).
