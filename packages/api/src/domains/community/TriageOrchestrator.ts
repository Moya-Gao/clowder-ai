...
interface TriageOrchestratorDeps {
  ...
  //   low  → pending-decision (CVO reviews in Decision Queue)
...
export class TriageOrchestrator {
  ...
  async recordTriageEntry(issueId: string, entry: TriageEntry): Promise<TriageAction> {
    ...
    //   - low confidence  → pending-decision for CVO review in Decision Queue
    ...
    if (this.deps.repoConfigStore && consensus.verdict === 'WELCOME' && !consensus.needsOwner) {
      ...
      if (confidence === 'high') {
        ...
        // Low confidence → pending-decision for CVO review
        state = 'pending-decision';
      }
    }
  ...
  async routeAccepted(
    ...
    if (relatedFeature) {
      await this.deps.communityIssueStore.update(issueId, {
        ...
        // P1-R2-2: CVO manual routing is final — mark as accepted so stale
