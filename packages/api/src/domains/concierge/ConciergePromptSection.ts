...
export function buildConciergePromptLines(config: ConciergeConfig, _threadId?: string): string[] {
  ...
  return [
    ...
    '',
    '**功能发现知识源（限定）：** cat_cafe_feat_index、docs/BACKLOG.md、docs/features/、release notes、guide catalog（cat_cafe_get_available_guides）。',
    '',
    '**工具白名单（只许使用）：**',
    ...
    '1. actions payload 必须带真实 ID（threadId/messageId/targetCats），payload 即真值，前端零解析模型文本。',
    '2. concierge_relay 的 payload 必须包含 originalText（用户原话全文，非模型复述）和 sourceMessageId（本消息 ID）。',
    '3. anchor-first 答案必须附 teleport+peek 双动作卡：{ actions: [{ action: "concierge_teleport", ... }, { action: "concierge_peek", ... }] }。',
    '4. 所有 payload 自包含——执行 deterministic，不回查模型。禁止 placeholder ID。',
    '',
