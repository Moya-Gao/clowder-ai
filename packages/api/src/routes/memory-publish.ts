...
import { z } from 'zod';
import type { IMemoryGovernanceStore } from '../domains/cats/services/MemoryGovernanceStore.js';
import { GovernanceConflictError } from '../domains/cats/services/MemoryGovernanceStore.js';
...
export const memoryPublishRoutes: FastifyPluginAsync<MemoryPublishRoutesOptions> = async (app, opts) => {
  app.post('/api/memory/publish', async (request, reply) => {
    ...
    try {
      // Auto-create entry as draft if it doesn't exist yet
      if (!opts.governanceStore.get(entryId)) {
        opts.governanceStore.create(entryId, actor);
      }

      const before = opts.governanceStore.get(entryId)!;
