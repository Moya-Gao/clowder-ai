...
export interface SkillIssue {
  skill: string;
  type: SkillIssueType;
  /** Mount-point provider id for conflict / mount-missing issues. */
  provider?: string;
...
function conflictToIssue(conflict: DriftConflict): SkillIssue {
  ...
  return {
    ...
    message: `${conflict.provider} ${occupancy}${pointer}（${CONFLICT_OVERWRITE_WARNING}）`,
  };
}
...
function buildSkillIssues(scenarios: {
  conflicts: DriftConflict[];
  missingMounts: ReadonlyArray<{ skill: string; providers: string[] }>;
  ...
  for (const missing of scenarios.missingMounts) {
    issues.push({
      skill: missing.skill,
      type: 'mount-missing',
      message: missing.providers.length > 0 ? `${missing.providers.join('、')} 未挂载` : '挂载缺失',
    });
  }
...
interface MountDriftResult {
  /** Skills with at least one missing managed symlink, with the missing provider ids. */
  missingMounts: Array<{ skill: string; providers: string[] }>;
  conflicts: DriftConflict[];
...
async function checkMountDrift(
  ...
  const missingMounts: Array<{ skill: string; providers: string[] }> = [];
    ...
    for (const skillName of expectedSet) {
      ...
      for (const target of mountTargets.filter((t) => skillAllowsMountProvider(policy, skillName, t.provider))) {
        ...
        if (result.kind === 'missing') {
          missingProviders.push(target.provider);
          continue;
        }
      ...
      else if (missingProviders.length > 0) missingMounts.push({ skill: skillName, providers: missingProviders });
    }
  }
