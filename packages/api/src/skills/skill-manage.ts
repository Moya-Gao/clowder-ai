  ...
  for (const target of targets) {
    if (allowed && !allowed.has(target.id)) {
      for (const dir of target.dirs) {
        // Guard: skip symlinked provider dirs (same as mount branch below)
        try {
    ...
    for (const dir of target.dirs) {
      ...
      try {
        ...
        if (dirStat.isSymbolicLink() || !dirStat.isDirectory()) {
          result.conflicts.push({ skillName, providerId: target.id, path: dir });
          continue;
        }
      } catch (err) {
...
export async function unmountSkillSymlinks(
  ...
  for (const dir of allProviderDirs(projectRoot, mountRules)) {
    // Guard: skip symlinked provider dirs to avoid following into external targets
    try {
      const dirStat = await lstat(dir);
