...
export async function syncProject(
  ...
  //   - Has non-empty mountPaths in project config → local override (R5 P2:
  //     after P2-1 fix, inherited paths are no longer frozen into project config,
  //     so non-empty configMountPaths reliably signals user intent)
  //   - Enabled + never cascade-involved + no mountPaths → likely read-materialized, must NOT block
  const projectConfiguredSkills = new Set(
    previousNames.filter((name) => {
      ...
      if (configDisabledSet.has(name)) return true;
      const mp = configMountPaths.get(name);
      return mp !== undefined && mp.length > 0;
    }),
  );
  ...
  for (const skillsDir of allProviderDirs(projectRoot, mountRules)) {
    try {
      if (await isManagedDirectoryLevelSkillsSymlink(skillsDir, skillsSource)) {
        // R1 P2: disabled provider's directory-level symlink must be removed.
        // Phase 1 only converts active providers, so disabled ones are never
        // cleaned up and keep exposing all skills through the legacy root.
        if (!activeDirSet.has(skillsDir)) {
          await rm(skillsDir);
          result.unmounted.push({ skillName: '*', providerId: 'cleanup', path: skillsDir });
        }
        continue;
      }
    } catch {
