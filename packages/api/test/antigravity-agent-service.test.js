...
describe('AntigravityAgentService (Bridge)', () => {
  ...
  test('injects callback fallback instructions when callbackEnv is available', async () => {
    ...
    assert.ok(sentPrompt.includes('graph_resolve / list_recent'), 'should advertise current readonly memory entrypoints');
    assert.ok(!sentPrompt.includes('search_evidence / reflect / session-chain'), 'should not advertise removed reflect MCP');
