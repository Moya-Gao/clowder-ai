/**
 * F233 PR3 — explicit CVO handoff source event.
 */
...
describe('F233 PR3: explicit CVO handoff event', () => {
  test('line-start @landy records ball.handed_cvo without natural-language intent classification', async () => {
    ...
    const deps = createMockDeps({
      opus: {
        async *invoke() {
          ...
          yield { type: 'text', catId: 'opus', content: '@landy\n需要 CVO 决策', timestamp: Date.now() };
    ...
    assert.ok(cvoEvent, 'line-start @landy must record ball.handed_cvo');
  ...
  test('callback-only line-start @landy records ball.handed_cvo from callback message id', async () => {
    const { routeSerial } = await import('../dist/domains/cats/services/agents/routing/route-serial.js');
    const recorded = [];
    const deps = createMockDeps({
      opus: {
        async *invoke() {
          ...
          yield {
            ...
            toolInput: JSON.stringify({ content: '@landy\ncallback-only escalation' }),
            timestamp: Date.now(),
          };
          yield {
            type: 'tool_result',
    ...
    const cvoEvent = recorded.find((event) => event.kind === 'ball.handed_cvo');
    assert.ok(cvoEvent, 'callback-only line-start @landy must record ball.handed_cvo');
  ...
  test('later callback post without line-start @landy does not inherit prior CVO handoff', async () => {
    const { routeSerial } = await import('../dist/domains/cats/services/agents/routing/route-serial.js');
    const recorded = [];
    const deps = createMockDeps({
      opus: {
        async *invoke() {
          ...
          yield {
            ...
            catId: 'opus',
            toolName: 'cat_cafe_post_message',
            toolInput: JSON.stringify({ content: 'status update without CVO handoff' }),
            timestamp: Date.now(),
          };
          yield {
            type: 'tool_result',
    ...
    assert.deepEqual(
      ...
      'only the callback that actually contained line-start @landy should record ball.handed_cvo',
    );
  });

  test('same-name pending callback posts use toolUseId to bind CVO handoff to the actual callback', async () => {
    const { routeSerial } = await import('../dist/domains/cats/services/agents/routing/route-serial.js');
    const recorded = [];
    const deps = createMockDeps({
      opus: {
  ...
  test('cross-post callback line-start @landy records CVO handoff on the callback target only', async () => {
    const { routeSerial } = await import('../dist/domains/cats/services/agents/routing/route-serial.js');
    const recorded = [];
    const deps = createMockDeps({
      opus: {
