...
describe('callback-anchor-helpers (F236 Phase A)', () => {
  ...
  test('anchorThreadMessage: keywordTerms route preview through match window', () => {
    ...
    const a = anchorThreadMessage(item, { effectiveThreadId: 't', speaker: '铲屎官', keywordTerms: ['KWHIT'] });
  ...
  test('anchorThreadMessage: agent-key caller drillDown carries agentKeyCatId (F236 R1/云端 P2)', () => {
    ...
    const a = anchorThreadMessage(item, { effectiveThreadId: 't', speaker: '铲屎官', agentKeyCatId: 'antig-opus' });
  ...
  test('anchorThreadMessage: invocation caller drillDown omits agentKeyCatId', () => {
    ...
    const a = anchorThreadMessage(item, { effectiveThreadId: 't', speaker: '铲屎官' });
  ...
  test('anchorThreadMessage: short message → anchor shape with injected threadId', () => {
    ...
    const a = anchorThreadMessage(item, { effectiveThreadId: 'thread-x', speaker: '铲屎官' });
    assert.equal(a.id, 'm1');
    assert.equal(a.threadId, 'thread-x'); // AC: injected effectiveThreadId
    assert.equal(a.timestamp, 1);
    assert.equal(a.speaker, '铲屎官'); // F236 R1: caller-supplied via sender-display, never raw userId
  ...
  test('anchorPendingMention: short mention keeps from/message, requiresDrill=false', () => {
    ...
    const a = anchorPendingMention(item, { from: '铲屎官' });
    assert.equal(a.id, 'p1');
    assert.equal(a.from, '铲屎官'); // F236 R1: caller-supplied, never raw userId
  ...
  test('anchorPendingMention: includes acked only when provided', () => {
    ...
    assert.equal(anchorPendingMention(item, { from: '铲屎官', acked: true }).acked, true);
    assert.equal('acked' in anchorPendingMention(item, { from: '铲屎官' }), false);
  });
