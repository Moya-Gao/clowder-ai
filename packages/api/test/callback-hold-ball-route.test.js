...
describe('F167 C1: /api/callbacks/hold-ball auth + body validation', () => {
  ...
  test('400 on invalid body: reason missing', async () => {
    ...
    const { invocationId, callbackToken } = registry.create('user-hb-400a', 'codex', thread.id);
    const response = await app.inject({
      method: 'POST',
      url: '/api/callbacks/hold-ball',
  ...
  test('400 on invalid body: wakeAfterMs below 5s minimum', async () => {
    ...
    const { invocationId, callbackToken } = registry.create('user-hb-400b', 'codex', thread.id);
    const response = await app.inject({
      method: 'POST',
      url: '/api/callbacks/hold-ball',
      headers: { 'x-invocation-id': invocationId, 'x-callback-token': callbackToken },
  ...
  test('400 on invalid body: wakeAfterMs above 1h maximum', async () => {
    ...
    const { invocationId, callbackToken } = registry.create('user-hb-400c', 'codex', thread.id);
    const response = await app.inject({
      method: 'POST',
      url: '/api/callbacks/hold-ball',
      headers: { 'x-invocation-id': invocationId, 'x-callback-token': callbackToken },
