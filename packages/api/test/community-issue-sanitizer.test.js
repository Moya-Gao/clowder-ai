...
describe('CommunityIssueSanitizer', () => {
  ...
  describe('absolute paths', () => {
    it('redacts /Users/xxx paths', async () => {
      const { sanitize } = await loadModule();
      const result = sanitize('Title', 'File at /Users/lysander/projects/relay-station/cat-cafe/src/index.ts');
      assert.ok(!result.bodyMarkdown.includes('/Users/lysander'));
  ...
  describe('multiple patterns in one text', () => {
    it('redacts all forbidden patterns', async () => {
      const { sanitize } = await loadModule();
      const result = sanitize('Bug report', 'thread_abc in /Users/lysander/code with fi_xyz123 and usr_test');
