...
describe('F146 validation scenario: browser 3-backend integration', () => {
  ...
  test('install pinchtab via write path', async () => {
    ...
    assert.ok(cap);
    assert.equal(cap.mcpServer?.command, '/Users/lysander/.pinchtab/bin/pinchtab-darwin-arm64');
  });
  ...
  test('full scenario: install 3 → delete 1 → verify audit', async () => {
    const backends = [
      ...
      { id: 'pinchtab', command: '/Users/lysander/.pinchtab/bin/pinchtab-darwin-arm64', args: ['mcp'] },
