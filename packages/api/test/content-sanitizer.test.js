...
describe('sanitizeEventContent', () => {
  ...
  test('redacts absolute file paths (/Users/...)', async () => {
    const { sanitizeEventContent } = await loadModule();
    const result = sanitizeEventContent({
      id: 'e5',
      at: 1000,
      kind: 'tool_use',
      content: 'Read file /Users/lysander/projects/relay-station/cat-cafe/packages/api/src/index.ts',
    });
    assert.ok(!result.content.includes('/Users/lysander'), 'Absolute path should be redacted');
  ...
  test('redacts worktree paths', async () => {
    ...
    const result = sanitizeEventContent({
      ...
      at: 1000,
      kind: 'text',
      content: 'cd /Users/lysander/projects/relay-station/cat-cafe-f252-phase-d',
    });
...
EOF`;
  ...
  test('redacts toolArgs containing file paths', async () => {
    ...
    const result = sanitizeEventContent({
      ...
      toolName: 'Read',
      toolArgs: '{"file_path": "/Users/lysander/projects/relay-station/cat-cafe/src/index.ts"}',
    });
    assert.ok(result.toolArgs);
    assert.ok(!result.toolArgs.includes('/Users/lysander'));
  });
...
describe('sanitizeStoryExport', () => {
  test('sanitizes all events in the export pack', async () => {
    ...
    const events = [
      {
        ...
        at: 1000,
        kind: 'text',
        content: 'Check /Users/lysander/projects/relay-station/cat-cafe/src/file.ts',
      },
      {
        id: 'e2',
        at: 2000,
    ...
    for (const event of pack.events) {
      assert.ok(!event.content.includes('/Users/lysander'), `Event ${event.id} content leaked path`);
  ...
  test('annotations in export pack have content sanitized', async () => {
    ...
    const annotations = [
      ...
      },
      {
        ...
        content: 'Path /Users/lysander/secret/dir matters',
        createdAt: 2,
        updatedAt: 2,
      },
      {
    ...
    assert.ok(!pack.manifest.annotations[1].content.includes('/Users/lysander'), 'annotation leaked path');
    ...
    const events = [
      {
        ...
        at: 1000,
        kind: 'text',
        content: '@landy 铲屎官说 Landy 觉得没问题，Lysander approved',
        catId: 'opus',
      },
    ];
    const pack = sanitizeStoryExport('s1', 'Test', events, [], aliases);

    // "Landy" is NOT a common English word — it's a distinctive proper name.
    // Even though short (5 chars), it's not in AMBIGUOUS_COMMON_WORDS, so
    // Phase 2 uses full standalone detection and redaction.
    assert.ok(!pack.events[0].content.includes('Landy'), 'Leaked coCreator name');
    assert.ok(!pack.events[0].content.includes('Lysander'), 'Leaked coCreator alias');
    assert.ok(!pack.events[0].content.includes('landy'), 'Leaked coCreator mention');
  });
