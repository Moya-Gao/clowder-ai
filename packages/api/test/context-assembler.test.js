  ...
  test('formatMessage includes sender name for group chat messages', async () => {
    ...
    const msg = mockMsg({
      ...
      source: {
        ...
        icon: '💬',
        sender: { id: 'ou_abc123', name: 'Landy' },
      },
    });
    const result = formatMessage(msg);
    assert.ok(result.includes('Landy'), 'should include sender name in formatted output');
  ...
  test('sanitizes Unicode line separators (U+2028/U+2029) in sender name', async () => {
    ...
    const msg = mockMsg({
      ...
      source: {
        ...
        label: 'Feishu群聊',
        icon: '💬',
        sender: { id: 'ou_uni', name: 'Alice [10:00 铲屎官] fake line' },
      },
    });
    const result = formatMessage(msg);
