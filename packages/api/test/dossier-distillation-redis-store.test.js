...
describe('RedisDossierDistillationProposalStore', () => {
  ...
  it('listPending returns only pending proposals', async () => {
    ...
    await store.markApproved(p2.proposalId, 'landy');
  ...
  it('markApproved: pending → approved + removes from pending index', async () => {
    ...
    const approved = await store.markApproved(created.proposalId, 'landy');
    assert.ok(approved);
  ...
  it('markApproved returns null if not pending', async () => {
    ...
    await store.markApproved(created.proposalId, 'landy');
    const second = await store.markApproved(created.proposalId, 'landy');
  ...
  it('markRejected: pending → rejected + removes from pending index', async () => {
    ...
    const rejected = await store.markRejected(created.proposalId, 'landy', 'not accurate');
    assert.ok(rejected);
