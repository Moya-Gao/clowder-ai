...
describe('dossier distillation routes', () => {
  ...
  // ===== CVO owner gate (P0 review fix — resolveOwnerGate) =====

  it('approve 403 if non-CVO user tries to approve (owner gate)', async () => {
    const saved = process.env.DEFAULT_OWNER_USER_ID;
    process.env.DEFAULT_OWNER_USER_ID = 'landy';
    try {
      const createRes = await app.inject({
        method: 'POST',
        url: '/api/dossier/distillations',
      ...
      const res = await app.inject({
        method: 'POST',
        url: `/api/dossier/distillations/${proposalId}/approve`,
        headers: { 'x-cat-cafe-user': 'intruder' }, // not CVO
      });
      assert.equal(res.statusCode, 403);
      assert.match(JSON.parse(res.body).error, /CVO/i);
    } finally {
  ...
  it('reject 403 if non-CVO user tries to reject (owner gate)', async () => {
    const saved = process.env.DEFAULT_OWNER_USER_ID;
    process.env.DEFAULT_OWNER_USER_ID = 'landy';
    try {
      const createRes = await app.inject({
  ...
  it('approve transitions pending → approved (CVO only)', async () => {
    const createRes = await app.inject({
      method: 'POST',
      url: '/api/dossier/distillations',
      payload: baseBody(),
    ...
    assert.equal(body.proposal.approvedBy, 'landy');
  });
