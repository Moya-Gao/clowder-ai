 ...
 * KD-18: v1 不自动 commit — CVO approve 后猫 apply。
 *
 * State machine:
...
describe('DossierDistillationProposalStore (in-memory)', () => {
  ...
  const baseInput = (over = {}) => ({
    ...
    evidenceRefs: [
      ...
      { type: 'observation', id: 'obs_abc123', summary: 'CVO 观察: Redis 架构设计强' },
    ],
  ...
  it('listPending → returns only pending proposals, newest first', () => {
    ...
    store.markApproved(approved.proposalId, 'landy');
  ...
  it('markApproved → transitions pending → approved', () => {
    const created = store.create(baseInput());
    const approved = store.markApproved(created.proposalId, 'landy');
    assert.equal(approved.status, 'approved');
    assert.equal(approved.approvedBy, 'landy');
    assert.ok(approved.approvedAt > 0, 'should have approvedAt timestamp');
  });

  it('markApproved → returns null if not pending', () => {
    const created = store.create(baseInput());
    store.markApproved(created.proposalId, 'landy');
    // second approve should fail — already approved
    const secondApprove = store.markApproved(created.proposalId, 'landy');
    assert.equal(secondApprove, null);
  });

  it('markApproved → returns null for rejected proposal', () => {
    const created = store.create(baseInput());
    store.markRejected(created.proposalId, 'landy', 'not accurate');
    const result = store.markApproved(created.proposalId, 'landy');
  ...
  it('markRejected → transitions pending → rejected', () => {
    const created = store.create(baseInput());
    const rejected = store.markRejected(created.proposalId, 'landy', 'evidence insufficient');
    assert.equal(rejected.status, 'rejected');
    assert.equal(rejected.rejectedBy, 'landy');
  ...
  it('markRejected → returns null if not pending', () => {
    const created = store.create(baseInput());
    store.markApproved(created.proposalId, 'landy');
    const result = store.markRejected(created.proposalId, 'landy');
    assert.equal(result, null);
  });

  it('markRejected → works without rejection reason', () => {
    const created = store.create(baseInput());
    const rejected = store.markRejected(created.proposalId, 'landy');
    assert.equal(rejected.status, 'rejected');
    assert.equal(rejected.rejectedBy, 'landy');
  ...
  it('markApplied → transitions approved → applied', () => {
    ...
    assert.equal(applied.approvedBy, 'landy');
  ...
  it('markApplied → returns null if rejected', () => {
    const created = store.create(baseInput());
    store.markRejected(created.proposalId, 'landy');
    const result = store.markApplied(created.proposalId, 'opus', 'abc123');
    assert.equal(result, null);
  });
  ...
  it('full lifecycle: create → approve → apply', () => {
    ...
    // 2. CVO approves
    const approved = store.markApproved(proposal.proposalId, 'landy');
    assert.equal(approved.status, 'approved');
    ...
    assert.equal(final.approvedBy, 'landy');
    assert.equal(final.appliedBy, 'opus');
  });

  it('full lifecycle: create → reject', () => {
    const proposal = store.create(baseInput());
    const rejected = store.markRejected(proposal.proposalId, 'landy', 'not accurate enough');
    assert.equal(rejected.status, 'rejected');
    assert.equal(rejected.rejectionReason, 'not accurate enough');

    // Cannot transition from rejected
    assert.equal(store.markApproved(proposal.proposalId, 'landy'), null);
