...
describe('RedisDossierObservationStore', () => {
  ...
  it('add persists observation to Redis hash + sorted set index', async () => {
    ...
    const obs = await store.add({ catId: 'opus', content: 'Strong at architecture', author: 'landy' });
    ...
    assert.equal(obs.provenance.author, 'landy');
    ...
    assert.equal(hash.provenanceAuthor, 'landy');
  ...
  it('list returns observations newest first', async () => {
    if (!store) return;
    const obs1 = await store.add({ catId: 'codex', content: 'First observation', author: 'landy' });
    // Small delay to ensure different timestamps
    await new Promise((r) => setTimeout(r, 5));
    const obs2 = await store.add({ catId: 'codex', content: 'Second observation', author: 'landy' });
  ...
  it('list respects limit parameter', async () => {
    if (!store) return;
    await store.add({ catId: 'limited-cat', content: 'A', author: 'landy' });
    await store.add({ catId: 'limited-cat', content: 'B', author: 'landy' });
    await store.add({ catId: 'limited-cat', content: 'C', author: 'landy' });
  ...
  it('listAll groups observations by catId', async () => {
    if (!store) return;
    await store.add({ catId: 'cat-a', content: 'Obs for A', author: 'landy' });
    await store.add({ catId: 'cat-b', content: 'Obs for B', author: 'landy' });
  ...
  it('get returns observation by id', async () => {
    if (!store) return;
    const obs = await store.add({ catId: 'opus', content: 'Retrievable', author: 'landy' });
  ...
  it('delete removes from both hash and index', async () => {
    if (!store) return;
    const obs = await store.add({ catId: 'delete-test', content: 'To delete', author: 'landy' });
  ...
  it('observations have no TTL (Iron Rule #5: user state persists)', async () => {
    if (!store) return;
    const obs = await store.add({ catId: 'ttl-test', content: 'Must persist', author: 'landy' });
