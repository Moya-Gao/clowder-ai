...
test('collectMention（via 整合）: 尾部猫@landy 无 CVO 回应 → 候选；有 CVO 回应 → 不报', async () => {
      ...
      getByThread: async () => [
        {
          ...
          mentionsUser: true,
          catId: 'opus',
          content: '@landy 这个要不要做？',
          timestamp: NOW - 2 * HOUR,
          deliveredAt: NOW - 2 * HOUR,
        },
      ],
      getByThreadAfter: async () => [], // 无后续 → 球还在 CVO 手上
    },
  });
  ...
  // 有 CVO（catId==null）回应 → 不报
  const answeredDeps = mockDeps({
    threadStore: { list: async () => [{ id: 'thr-m', lastActiveAt: NOW - 1 * HOUR, title: 't' }] },
    messageStore: {
      getByThread: async () => [
        { id: 'msg-1', mentionsUser: true, catId: 'opus', content: '@landy ?', timestamp: NOW - 2 * HOUR },
      ],
      getByThreadAfter: async () => [{ id: 'msg-2', catId: null, content: '好的' }],
    },
  });
  const answered = await collectDutyBriefingInput(answeredDeps);
  assert.equal(answered.mentionCandidates.length, 0, 'CVO 回应了 → 球不在 CVO 手上');
});

test('collectMention: system/briefing 消息不算 CVO 回应，不能压掉 needs-user 候选', async () => {
  const input = await collectDutyBriefingInput(
    mockDeps({
  ...
  assert.equal(input.mentionCandidates.length, 1, 'system bubble 不是 CVO 回复');
});
...
test('collectMention: 非近期活跃 thread（超 72h）跳过', async () => {
  const input = await collectDutyBriefingInput(
    mockDeps({
      ...
      messageStore: {
        getByThread: async () => [
          { id: 'm', mentionsUser: true, catId: 'opus', content: '@landy', timestamp: NOW - 100 * HOUR },
        ],
        getByThreadAfter: async () => [],
      },
    }),
