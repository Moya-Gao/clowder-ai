...
describe('F235: FrustrationIssueSourceAdapter', () => {
  ...
  it('sanitizes internal IDs from userDescription (deny-list defense-in-depth)', async () => {
    ...
    const issueWithInternalIds = {
      ...
      context: {
        recentMessages: [
          {
            role: 'user',
            content: 'Error in thread_abc123 for session_xyz789 at /Users/lysander/projects/foo',
            timestamp: 1000,
          },
        ],
      },
    ...
    assert.ok(!draft.bodyMarkdown.includes('/Users/lysander'), 'raw message content excluded by B-lite');
  ...
  it('adds "Reported via Cat Cafe" footer', async () => {
    const draft = await createDraftFromFrustrationIssue(confirmedIssue, {
      draftStore: store,
      config: defaultConfig,
    });
    assert.ok(
      draft.bodyMarkdown.includes('Reported via Cat Cafe') || draft.bodyMarkdown.includes('Cat Cafe'),
      'body should have Cat Cafe footer',
    );
  });
