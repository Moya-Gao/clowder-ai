...
describe('Skills sync state (F228)', () => {
  ...
  test('updateSkillMountPaths preserves desired mountPaths when empty providers without forceDisabled (F228 P1)', async () => {
    ...
    // Empty providers without forceDisabled → preserve existing mountPaths AND enabled
    await updateSkillMountPaths(tempDir, ['debugging'], []);
    // Non-empty providers → always update mountPaths and set enabled=true
    await updateSkillMountPaths(tempDir, ['worktree'], ['codex']);
    // New entry with empty providers, no forceDisabled → defaults to enabled=true, mountPaths=[]
    await updateSkillMountPaths(tempDir, ['tdd'], []);

    const config = await readCapabilitiesConfig(tempDir);
    const byId = new Map(config?.capabilities.map((cap) => [cap.id, cap]));

    // F228: empty providers ≠ disabled. Desired mountPaths ['claude'] must survive
    // temporary provider unavailability so re-enabling can restore the mount.
  ...
  test('updateSkillMountPaths explicit mount points lifecycle (F228 state-record model)', async () => {
    ...
    // Step 3: Empty providers → preserve existing state (no resolvedEnabled)
    await updateSkillMountPaths(tempDir, ['tdd'], []);

    const config3 = await readCapabilitiesConfig(tempDir);
    const tdd3 = config3?.capabilities.find((c) => c.id === 'tdd');
    assert.equal(tdd3.enabled, true, 'still enabled — empty providers = no change');
    assert.deepStrictEqual(tdd3.mountPaths, ['kimi'], 'existing policy preserved when no providers passed');
