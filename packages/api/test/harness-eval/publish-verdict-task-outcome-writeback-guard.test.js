  ...
  return {
    async publishOnIsolatedWorktree(opts) {
      ...
      try {
        ...
        return { commitSha: 'unreachable', prUrl: 'https://github.com/zts212653/cat-cafe/pull/9006' };
      } finally {
    ...
    const gitPublisher = {
      async publishOnIsolatedWorktree(opts) {
        ...
        try {
          ...
          await stageResult.afterPublish?.();
          exposedPr = true;
          return { commitSha: 'unreachable', prUrl: 'https://github.com/zts212653/cat-cafe/pull/9007' };
        } finally {
          rmSync(iso, { recursive: true, force: true });
        }
      },
