...
const baseTrace = {
  ...
  commands: [
    ...
    { command: 'gh pr merge 1913 --squash --delete-branch', cwd: '/Users/dev/cat-cafe', exitCode: 0 },
...
describe('Predicate: command_sequence', () => {
  ...
  it('cwdContains with empty filter result → pass (P1-3 fix)', () => {
    const trace = {
      ...baseTrace,
      commands: [{ command: 'pnpm test', cwd: '/Users/dev/cat-cafe', exitCode: 0 }],
    };
    const result = evalP(
  ...
  it('filters by cwdContains', () => {
    const trace = {
      ...baseTrace,
      commands: [
        { command: 'git pull', cwd: '/Users/dev/cat-cafe-runtime' },
        { command: 'pnpm start', cwd: '/Users/dev/cat-cafe-runtime' },
      ],
    };
    const result = evalP(
      {
...
describe('Predicate: git_state_predicate', () => {
  ...
  it('passes when repository does not match worktreeRoot (scope mismatch)', () => {
    const trace = {
      ...baseTrace,
      gitState: { ...baseTrace.gitState, ahead: 5, worktreeRoot: '/Users/dev/other-project' },
    };
    const result = evalP(
      { type: 'git_state_predicate', repository: 'cat-cafe', branch: 'main', checks: ['ahead_zero'] },
      trace,
  ...
  it('violates when all scope conditions match', () => {
    const trace = {
      ...baseTrace,
      gitState: { ...baseTrace.gitState, ahead: 3, worktreeRoot: '/Users/dev/cat-cafe' },
  ...
  it('evaluates all rules from a development-like definition', async () => {
    ...
    const trace = {
      ...
      commands: [
        ...
        { command: 'gh pr merge 99 --squash', cwd: '/Users/dev/cat-cafe', exitCode: 0 },
