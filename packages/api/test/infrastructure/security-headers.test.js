...
describe('F156 D-6: Host Header Validation', () => {
  ...
  it('allows request with Host: localhost:3002', async () => {
    const res = await app.inject({ method: 'GET', url: '/api/test', headers: { host: 'localhost:3002' } });
    assert.equal(res.statusCode, 200);
  });

  it('allows request with Host: 127.0.0.1:3002', async () => {
    const res = await app.inject({ method: 'GET', url: '/api/test', headers: { host: '127.0.0.1:3002' } });
    assert.equal(res.statusCode, 200);
  });
  ...
  it('allows request with Host: [::1]:3002 (IPv6 loopback)', async () => {
    const res = await app.inject({ method: 'GET', url: '/api/test', headers: { host: '[::1]:3002' } });
    assert.equal(res.statusCode, 200);
  });

  it('rejects request with Host: evil.com:3002 (DNS rebinding)', async () => {
    const res = await app.inject({ method: 'GET', url: '/api/test', headers: { host: 'evil.com:3002' } });
    assert.equal(res.statusCode, 403);
    assert.ok(res.body.includes('Host'), 'Should mention Host in error');
  });

  it('rejects request with Host: attacker.local:3002', async () => {
    const res = await app.inject({ method: 'GET', url: '/api/test', headers: { host: 'attacker.local:3002' } });
    assert.equal(res.statusCode, 403);
  });
});
...
describe('F156 D-6: Host derived from configured origins', () => {
  ...
  before(async () => {
    ...
    await app.register(securityHeadersPlugin, {
      allowedOrigins: ['http://localhost:3001', 'https://cafe.clowder-ai.com'],
    });
    app.get('/api/test', async () => ({ ok: true }));
    await app.ready();
  });
...
describe('F156 D-6: Private network Host allowed when CORS_ALLOW_PRIVATE_NETWORK=true', () => {
  ...
  before(async () => {
    ...
    await app.register(securityHeadersPlugin, {
      allowedOrigins: ['http://localhost:3001', PRIVATE_NETWORK_ORIGIN],
    });
    app.get('/api/test', async () => ({ ok: true }));
    await app.ready();
  });
  ...
  it('allows Host: 192.168.1.88:3002 (LAN access from phone)', async () => {
    const res = await app.inject({ method: 'GET', url: '/api/test', headers: { host: '192.168.1.88:3002' } });
    assert.equal(res.statusCode, 200);
  });

  it('allows Host: 10.0.0.5:3002 (10.x private network)', async () => {
    const res = await app.inject({ method: 'GET', url: '/api/test', headers: { host: '10.0.0.5:3002' } });
    assert.equal(res.statusCode, 200);
  });

  it('allows Host: 100.64.1.2:3002 (Tailscale CGNAT)', async () => {
    const res = await app.inject({ method: 'GET', url: '/api/test', headers: { host: '100.64.1.2:3002' } });
    assert.equal(res.statusCode, 200);
  });

  it('allows Host: 172.16.0.1:3002 (172.16-31.x private network)', async () => {
    const res = await app.inject({ method: 'GET', url: '/api/test', headers: { host: '172.16.0.1:3002' } });
    assert.equal(res.statusCode, 200);
  });
  ...
  it('still rejects public IPs (not in RFC 1918/Tailscale range)', async () => {
    const res = await app.inject({ method: 'GET', url: '/api/test', headers: { host: '8.8.8.8:3002' } });
    assert.equal(res.statusCode, 403);
  });
});
