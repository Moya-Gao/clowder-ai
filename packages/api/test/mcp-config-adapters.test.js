...
describe('writeClaudeMcpConfig', () => {
  ...
  it('injects workspace env into managed Cat Cafe servers', async () => {
    ...
    try {
      ...
      process.env.CAT_CAFE_WORKSPACE_ROOT = '/Users/landy/projects/cat-cafe';
      ...
      const data = JSON.parse(await readFile(file, 'utf-8'));
      assert.equal(data.mcpServers['cat-cafe'].env.ALLOWED_WORKSPACE_DIRS, '/Users/landy/projects/cat-cafe');
      assert.deepEqual(data.mcpServers['cat-cafe-memory'].env, {
        ALLOWED_WORKSPACE_DIRS: '/Users/landy/projects/cat-cafe',
        EXTRA_FLAG: 'keep-me',
      });
...
describe('writeCodexMcpConfig', () => {
  ...
  it('injects workspace env into managed Cat Cafe servers', async () => {
    ...
    try {
      ...
      assert.ok(main);
      assert.equal(main.env?.ALLOWED_WORKSPACE_DIRS, '/Users/landy/projects/cat-cafe');
      assert.ok(memory);
