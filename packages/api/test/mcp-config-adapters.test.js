...
describe('readClaudeMcpConfig', () => {
  ...
  beforeEach(async () => { dir = await makeTmpDir('claude-read'); });
...
describe('readCodexMcpConfig', () => {
  ...
  beforeEach(async () => { dir = await makeTmpDir('codex-read'); });
  afterEach(async () => { await rm(dir, { recursive: true, force: true }); });
...
describe('readGeminiMcpConfig', () => {
  ...
  beforeEach(async () => { dir = await makeTmpDir('gemini-read'); });
  afterEach(async () => { await rm(dir, { recursive: true, force: true }); });
...
describe('writeClaudeMcpConfig', () => {
  ...
  beforeEach(async () => { dir = await makeTmpDir('claude-write'); });
  afterEach(async () => { await rm(dir, { recursive: true, force: true }); });
  ...
  it('writes env and cwd when present', async () => {
    const file = join(dir, '.mcp.json');
    await writeClaudeMcpConfig(file, [
      { name: 'test', command: 'node', args: [], enabled: true, source: 'external',
        env: { KEY: 'val' }, workingDir: '/tmp' },
    ]);
...
describe('writeCodexMcpConfig', () => {
  ...
  beforeEach(async () => { dir = await makeTmpDir('codex-write'); });
  afterEach(async () => { await rm(dir, { recursive: true, force: true }); });
  ...
  it('preserves existing non-MCP config', async () => {
    ...
    await writeCodexMcpConfig(file, [
      { name: 'test', command: 'echo', args: [], enabled: true, source: 'external' },
    ]);
...
describe('writeGeminiMcpConfig', () => {
  ...
  beforeEach(async () => { dir = await makeTmpDir('gemini-write'); });
  afterEach(async () => { await rm(dir, { recursive: true, force: true }); });
...
describe('round-trip: read → write → read', () => {
  ...
  it('Claude .mcp.json round-trips correctly', async () => {
    const servers = [
      { name: 'cat-cafe', command: 'node', args: ['./mcp/index.js'],
        env: { PORT: '3000' }, enabled: true, source: /** @type {const} */ ('cat-cafe') },
      { name: 'fs', command: 'npx', args: ['-y', '@mcp/fs'],
        enabled: true, source: /** @type {const} */ ('external') },
    ];
  ...
  it('Codex config.toml round-trips correctly', async () => {
    const servers = [
      { name: 'cat_cafe', command: 'node', args: ['index.js'],
        enabled: true, source: /** @type {const} */ ('cat-cafe') },
      { name: 'disabled', command: 'echo', args: [],
        enabled: false, source: /** @type {const} */ ('external') },
    ];
  ...
  it('Gemini settings.json round-trips correctly', async () => {
    const servers = [
      { name: 'cat-cafe', command: 'node', args: ['index.js'],
        enabled: true, source: /** @type {const} */ ('cat-cafe'), workingDir: '/tmp' },
    ];
