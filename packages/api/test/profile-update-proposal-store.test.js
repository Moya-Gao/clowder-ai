...
describe('ProfileUpdateProposalStore (in-memory)', () => {
  ...
  it('INV-3: concurrent claim — only one wins (CAS pending→approving)', () => {
    const p = store.create(baseInput());
    const first = store.claimForApproval(p.proposalId, 'landy');
    const second = store.claimForApproval(p.proposalId, 'landy');
  ...
  it('recordCheckpoint persists BOTH paths without changing status (P1-1)', () => {
    const p = store.create(baseInput());
    store.claimForApproval(p.proposalId, 'landy');
  ...
  it('markRejected: pending→rejected (null if already approving)', () => {
    const p = store.create(baseInput());
    assert.equal(store.markRejected(p.proposalId, 'landy', 'not accurate').status, 'rejected');
    assert.equal(store.get(p.proposalId).rejectionReason, 'not accurate');
    const p2 = store.create(baseInput());
    store.claimForApproval(p2.proposalId, 'landy');
    assert.equal(store.markRejected(p2.proposalId, 'landy'), null, 'cannot reject approving');
  });

  it('ADV-5: reject then approve — claim returns null', () => {
    const p = store.create(baseInput());
    store.markRejected(p.proposalId, 'landy');
    assert.equal(store.claimForApproval(p.proposalId, 'landy'), null, 'cannot claim rejected');
  });
  ...
  it('listPending returns only pending for the user (newest first)', () => {
    ...
    const p2 = store.create(baseInput());
    store.markRejected(p2.proposalId, 'landy');
