...
describe('F177 Phase H — route-serial routing guard remedial invoke', () => {
        ...
        for (const hasToolUseId of [true, false]) {
          for (const mentionCase of mentionCases) {
            ...
            assert.equal(
              ...
              `${label}: source mentionsUser must follow local CVO state`,
            );
            ...
            assert.deepEqual(
              ...
              expectedCvo,
              `${label}: CVO handoff must bind to the callback delivery scope`,
            );
          }
        }
      }
  ...
  test('cross-post CVO callback does not mark the source thread as mentioning the user', async () => {
    const recorded = [];
    const service = createSequenceService('codex', [
      [
        {
    ...
    assert.equal(calls.length, 1, 'confirmed cross-post CVO callback must satisfy the routing guard');
    assert.deepEqual(
      recorded
        .filter((event) => event.kind === 'ball.handed_cvo')
        .map((event) => [event.sourceEventId, event.subjectKey]),
      [['route:msg-routing-cross-cvo', 'ball:thread:thread-routing-cross-cvo-target']],
      'cross-post CVO handoff must bind to the target thread',
    );
    ...
    assert.equal(
      ...
      undefined,
      'target-thread CVO callback must not create a source-thread user mention notification',
    );
  });
