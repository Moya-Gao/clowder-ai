...
describe('PATCH /api/config/env — sensitive env owner gate', () => {
  ...
  it('owner can write sensitive env vars and triggers dual audit trail', async () => {
    ...
    try {
      ...
      const res = await app.inject({
        method: 'PATCH',
        url: '/api/config/env',
        headers: { 'x-test-session-user': 'lysander' },
      ...
      assert.equal(auditEvents[1].data.operator, 'lysander');
    } finally {
      await app.close();
      rmSync(tempRoot, { recursive: true, force: true });
    }
  ...
  it('sensitive write audit uses session identity, not forgeable header', async () => {
    ...
    try {
      ...
      const res = await app.inject({
        method: 'PATCH',
        url: '/api/config/env',
        headers: { 'x-test-session-user': 'lysander', 'x-cat-cafe-user': 'attacker' },
        payload: { updates: [{ name: 'F102_API_KEY', value: 'sk-new' }] },
      });

      assert.equal(res.statusCode, 200);
      assert.equal(auditEvents.length, 2);
      assert.equal(auditEvents[0].data.operator, 'lysander', 'CONFIG_UPDATED must use session identity');
      assert.equal(auditEvents[1].data.operator, 'lysander', 'ENV_SENSITIVE_WRITE must use session identity');
  ...
  it('rejects DEFAULT_OWNER_USER_ID edits (trust anchor protection, P1 fix)', async () => {
    ...
    const tempRoot = mkdtempSync(resolve(tmpdir(), 'cat-cafe-env-'));
    const envFilePath = resolve(tempRoot, '.env');
    writeFileSync(envFilePath, 'DEFAULT_OWNER_USER_ID=lysander\n', 'utf8');
