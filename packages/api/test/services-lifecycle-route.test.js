...
const SESSION_HEADERS = { 'x-test-session-user': 'lysander' };
const TRUSTED_ORIGIN_HEADERS = { origin: 'http://localhost:3001', host: 'localhost:3001' };
    ...
    try {
      ...
      for (let attempt = 0; attempt < 20; attempt += 1) {
        ...
        if (readyEvents.length > 0) {
          assert.deepEqual(readyEvents, [{ serviceId: 'embedding-model', operator: 'lysander', reason: 'readiness' }]);
          return;
        }
      }
