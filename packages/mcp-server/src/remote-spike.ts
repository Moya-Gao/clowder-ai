...
/**
 * Cat Café MCP Server — Remote Spike Entry (F247 Phase B1a v4)
 *
...
function validateB1aEnv(): void {
  ...
  // every collab/memory tool would fail as "Cat Café callback not configured"
  ...
  if (!process.env.CAT_CAFE_API_URL) {
    errors.push(
      'CAT_CAFE_API_URL missing — getCallbackConfig() returns null without it, ' +
        'all collab/memory tools would fail as "Cat Café callback not configured".',
    );
  }
