...
function makeTheme(): ThemeTokens {
  return {
    ...
    brand: {
      colors: {
        primary: 'CF0A2C', secondary: '333333', accent: 'E60012',
        background: 'FFFFFF', surface: 'F5F5F5', surfaceAlt: 'EEEEEE', white: 'FFFFFF',
        text: { primary: '333333', secondary: '666666', muted: '999999', onPrimary: 'FFFFFF' },
      },
      typography: {
        headingFont: 'Noto Sans SC', bodyFont: 'Noto Sans SC',
        monoFont: 'IBM Plex Mono', cjkFont: 'Noto Sans SC',
    ...
    slide: {
      ...
      chart: { palette: ['CF0A2C', 'E60012', 'FF6B35'], gridColor: 'EEEEEE', gridSize: 0.5, axisLabelColor: '666666', axisLabelSize: 9, dataLabelColor: '333333', dataLabelSize: 9, bgColor: 'FFFFFF' },
...
describe('buildDeck', () => {
  ...
  it('rejects theme with # prefixed brand colors at build time (P2)', () => {
    ...
    assert.throws(
      () => buildDeck(makeBlueprint(), badTheme),
      /must not start with #/,
    );
  });
  ...
  it('throws on unsupported ImageElement instead of silent drop (P1-2)', () => {
    ...
    assert.throws(
      () => buildDeck(bp, makeTheme()),
      /not supported.*Phase A/i,
    );
  });
});
