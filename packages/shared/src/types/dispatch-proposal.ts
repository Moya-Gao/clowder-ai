 ...
 * the message is held as a DispatchProposal pending CVO approval in the
 ...
 * - assign_work: recipient opens worktree + writes code ("fix this bug") → needs CVO approval
 */
...
/**
 * A cross-thread assign_work dispatch held for CVO approval.
 *
 * Created when a cat sends cross_post_message with effectClass='assign_work'.
 * The message content is held (NOT delivered to target thread) until the CVO
...
export interface DispatchProposal {
  ...
  senderCatId: string;
  /** CVO user ID — only this user can approve/reject. */
  ownerUserId: string;
  ...
  createdAt: number;
  /** When CVO approved/rejected (epoch ms). */
  decidedAt?: number;
  /** CVO userId who decided. */
  decidedBy?: string;
}
