 ...
 *
 * Observations (Phase D) accumulate raw CVO/peer signals. Distillation promotes
 * those signals into the cat-dossier.md summary layer — but only through CVO
 * approval (KD-3: summary layer must be peer/CVO judged, not algorithm-generated).
 *
 ...
 *   pending → approved    (CVO approves in Hub)
 *   pending → rejected    (CVO rejects in Hub)
...
export interface DistillationEvidenceRef {
  ...
  id: string;
  /** Short human-readable summary for CVO display. */
  summary?: string;
}
...
export interface DossierDistillationProposal {
  ...
  // ---- Content ----
  /** Current field values at proposal time (for CVO diff display). */
