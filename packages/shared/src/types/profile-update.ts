 ...
 * the CVO sees a card and approves/rejects. Only on approve does the backend
 ...
 *   cvo-instructed   — CVO explicitly told the cat to record something
 *   cat-declared     — cat proactively observed and declared a signal
 *   magic-word       — CVO used a magic word (shared-rules.md)
 *   message-coordinate — signal anchored to a specific message coordinate
 *   sign-off         — CVO signed off / rejected something (explicit decision)
 *   reaction         — CVO reaction (emoji, quick feedback)
 */
...
/**
 * A profile-update proposal created by a cat, awaiting CVO decision.
 */
...
/** Fields the CVO may override at approve time (AC-C1: afterContent edit only). */
