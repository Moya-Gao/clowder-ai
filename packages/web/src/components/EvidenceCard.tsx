...
export function EvidenceCard({ result }: { result: EvidenceResult }) {
  ...
  return (
    <div
      className={`flex gap-2.5 p-3 rounded-xl bg-[var(--color-base-white)] border border-transparent hover:border-[var(--color-owner-primary)] hover:shadow-sm transition-all duration-200 group relative ${status?.className ?? ''}`}
    >
      {/* Source type icon */}
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2">
          <div className="flex flex-col gap-0.5 min-w-0">
            <h4
              className={`text-xs font-bold text-[var(--color-base-black)] leading-snug line-clamp-2 ${result.status === 'archived' ? 'line-through decoration-gray-400/50' : ''}`}
            >
              {result.title}
            </h4>
          </div>
        ...
        <p className="text-[11px] text-gray-500 leading-relaxed mt-1.5 line-clamp-2">{snippet}</p>
