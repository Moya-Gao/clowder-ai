    ...
    <div className="group relative">
      ...
      {canAct && (
        <div className="opacity-0 group-hover:opacity-100 absolute top-1 right-1 flex gap-0.5 transition-opacity bg-white/90 rounded-lg shadow-sm border border-gray-200 px-1 py-0.5">
          ...
          <button onClick={handleHardDelete} className="p-1 rounded hover:bg-gray-100 text-gray-400 hover:text-red-600 transition-colors" title="永久删除">
      ...
      <ConfirmDialog
        ...
        requireInput={
          dialog.type === 'hard-delete'
            ? (dialog.threadTitle ?? '确认删除')
            : undefined
        }
        inputPlaceholder={
          dialog.type === 'hard-delete' && dialog.threadTitle
            ? '输入对话标题'
            : '输入 "确认删除"'
        }
        confirmLabel="永久删除"
        ...
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50" onClick={close}>
          <div className="bg-white rounded-xl shadow-xl p-6 max-w-lg w-full mx-4" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex justify-end gap-2">
              <button onClick={close} className="px-4 py-2 text-sm text-gray-600 hover:bg-gray-100 rounded-lg">取消</button>
              <button
