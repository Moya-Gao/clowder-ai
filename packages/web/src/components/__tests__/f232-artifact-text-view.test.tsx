...
describe('F232 AC-A7 text view 看正文', () => {
  ...
  it('内容加载失败 → 降级到「跳回原消息」', () => {
    contentState.value = { content: null, path: 'docs/BACKLOG.md', isMarkdown: true, loading: false, error: true };
    const { container } = renderDetail({
      type: 'file',
      name: 'BACKLOG.md',
      ref: 'docs/BACKLOG.md',
  ...
  it('加载中 → 显示加载态', () => {
    contentState.value = { content: null, path: 'docs/BACKLOG.md', isMarkdown: true, loading: true, error: false };
    const { container } = renderDetail({
      type: 'file',
      name: 'BACKLOG.md',
      ref: 'docs/BACKLOG.md',
