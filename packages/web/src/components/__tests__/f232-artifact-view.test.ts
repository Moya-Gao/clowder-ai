...
describe('F232 AC-A7 classifyArtifactView — 产物 → 内容查看策略', () => {
  ...
  it('file 文本扩展名(.md) → text (panel 内看正文)', () => {
    expect(classifyArtifactView({ type: 'file', name: 'BACKLOG.md', ref: 'docs/BACKLOG.md' })).toBe('text');
  });
...
describe('F232 AC-A7 prRefToUrl — PR ref → GitHub url', () => {
  it('org/repo#123 → github pull url', () => {
    expect(prRefToUrl('zts212653/cat-cafe#2247')).toBe('https://github.com/zts212653/cat-cafe/pull/2247');
  });
