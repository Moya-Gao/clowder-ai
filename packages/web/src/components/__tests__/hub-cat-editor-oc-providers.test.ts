...
import { KNOWN_OC_PROVIDERS, resolveOpenCodeEndpoint, buildCallHint } from '@/components/hub-cat-editor.sections';

/** Minimal profile stub for buildCallHint tests */
function mkProfile(baseUrl: string) {
  return { id: 't', displayName: '', name: '', authType: 'api-key' as const, kind: 'opencode' as const, builtin: false, mode: 'default' as const, baseUrl };
}
