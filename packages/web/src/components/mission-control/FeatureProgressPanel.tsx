...
export function FeatureProgressPanel({ detail }: FeatureProgressPanelProps) {
  ...
  if (detail.phases.length === 0) {
    return (
      <p className="text-xs text-cafe-muted" data-testid="mc-progress-empty">
  ...
  return (
    <div className="space-y-3" data-testid="mc-progress-panel">
      <p className="text-xs font-bold uppercase tracking-wider text-cafe-secondary">Phase 进度</p>
      ...
      {detail.risks.length > 0 && (
        <div className="mt-2">
          <p className="mb-1 text-xs font-bold uppercase tracking-wider text-cafe-secondary">风险</p>
          <div className="space-y-1">
            {detail.risks.map((r, i) => (
              <div key={`risk-${i}`} className="flex gap-2 text-xs">
                <span className="text-[var(--mc-status-risk)]">• {r.risk}</span>
                <span className="text-cafe-secondary">→ {r.mitigation}</span>
              </div>
            ))}
          </div>
        </div>
...
function PhaseRow({ phase, expanded, onToggle }: { phase: FeatureDocPhase; expanded: boolean; onToggle: () => void }) {
  ...
  const barColor =
    pct === 100
      ? 'bg-[var(--mc-status-done-dot)]'
      : pct > 0
        ? 'bg-[var(--mc-status-dispatched-dot)]'
        : 'bg-[var(--mc-status-open-dot)]';
  const pctColor =
    pct === 100
      ? 'text-[var(--mc-status-done-dot)]'
      : pct > 0
        ? 'text-[var(--mc-status-dispatched-dot)]'
        : 'text-cafe-muted';
  ...
  return (
    <div data-testid={`mc-phase-${phase.id}`}>
      ...
      >
        <span className="w-[60px] shrink-0 text-xs font-medium text-cafe-secondary">Phase {phase.id}</span>
        <div className="h-[6px] flex-1 overflow-hidden rounded-full bg-[var(--console-border-soft)]">
        ...
        <div className="ml-[68px] mt-1 space-y-0.5" data-testid={`mc-phase-acs-${phase.id}`}>
          ...
          {phase.acs.map((ac) => (
            <div key={ac.id} className="flex items-center gap-1.5 text-xs">
              {ac.done ? (
                <svg
                  className="h-3.5 w-3.5 shrink-0 text-[var(--mc-status-done-dot)]"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2"
              ...
              <span className={ac.done ? 'text-cafe-secondary line-through' : 'text-cafe'}>
                {ac.id}: {ac.text}
              </span>
            </div>
          ))}
