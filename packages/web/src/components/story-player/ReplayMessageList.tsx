 ...
 * CVO iron rule: "100% 看起来就是你们平时的样子" — we reuse
...
const ReplayMessage = memo(function ReplayMessage({
  ...
  // Text message (thinking shown only in faithful mode)
  return (
    <MessageBubble
      messageId={msg.id}
      avatar={avatar}
    ...
    >
      ...
      <CollapsibleMarkdown content={msg.content} className="text-[var(--cat-msg-text,#e0e0e0)]" />
    </MessageBubble>
  );
});
...
export function ReplayMessageList({ messages, autoScroll = true, displayMode = 'cinematic' }: ReplayMessageListProps) {
  const bottomRef = useRef<HTMLDivElement>(null);

  // Auto-scroll when new messages appear — messageCount is intentionally in deps
  // to trigger scroll when replay engine advances and new messages appear
  const messageCount = messages.length;
  // biome-ignore lint/correctness/useExhaustiveDependencies: messageCount triggers scroll on new events
  useEffect(() => {
  ...
  if (messages.length === 0) {
    return (
      <div className="flex items-center justify-center h-full text-[var(--console-text-tertiary,#888)]">
        <p className="text-sm">Press play to start replay</p>
      </div>
    );
  }
