...
function StaleBranchRow({ branch }: { branch: StaleBranch }) {
  ...
  return (
    <div className="flex items-center gap-1.5 text-xs font-mono py-0.5 px-1 rounded hover:bg-owner-light/30">
      <Badge label="stale" variant="warning" />
      <span className="truncate text-cafe-black/80 flex-1">{branch.name}</span>
      <span className="text-[10px] text-owner-dark/40 shrink-0">{branch.author}</span>
      <span className="text-[10px] text-owner-dark/30 shrink-0">{relDate}</span>
    </div>
  );
}
...
function WorktreeRow({ wt }: { wt: WorktreeHealth }) {
  ...
  return (
    <div className="flex items-center gap-1.5 text-xs font-mono py-0.5 px-1 rounded hover:bg-owner-light/30">
      ...
      <span className="text-[10px] text-owner-dark/40 shrink-0">{wt.branch}</span>
      <span className="text-[10px] text-owner-dark/30 shrink-0">{wt.head}</span>
    </div>
  );
}
...
function DriftSection({ drift }: { drift: RuntimeDrift }) {
  if (!drift.available) {
    return <div className="text-[10px] text-owner-dark/40 px-1">Runtime drift unavailable</div>;
  }
  ...
  return (
    <div className="border-b border-owner-light/40">
      ...
      >
        ...
        <span className="text-[10px] text-owner-dark/40">{collapsed ? '\u25b8' : '\u25be'}</span>
      </button>

      {!collapsed && (
        <div className="px-3 pb-2 space-y-2">
          {loading && <div className="text-[10px] text-owner-dark/40">Loading...</div>}
