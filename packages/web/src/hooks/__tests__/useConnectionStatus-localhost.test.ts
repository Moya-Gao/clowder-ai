...
describe('useConnectionStatus localhost handling', () => {
  it('does not force loopback API targets offline when the browser reports offline', () => {
    expect(shouldForceBrowserOffline(false, 'http://localhost:3002')).toBe(false);
    expect(shouldForceBrowserOffline(false, 'http://127.0.0.1:3002')).toBe(false);
    expect(shouldForceBrowserOffline(false, 'http://[::1]:3002')).toBe(false);
  });
  ...
  it('does not downgrade a live loopback socket just because navigator.onLine is false', () => {
    expect(deriveSocketLevel(false, true, 'http://localhost:3002')).toBe('online');
  });
