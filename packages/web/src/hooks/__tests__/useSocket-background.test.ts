...
describe('background thread socket handling', () => {
  ...
  describe('R2-P2: text(isFinal) clears hasActiveInvocation', () => {
    ...
    it('does not suppress the new bg invocation after a fenced late callback merged into the previous bubble', () => {
      const now = Date.now();
      const responseText = 'Response from inv-1';
      useChatStore.getState().setThreadCatInvocation('thread-bg', 'opus', { invocationId: 'inv-bg-p2' });
      ...
      simulateBackgroundMessage({
        ...
        catId: 'opus',
        threadId: 'thread-bg',
        content: 'Fresh stream from inv-bg-p2-v2',
        timestamp: now + 4,
      });
      ...
      expect(ts.messages[1]).toEqual(
        expect.objectContaining({
          ...
          origin: 'stream',
          isStreaming: true,
          extra: { stream: { invocationId: 'inv-bg-p2-v2' } },
        }),
      );
    });
    ...
    it('P2 regression: suppresses late bg stream chunks when finalized_fallback infers invocationId from thread state', () => {
      const now = Date.now();
      const responseText = 'Late-bound bg response';

      useChatStore.getState().addMessageToThread('thread-bg', {
        id: 'bg-stream-late-bound',
        type: 'assistant',
        catId: 'opus',
        content: responseText,
        origin: 'stream',
      ...
      // Late invocation binding arrives after finalization, before the callback.
      useChatStore.getState().setThreadCatInvocation('thread-bg', 'opus', { invocationId: 'inv-bg-late-bound' });
      ...
      simulateBackgroundMessage({
        ...
        origin: 'callback',
        content: responseText,
        messageId: 'bg-callback-late-bound',
        timestamp: now + 2,
      });
      ...
      simulateBackgroundMessage({
        ...
        invocationId: 'inv-bg-late-bound',
        timestamp: now + 3,
      });
    ...
    it('P2 regression: background finalized_fallback does not merge callback onto a different reply target', () => {
      ...
      simulateBackgroundMessage({
        ...
        origin: 'callback',
        content: responseText,
        messageId: 'bg-callback-reply-mismatch',
    ...
    it('P2 regression: background finalized_fallback still merges when stream reply target is unknown', () => {
      const now = Date.now();
      const responseText = 'Same text, reply metadata arrives late';
      useChatStore.getState().setThreadCatInvocation('thread-bg', 'opus', { invocationId: 'inv-bg-old' });

      useChatStore.getState().addMessageToThread('thread-bg', {
        id: 'bg-stream-no-reply',
        type: 'assistant',
        catId: 'opus',
        content: responseText,
        origin: 'stream',
      ...
      testBgStreamRefs.set('thread-bg::opus', { id: 'bg-stream-no-reply', threadId: 'thread-bg', catId: 'opus' });
      ...
      simulateBackgroundMessage({
        ...
        origin: 'callback',
        content: responseText,
        messageId: 'bg-callback-reply-known',
        replyTo: 'msg-parent-known-late',
        replyPreview: { senderCatId: 'user', content: 'late-known parent' },
        timestamp: now + 3,
      });
    ...
    it('P2 regression: background finalized_fallback still merges when callback reply target is missing', () => {
      const now = Date.now();
      const responseText = 'Same text, callback reply metadata missing';
      useChatStore.getState().setThreadCatInvocation('thread-bg', 'opus', { invocationId: 'inv-bg-old' });

      useChatStore.getState().addMessageToThread('thread-bg', {
        id: 'bg-stream-known-reply',
        type: 'assistant',
        catId: 'opus',
        content: responseText,
        origin: 'stream',
      ...
      testBgStreamRefs.set('thread-bg::opus', { id: 'bg-stream-known-reply', threadId: 'thread-bg', catId: 'opus' });
      ...
      simulateBackgroundMessage({
        ...
        origin: 'callback',
        content: responseText,
        messageId: 'bg-callback-missing-reply',
        timestamp: now + 3,
      });
