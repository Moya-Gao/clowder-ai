 ...
 * Matches the Cat Café roster — handles that appear in real transcripts.
 */
const CAT_HANDLES = [
  'opus',
  'sonnet',
...
/** Collaboration tool names that indicate pass-ball events */
const PASS_BALL_TOOLS = new Set(['cat_cafe_cross_post_message', 'cat_cafe_multi_mention']);
...
export function isPassBallEvent(event: ReplayEvent): boolean {
  ...
  if (event.type === 'tool_call') {
    ...
    if (PASS_BALL_TOOLS.has(toolName)) return true;
