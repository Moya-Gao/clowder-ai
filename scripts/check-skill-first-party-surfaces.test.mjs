...
\`\`\`bash
curl -X POST http://localhost:3002/api/workspace/navigate \\
...
`);
  ...
  it('blocks scheme-less localhost curl guidance', () => {
    const hits = scan(`
\`\`\`bash
curl -X POST localhost:3002/api/workspace/navigate \\
  -d '{"worktreeId":"cat-cafe","path":"docs/foo.md","action":"open"}'
\`\`\`
`);
    assert.equal(hits.length, 1);
...
`);
  ...
  it('allows explicit instead-of-raw-curl guidance', () => {
    const hits = scan(
      'Use `cat_cafe_workspace_navigate` instead of raw curl to `localhost:3002/api/workspace/navigate`.',
    );
    assert.deepEqual(hits, []);
  });
...
`);
  ...
  it('honors reviewed allowlist entries', () => {
    const hits = scan('curl -X POST http://localhost:3002/api/workspace/navigate', [
      { path: REL_PATH, pattern: 'http://localhost:3002/api/workspace/navigate', reason: 'fixture' },
    ]);
    assert.deepEqual(hits, []);
  });
