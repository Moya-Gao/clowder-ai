...
function formatFseventsdPressureFailure(row, maxRssKb) {
  return [
    ...
    `  diagnose stale/no-listener Cat Cafe dev/watch process groups: pnpm process:doctor`,
    `  safe cleanup for Cat Cafe-owned stale rows: pnpm process:cleanup`,
    ...
    `  Manual gate bypass is a CVO override, not a pnpm gate pass.`,
  ].join('\n');
}
