...
describe('outbound text scanning', () => {
  it('detects home-only term "Cat Café" in a text file', () => {
    const file = join(tmpDir, 'readme.md');
    writeFileSync(file, 'Welcome to Cat Café — the best place for AI cats.\n');
  ...
  it('detects multiple terms in one file', () => {
    ...
    writeFileSync(file, 'Cat Café is run by 铲屎官 and 宪宪.\n');
...
describe('JSON field-path reporting', () => {
  it('reports $.name for a violation in a JSON name field', () => {
    ...
    writeFileSync(file, JSON.stringify({ name: 'Cat Café Hub', version: '1.0' }));
    const { stdout } = run(['--direction', 'outbound', file]);
    const findings = parseFindings(stdout);
  ...
  it('reports nested JSON field paths', () => {
    ...
    writeFileSync(
      file,
      JSON.stringify({
        app: { title: '三只 AI 猫猫的协作空间', debug: false },
      }),
    );
    const { stdout } = run(['--direction', 'outbound', file]);
    const findings = parseFindings(stdout);
...
describe('exit codes', () => {
  ...
  it('exits 1 for P1 violation', () => {
    ...
    writeFileSync(file, 'Welcome to Cat Café.\n');
    const { exitCode } = run(['--direction', 'outbound', file]);
  ...
  it('exits 2 for P2-only violations', () => {
    ...
    // CVO is P2, avoid any P1 terms
    writeFileSync(file, 'The CVO decides.\n');
    const { exitCode } = run(['--direction', 'outbound', file]);
...
describe('R1-P1: word boundary — no substring false positives', () => {
  ...
  it('still flags CJK terms without word boundaries', () => {
    const file = join(tmpDir, 'doc.txt');
    writeFileSync(file, '问铲屎官就好了\n');
    const { stdout } = run(['--direction', 'outbound', file]);
    const findings = parseFindings(stdout);
    assert.ok(
      findings.some((f) => f.termId === 'role.co_creator'),
      '铲屎官 should still match even without space boundaries',
    );
  });
});

describe('R1-P2: overlap suppression — longer match wins', () => {
  it('reports only product.hub, not product.primary, for "Cat Cafe Hub"', () => {
    const file = join(tmpDir, 'manifest.json');
    writeFileSync(file, JSON.stringify({ name: 'Cat Cafe Hub' }));
    const { stdout } = run(['--direction', 'outbound', file]);
    const findings = parseFindings(stdout);
  ...
  it('still reports product.primary when "Cat Cafe" appears alone', () => {
    const file = join(tmpDir, 'doc.txt');
    writeFileSync(file, 'Welcome to Cat Cafe.\n');
    const { stdout } = run(['--direction', 'outbound', file]);
    const findings = parseFindings(stdout);
...
describe('R4-P1: canonical-first — non-parallel lists use canonical, not index', () => {
  it('suggests 铲屎官 for inbound "operator" (not @landy)', () => {
    // role.co_creator has 3 public variants but 7 home variants — NOT parallel.
    // home.canonical = "铲屎官" must win over index-based home.variants[1] = "@landy".
    const file = join(tmpDir, 'doc.txt');
    writeFileSync(file, 'The operator manages the system.\n');
    const { stdout } = run(['--direction', 'inbound', file]);
    const findings = parseFindings(stdout);
    ...
    assert.equal(f.suggestion, '铲屎官', 'canonical should win over index mapping for non-parallel lists');
  });
